...
public final class BarnManagerValidationService {
    ...
    public static ValidationResult validateForTier(ServerLevel level, BlockPos managerPos, int targetTier) {
        ...
        if (!scan.hasInteriorSpace()) {
            reasons.add("Не обнаружено внутреннее пространство амбара (убедитесь, что рядом с менеджером есть свободное пространство для стояния)");
        }
        ...
        if (scan.interiorAirCount() < requirement.minInteriorBlocks()) {
            reasons.add("Недостаточно внутреннего пространства амбара: требуется минимум " + requirement.minInteriorBlocks() + " клеток, сейчас " + scan.interiorAirCount() + " клеток");
        }

        if (Config.BARN_REQUIRE_ENCLOSED.get() && !scan.enclosed()) {
            reasons.add("Амбар не закрыт (обнаружена связь внутреннего пространства с границей сканирования)");
        }

        if (Config.BARN_REQUIRE_DOOR.get() && scan.doorCount() < Config.BARN_MIN_DOOR_COUNT.get()) {
            reasons.add("Недостаточно дверей/ворот: требуется " + Config.BARN_MIN_DOOR_COUNT.get() + ", сейчас " + scan.doorCount());
        }
