...
public class BeeHouseBlockEntityRenderer implements BlockEntityRenderer<BeeHouseBlockEntity> {
	...
	@Override
	public void render(@SuppressWarnings("null") BeeHouseBlockEntity be, float partialTick, @SuppressWarnings("null") PoseStack poseStack, @SuppressWarnings("null") MultiBufferSource buffer, int packedLight, int packedOverlay) {
		if (be == null || !be.isReady()) {
			return;
		}

		ItemStack product = be.getCurrentProduct();
		if (product.isEmpty()) {
			return;
		}
