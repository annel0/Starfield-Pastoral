...
public class FishSmokerBlockEntityRenderer implements BlockEntityRenderer<FishSmokerBlockEntity> {
    ...
    public FishSmokerBlockEntityRenderer(BlockEntityRendererProvider.Context context) {
    }

    @SuppressWarnings({ "null", "deprecation" })
    @Override
    public void render(@Nonnull FishSmokerBlockEntity be, float partialTick, @Nonnull PoseStack poseStack, @Nonnull MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        if (level != null) {
            ...
            if (be.isWorking() && !ready) {
                applyWorkingPose(poseStack, level, be.getBlockPos(), partialTick);
            }
        ...
        poseStack.pushPose();
        poseStack.translate(0.5f, 1.8f, 0.5f);
    ...
    private static void applyWorkingPose(PoseStack poseStack, Level level, net.minecraft.core.BlockPos pos, float partialTick) {
