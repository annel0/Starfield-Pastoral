...
public final class DwarvishTranslationGuideClientFx {
    private static final int BROWN_BRIGHT = 0xFFE9C28C;
    private static final int BROWN_MID = 0xFF9A6130;
    private static final int BROWN_DEEP = 0xFF4B2A13;
    private static final int BG_TINT_TOP = 0xF0180E05;
    private static final int BG_TINT_BOTTOM = 0xF0080401;
    private static final float BORDER_PERIOD_S = 3.0F;

    private static final int TYPE_BASE_RGB = 0x8B5A2B;
    private static final int TYPE_HIGHLIGHT_RGB = 0xE4B77A;
    private static final float TYPE_SWEEP_SPEED = 0.36F;
    private static final float TYPE_SWEEP_HALF = 0.28F;

    private DwarvishTranslationGuideClientFx() {
    }

    @SubscribeEvent
    public static void onTooltipColor(RenderTooltipEvent.Color event) {
        ItemStack stack = event.getItemStack();
        if (stack.isEmpty() || !stack.is(ModItems.DWARVISH_TRANSLATION_GUIDE.get())) {
            return;
        }
        ...
        float kShift = 0.5F + 0.5F * (float) Math.sin(phase + Math.PI * 0.5);

        event.setBorderStart(lerpArgb(BROWN_MID, BROWN_BRIGHT, k));
        event.setBorderEnd(lerpArgb(BROWN_DEEP, BROWN_MID, kShift));
