...
public final class GalaxyDaggerMarkRenderer {
    ...
    private static final ResourceLocation MARK_TEXTURE = ResourceLocation.fromNamespaceAndPath(
        StardewCraft.MODID,
        "textures/gui/weapon_skill/holy_blade_ring.png"
    );

    @SuppressWarnings("null")
    private static final RenderType MARK_RENDER_TYPE = RenderType.create(
        "stardew_galaxy_dagger_mark",
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        for (LivingEntity entity : entities) {
            if (!GalaxyDaggerMarkClientState.isMarked(entity.getId(), nowTick)) {
                continue;
            }

            float remainingRatio = GalaxyDaggerMarkClientState.getRemainingRatio(entity.getId(), nowTick);
            float fade = remainingRatio < 0.35f ? (remainingRatio / 0.35f) : 1.0f;
            float pulse = 0.85f + 0.15f * Mth.sin((nowTick + partial + entity.getId()) * 0.35f);
            int alpha = Math.min(255, Math.max(20, (int) (210 * fade * pulse)));
            ...
            float rot = (nowTick + partial) * 6.0f;
