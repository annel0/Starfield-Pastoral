...
public final class DwarfDaggerThrustClientState {
    ...
    public static void onClientTick(ClientTickEvent.Post event) {
        ...
        if (elapsed == windupTicks) {
            player.playSound(SoundEvents.PLAYER_ATTACK_SWEEP, 0.75f, 1.3f);
            player.playSound(SoundEvents.TRIDENT_THROW.value(), 0.55f, 1.25f);
        }

        Vec3 current = player.position();
        double progress = Math.min(1.0, elapsed / (double) totalTicks);
        double accelFactor = elapsed < windupTicks ? 0.25 : Math.min(1.6, 0.85 + progress * 1.15);
    ...
    private static void spawnDashTrail(Player player, Vec3 current, Vec3 end) {
        ...
        if (player.level().getRandom().nextFloat() < 0.4f) {
