...
public enum PreservesJarJadeProvider implements IBlockComponentProvider, IServerDataProvider<BlockAccessor> {
	...
	public void appendServerData(CompoundTag tag, BlockAccessor accessor) {
		if (!(accessor.getBlockEntity() instanceof PreservesJarBlockEntity jar)) {
			return;
		}

		tag.putBoolean(NBT_READY, jar.isReady());

		ItemStack input = jar.getInput();
		ItemStack product = jar.getProduct();
		...
		PreservesJarBlockEntity.RemainingTime rt = jar.getRemainingTime();
	...
	public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
		...
		if (ready && !productItemId.isEmpty()) {
			...
			tooltip.add(Component.translatable("stardewcraft.tooltip.preserves_jar.ready")
					.withStyle(ChatFormatting.GREEN, ChatFormatting.BOLD));
			return;
		}

		if (inputItemId.isEmpty()) {
			tooltip.add(Component.translatable("stardewcraft.tooltip.preserves_jar.input")
					.append(": ")
					.append(Component.translatable("stardewcraft.tooltip.preserves_jar.input.none"))
					.withStyle(ChatFormatting.WHITE));
			return;
		}
		...
		if (!inputStack.isEmpty()) {
			tooltip.add(List.of(
				...
				helper.spacer(4, 0),
				helper.text(Component.translatable("stardewcraft.tooltip.preserves_jar.input")
					.append(": ")
		...
		var remaining = RemainingTimeTooltip.build("stardewcraft.tooltip.preserves_jar.remaining", days, hours, minutes)
