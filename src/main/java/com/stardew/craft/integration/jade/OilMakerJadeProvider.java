...
public enum OilMakerJadeProvider implements IBlockComponentProvider, IServerDataProvider<BlockAccessor> {
    INSTANCE;

    private static final ResourceLocation UID = ResourceLocation.fromNamespaceAndPath(StardewCraft.MODID, "oil_maker");
    ...
    public void appendServerData(CompoundTag tag, BlockAccessor accessor) {
        if (!(accessor.getBlockEntity() instanceof OilMakerBlockEntity maker)) {
            return;
        }

        tag.putBoolean(NBT_READY, maker.isReady());

        ItemStack input = maker.getInput();
        ItemStack product = maker.getProduct();
        ...
        OilMakerBlockEntity.RemainingTime rt = maker.getRemainingTime();
    ...
    public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
        ...
        if (ready && !productItemId.isEmpty()) {
            ...
            tooltip.add(Component.translatable("stardewcraft.tooltip.oil_maker.ready")
                    .withStyle(ChatFormatting.GREEN, ChatFormatting.BOLD));
            return;
        }

        if (inputItemId.isEmpty()) {
            tooltip.add(Component.translatable("stardewcraft.tooltip.oil_maker.input")
                    .append(": ")
                    .append(Component.translatable("stardewcraft.tooltip.oil_maker.input.none"))
                    .withStyle(ChatFormatting.WHITE));
            return;
        }
        ...
        if (!inputStack.isEmpty()) {
            tooltip.add(List.of(
                    ...
                    helper.spacer(4, 0),
                    helper.text(Component.translatable("stardewcraft.tooltip.oil_maker.input")
                            .append(": ")
        ...
        var remaining = RemainingTimeTooltip.build("stardewcraft.tooltip.oil_maker.remaining", days, hours, minutes)
