...
public enum SeedMakerJadeProvider implements IBlockComponentProvider, IServerDataProvider<BlockAccessor> {
    INSTANCE;

    private static final ResourceLocation UID = ResourceLocation.fromNamespaceAndPath(StardewCraft.MODID, "seed_maker");
    ...
    public void appendServerData(CompoundTag tag, BlockAccessor accessor) {
        ...
        tag.putBoolean(NBT_READY, seedMaker.isReady());

        ItemStack input = seedMaker.getInput();
        ItemStack product = seedMaker.getProduct();
    ...
    public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
        ...
        if (ready && !productItemId.isEmpty()) {
            ...
            tooltip.add(Component.translatable("stardewcraft.tooltip.seed_maker.ready")
                    .withStyle(ChatFormatting.GREEN, ChatFormatting.BOLD));
            return;
        }

        if (inputItemId.isEmpty()) {
            tooltip.add(Component.translatable("stardewcraft.tooltip.seed_maker.input")
                    .append(": ")
                    .append(Component.translatable("stardewcraft.tooltip.seed_maker.input.none"))
                    .withStyle(ChatFormatting.WHITE));
            return;
        }
        ...
        if (!inputStack.isEmpty()) {
            tooltip.add(List.of(
                ...
                helper.spacer(4, 0),
                helper.text(Component.translatable("stardewcraft.tooltip.seed_maker.input")
                    .append(": ")
        ...
        var remaining = RemainingTimeTooltip.build("stardewcraft.tooltip.seed_maker.remaining", days, hours, minutes)
