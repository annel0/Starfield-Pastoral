...
/** Seed item. */
public class PowderMelonSeedItem extends Item implements IStardewItem {

    public PowderMelonSeedItem(Item.Properties properties) {
        super(properties);
    }
    ...
    public InteractionResult useOn(@SuppressWarnings("null") UseOnContext context) {
        ...
        if (!level.isClientSide) {
            ...
            if (!com.stardew.craft.farming.SeasonLocationRules.isPlantingSeasonAllowed(level, abovePos, season, 3)) {
        ...
        if (!level.isClientSide) {
            level.setBlock(abovePos, ModBlocks.POWDER_MELON_CROP.get().defaultBlockState(), 3);
