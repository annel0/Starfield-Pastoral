...
public class GoldenPumpkinItem extends Item implements IStardewItem {
    private static final int SELL_PRICE = 2500;
    private static final int NAME_BASE_RGB = 0xD68A18;
    private static final int NAME_HIGHLIGHT_RGB = 0xFFF2A8;

    public GoldenPumpkinItem(Properties properties) {
        super(properties);
    }
    ...
    public void appendHoverText(@Nonnull ItemStack stack, @Nonnull TooltipContext context,
        ...
        tooltipComponents.add(Component.translatable("stardewcraft.item.golden_pumpkin.tooltip.special")
                .withStyle(ChatFormatting.DARK_GRAY));
    }

    private static MutableComponent sweepHighlight(String raw, int baseRgb, int highlightRgb) {
        long ms = System.currentTimeMillis();
        float pos = (((ms % 60_000L) / 1000.0F * 0.36F) % 1.64F) - 0.32F;
