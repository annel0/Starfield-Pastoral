...
public class SheepEntity extends BaseCoopAnimalEntity {
    ...
    protected EntityType<? extends Animal> getOffspringType() {
        return ModEntities.SHEEP.get();
    }

    @Override
    public @Nonnull InteractionResult mobInteract(@Nonnull net.minecraft.world.entity.player.Player player,
                                                  @Nonnull InteractionHand hand) {
        if (hand == InteractionHand.MAIN_HAND && !this.level().isClientSide) {
            ItemStack held = player.getItemInHand(hand);
            if (held.is(ModItems.SHEARS.get()) && this.level() instanceof ServerLevel serverLevel && getManagedAnimalId() > 0L) {
                ...
                if (record != null && !record.isBaby() && !record.currentProduceId().isBlank()) {
                    ...
                    if (!produce.isEmpty()) {
                        ...
                        data.markChanged();
                        serverLevel.playSound(null, this.blockPosition(), SoundEvents.SHEEP_SHEAR, SoundSource.NEUTRAL, 1.0F, 1.0F);
