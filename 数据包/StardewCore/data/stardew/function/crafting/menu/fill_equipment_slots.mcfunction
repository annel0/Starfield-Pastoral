# data/stardew/function/crafting/menu/fill_equipment_slots.mcfunction
# [执行者: 玩家] 填充设备配方菜单的7个槽位
# 根据解锁配方列表和当前页码动态填充
...
# 填充配方（使用宏函数循环填充）
# 获取当前页的配方数据
execute store result score #SlotIndex sd_temp run scoreboard players get @s sd_menu_page
scoreboard players operation #SlotIndex sd_temp *= #7 sd_const

# 逐个检查并填充
function stardew:crafting/menu/fill_slot_recursive {slot:1}
