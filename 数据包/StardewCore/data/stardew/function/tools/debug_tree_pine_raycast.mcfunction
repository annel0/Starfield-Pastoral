# data/stardew/function/tools/debug_tree_pine_raycast.mcfunction
# Debug松树生成射线检测

# 1. 检查是否为合适的地面
execute unless block ~ ~ ~ #stardew:tillable run scoreboard players add @s sd_ray_steps 1
execute unless block ~ ~ ~ #stardew:tillable if score @s sd_ray_steps matches ..30 positioned ^ ^ ^0.2 run function stardew:tools/debug_tree_pine_raycast
...
execute unless block ~ ~1 ~ minecraft:air if score @s sd_ray_steps matches ..30 positioned ^ ^ ^0.2 run function stardew:tools/debug_tree_pine_raycast
...
# 3. 生成成熟松树 - 在与种树系统相同的高度生成
execute align xyz positioned ~0.5 ~1.5 ~0.5 run function stardew:tree/spawn_pine
tellraw @s [{"text":"[DEBUG] ","color":"gold"},{"text":"成功生成松树","color":"green"}]
return 1