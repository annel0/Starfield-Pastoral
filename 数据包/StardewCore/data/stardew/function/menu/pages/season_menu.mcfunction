# data/stardew/function/menu/pages/season_menu.mcfunction
# 季节子菜单 (Debug -> 季节)
# 执行者: 玩家 (@s)

# 1. 设置菜单层级为21（季节子菜单）
scoreboard players set @s sd_menu_level 21
...
# 2. 配置5个季节按钮 (居中布局: slot 3-7，向右移一格)

# slot_3: 切换春季 (CMD: 30)
execute as @e[type=item_display,tag=sd_menu_button,tag=sd_menu_slot_3] if score @s sd_menu_entity_num = #CurrentPlayer sd_menu_ctrl run data merge entity @s {item:{id:"minecraft:paper",count:1,components:{"minecraft:custom_model_data":30}}}

# slot_4: 切换夏季 (CMD: 31)
execute as @e[type=item_display,tag=sd_menu_button,tag=sd_menu_slot_4] if score @s sd_menu_entity_num = #CurrentPlayer sd_menu_ctrl run data merge entity @s {item:{id:"minecraft:paper",count:1,components:{"minecraft:custom_model_data":31}}}

# slot_5: 切换秋季 (CMD: 32)
execute as @e[type=item_display,tag=sd_menu_button,tag=sd_menu_slot_5] if score @s sd_menu_entity_num = #CurrentPlayer sd_menu_ctrl run data merge entity @s {item:{id:"minecraft:paper",count:1,components:{"minecraft:custom_model_data":32}}}

# slot_6: 切换冬季 (CMD: 33)
execute as @e[type=item_display,tag=sd_menu_button,tag=sd_menu_slot_6] if score @s sd_menu_entity_num = #CurrentPlayer sd_menu_ctrl run data merge entity @s {item:{id:"minecraft:paper",count:1,components:{"minecraft:custom_model_data":33}}}
