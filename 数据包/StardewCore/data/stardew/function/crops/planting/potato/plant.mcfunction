# data/stardew/functions/crops/planting/plant_potato.mcfunction
...
execute unless score Global sd_season matches 1 run tellraw @s {"text":"土豆只能在春季种植","color":"red"}
execute unless score Global sd_season matches 1 run return 1

# 3. 种植 (召唤实体)
execute if block ~ ~ ~ minecraft:farmland align xyz run function stardew:crops/summon_entities/potato/summon_entities
execute if block ~ ~-1 ~ minecraft:farmland align xyz positioned ~ ~-1 ~ run function stardew:crops/summon_entities/potato/summon_entities
...
# 5. 消耗种子与初始化
execute align xyz positioned ~0.5 ~1.375 ~0.5 if entity @e[tag=init_crop,distance=..0.5] if entity @s[gamemode=!creative] run clear @s carrot_on_a_stick[minecraft:custom_model_data=2103] 1
...
execute align xyz positioned ~0.5 ~1.375 ~0.5 as @e[tag=init_crop,distance=..0.5] run scoreboard players set @s sd_max_crop_age 6
