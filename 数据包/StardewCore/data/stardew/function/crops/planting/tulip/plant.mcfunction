# data/stardew/functions/crops/planting/plant_tulip.mcfunction
...
execute unless score Global sd_season matches 1 run tellraw @s {"text":"郁金香只能在春季种植","color":"red"}
execute unless score Global sd_season matches 1 run return 1

# 3. 种植 (召唤实体)
execute if block ~ ~ ~ minecraft:farmland align xyz run function stardew:crops/summon_entities/tulip/summon_entities
execute if block ~ ~-1 ~ minecraft:farmland align xyz positioned ~ ~-1 ~ run function stardew:crops/summon_entities/tulip/summon_entities
