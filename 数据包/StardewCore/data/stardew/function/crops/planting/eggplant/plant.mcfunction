# data/stardew/functions/crops/planting/plant_eggplant.mcfunction
...
execute unless score Global sd_season matches 3 run tellraw @s {"text":"茄子只能在秋季种植","color":"red"}
execute unless score Global sd_season matches 3 run return 1

# 3. 种植 (召唤实体)
execute if block ~ ~ ~ minecraft:farmland align xyz run function stardew:crops/summon_entities/eggplant/summon_entities
execute if block ~ ~-1 ~ minecraft:farmland align xyz positioned ~ ~-1 ~ run function stardew:crops/summon_entities/eggplant/summon_entities
