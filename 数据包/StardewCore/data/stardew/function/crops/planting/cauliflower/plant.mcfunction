# data/stardew/functions/crops/planting/plant_cauliflower.mcfunction
...
execute unless score Global sd_season matches 1 run tellraw @s {"text":"花椰菜只能在春季种植","color":"red"}
execute unless score Global sd_season matches 1 run return 1

# 3. 种植 (召唤实体)
execute if block ~ ~ ~ minecraft:farmland align xyz run function stardew:crops/summon_entities/cauliflower/summon_entities
execute if block ~ ~-1 ~ minecraft:farmland align xyz positioned ~ ~-1 ~ run function stardew:crops/summon_entities/cauliflower/summon_entities
...
# 5. 消耗种子与初始化
execute align xyz positioned ~0.5 ~1.375 ~0.5 if entity @e[tag=init_crop,distance=..0.5] if entity @s[gamemode=!creative] run clear @s carrot_on_a_stick[minecraft:custom_model_data=2115] 1
...
execute align xyz positioned ~0.5 ~1.375 ~0.5 as @e[tag=init_crop,distance=..0.5] run scoreboard players set @s sd_max_crop_age 12
execute align xyz positioned ~0.5 ~1.375 ~0.5 as @e[tag=init_crop,distance=..0.5] run scoreboard players set @s sd_watered 0
# 添加季节标签
execute align xyz positioned ~0.5 ~1.375 ~0.5 as @e[tag=init_crop,distance=..0.5] run tag @s add season_1

# 5.5 肥料继承 - 检查并继承已有肥料
execute align xyz positioned ~0.5 ~1.375 ~0.5 as @e[tag=init_crop,distance=..0.5] at @s run tag @s add sd_new_crop
execute align xyz positioned ~0.5 ~1.375 ~0.5 as @e[tag=sd_new_crop] at @s run function stardew:farming/fertilizer/inherit_on_plant
execute align xyz positioned ~0.5 ~1.375 ~0.5 as @e[tag=sd_new_crop] run tag @s remove sd_new_crop
