# data/stardew/functions/crops/planting/plant_fairy_rose.mcfunction
...
execute unless score Global sd_season matches 3 run tellraw @s {"text":"玫瑰仙子只能在秋季种植","color":"red"}
execute unless score Global sd_season matches 3 run return 1

# 3. 种植 (召唤实体)
execute if block ~ ~ ~ minecraft:farmland align xyz run function stardew:crops/summon_entities/fairy_rose/summon_entities
execute if block ~ ~-1 ~ minecraft:farmland align xyz positioned ~ ~-1 ~ run function stardew:crops/summon_entities/fairy_rose/summon_entities
