# data/stardew/functions/crops/planting/plant_coffee_bean.mcfunction
...
execute unless score Global sd_season matches 1..2 run tellraw @s {"text":"咖啡豆只能在春季、夏季种植","color":"red"}
execute unless score Global sd_season matches 1..2 run return 1

# 3. 种植 (召唤实体)
execute if block ~ ~ ~ minecraft:farmland align xyz run function stardew:crops/summon_entities/coffee_bean/summon_entities
execute if block ~ ~-1 ~ minecraft:farmland align xyz positioned ~ ~-1 ~ run function stardew:crops/summon_entities/coffee_bean/summon_entities
