# data/stardew/functions/crops/planting/plant_sunflower.mcfunction
...
execute unless score Global sd_season matches 2..3 run tellraw @s {"text":"向日葵只能在夏季、秋季种植","color":"red"}
execute unless score Global sd_season matches 2..3 run return 1

# 3. 种植 (召唤实体)
execute if block ~ ~ ~ minecraft:farmland align xyz run function stardew:crops/summon_entities/sunflower/summon_entities
execute if block ~ ~-1 ~ minecraft:farmland align xyz positioned ~ ~-1 ~ run function stardew:crops/summon_entities/sunflower/summon_entities
...
execute align xyz positioned ~0.5 ~1.375 ~0.5 as @e[tag=init_crop,distance=..0.5] run scoreboard players set @s sd_max_crop_age 8
