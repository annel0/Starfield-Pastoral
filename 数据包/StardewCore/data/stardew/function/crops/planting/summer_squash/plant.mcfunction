# data/stardew/functions/crops/planting/summer_squash/plant.mcfunction
...
execute unless score Global sd_season matches 2 run tellraw @s {"text":"金皮西葫芦只能在夏季种植","color":"red"}
execute unless score Global sd_season matches 2 run return 1

# 3. 种植 (召唤实体)
execute if block ~ ~ ~ minecraft:farmland align xyz run function stardew:crops/summon_entities/summer_squash/summon_entities
execute if block ~ ~-1 ~ minecraft:farmland align xyz positioned ~ ~-1 ~ run function stardew:crops/summon_entities/summer_squash/summon_entities
...
execute as @e[tag=init_crop,distance=..0.5,limit=1] run tag @s add crop_summer_squash
execute as @e[tag=init_crop,distance=..0.5,limit=1] run tag @s add season_summer
execute as @e[tag=init_crop,distance=..0.5,limit=1] run tag @s add season_2
execute as @e[tag=init_crop,distance=..0.5,limit=1] run scoreboard players set @s sd_crop_age 0
execute as @e[tag=init_crop,distance=..0.5,limit=1] run scoreboard players set @s sd_max_crop_age 6
execute as @e[tag=init_crop,distance=..0.5,limit=1] run scoreboard players set @s sd_watered 0
execute as @e[tag=init_crop,distance=..0.5,limit=1] run scoreboard players set @s sd_regrow_days 3
