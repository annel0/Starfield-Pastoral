# data/stardew/functions/crops/planting/plant_crystal_fruit.mcfunction
...
execute unless score Global sd_season matches 4 run tellraw @s {"text":"水晶果只能在冬季种植","color":"red"}
execute unless score Global sd_season matches 4 run return 1

# 3. 种植 (召唤实体)
execute if block ~ ~ ~ minecraft:farmland align xyz run function stardew:crops/summon_entities/crystal_fruit/summon_entities
execute if block ~ ~-1 ~ minecraft:farmland align xyz positioned ~ ~-1 ~ run function stardew:crops/summon_entities/crystal_fruit/summon_entities
...
# 添加季节标签
execute align xyz positioned ~0.5 ~1.375 ~0.5 as @e[tag=init_crop,distance=..0.5] run tag @s add season_4
