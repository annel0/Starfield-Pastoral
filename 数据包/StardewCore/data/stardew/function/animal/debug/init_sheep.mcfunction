...
# 初始化羊毛生产计数器
scoreboard players set @s stardew.animal.produce_days 0

# 成年绵羊立即生成一次羊毛（用于测试）
execute if score @s stardew.animal.age matches 5.. run function stardew:animal/produce/check_single_sheep
...
# 显示调试信息
tellraw @a[distance=..10] [{"text":"绵羊初始化完成 - ID: ","color":"gray"},{"score":{"name":"@s","objective":"stardew.animal.id"},"color":"white"},{"text":" | 类型: ","color":"gray"},{"score":{"name":"@s","objective":"stardew.animal.type"},"color":"white"}]
