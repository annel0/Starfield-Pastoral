...
# 更新动画状态
function stardew:animal/animated_java/update_chicken_baby_animation
...
# 如果需要成长，进行模型切换
# 注意：check_chicken_growth 需要 #check_id，所以要复制 #sync_id 到 #check_id
...
execute if score #need_grow stardew.animal.temp matches 1 as @e[type=chicken,tag=stardew.animal] if score @s stardew.animal.id = #sync_id stardew.animal.temp run function stardew:animal/animated_java/check_chicken_growth
