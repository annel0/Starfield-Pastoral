...
# 星露谷物语 - 同步单个幼年鸭
# ================================================================
# 用途：让 root entity tp 到对应的逻辑鸭位置
# @s = AJ root entity (chicken_baby 用于幼年鸭)
...
execute as @e[type=chicken,tag=stardew.animal] if score @s stardew.animal.id = #sync_id stardew.animal.temp at @s run tp @e[tag=aj.chicken_baby.root,tag=stardew.animal.aj_bound,limit=1,sort=nearest] ~ ~ ~ ~ 0

# 更新动画状态
function stardew:animal/animated_java/update_duck_baby_animation
...
# 如果需要成长，进行模型切换
# 注意：check_duck_growth 需要 #check_id，所以要复制 #sync_id 到 #check_id
...
execute if score #need_grow stardew.animal.temp matches 1 as @e[type=chicken,tag=stardew.animal] if score @s stardew.animal.id = #sync_id stardew.animal.temp run function stardew:animal/animated_java/check_duck_growth
