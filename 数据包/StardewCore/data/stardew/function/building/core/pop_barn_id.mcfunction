...
# 找到栈顶
tag @e[type=marker,tag=stardew.id_stack.barn,limit=1,sort=random] add stardew.temp.pop
...
tellraw @a[tag=debug] [{"text":"[ID栈] ","color":"aqua"},{"text":"从栈中分配畜棚ID: "},{"score":{"name":"@s","objective":"stardew.building.id"}}]
