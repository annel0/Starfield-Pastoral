# data/stardew/function/utility/fu# 6. 改变视觉实体为工作状态 (CMD 102)
...
# 8. 在熔炉上方召唤产物展示 (钻石)，并设置相同的IDmelt_diamond.mcfunction
# 熔炼钻石锭 - 玩家拿钻石右键熔炉时调用
# 执行者: 玩家 (@s)
# 前提: 附近有一个标记为 sd_interacting_furnace 的熔炉

# 1. 检查玩家是否有5个钻石和1个煤炭
execute store result score @s sd_temp run clear @s minecraft:paper[custom_model_data=7006] 0
execute unless score @s sd_temp matches 5.. run tellraw @s {"text":"需要5个钻石！","color":"red"}
...
# 2. 扣除材料
clear @s minecraft:paper[custom_model_data=7006] 5
...
scoreboard players set @e[tag=sd_current_furnace] sd_furnace_type 4
# 初始化已工作时间为 0，设置需要的总时间（480分钟，8小时）
scoreboard players set @e[tag=sd_current_furnace] sd_furnace_timer 0
scoreboard players set @e[tag=sd_current_furnace] sd_furnace_max_time 480
...
# 7. 在熔炉上方召唤产物展示（根据旋转角度）(钻石锭)，并设置相同的ID
...
# 8. 召唤时间文本显示，并设置相同的ID
execute as @e[tag=sd_current_furnace_visual] at @s run summon minecraft:text_display ~ ~1.7 ~ {Tags:["sd_furnace_time","sd_current_time"],text:'{"text":"480分钟","color":"yellow"}',billboard:"center",alignment:"center",background:0}
