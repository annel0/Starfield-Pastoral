# data/stardew/function/utility/furnace/smelt_copper.mcfunction
# 熔炼铜锭 - 玩家拿铜粒右键熔炉时调用
# 执行者: 玩家 (@s)
# 前提: 附近有一个标记为 sd_interacting_furnace 的熔炉

# 1. 检查玩家是否有5个铜粒和1个煤炭
execute store result score @s sd_temp run clear @s minecraft:paper[custom_model_data=7003] 0
execute unless score @s sd_temp matches 5.. run tellraw @s {"text":"需要5个铜粒！","color":"red"}
...
# 2. 扣除材料
clear @s minecraft:paper[custom_model_data=7003] 5
...
scoreboard players set @e[tag=sd_current_furnace] sd_furnace_type 1
# 初始化已工作时间为 0，设置需要的总时间（30分钟）
scoreboard players set @e[tag=sd_current_furnace] sd_furnace_timer 0
scoreboard players set @e[tag=sd_current_furnace] sd_furnace_max_time 30
...
# 8. 在熔炉上方召唤产物展示（根据旋转角度）(铜锭)，并设置相同的ID
...
# 9. 召唤时间文本显示，并设置相同的ID
execute as @e[tag=sd_current_furnace_visual] at @s run summon minecraft:text_display ~ ~1.7 ~ {Tags:["sd_furnace_time","sd_current_time"],text:'{"text":"30分钟","color":"yellow"}',billboard:"center",alignment:"center",background:0}
