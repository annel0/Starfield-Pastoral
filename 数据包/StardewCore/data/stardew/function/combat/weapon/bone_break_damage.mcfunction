# 骨裂打击伤害计算
# 130%武器伤害已在主函数计算好，存储在 #damage sd_temp 中

# 【重构】统一的暴击计算系统
execute store result score #weapon_crit sd_temp run data get entity @p[tag=sd_using_bone_break] SelectedItem.components."minecraft:custom_data".weapon_crit_chance 100
execute as @p[tag=sd_using_bone_break] run function stardew:combat/calculate_crit
execute as @p[tag=sd_using_bone_break] run function stardew:combat/apply_crit_damage
...
data modify storage stardew:temp color set value "#E0E0E0"
...
execute if score #is_critical sd_temp matches 1 run playsound minecraft:entity.player.attack.crit player @a ~ ~ ~ 1.5 0.7
execute unless score #is_critical sd_temp matches 1 run playsound minecraft:entity.player.attack.strong player @a ~ ~ ~ 1 0.7
...
execute store result score #player_x sd_temp run data get entity @p[tag=sd_using_bone_break] Pos[0] 1000
execute store result score #player_z sd_temp run data get entity @p[tag=sd_using_bone_break] Pos[2] 1000
...
execute store result entity @s Motion[0] double 0.00018 run scoreboard players get #motion_x sd_temp
execute store result entity @s Motion[2] double 0.00018 run scoreboard players get #motion_z sd_temp
data modify entity @s Motion[1] set value 0.22
