...
# 【重构】统一的暴击计算系统
execute store result score #weapon_crit sd_temp run data get entity @p[tag=sd_using_rapid_strike] SelectedItem.components."minecraft:custom_data".weapon_crit_chance 100
execute as @p[tag=sd_using_rapid_strike] run function stardew:combat/calculate_crit
execute as @p[tag=sd_using_rapid_strike] run function stardew:combat/apply_crit_damage

# 如果是怪物，扣除 sd_monster_hp（使用暴击后的伤害）
...
playsound minecraft:entity.generic.hurt hostile @a ~ ~ ~ 0.7 1.2
...
execute store result score #player_x sd_temp run data get entity @p[tag=sd_using_rapid_strike] Pos[0] 1000
execute store result score #player_z sd_temp run data get entity @p[tag=sd_using_rapid_strike] Pos[2] 1000
