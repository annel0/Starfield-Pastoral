...
# 【重构】统一的暴击计算系统
execute store result score #weapon_crit sd_temp run data get entity @p[tag=sd_using_whirlwind] SelectedItem.components."minecraft:custom_data".weapon_crit_chance 100
execute as @p[tag=sd_using_whirlwind] run function stardew:combat/calculate_crit
execute as @p[tag=sd_using_whirlwind] run function stardew:combat/apply_crit_damage
...
execute unless score #is_critical sd_temp matches 1 run particle minecraft:damage_indicator ~ ~1 ~ 0.3 0.5 0.3 0 3 force
execute unless score #is_critical sd_temp matches 1 run particle minecraft:crit ~ ~1 ~ 0.3 0.3 0.3 0.2 10 force
execute if score #is_critical sd_temp matches 1 run playsound minecraft:entity.player.attack.crit player @a ~ ~ ~ 1.5 1.2
playsound minecraft:entity.player.attack.sweep player @a ~ ~ ~ 0.7 1
playsound minecraft:entity.generic.hurt hostile @a ~ ~ ~ 0.7 1
...
execute store result score #player_x sd_temp run data get entity @p[tag=sd_using_whirlwind] Pos[0] 1000
execute store result score #player_z sd_temp run data get entity @p[tag=sd_using_whirlwind] Pos[2] 1000
