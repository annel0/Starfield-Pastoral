...
# 标记附近的目标（前方2.5格）
execute positioned ~ ~1.5 ~ positioned ^ ^ ^2.5 run tag @e[type=!#minecraft:non_attackable,type=!minecraft:player,type=!minecraft:item,distance=..2,sort=nearest,limit=1] add sd_star_flurry_check

# 没有目标 - 不进入冷却
execute unless entity @e[tag=sd_star_flurry_check] run tellraw @s {"text":"⚠ 附近没有目标！","color":"yellow"}
execute unless entity @e[tag=sd_star_flurry_check] run return 0
tag @e[tag=sd_star_flurry_check] remove sd_star_flurry_check
...
function stardew:combat/cooldown/set_star_flurry_cooldown_max

# 标记正在使用星流连斩技能冷却
tag @s add sd_using_star_flurry

# 标记目标（保留tag用于schedule）
execute positioned ~ ~1.5 ~ positioned ^ ^ ^2.5 run tag @e[type=!#minecraft:non_attackable,type=!minecraft:player,type=!minecraft:item,distance=..2,sort=nearest,limit=1] add sd_star_flurry_target
...
schedule function stardew:combat/weapon/star_flurry_attack1 1t
schedule function stardew:combat/weapon/star_flurry_attack2 4t
schedule function stardew:combat/weapon/star_flurry_attack3 7t
schedule function stardew:combat/weapon/star_flurry_attack4 10t
schedule function stardew:combat/weapon/star_flurry_attack5 13t

# 提示
title @s subtitle [{"text":"✨ 星流连斩","color":"aqua","bold":true},{"text":" - 5次连斩，每次30%伤害","color":"gray"}]
