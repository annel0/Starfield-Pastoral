...
# 【重构】统一的暴击计算系统
execute store result score #weapon_crit sd_temp run data get entity @p[tag=sd_using_shadow_step] SelectedItem.components."minecraft:custom_data".weapon_crit_chance 100
execute as @p[tag=sd_using_shadow_step] run function stardew:combat/calculate_crit
execute as @p[tag=sd_using_shadow_step] run function stardew:combat/apply_crit_damage
...
particle minecraft:smoke ~ ~1 ~ 0.4 0.6 0.4 0.1 20 force
particle minecraft:portal ~ ~1 ~ 0.3 0.5 0.3 0.3 15 force
execute if score #is_critical sd_temp matches 1 run playsound minecraft:entity.player.attack.crit player @a ~ ~ ~ 1.5 0.9
playsound minecraft:entity.player.attack.sweep player @a ~ ~ ~ 1.2 0.9
playsound minecraft:entity.generic.hurt hostile @a ~ ~ ~ 1 0.8
...
execute store result score #player_x sd_temp run data get entity @p[tag=sd_using_shadow_step] Pos[0] 1000
execute store result score #player_z sd_temp run data get entity @p[tag=sd_using_shadow_step] Pos[2] 1000
...
data modify entity @s Motion[1] set value 0.18
