# data/stardew/functions/crops/planting/plant_summer_spangle.mcfunction
...
execute unless score Global sd_season matches 2 run tellraw @s {"text":"夏季亮片只能在夏季种植","color":"red"}
execute unless score Global sd_season matches 2 run return 1

# 3. 种植 (召唤实体)
execute if block ~ ~ ~ minecraft:farmland align xyz run function stardew:crops/summon_entities/summer_spangle/summon_entities
execute if block ~ ~-1 ~ minecraft:farmland align xyz positioned ~ ~-1 ~ run function stardew:crops/summon_entities/summer_spangle/summon_entities
