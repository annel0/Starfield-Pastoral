...
# 5. 消耗种子与初始化
execute if entity @e[tag=init_crop,distance=..0.5] if entity @s[gamemode=!creative] run clear @s carrot_on_a_stick[minecraft:custom_model_data=2107] 1
...
execute as @e[tag=init_crop,distance=..0.5] run scoreboard players set @s sd_max_crop_age 10
execute as @e[tag=init_crop,distance=..0.5] run scoreboard players set @s sd_watered 0
# 添加季节标签
execute as @e[tag=init_crop,distance=..0.5] run tag @s add season_1
