...
public class NetWorldState : INetObject<NetFields>
{
	...
	public void MarkUnderConstruction(string builderName, Building building)
	{
		...
		{
			builders[builderName] = new BuilderData(building.buildingType.Value, daysUntilFinished, building.parentLocationName.Value, new Point(building.tileX.Value, building.tileY.Value), upgradeDays > 0 && buildDays <= 0);
		}
	}
