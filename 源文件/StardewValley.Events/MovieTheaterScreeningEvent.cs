	...
	public Event getMovieEvent(string movieId, List<List<Character>> player_and_guest_audience_groups, List<List<Character>> npcOnlyAudienceGroups, Dictionary<Character, MovieConcession> concessions_data = null)
	{
		...
		if (playerAndGuestAudienceGroups.Count > 0)
		{
			for (int i2 = 0; i2 < 8; i2++)
			{
				...
				{
					backRow[seat + j] = ((playerAndGuestAudienceGroups[whichGroup][j] is Farmer) ? ("farmer" + Utility.getFarmerNumberFromFarmer(playerAndGuestAudienceGroups[whichGroup][j] as Farmer)) : playerAndGuestAudienceGroups[whichGroup][j].name.Value);
					if (j > 0)
					{
						i2++;
					}
		...
		if (whichGroup < playerAndGuestAudienceGroups.Count)
		{
			...
			for (int i3 = 0; i3 < 4; i3++)
			{
				...
				for (int j2 = 0; j2 < playerAndGuestAudienceGroups[whichGroup].Count && seat2 + j2 < frontRow.Length; j2++)
				{
					frontRow[seat2 + j2] = ((playerAndGuestAudienceGroups[whichGroup][j2] is Farmer) ? ("farmer" + Utility.getFarmerNumberFromFarmer(playerAndGuestAudienceGroups[whichGroup][j2] as Farmer)) : playerAndGuestAudienceGroups[whichGroup][j2].name.Value);
					if (j2 > 0)
					{
						i3++;
					}
			...
			if (whichGroup < playerAndGuestAudienceGroups.Count)
			{
				...
				for (int i4 = 0; i4 < 6; i4++)
				{
					...
					for (int j3 = 0; j3 < playerAndGuestAudienceGroups[whichGroup].Count && seat3 + j3 < midRow.Length; j3++)
					{
						midRow[seat3 + j3] = ((playerAndGuestAudienceGroups[whichGroup][j3] is Farmer) ? ("farmer" + Utility.getFarmerNumberFromFarmer(playerAndGuestAudienceGroups[whichGroup][j3] as Farmer)) : playerAndGuestAudienceGroups[whichGroup][j3].name.Value);
						if (j3 > 0)
						{
							i4++;
						}
		...
		for (int i6 = 0; i6 < backRow.Length; i6++)
		{
			...
			Point seat5 = getBackRowSeatTileFromIndex(i6);
		...
		for (int i7 = 0; i7 < midRow.Length; i7++)
		{
			...
			sittingTogetherCount++;
			Point seat6 = getMidRowSeatTileFromIndex(i7);
		...
		for (int i8 = 0; i8 < frontRow.Length; i8++)
		{
			...
			sittingTogetherCount++;
			Point seat7 = getFrontRowSeatTileFromIndex(i8);
		...
		for (int i9 = 0; i9 < backRow.Length; i9++)
		{
			...
			{
				Point seat8 = getBackRowSeatTileFromIndex(i9);
				if (soFar == 1)
				{
		...
		for (int i10 = 0; i10 < midRow.Length; i10++)
		{
			...
			{
				Point seat9 = getMidRowSeatTileFromIndex(i10);
				if (soFar == 1)
				{
		...
		for (int i11 = 0; i11 < frontRow.Length; i11++)
		{
			...
			{
				Point seat10 = getFrontRowSeatTileFromIndex(i11);
				if (soFar == 1)
				{
		...
		for (int i12 = 0; i12 < backRow.Length; i12++)
		{
			...
			{
				Point seat11 = getBackRowSeatTileFromIndex(i12);
		...
		for (int i13 = 0; i13 < midRow.Length; i13++)
		{
			...
			{
				Point seat12 = getMidRowSeatTileFromIndex(i13);
		...
		for (int i14 = 0; i14 < frontRow.Length; i14++)
		{
			...
			{
				Point seat13 = getFrontRowSeatTileFromIndex(i14);
		...
		foreach (MovieScene scene in movieData.Scenes)
		{
			...
			for (int i16 = 0; i16 < responding_characters.Count; i16++)
			{
				...
				{
					if (!movie_reaction.ShouldApplyToMovie(movieData, MovieTheater.GetPatronNames(), MovieTheater.GetResponseForMovie(responding_characters[i16] as NPC)) || movie_reaction.SpecialResponses?.DuringMovie == null || (!(movie_reaction.SpecialResponses.DuringMovie.ResponsePoint == scene.ResponsePoint) && movie_reaction.Whitelist.Count <= 0))
					{
						continue;
					}
					if (!_whiteListDependencyLookup.ContainsKey(responding_characters[i16]))
					{
						_responseOrder[current_response_index] = responding_characters[i16];
						if (movie_reaction.Whitelist != null)
						{
							for (int j5 = 0; j5 < movie_reaction.Whitelist.Count; j5++)
							{
								...
								_whiteListDependencyLookup[white_list_character] = responding_characters[i16];
			...
			if (!found_reaction)
			{
				for (int i17 = 0; i17 < responding_characters.Count; i17++)
				{
					...
					{
						if (!movie_reaction2.ShouldApplyToMovie(movieData, MovieTheater.GetPatronNames(), MovieTheater.GetResponseForMovie(responding_characters[i17] as NPC)) || movie_reaction2.SpecialResponses?.DuringMovie == null || !(movie_reaction2.SpecialResponses.DuringMovie.ResponsePoint == current_response_index.ToString()))
						{
							continue;
						}
						if (!_whiteListDependencyLookup.ContainsKey(responding_characters[i17]))
						{
							_responseOrder[current_response_index] = responding_characters[i17];
							if (movie_reaction2.Whitelist != null)
							{
								for (int j6 = 0; j6 < movie_reaction2.Whitelist.Count; j6++)
								{
									...
									_whiteListDependencyLookup[white_list_character2] = responding_characters[i17];
		...
		{
			if (!_whiteListDependencyLookup.ContainsKey(responding_characters[i18]))
			{
				for (; _responseOrder.ContainsKey(current_response_index); current_response_index++)
				{
				}
				_responseOrder[current_response_index] = responding_characters[i18];
				current_response_index++;
			}
		}
