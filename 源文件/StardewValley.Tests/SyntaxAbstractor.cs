...
public class SyntaxAbstractor
{
	...
	{
		if (SyntaxHandlers.TryGetValue(baseAssetName, out var handler))
		{
			return handler;
		}
		int splitIndex = baseAssetName.LastIndexOf('/');
		if (splitIndex != -1 && SyntaxHandlers.TryGetValue(baseAssetName.Substring(0, splitIndex) + "/*", out handler))
		{
			return handler;
		}
		return null;
	...
	public string ExtractDialogueSyntax(string value)
	{
		...
		int index = 0;
		ExtractDialogueSyntaxImpl(value, '#', ref index, syntax);
	...
	public string ExtractEventSyntax(string value)
	{
		StringBuilder syntax = new StringBuilder();
		int index = 0;
		ExtractEventSyntaxImpl(value, ref index, syntax);
		return syntax.ToString();
	}
	...
	public string ExtractCreditsSyntax(string text)
	{
		...
		for (; index < text.Length; index++)
		{
			if (text[index] == '[')
			{
				...
				ExtractTagSyntax(text, ref index, syntax);
			}
			else
			{
				hasText = true;
	...
	public string ExtractMailSyntax(string text)
	{
		...
		for (; index < text.Length; index++)
		{
			...
			case '%':
				...
				EndTextContext(ref hasText, syntax);
				ExtractMailCommandSyntax(text, ref index, syntax);
				break;
			default:
				if (!hasText)
				{
	...
	public string ExtractDelimitedDataSyntax(string text, char delimiter, int[] textFields, int[] dialogueFields)
	{
		...
		if (dialogueFields != null)
		{
			...
			foreach (int index2 in array)
			{
				...
				{
					parts[index2] = ExtractDialogueSyntax(parts[index2]);
				}
			}
		}
			...
			if (!string.IsNullOrWhiteSpace(rawCommand))
			{
				...
				if (Event.TryResolveCommandName(commandName, out var actualName))
				{
					...
					case "End":
					{
						...
						{
							AppendEventCommandArg(syntax, args, 1);
							AppendEventCommandArg(syntax, args, 2);
							AppendEventCommandDialogueArg(syntax, args, 3);
							nextArg = 4;
						}
						break;
					}
					case "Message":
						AppendEventCommandDialogueArg(syntax, args, 1);
						nextArg = 2;
						break;
					case "Question":
						AppendEventCommandArg(syntax, args, 1);
						AppendEventCommandDialogueArg(syntax, args, 2);
						nextArg = 3;
						break;
					case "QuickQuestion":
					{
						...
						syntax.Append(" \"");
						AppendEventCommandDialogueArg(syntax, questionAndAnswerSplit, 0, prependSpace: true, quote: false);
						for (int k = 1; k < questionAndAnswerSplit.Length; k++)
						{
							syntax.Append('#');
							AppendEventCommandDialogueArg(syntax, questionAndAnswerSplit, k, prependSpace: false, quote: false);
						}
						for (int l = 1; l < masterSplit.Length; l++)
						{
							...
							syntax.Append("(break)");
							int tempIndex = 0;
							ExtractEventSyntaxImpl(masterSplit[l], ref tempIndex, syntax);
						}
						syntax.Append('"');
						nextArg = args.Length;
						break;
					...
					case "SplitSpeak":
					{
						...
						if (dialogues != null)
						{
							...
							for (int j = 0; j < dialogues.Length; j++)
							{
								...
								AppendEventCommandDialogueArg(syntax, dialogues, j, prependSpace: false, quote: false);
							}
							syntax.Append('"');
						}
						nextArg = 3;
				...
				{
					AppendEventCommandArg(syntax, args, nextArg);
				}
			}
	...
	private void AppendEventCommandDialogueArg(StringBuilder syntax, string[] args, int index, bool prependSpace = true, bool quote = true)
	{
		if (ArgUtility.HasIndex(args, index))
		{
			...
			ExtractDialogueSyntaxImpl(text, '/', ref tempIndex, syntax);
			if (quote)
			{
				syntax.Append('"');
			}
	...
	private string ExtractNpcGenderedDialogueSyntax(string text)
	{
		...
		for (int i = 0; i < parts.Length; i++)
		{
			parts[i] = ExtractDialogueSyntax(parts[i]);
		}
