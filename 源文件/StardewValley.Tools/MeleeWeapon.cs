...
public class MeleeWeapon : Tool
{
	...
	protected void ReloadData()
	{
		...
		{
			cachedData = data;
			Name = data.Name ?? itemData.InternalName;
	...
	public override void drawInMenu(SpriteBatch spriteBatch, Vector2 location, float scaleSize, float transparency, float layerDepth, StackDrawType drawStackNumber, Color color, bool drawShadow)
	{
		...
		spriteBatch.Draw(texture, location + ((type.Value == 1) ? new Vector2(38f, 25f) : new Vector2(32f, 32f)), sourceRect, color * transparency, 0f, new Vector2(8f, 8f), 4f * (scaleSize + addedScale), SpriteEffects.None, layerDepth);
	...
	public virtual int getItemLevel()
	{
		...
		weaponPoints += (float)(int)((double)((maxDamage.Value + minDamage.Value) / 2) * (1.0 + 0.03 * (double)(Math.Max(0, speed.Value) + ((type.Value == 1) ? 15 : 0))));
		weaponPoints += (float)(int)((double)(addedPrecision.Value / 2 + addedDefense.Value) + ((double)critChance.Value - 0.02) * 200.0 + (double)((critMultiplier.Value - 3f) * 6f));
		...
		weaponPoints += (float)(addedDefense.Value * 2);
	...
	public static Item attemptAddRandomInnateEnchantment(Item item, Random r, bool force = false, List<BaseEnchantment> enchantsToReroll = null)
	{
		...
		if (item is MeleeWeapon weapon && (force || r.NextBool()))
		{
			while (true)
			{
	...
	public override string getDescription()
	{
		if (!isScythe())
		{
			...
			if (speed.Value != 0)
			{
				b.AppendLine(Game1.content.LoadString("Strings\\StringsFromCSFiles:MeleeWeapon.cs.14134", (speed.Value > 0) ? "+" : "-", Math.Abs(speed.Value)));
			}
			...
			if (knockback.Value != defaultKnockBackForThisType(type.Value))
			{
				b.AppendLine(Game1.content.LoadString("Strings\\StringsFromCSFiles:MeleeWeapon.cs.14140", (knockback.Value > defaultKnockBackForThisType(type.Value)) ? "+" : "", (int)Math.Ceiling(Math.Abs(knockback.Value - defaultKnockBackForThisType(type.Value)) * 10f)));
			}
			return b.ToString();
		}
	...
	public virtual Rectangle getAreaOfEffect(int x, int y, int facingDirection, ref Vector2 tileLocation1, ref Vector2 tileLocation2, Rectangle wielderBoundingBox, int indexInCurrentAnimation)
	{
		...
		areaOfEffect.Inflate(addedAreaOfEffect.Value, addedAreaOfEffect.Value);
		return areaOfEffect;
	}
	...
	public virtual void FireProjectile(Farmer who)
	{
		...
		foreach (WeaponProjectile data in cachedData.Projectiles)
		{
			...
			if (data.Item != null)
			{
				shotItemId = ItemQueryResolver.TryResolveRandomItem(data.Item, new ItemQueryContext(who.currentLocation, who, null, $"weapon '{base.QualifiedItemId}' > projectile data '{data.Id}'"))?.QualifiedItemId;
				if (shotItemId == null)
				{
					continue;
				}
	...
	public virtual void DoDamage(GameLocation location, int x, int y, int facingDirection, int power, Farmer who)
	{
		...
		if (PlayUseSounds && !soundToPlay.Equals(""))
		{
	...
	public override void drawTooltip(SpriteBatch spriteBatch, ref int x, ref int y, SpriteFont font, float alpha, StringBuilder overrideText)
	{
		...
		if (speed.Value != ((type.Value == 2) ? (-8) : 0))
		{
			...
			bool negativeSpeed = (type.Value == 2 && speed.Value < -8) || (type.Value != 2 && speed.Value < 0);
			Color c = Game1.textColor;
			if (hasEnchantmentOfType<EmeraldEnchantment>())
			{
				c = new Color(0, 120, 120);
			}
			Utility.drawTextWithShadow(spriteBatch, Game1.content.LoadString("Strings\\UI:ItemHover_Speed", ((((type.Value == 2) ? (speed.Value - -8) : speed.Value) > 0) ? "+" : "") + ((type.Value == 2) ? (speed.Value - -8) : speed.Value) / 2), font, new Vector2(x + 16 + 52, y + 16 + 12), negativeSpeed ? Color.DarkRed : (c * 0.9f * alpha));
			y += (int)Math.Max(font.MeasureString("TT").Y, 48f);
		}
		if (addedDefense.Value > 0)
		{
			...
			Utility.drawTextWithShadow(spriteBatch, Game1.content.LoadString("Strings\\UI:ItemHover_DefenseBonus", addedDefense), font, new Vector2(x + 16 + 52, y + 16 + 12), c2 * 0.9f * alpha);
			y += (int)Math.Max(font.MeasureString("TT").Y, 48f);
		}
		float effectiveCritChance = critChance.Value;
		if (type.Value == 1)
		...
		if ((double)effectiveCritChance / 0.02 >= 1.100000023841858)
		{
			...
			Utility.drawTextWithShadow(spriteBatch, Game1.content.LoadString("Strings\\UI:ItemHover_CritChanceBonus", (int)Math.Round((double)(effectiveCritChance - 0.001f) / 0.02)), font, new Vector2(x + 16 + 52, y + 16 + 12), c3 * 0.9f * alpha);
			y += (int)Math.Max(font.MeasureString("TT").Y, 48f);
		}
		if ((double)(critMultiplier.Value - 3f) / 0.02 >= 1.0)
		{
	...
	{
		if (item is MeleeWeapon other_weapon && other_weapon.type.Value == type.Value)
		{
			return true;
		}
