...
public class Pan : Tool
{
	...
	public List<Item> getPanItems(GameLocation location, Farmer who)
	{
		...
		roll -= (double)(upgradeLevel.Value - 1) * 0.05;
		if (roll < 0.01)
		{
			whichOre = "386";
		}
		...
		orePieces += upgradeLevel.Value - 1;
		roll = r.NextDouble() - who.DailyLuck;
		int numRolls = upgradeLevel.Value;
		bool gotRing = false;
		double extraChance = (double)(upgradeLevel.Value - 1) * 0.04;
		...
		{
			roll = r.NextDouble() - who.DailyLuck;
			roll -= (double)(upgradeLevel.Value - 1) * 0.005;
			whichExtra = "382";
