...
public class Hoe : Tool
{
	...
	public override void DoFunction(GameLocation location, int x, int y, int power, Farmer who)
	{
		...
		foreach (Vector2 tileLocation in tileLocations)
		{
			...
			if (location is MineShaft && !location.IsTileOccupiedBy(tileLocation, CollisionMask.All, CollisionMask.None, useFarmerTile: true))
			{
				if (location.makeHoeDirt(tileLocation))
				{
					if (PlayUseSounds)
					{
			...
			else if (location.isTilePassable(new Location((int)tileLocation.X, (int)tileLocation.Y), Game1.viewport) && location.makeHoeDirt(tileLocation))
			{
				if (PlayUseSounds)
				{
					location.playSound("hoeHit", tileLocation);
