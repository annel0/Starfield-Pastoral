...
public class FishingRod : Tool
{
	...
	public override void DoFunction(GameLocation location, int x, int y, int power, Farmer who)
	{
		...
		if (isNibbling)
		{
			...
			o = location.getFish(fishingNibbleAccumulator, bait?.QualifiedItemId, clearWaterDistance + (splashPoint ? 1 : 0), who, baitPotency + (splashPoint ? 0.4 : 0.0), bobberTile);
		...
		else
		{
			...
			animations.Add(new TemporaryAnimatedSprite("TileSheets\\bobbers", sourceRect2, t2, 1, 0, bobber.Value + new Vector2(-32f, -48f), flicker: false, flipped: false, bobber.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, (float)Game1.random.Next(-20, 20) / 100f)
			{
	...
	{
		if (Game1.currentLocation.isTileBuildingFishable((int)bobberTile.X, (int)bobberTile.Y) && Game1.currentLocation.getBuildingAt(bobberTile) is FishPond pond && pond.currentOccupants.Value > 0)
		{
	...
	private void doPullFishFromWater(BinaryReader argReader)
	{
		...
		else
		{
			...
			if (numberOfFishCaught > 1)
			{
				for (int i2 = 1; i2 < numberOfFishCaught; i2++)
				{
					...
					gravity2 = 0.004f - (float)i2 * 0.0005f;
					...
					animations.Add(new TemporaryAnimatedSprite(sprite_sheet_name, sprite_rect, t, 1, 0, new Vector2(bobber.X, bobber.Y), flicker: false, flipped: false, bobber.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, 0f)
					{
						...
						endSound = "fishSlap",
						Parent = who.currentLocation,
						delayBeforeAnimationStart = (i2 - 1) * 100
					});
				}
			}
		}
	...
	public override void draw(SpriteBatch b)
	{
		...
		{
			Vector2 bobberPos2 = (isFishing ? bobber.Value : ((animations.Count > 0) ? (animations[0].position + new Vector2(0f, 4f * scale)) : Vector2.Zero));
			if (whichFish != null)
			{
			...
			Vector2 localBobber = Game1.GlobalToLocal(Game1.viewport, bobberPos2 + new Vector2(0f, -2.5f * scale + (float)((bobberBob == 1) ? 4 : 0)));
			...
			for (float i2 = 0f; i2 < 1f; i2 += 0.025f)
			{
				Vector2 current = Utility.GetCurvePoint(i2, v1, v2, v3, v4);
		...
		else
		{
			...
			if (fishIsObject)
			{
				...
				if (numberOfFishCaught > 1)
				{
					Utility.drawTinyDigits(numberOfFishCaught, b, Game1.GlobalToLocal(Game1.viewport, who.Position + new Vector2(-120f, -284f + yOffset2) + new Vector2(23f, 29f) * 4f), 3f, (float)playerStandingY / 10000f + 0.0001f + 0.061f, Color.White);
				}
				...
				if (numberOfFishCaught > 1)
				{
					...
					{
						b.Draw(texture, Game1.GlobalToLocal(Game1.viewport, who.Position + new Vector2(-(12 * j), -56f)), sourceRect, Color.White, (fishSize == -1 || whichFish.QualifiedItemId == "(O)800" || whichFish.QualifiedItemId == "(O)798" || whichFish.QualifiedItemId == "(O)149" || whichFish.QualifiedItemId == "(O)151") ? 0f : ((j == 2) ? ((float)Math.PI) : ((float)Math.PI * 4f / 5f)), new Vector2(8f, 8f), 3f, SpriteEffects.None, (float)playerStandingY / 10000f + 0.002f + 0.058f);
					}
				}
			}
			else
			...
			b.DrawString(Game1.smallFont, name, Game1.GlobalToLocal(Game1.viewport, who.Position + new Vector2(26f - Game1.smallFont.MeasureString(name).X / 2f, -278f + yOffset2)), bossFish ? new Color(126, 61, 237) : Game1.textColor, 0f, Vector2.Zero, 1f, SpriteEffects.None, (float)playerStandingY / 10000f + 0.002f + 0.06f);
			if (fishSize != -1)
			{
	...
	public override void tickUpdate(GameTime time, Farmer who)
	{
		...
		if (castingChosenCountdown > 0f)
		{
			...
			if (castingChosenCountdown <= 0f && who.CurrentTool != null)
			{
				...
				{
					float distance2 = 0f - Math.Max(128f, castingPower * (float)(getAddedDistance(who) + 3) * 64f);
					...
					if (who.IsLocalPlayer)
					{
						...
						bobber.Set(new Vector2(playerPixel2.X, (float)playerPixel2.Y - distance2));
					}
					Rectangle sourceRect2 = Game1.getSourceRectForStandardTileSheet(Game1.bobbersTexture, getBobberStyle(who), 16, 32);
					sourceRect2.Height = 16;
					animations.Add(new TemporaryAnimatedSprite("TileSheets\\bobbers", sourceRect2, t2, 1, 0, who.Position + new Vector2(0f, -96f), flicker: false, flipped: false, bobber.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, (float)Game1.random.Next(-20, 20) / 100f)
					{
						alphaFade = 0.0001f,
		...
		else if (isFishing)
		{
			...
			if (timeUntilFishingBite != -1f)
			{
				...
				if (fishingBiteAccumulator > timeUntilFishingBite)
				{
					...
					Game1.screenOverlayTempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(395, 497, 3, 8), new Vector2(playerPixel3.X - Game1.viewport.X, playerPixel3.Y - 128 - 8 - Game1.viewport.Y), flipped: false, 0.02f, Color.White)
					{
						scale = 5f,
						scaleChange = -0.01f,
		...
		if (num)
		{
			...
			while (Game1.random.NextDouble() <= (double)chance)
			{
				...
				case 0:
				{
					...
					possibles.Add(382);
					Item treasure3 = ItemRegistry.Create(Game1.random.ChooseFrom(possibles).ToString(), Game1.random.Next(2, 7) * ((!(Game1.random.NextDouble() < 0.05 + (double)who.luckLevel.Value * 0.015)) ? 1 : 2));
				...
				case 3:
					...
					case 1:
					{
						...
						if (clearWaterDistance >= 4)
						{
							treasures.Add(treasure2 = ItemRegistry.Create("(O)" + ((Game1.random.NextDouble() < 0.3) ? 82 : Game1.random.Choose(64, 60)), Game1.random.Next(1, 3)));
						}
						else if (clearWaterDistance >= 3)
						{
							treasures.Add(treasure2 = ItemRegistry.Create("(O)" + ((Game1.random.NextDouble() < 0.3) ? 84 : Game1.random.Choose(70, 62)), Game1.random.Next(1, 3)));
						}
						else
						{
							treasures.Add(treasure2 = ItemRegistry.Create("(O)" + ((Game1.random.NextDouble() < 0.3) ? 86 : Game1.random.Choose(66, 68)), Game1.random.Next(1, 3)));
						}
						if (Game1.random.NextDouble() < 0.028 * (double)((float)clearWaterDistance / 5f))
						{
							treasures.Add(treasure2 = ItemRegistry.Create("(O)72"));
						}
