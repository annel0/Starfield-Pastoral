...
public class GameLocation : INetObject<NetFields>, IEquatable<GameLocation>, IAnimalLocation, IHaveModData
{
	...
	public virtual void MakeMapModifications(bool force = false)
	{
		...
		case 7:
			if (value == "Sunroom")
			{
				...
				tileSheet.ImageSource = Path.Combine(imageDir, "CarolineGreenhouseTiles" + ((IsRainingHere() || Game1.timeOfDay > Game1.getTrulyDarkTime(this)) ? "_rainy" : ""));
	...
	public virtual void OnRemoved()
	{
		...
		{
			characters[i].OnLocationRemoved();
		}
	}
	...
	private Season? LoadSeasonOverride()
	{
		...
		if (map != null && FrameworkExtensions.TryGetValue(map.Properties, "SeasonOverride", out var propertyValue) && !string.IsNullOrWhiteSpace(propertyValue))
		{
	...
	public void cleanUpTileForMapOverride(Point tile, string exceptItemId)
	{
		...
		if (Objects.TryGetValue(tileVector, out var o) && (exceptItemId == null || !ItemRegistry.HasItemId(o, exceptItemId)))
		{
	...
	public void ApplyMapOverride(Map override_map, string override_key, Microsoft.Xna.Framework.Rectangle? source_rect = null, Microsoft.Xna.Framework.Rectangle? dest_rect = null, Action<Point> perTileCustomAction = null)
	{
		...
		foreach (TileSheet override_tile_sheet in override_map.TileSheets)
		{
			...
			{
				map_tilesheet = new TileSheet(GetAddedMapOverrideTilesheetId(override_key, override_tile_sheet.Id), map, override_tile_sheet.ImageSource, override_tile_sheet.SheetSize, override_tile_sheet.TileSize);
		...
		{
			map_width = Math.Max(map_width, map.Layers[k].LayerWidth);
			map_height = Math.Max(map_height, map.Layers[k].LayerHeight);
		}
		bool layersDirty = false;
		for (int l = 0; l < override_map.Layers.Count; l++)
		{
			Layer original_layer = map.GetLayer(override_map.Layers[l].Id);
			if (original_layer == null)
			{
				original_layer = new Layer(override_map.Layers[l].Id, map, new Size(map_width, map_height), override_map.Layers[l].TileSize);
					...
					if (paths != null)
					{
						for (int x = 0; x < paths.LayerWidth; x++)
						{
							for (int y = 0; y < paths.LayerHeight; y++)
							{
								...
								if (o2 == null && getTileIndexAt(x, y, "Paths") == 22 && isTileLocationOpen(location2) && !IsTileOccupiedBy(location2))
								{
	...
	public virtual Warp isCollidingWithDoors(Microsoft.Xna.Framework.Rectangle position, Character character = null)
	{
		for (int i = 0; i < 4; i++)
		{
			...
			foreach (KeyValuePair<Point, string> pair2 in doors.Pairs)
			{
			...
			foreach (Building building in buildings)
			{
				...
				{
					Warp warp2 = getWarpFromDoor(point, character);
					if (warp2 != null)
					{
						return warp2;
					}
	...
	public virtual bool CanWakeUpHere(Farmer who, Point? tile = null)
	{
		...
		{
			return TryGetMapPropertyAs("AllowWakeUpWithoutBed", out allowWakeUpWithoutBed, false) && allowWakeUpWithoutBed;
		}
		return true;
	}
	...
	public virtual Microsoft.Xna.Framework.Rectangle GetBuildableRectangle()
	{
		...
		{
			_buildableTileRect = (TryGetMapPropertyAs("ValidBuildRect", out Microsoft.Xna.Framework.Rectangle area, false) ? area : Microsoft.Xna.Framework.Rectangle.Empty);
	...
	public virtual bool isCollidingPosition(Microsoft.Xna.Framework.Rectangle position, xTile.Dimensions.Rectangle viewport, bool isFarmer, int damagesFarmer, bool glider, Character character, bool pathfinding, bool projectile = false, bool ignoreCharacterRequirement = false, bool skipCollisionEffects = false)
	{
		...
		if (IsOutOfBounds(position))
		{
			if (isFarmer && Game1.eventUp)
			{
				...
				if (flag.HasValue && flag == true && currentEvent.checkForCollision(position, (character as Farmer) ?? Game1.player))
				{
					return true;
				}
			}
		...
		else
		{
			...
			{
				if (_TestCornersTiles(nextTopRight, nextTopLeft, nextBottomRight, nextBottomLeft, nextTopMid, nextBottomMid, currentTopRight, currentTopLeft, currentBottomRight, currentBottomLeft, currentTopMid, currentBottomMid, nextLargerThanTile, delegate(Vector2 vector)
				{
		...
		if (!isFarmer || !Game1.player.isRafting)
		{
			if ((!(character is FarmAnimal animal3) || !animal3.IsActuallySwimming()) && _TestCornersTiles(nextTopRight, nextTopLeft, nextBottomRight, nextBottomLeft, nextTopMid, nextBottomMid, currentTopRight, currentTopLeft, currentBottomRight, currentBottomLeft, currentTopMid, currentBottomMid, nextLargerThanTile, delegate(Vector2 vector)
			{
	...
	public virtual void checkForMusic(GameTime time)
	{
		...
		if (IsOutdoors && !IsRainingHere() && !Game1.eventUp)
		{
			...
			else
			{
				...
				LocationContextData context3 = GetLocationContext();
	...
	public bool moveContents(int oldX, int oldY, int newX, int newY, string unlessItemId)
	{
		...
		bool movedAny = false;
		if (objects.TryGetValue(oldTile, out var o) && !objects.ContainsKey(newTile) && (unlessItemId == null || !ItemRegistry.HasItemId(o, unlessItemId)))
		{
			objects.Remove(oldTile);
			objects.Add(newTile, o);
			movedAny = true;
		}
		for (int i = furniture.Count - 1; i >= 0; i--)
		{
			Furniture f = furniture[i];
			if (f.boundingBox.Value.Intersects(oldPixelArea) && (unlessItemId == null || !ItemRegistry.HasItemId(f, unlessItemId)) && !furniture.Any((Furniture p) => p.boundingBox.Value.Intersects(newPixelArea)))
			{
	...
	public virtual void performTouchAction(string[] action, Vector2 playerStandingPosition)
	{
		...
		try
		{
			...
			else if (registeredTouchActions.TryGetValue(actionType, out actionHandler))
			{
			...
			else
			{
				...
				case 4:
					...
					case 'D':
					{
						...
						for (int i2 = 1; i2 < action.Length && (!(action[i2] == "Sebastian") || !IsGreenRainingHere() || Game1.year != 1); i2++)
						{
				...
				case 15:
				{
					...
					string message = doesTileHaveProperty((int)playerStandingPosition.X / 64, (int)playerStandingPosition.Y / 64, "LockedDoorMessage", "Back");
					if (message != null)
					{
				...
				case 13:
					if (actionType == "FaceDirection")
					{
						...
						{
							getCharacterFromName(npcName2)?.faceDirection(direction);
						}
					}
					break;
				case 14:
	...
	public virtual void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		for (int i2 = _activeTerrainFeatures.Count - 1; i2 >= 0; i2--)
		{
			TerrainFeature feature = _activeTerrainFeatures[i2];
			if (feature.tickUpdate(time))
			{
				terrainFeatures.Remove(feature.Tile);
			}
		}
		largeTerrainFeatures?.RemoveWhere((LargeTerrainFeature largeTerrainFeature) => largeTerrainFeature.tickUpdate(time));
		foreach (ResourceClump resourceClump in resourceClumps)
		{
		...
		foreach (KeyValuePair<long, FarmAnimal> kvp2 in tempAnimals)
		{
			if (kvp2.Value.updateWhenCurrentLocation(time, this))
			{
				animals.Remove(kvp2.Key);
			}
		}
		tempAnimals.Clear();
		foreach (Building building in buildings)
		...
		{
			if (!objects.TryGetValue(player_position + new Vector2(offsetX, offsetY), out var tileObject) || !tileObject.IsTextSign())
			{
				return null;
			}
			return tileObject;
	...
	public virtual void updateEvenIfFarmerIsntHere(GameTime time, bool ignoreWasUpdatedFlush = false)
	{
		...
		{
			Building containingBuilding = ParentBuilding;
		...
		for (int j = characters.Count - 1; j >= 0; j--)
		{
			if (j < characters.Count && characters[j] is Monster { IsMonster: not false, Health: >0 } monster && monster.TakesDamageFromHitbox(areaOfEffect))
			{
				...
				if (!monster.IsInvisible && !monster.isInvincible() && (isBomb || isProjectile || isMonsterDamageApplicable(who, monster) || isMonsterDamageApplicable(who, monster, horizontalBias: false)))
				{
					...
					if (monster.Health <= 0)
					{
						onMonsterKilled(who, monster, monsterBox, isBomb);
					}
		...
		for (int i = 0; i < numberOfNewWeeds; i++)
		{
			...
			while (numberOfTries < 3)
			{
				...
				else if (o == null && doesTileHaveProperty(xCoord, yCoord, "Diggable", "Back") != null && isTileLocationOpen(new Location(xCoord, yCoord)) && !IsTileOccupiedBy(location) && !isWaterTile(xCoord, yCoord))
				{
					...
					if (grass != -1 && GetSeason() != Season.Winter && name.Value == "Farm")
					{
	...
	public virtual void DayUpdate(int dayOfMonth)
	{
		...
		objects.Lock();
		foreach (KeyValuePair<Vector2, Object> pair3 in objects.Pairs)
		{
		...
		{
			debris.RemoveWhere((Debris debris) => debris.item == null && debris.itemId.Value == null);
		}
		...
		terrainFeatures.RemoveWhere((KeyValuePair<Vector2, TerrainFeature> keyValuePair) => keyValuePair.Value is HoeDirt hoeDirt && (hoeDirt.crop == null || hoeDirt.crop.forageCrop.Value) && (!objects.TryGetValue(keyValuePair.Key, out var value) || value == null || !value.IsSpawnedObject || !value.isForage()) && Game1.random.NextBool(GetDirtDecayChance(keyValuePair.Key)));
		lightLevel.Value = 0f;
	...
	{
		if (TryGetMapPropertyAs("DirtDecayChance", out double chance, false))
		{
			return chance;
		}
		if (IsGreenhouse)
	...
	public void addLightGlows()
	{
		...
		if (isOutdoors.Value || (Game1.timeOfDay >= night_tiles_time && !Game1.newDay))
		{
			return;
		}
		lightGlows.Clear();
	...
	{
		if (map != null && FrameworkExtensions.TryGetValue(Map.Properties, propertyName, out var rawValue))
		{
			if (rawValue == null)
			{
				return false;
	...
	public void setFireplace(bool on, int tileLocationX, int tileLocationY, bool playSound = true, int xOffset = 0, int yOffset = 0)
	{
		...
		string lightSourceId = $"{NameOrUniqueName}_Fireplace_{tileLocationX}_{tileLocationY}";
		if (on)
		{
	...
	public void addSquirrels(double chance, bool onlyIfOnScreen = false)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			if (!Utility.TryGetRandom(terrainFeatures, out var pos, out var feature) || !(feature is Tree tree) || tree.growthStage.Value < 5 || tree.stump.Value)
			{
				continue;
			}
	...
	public void setUpLocationSpecificFlair()
	{
		...
		case 17:
			...
			{
				Point position = new Point(8, 8);
				Game1.currentLightSources.Add(new LightSource("AbandonedJojaMart", 4, new Vector2(position.X * 64, position.Y * 64), 1f, LightSource.LightContext.None, 0L, NameOrUniqueName));
	...
	protected virtual void resetLocalState()
	{
		...
		if (!ignoreLights.Value)
		{
			...
			for (int i = 0; i < lights.Length; i += 3)
			{
				...
				Game1.currentLightSources.Add(new LightSource($"{lightIdPrefix}_{tile.X}_{tile.Y}", textureIndex, new Vector2(tile.X * 64 + 32, tile.Y * 64 + 32), 1f, LightSource.LightContext.MapLight, 0L, NameOrUniqueName));
			}
			if (!Game1.isTimeToTurnOffLighting(this) && !Game1.isRaining)
			{
				...
				for (int i2 = 0; i2 < windowLights.Length; i2 += 3)
				{
					...
					{
						LogMapPropertyError("WindowLight", windowLights, error2);
						continue;
					}
					Game1.currentLightSources.Add(new LightSource($"{lightIdPrefix}_{tile2.X}_{tile2.Y}_Window", textureIndex2, new Vector2(tile2.X * 64 + 32, tile2.Y * 64 + 32), 1f, LightSource.LightContext.WindowLight, 0L, NameOrUniqueName));
				}
				foreach (Vector2 v in lightGlows)
				{
					Game1.currentLightSources.Add(new LightSource($"{lightIdPrefix}_{v.X}_{v.Y}_Glow", 6, v, 1f, LightSource.LightContext.WindowLight, 0L, NameOrUniqueName));
				}
			}
		}
		if (isOutdoors.Value || treatAsOutdoors.Value)
		{
			...
			for (int i3 = 0; i3 < sounds.Length; i3 += 3)
			{
				...
				{
					LogMapPropertyError("BrookSounds", sounds, error3);
				}
				else
				{
		...
		foreach (KeyValuePair<Vector2, TerrainFeature> pair2 in terrainFeatures.Pairs)
		{
		...
		if (Game1.timeOfDay < night_tiles_time && (!IsRainingHere() || name.Equals("SandyHouse")))
		{
			...
			for (int i4 = 0; i4 < dayTiles.Length; i4 += 4)
			{
				...
				{
					LogMapPropertyError("DayTiles", dayTiles, error4);
				}
				else if (tileIndex != 720 || !Game1.MasterPlayer.mailReceived.Contains("pamHouseUpgrade"))
				{
					Tile tile4 = map.RequireLayer(layerId).Tiles[position.X, position.Y];
					if (tile4 != null)
					{
		...
		foreach (KeyValuePair<string, LightSource> pair in sharedLights.Pairs)
		{
			...
			foreach (string builder in Game1.netWorldState.Value.Builders.Keys)
			{
				...
				if (builderData.buildingLocation.Value == NameOrUniqueName && builderData.daysUntilBuilt.Value > 0)
				{
					...
					if (buildCharacter != null && buildCharacter.currentLocation.Equals(this))
					{
						...
						if (b != null)
						{
							temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(399, 262, (b.daysOfConstructionLeft.Value == 1) ? 29 : 9, 43), new Vector2(b.tileX.Value + b.tilesWide.Value / 2, b.tileY.Value + b.tilesHigh.Value / 2) * 64f + new Vector2(-16f, -144f), flipped: false, 0f, Color.White)
							{
								id = 16846,
								scale = 4f,
								interval = 999999f,
	...
	{
		if (identifier == null || !sharedLights.TryGetValue(identifier, out var light))
		{
			return null;
		}
		return light;
	...
	public virtual bool CanPlaceThisFurnitureHere(Furniture furniture)
	{
		...
		bool isIndoor = this is DecoratableLocation || !IsOutdoors;
		if (furniture.furniture_type.Value == 15)
		{
			if (!TryGetMapPropertyAs("AllowBeds", out bool allowBedsHere, false))
			{
				allowBedsHere = this is FarmHouse || this is IslandFarmHouse || (isIndoor && ParentBuilding != null);
			}
			if (!allowBedsHere)
			{
				return false;
	...
	public void playTerrainSound(Vector2 tileLocation, Character who = null, bool showTerrainDisturbAnimation = true)
	{
		...
		if (terrainFeatures.TryGetValue(tileLocation, out var terrainFeature) && terrainFeature is Flooring)
		{
			currentStep = ((Flooring)terrainFeatures[tileLocation]).getFootstepSound();
		}
		...
		else if (who != null && showTerrainDisturbAnimation && GetSeason() == Season.Winter && currentStep == "grassyStep")
		{
			...
			temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(247, 407, 6, 6), 2000f, 1, 10000, new Vector2(who.Position.X, who.Position.Y) + offset2, flicker: false, flipped: false, 0.0001f, 0.001f, Color.White, 1f, 0.01f, 0f, 0f));
		}
	...
	public bool checkForTerrainFeaturesAndObjectsButDestroyNonPlayerItems(int x, int y)
	{
		...
		if (objects.TryGetValue(v, out var tileObj))
		{
	...
	public virtual bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
	{
		...
		int tileIndex = getTileIndexAt(tileLocation, "Buildings", "untitled tile sheet");
		if (NameOrUniqueName == "SkullCave" && (tileIndex == 344 || tileIndex == 349))
		{
		...
		if (objects.TryGetValue(tilePos, out var obj))
		{
			...
			if (obj.Type != null || isErrorItem)
			{
				...
				else if (objects.TryGetValue(tilePos, out obj) && (obj.isSpawnedObject.Value || isErrorItem))
				{
					int oldQuality = obj.quality.Value;
					Random r = Utility.CreateDaySaveRandom(tilePos.X, tilePos.Y * 777f);
					if (obj.isForage())
					{
						obj.Quality = GetHarvestSpawnedObjectQuality(who, obj.isForage(), obj.TileLocation, r);
					}
					...
					if (who.couldInventoryAcceptThisItem(obj))
					{
						...
						if (who.professions.Contains(13) && r.NextDouble() < 0.2 && !obj.questItem.Value && who.couldInventoryAcceptThisItem(obj) && !isFarmBuildingInterior())
						{
							who.addItemToInventoryBool(obj.getOne());
		...
		if (tile == null || !FrameworkExtensions.TryGetValue(tile.Properties, "Action", out var action))
		{
	...
	public virtual bool LowPriorityLeftClick(int x, int y, Farmer who)
	{
		...
		for (int i2 = furniture.Count - 1; i2 >= 0; i2--)
		{
			Furniture furnitureItem2 = furniture[i2];
			if ((CanFreePlaceFurniture() || furnitureItem2.IsCloseEnoughToFarmer(who)) && furnitureItem2.isPassable() && furnitureItem2.boundingBox.Value.Contains(x, y) && furnitureItem2.canBeRemoved(who))
			{
	...
	public virtual bool shouldShadowBeDrawnAboveBuildingsLayer(Vector2 p)
	{
		...
		{
			int tileIndex = getTileIndexAt((int)p.X, (int)p.Y, "Buildings", "Town");
	...
	public void openDoor(Location tileLocation, bool playSound)
	{
		try
		{
			int tileIndex = getTileIndexAt(tileLocation.X, tileLocation.Y, "Buildings", "1");
	...
	public virtual bool performAction(string[] action, Farmer who, Location tileLocation)
	{
		...
		if (who.IsLocalPlayer)
		{
					...
					if (who?.ActiveObject?.QualifiedItemId == "(O)103")
					{
						...
						tempSprite.reachedStopCoordinate = delegate
						{
							...
							DelayedAction.functionAfterDelay(delegate
							{
								...
								for (int j = 0; j < 13; j++)
								{
									temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\terraria_cat", new Microsoft.Xna.Framework.Rectangle(134, (j == 12) ? 54 : 41, 21, 12), 9999f, 1, 1, tempSprite.position, flicker: false, flipped: false, 0.97f - (float)j * 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f)
									{
										xPeriodic = true,
			...
			case "LockedDoorWarp":
			{
				...
				who.faceGeneralDirection(new Vector2(tileLocation.X, tileLocation.Y) * 64f);
				lockedDoorWarp(tile3, locationName3, openTime3, closeTime3, npcName3, minFriendship);
				break;
			}
			case "ConditionalDoor":
				if (action.Length > 1 && !Game1.eventUp)
				{
					...
					string message2 = doesTileHaveProperty(tileLocation.X, tileLocation.Y, "LockedDoorMessage", "Buildings");
					if (message2 != null)
					{
			...
			case "BuildingSilo":
			{
				...
				if (activeObj?.QualifiedItemId == "(O)178")
				{
					...
					int stored = activeObj.Stack - tryToAddHay(activeObj.Stack);
					if (stored > 0)
					{
	...
	public bool characterDestroyObjectWithinRectangle(Microsoft.Xna.Framework.Rectangle rect, bool showDestroyedObject)
	{
		...
		for (int i2 = resourceClumps.Count - 1; i2 >= 0; i2--)
		{
			ResourceClump clump = resourceClumps[i2];
			if (clump.IsGreenRainBush() && clump.getBoundingBox().Intersects(rect) && clump.destroy(null, this, clump.Tile))
			{
				resourceClumps.RemoveAt(i2);
			}
		}
		return false;
	}
	...
	public virtual bool answerDialogueAction(string questionAndAnswer, string[] questionParams)
	{
		...
		case "SquidFestBooth_Rewards":
		{
			...
			if (availableRewards.Count > 0)
			{
				...
				foreach (string item in availableRewards)
				{
					switch (item)
					{
					case "12_0":
		...
		case "Blacksmith_Upgrade":
			...
			{
				NPC n2 = getCharacterFromName("Clint");
				if (n2 != null)
				{
		...
		case "telephone_SeedShop_CheckSeedStock":
			...
			{
				if (Utility.TryOpenShopMenu("SeedShop", null, true) && Game1.activeClickableMenu is ShopMenu menu)
				{
					menu.readOnly = true;
	...
	{
		if (!objects.TryGetValue(tileLocation, out var obj) || obj.fragility.Value == 2 || obj is Chest || !(obj.QualifiedItemId != "(BC)165"))
		{
			return;
		}
		bool remove = false;
	...
	public void addOneTimeGiftBox(Item i, int x, int y, int whichGiftBox = 2)
	{
		...
		{
			Vector2 v = new Vector2(x, y);
			if (!(overlayObjects.GetValueOrDefault(v) is Chest oldChest) || !(oldChest.mailToAddOnItemDump == id))
			{
	...
	public virtual string GetLocationContextId()
	{
		if (locationContextId == null)
		{
			...
			if (map != null && FrameworkExtensions.TryGetValue(map.Properties, "LocationContext", out var contextId))
			{
	...
	public virtual bool doesEitherTileOrTileIndexPropertyEqual(int xTile, int yTile, string propertyName, string layerName, string propertyValue)
	{
		...
		if (layer != null)
		{
			...
			if (tmp != null && FrameworkExtensions.TryGetValue(tmp.TileIndexProperties, propertyName, out var property) && property == propertyValue)
			{
				return true;
			}
			if (tmp != null && FrameworkExtensions.TryGetValue(layer.PickTile(new Location(xTile * 64, yTile * 64), Game1.viewport.Size).Properties, propertyName, out var property2) && property2 == propertyValue)
			{
				return true;
			}
		}
	...
	public virtual string doesTileHaveProperty(int xTile, int yTile, string propertyName, string layerName, bool ignoreTileSheetProperties = false)
	{
		...
		if (!buildingOnTile && map != null)
		{
			...
			if (tile != null)
			{
				if (FrameworkExtensions.TryGetValue(tile.Properties, propertyName, out var propertyValue))
				{
		...
		for (int i = 0; i < radius * 2 + 1; i++)
		{
			for (int j = 0; j < radius * 2 + 1; j++)
			{
				...
				else if (circleOutline[i, j])
				{
					...
					if (insideCircle <= 0)
					{
						if (destroyObjects)
						{
							...
							if (terrainFeatures.TryGetValue(currentTile, out var terrainFeature) && terrainFeature.performToolAction(null, radius / 2, currentTile))
							{
				...
				if (insideCircle >= 1)
				{
					...
					if (destroyObjects)
					{
						if (objects.TryGetValue(currentTile, out var obj2) && obj2.onExplosion(who))
						{
							destroyObject(currentTile, who);
						}
						if (terrainFeatures.TryGetValue(currentTile, out var terrainFeature2) && terrainFeature2.performToolAction(null, radius / 2, currentTile))
						{
							terrainFeatures.Remove(currentTile);
						}
					}
		...
		for (int k = 0; k < radius * 2 + 1; k++)
		{
			for (int l = 0; l < radius * 2 + 1; l++)
			{
				...
				else if (circleOutline[k, l])
				{
					...
					if (insideCircle <= 0 && !objects.ContainsKey(currentTile) && Game1.random.NextDouble() < 0.9 && !isTileHoeDirt(currentTile) && makeHoeDirt(currentTile))
					{
						checkForBuriedItem((int)currentTile.X, (int)currentTile.Y, explosion: true, detectOnly: false, who);
					}
				}
				if (insideCircle >= 1 && !objects.ContainsKey(currentTile) && Game1.random.NextDouble() < 0.9 && !isTileHoeDirt(currentTile) && makeHoeDirt(currentTile))
				{
					checkForBuriedItem((int)currentTile.X, (int)currentTile.Y, explosion: true, detectOnly: false, who);
				}
				currentTile.Y += 1f;
	...
	public virtual bool performOrePanTenMinuteUpdate(Random r)
	{
		if (Game1.MasterPlayer.mailReceived.Contains("ccFishTank") && !(this is Beach) && orePanPoint.Value.Equals(Point.Zero) && r.NextBool())
		{
			...
			{
				Point p = new Point(r.Next(0, Map.RequireLayer("Back").LayerWidth), r.Next(0, Map.RequireLayer("Back").LayerHeight));
				if (isOpenWater(p.X, p.Y) && FishingRod.distanceToLand(p.X, p.Y, this, landMustBeAdjacentToWalkableTile: true) <= 1 && !hasTileAt(p, "Buildings"))
				{
					if (Game1.player.currentLocation.Equals(this))
					{
						playSound("slosh");
	...
	{
		LocationData locationData = GetData();
		ItemQueryContext itemQueryContext = new ItemQueryContext(this, who, r, "location '" + NameOrUniqueName + "' > artifact spots");
	...
	public virtual string checkForBuriedItem(int xLocation, int yLocation, bool explosion, bool detectOnly, Farmer who)
	{
		...
		if (!isFarm.Value && isOutdoors.Value && GetSeason() == Season.Winter && r.NextDouble() < 0.08 && !explosion && !detectOnly && !(this is Desert))
		{
		...
		LocationData data = GetData();
		if (isOutdoors.Value && r.NextBool(data?.ChanceForClay ?? 0.03) && !explosion)
		{
			if (detectOnly)
			{
	...
	{
		Layer mapLayer = map?.RequireLayer(layer);
	...
	public StaticTile setMapTile(int tileX, int tileY, int index, string layer, string tileSheetId, string action = null, bool copyProperties = true)
	{
		...
		{
			tile = (StaticTile)(mapLayer.Tiles[tileX, tileY] = new StaticTile(mapLayer, map.RequireTileSheet(tileSheetId), BlendMode.Alpha, index));
	...
	public virtual void shiftContents(int dx, int dy, Func<Vector2, object, bool> where = null)
	{
		...
		foreach (KeyValuePair<Vector2, Object> v in list)
		{
			...
			{
				removeLightSource(v.Value.lightSource?.Id);
		...
		foreach (KeyValuePair<Vector2, TerrainFeature> v2 in list2)
		{
			...
			terrainFeatures.Add(tile2, v2.Value);
		}
		foreach (LargeTerrainFeature v3 in largeTerrainFeatures)
		{
		...
		foreach (Furniture v4 in furniture)
		{
	...
	public void moveFurniture(int oldX, int oldY, int newX, int newY)
	{
		...
		if (objects.TryGetValue(oldSpot, out var o))
		{
	...
	protected virtual bool breakStone(string stoneId, int x, int y, Farmer who, Random r)
	{
		...
		if ((isOutdoors.Value || treatAsOutdoors.Value) && experience == 0)
		{
	...
	public virtual void spawnObjects()
	{
		...
		if (data != null && numberOfSpawnedObjectsOnMap < data.MaxSpawnedForageAtOnce)
		{
			...
			if (possibleForage.Any())
			{
				...
				ItemQueryContext itemQueryContext = new ItemQueryContext(this, null, r, "location '" + NameOrUniqueName + "' > forage");
				for (int i = 0; i < numberToSpawn; i++)
				{
					for (int attempt = 0; attempt < 11; attempt++)
					{
						...
						Vector2 location = new Vector2(xCoord, yCoord);
						if (objects.ContainsKey(location) || IsNoSpawnTile(location) || doesTileHaveProperty(xCoord, yCoord, "Spawnable", "Back") == null || doesEitherTileOrTileIndexPropertyEqual(xCoord, yCoord, "Spawnable", "Back", "F") || !CanItemBePlacedHere(location) || hasTileAt(xCoord, yCoord, "AlwaysFront") || hasTileAt(xCoord, yCoord, "AlwaysFront2") || hasTileAt(xCoord, yCoord, "AlwaysFront3") || hasTileAt(xCoord, yCoord, "Front") || isBehindBush(location) || (!r.NextBool(0.1) && isBehindTree(location)))
						{
							continue;
						}
		...
		for (int i2 = positionOfArtifactSpots.Count - 1; i2 >= 0; i2--)
		{
			...
			{
				objects.Remove(positionOfArtifactSpots[i2]);
		...
		while (r.NextDouble() < chanceForNewArtifactAttempt)
		{
			...
			if (CanItemBePlacedHere(location2) && !IsTileOccupiedBy(location2) && !hasTileAt(xCoord2, yCoord2, "AlwaysFront") && !hasTileAt(xCoord2, yCoord2, "Front") && !isBehindBush(location2) && (doesTileHaveProperty(xCoord2, yCoord2, "Diggable", "Back") != null || (GetSeason() == Season.Winter && doesTileHaveProperty(xCoord2, yCoord2, "Type", "Back") != null && doesTileHaveProperty(xCoord2, yCoord2, "Type", "Back").Equals("Grass"))))
			{
				if (name.Equals("Forest") && xCoord2 >= 93 && yCoord2 <= 22)
				{
					continue;
				}
				objects.Add(location2, ItemRegistry.Create<Object>(r.NextBool(0.166) ? "(O)SeedSpot" : "(O)590"));
			}
		...
		for (int i = 0; i < numWeedsAndStones; i++)
		{
			...
			if (num == tile_is_diggable && !IsNoSpawnTile(checked_tile) && doesTileHaveProperty(checked_tile_x, checked_tile_y, "Type", "Back") != "Wood")
			{
				...
				else if (spawnFromOldWeeds)
				{
					...
					{
						is_tile_clear = !greenRain && getLargeTerrainFeatureAt(checked_tile_x, checked_tile_y) == null;
					}
				}
				if (is_tile_clear)
				{
		...
		if (Name == "Trailer_Big")
		{
			...
			if (trailer_events != null)
			{
				foreach (string trailer_event_key in trailer_events.Keys)
				{
					...
					if (!(name.Value == "Trailer_Big") || !events.ContainsKey(trailer_event_key))
					{
	...
	public virtual void checkForEvents()
	{
		if (Game1.killScreen && !Game1.eventUp)
		{
			...
			else if (this is IslandLocation)
			{
				...
				currentEvent = new Event(Game1.content.LoadString("Data\\Events\\IslandSouth:PlayerKilled", rescuer2, uniquemessage2, ArgUtility.EscapeQuotes(Game1.player.Name)));
			}
		...
		else
		{
			...
			if (!Game1.eventUp && currentEvent == null && Game1.farmEvent == null && !IsGreenRainingHere())
			{
				...
				try
				{
					if (!TryGetLocationEvents(out eventAssetName, out events2))
					{
						return;
					}
				}
				...
				if (events2 != null)
				{
					foreach (string eventKey in events2.Keys)
					{
						...
						if (!string.IsNullOrEmpty(eventId) && eventId != "-1" && IsValidLocationEvent(eventKey, events2[eventKey]))
						{
							currentEvent = new Event(events2[eventKey], eventAssetName, eventId);
							break;
						}
					}
	...
	public virtual void draw(SpriteBatch b)
	{
		...
		for (int i = 0; i < _currentLocationFarmersForDisambiguating.Count; i++)
		{
			...
			{
				Farmer farmer2 = _currentLocationFarmersForDisambiguating[i];
	...
	{
		Season season = GetSeason();
		terrainFeatures.RemoveWhere((KeyValuePair<Vector2, TerrainFeature> keyValuePair) => keyValuePair.Value.seasonUpdate(onLoad));
	...
	{
		int piecesToAdd = Math.Min(GetHayCapacity() - piecesOfHay.Value, num);
	...
	public bool buildStructure(Building building, Vector2 tileLocation, Farmer who, bool skipSafetyChecks = false)
	{
		if (!skipSafetyChecks)
		{
			...
			foreach (BuildingPlacementTile additionalPlacementTile2 in building.GetAdditionalPlacementTiles())
			{
				...
				foreach (Point point in additionalPlacementTile2.TileArea.GetPoints())
				{
					...
					if (!isBuildable(currentGlobalTilePosition2, onlyNeedsToBePassable))
					{
						return false;
					}
		...
		for (int k = 0; k < building.tilesHigh.Value; k++)
		{
			for (int l = 0; l < building.tilesWide.Value; l++)
			{
				...
				if (!(terrainFeatures.GetValueOrDefault(currentGlobalTilePosition3) is Flooring) || !(building.GetData()?.AllowsFlooringUnderneath ?? false))
				{
					terrainFeatures.Remove(currentGlobalTilePosition3);
				}
			}
		}
	...
	public virtual void updateWarps()
	{
		...
		{
			if (!FrameworkExtensions.TryGetValue(map.Properties, propertyName, out var warpsUnparsed) || warpsUnparsed == null)
			{
				continue;
			}
			...
			for (int j = 0; j < fields.Length; j += 5)
			{
				...
				{
					Game1.log.Warn($"Failed parsing {(npcOnly ? "NPC warp" : "warp")} '{string.Join(" ", fields.Skip(j))}' for location '{NameOrUniqueName}'. Warps must have five fields in the form 'fromX fromY toLocationName toX toY', but " + ((!hasFields) ? "got insufficient fields." : "got a non-numeric value for one of the X/Y position fields."));
				}
				else
				{
					warps.Add(new Warp(fromX, fromY, fields[j + 2], toX, toY, flipFarmer: false, npcOnly));
				}
			}
		}
		if (warps.Count > 0)
	...
	public void loadObjects()
	{
			...
			if (parentTreeMap != null)
			{
				foreach (KeyValuePair<Vector2, TerrainFeature> pair in parentTreeMap.terrainFeatures.Pairs)
				{
					if (pair.Value is Tree tree)
					{
						...
						if (pathsLayer.HasTileAt(p.X, p.Y) && TryGetTreeIdForTile(pathsLayer.Tiles[p.X, p.Y], out var _, out var _, out var _, out var _))
						{
		...
		if ((isOutdoors.Value || name.Equals("BathHouse_Entry") || treatAsOutdoors.Value || map.Properties.ContainsKey("forceLoadObjects")) && pathsLayer != null)
		{
	...
	public void loadPathsLayerObjectsInArea(int startingX, int startingY, int width, int height)
	{
		...
		for (int x = startingX; x < startingX + width; x++)
		{
			for (int y = startingY; y < startingY + height; y++)
			{
				...
				case 30:
				{
					if (Game1.startingCabins > 0 && FrameworkExtensions.TryGetValue(t.Properties, "Order", out var rawOrder) && int.Parse(rawOrder) <= Game1.startingCabins && ((t.TileIndex == 29 && !Game1.cabinsSeparate) || (t.TileIndex == 30 && Game1.cabinsSeparate)))
					{
	...
	public bool TryGetTreeIdForTile(Tile tile, out string treeId, out int? growthStageOnLoad, out int? growthStageOnRegrow, out bool isFruitTree)
	{
		...
		case 34:
		{
			if (!FrameworkExtensions.TryGetValue(tile.Properties, "SpawnTree", out var property))
			{
	...
	public void BuildStartingCabins()
	{
		if (_startingCabinLocations.Count > 0)
		{
			...
			{
				removeObjectsAndSpawned((int)startingCabinsInOrder[k].X, (int)startingCabinsInOrder[k].Y, 5, 3);
				removeObjectsAndSpawned((int)startingCabinsInOrder[k].X + 2, (int)startingCabinsInOrder[k].Y + 3, 1, 1);
				...
				b.load();
				buildStructure(b, startingCabinsInOrder[k], Game1.player, skipSafetyChecks: true);
	...
	public void updateDoors()
	{
		...
		for (int layerHeight = buildingLayer.LayerHeight; y < layerHeight; y++)
		{
			...
			for (int layerWidth = buildingLayer.LayerWidth; x < layerWidth; x++)
			{
				...
				if (tile == null || !FrameworkExtensions.TryGetValue(tile.Properties, "Action", out var door) || !door.Contains("Warp"))
				{
					continue;
				}
				...
				if (!(name.Value == "Mountain") || x != 8 || y != 20)
				{
	...
	private void changeMapProperties(string propertyName, string toAdd)
	{
		try
		{
			if (!FrameworkExtensions.TryGetValue(map.Properties, propertyName, out var oldValue))
			{
