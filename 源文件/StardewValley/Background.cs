...
public class Background
{
	...
	public virtual void draw(SpriteBatch b)
	{
		...
		if (backgroundImage == null)
		{
			...
			for (int i2 = tempSprites.Count - 1; i2 >= 0; i2--)
			{
				if (tempSprites[i2].update(Game1.currentGameTime))
				{
					tempSprites.RemoveAt(i2);
				}
				else
				{
					tempSprites[i2].draw(b);
				}
			}
			return;
		}
		Vector2 v = Vector2.Zero;
		Microsoft.Xna.Framework.Rectangle r2 = new Microsoft.Xna.Framework.Rectangle(0, 0, chunkWidth, chunkHeight);
		for (int j = 0; j < chunks.Length; j++)
		{
			v.X = position.X + (float)(j * chunkWidth % (chunksWide * chunkWidth)) * zoom;
			v.Y = position.Y + (float)(j * chunkWidth / (chunksWide * chunkWidth) * chunkHeight) * zoom;
			if (backgroundImage == null)
			{
				b.Draw(Game1.staminaRect, new Microsoft.Xna.Framework.Rectangle((int)v.X, (int)v.Y, Game1.viewport.Width, Game1.viewport.Height), r2, c, 0f, Vector2.Zero, SpriteEffects.None, 0f);
				continue;
			}
			r2.X = chunks[j] * chunkWidth % backgroundImage.Width;
			r2.Y = chunks[j] * chunkWidth / backgroundImage.Width * chunkHeight;
			b.Draw(backgroundImage, v, r2, c, 0f, Vector2.Zero, zoom, SpriteEffects.None, 0f);
		}
	}
}
