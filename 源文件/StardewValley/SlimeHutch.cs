...
public class SlimeHutch : DecoratableLocation
{
	...
	{
		int matesLeft = slimeMatingsLeft.Value;
	...
	public override void DayUpdate(int dayOfMonth)
	{
		...
		{
			if (characters.Count > 1 && !isFull() && characters[Game1.random.Next(characters.Count)] is GreenSlime mate && mate.ageUntilFullGrown.Value <= 0)
			{
	...
	protected override void resetLocalState()
	{
		...
		if (objects.TryGetValue(new Vector2(1f, 4f), out var tileObj))
		{
			tileObj.Fragility = 0;
		}
	}
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		for (int i = 0; i < waterSpots.Length; i++)
		{
			int tileIndex = (waterSpots[i] ? 2135 : 2134);
			setMapTile(16, 6 + i, tileIndex, "Buildings", "untitled tile sheet");
		}
	}
}
