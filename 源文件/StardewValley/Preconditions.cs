...
public class Preconditions
{
	...
	public static bool SawEvent(GameLocation location, string eventId, string[] args)
	{
		...
		{
			if (!ArgUtility.TryGet(args, i, out var id, out var error, allowBlank: false, "string id"))
			{
	...
	{
		if (!ArgUtility.TryGetOptional(args, 1, out var petType, out var error, null, allowBlank: false, "string petType"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
		if (!Game1.player.hasPet())
		{
			return petType?.EqualsIgnoreCase(Game1.player.whichPetType) ?? true;
		}
		return false;
	}
	...
	{
		if (!ArgUtility.TryGet(args, 1, out var mailId, out var error, allowBlank: false, "string mailId"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	{
		if (!ArgUtility.TryGet(args, 1, out var worldStateId, out var error, allowBlank: false, "string worldStateId"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	{
		if (!ArgUtility.TryGetInt(args, 1, out var minMoney, out var error, "int minMoney"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	{
		if (!ArgUtility.TryGetInt(args, 1, out var minFreeSpots, out var error, "int minFreeSpots"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	{
		if (!ArgUtility.TryGet(args, 1, out var npcName, out var error, allowBlank: false, "string npcName"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	{
		if (!ArgUtility.TryGetInt(args, 1, out var minDaysPlayed, out var error, "int minDaysPlayed"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	public static bool Friendship(GameLocation location, string eventId, string[] args)
	{
		...
		{
			if (!ArgUtility.TryGet(args, i, out var npcName, out var error, allowBlank: false, "string npcName") || !ArgUtility.TryGetInt(args, i + 1, out var minPoints, out error, "int minPoints"))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
	...
	{
		if (!ArgUtility.TryGetFloat(args, 1, out var probability, out var error, "float probability"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	public static bool Shipped(GameLocation location, string eventId, string[] args)
	{
		for (int i = 1; i < args.Length; i += 2)
		{
			if (!ArgUtility.TryGet(args, i, out var itemId, out var error, allowBlank: false, "string itemId") || !ArgUtility.TryGetInt(args, i + 1, out var minShipped, out error, "int minShipped"))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
	...
	{
		if (!ArgUtility.TryGetInt(args, 1, out var secretNoteId, out var error, "int secretNoteId"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	public static bool ChoseDialogueAnswers(GameLocation location, string eventId, string[] args)
	{
		for (int i = 1; i < args.Length; i++)
		{
			if (!ArgUtility.TryGet(args, i, out var answerId, out var error, allowBlank: false, "string answerId"))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
	...
	{
		if (!ArgUtility.TryGetInt(args, 1, out var minWalnuts, out var error, "int minWalnuts"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	{
		if (!ArgUtility.TryGetOptionalInt(args, 1, out var minUpgradeLevel, out var error, 2, "int minUpgradeLevel"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	{
		if (!ArgUtility.TryGetInt(args, 1, out var minTime, out var error, "int minTime") || !ArgUtility.TryGetInt(args, 2, out var maxTime, out error, "int maxTime"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	{
		if (!ArgUtility.TryGet(args, 1, out var weather, out var error, allowBlank: false, "string weather"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
		if (!(weather == "rainy"))
	...
	public static bool DayOfWeek(GameLocation location, string eventId, string[] args)
	{
		...
		for (int i = 1; i < args.Length; i++)
		{
			if (!ArgUtility.TryGet(args, i, out var rawDayName, out var error, allowBlank: false, "string rawDayName"))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
	...
	public static bool Season(GameLocation location, string eventId, string[] args)
	{
		for (int i = 1; i < args.Length; i++)
		{
			if (!ArgUtility.TryGetEnum<Season>(args, 1, out var season, out var error, "Season season"))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
	...
	{
		if (!ArgUtility.TryGetOptionalInt(args, 1, out var minTimes, out var error, 1, "int minTimes"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	{
		if (!ArgUtility.TryGetInt(args, 1, out var desiredYear, out var error, "int desiredYear"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
		if (desiredYear != 1)
	...
	{
		if (!ArgUtility.TryGet(args, 1, out var gender, out var error, allowBlank: false, "string gender"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	{
		if (!ArgUtility.TryGet(args, 1, out var itemId, out var error, allowBlank: false, "string itemId"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	public static bool Tile(GameLocation location, string eventId, string[] args)
	{
		...
		{
			if (!ArgUtility.TryGetPoint(args, i, out var tile, out var error, "Point tile"))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
			if (tile == actualTile)
	...
	{
		if (!ArgUtility.TryGet(args, 1, out var id, out var error, allowBlank: false, "string id"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	{
		if (!ArgUtility.TryGet(args, 1, out var mailId, out var error, allowBlank: false, "string mailId") || !ArgUtility.TryGetOptionalBool(args, 2, out var inMailboxToday, out error, defaultValue: false, "bool inMailboxToday"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
		if (inMailboxToday)
	...
	public static bool DayOfMonth(GameLocation location, string eventId, string[] args)
	{
		...
		for (int i = 1; i < args.Length; i++)
		{
			if (!ArgUtility.TryGetInt(args, i, out var day, out var error, "int day"))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
	...
	{
		if (!ArgUtility.TryGetInt(args, 1, out var numberOfDays, out var error, "int numberOfDays"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
	...
	{
		string query = ArgUtility.UnsplitQuoteAware(args, ' ', 1);
	...
	{
		if (!ArgUtility.TryGet(args, 1, out var name, out var error, allowBlank: false, "string name") || !ArgUtility.TryGetInt(args, 2, out var minSkillLevel, out error, "int minSkillLevel"))
		{
			return Event.LogPreconditionError(location, eventId, args, error);
		}
