...
public class Farmer : Character, IComparable
{
	...
	public bool hasPendingCompletedQuests
	{
		get
		{
			...
			foreach (Quest quest2 in questLog)
			{
	...
	public void LearnDefaultRecipes()
	{
		...
		{
			if (!cookingRecipes.ContainsKey(recipe2.Key) && ArgUtility.Get(recipe2.Value.Split('/'), 3) == "default")
			{
				cookingRecipes.Add(recipe2.Key, 0);
			}
		}
	}
	...
	public void AddMissedMailAndRecipes()
	{
		...
		{
			if (CraftingRecipe.TryParseLevelRequirement(v.Key, v.Value, isCooking: true, out var reqSkillNumber, out var minLevel) && GetUnmodifiedSkillLevel(reqSkillNumber) >= minLevel)
			{
				cookingRecipes.TryAdd(v.Key, 0);
				addRobinKitchenLetter = true;
			}
		}
		foreach (KeyValuePair<string, string> v2 in CraftingRecipe.craftingRecipes)
		{
			if (CraftingRecipe.TryParseLevelRequirement(v2.Key, v2.Value, isCooking: false, out var reqSkillNumber2, out var minLevel2) && GetUnmodifiedSkillLevel(reqSkillNumber2) >= minLevel2)
			{
				craftingRecipes.TryAdd(v2.Key, 0);
			}
		}
		if (addRobinKitchenLetter && !hasOrWillReceiveMail("robinKitchenLetter"))
		{
	...
	private static void removeLowestUpgradeLevelTool(List<Item> items, Type toolType)
	{
		...
		{
			if (item is Tool tool && tool.GetType() == toolType && (lowestItem == null || tool.upgradeLevel.Value < lowestItem.upgradeLevel.Value))
			{
				lowestItem = tool;
			}
		}
	...
	public Point getMailboxPosition()
	{
		...
		{
			if (b.isCabin && b.HasIndoorsName(homeLocation.Value))
			{
	...
	public void shiftToolbar(bool right)
	{
		...
		{
			IList<Item> toMove2 = Items.GetRange(Items.Count - 12, 12);
			for (int i = 0; i < Items.Count - 12; i++)
			{
				toMove2.Add(Items[i]);
			}
			Items.OverwriteWith(toMove2);
		}
		netItemStowed.Set(newValue: false);
	...
	public virtual void gainExperience(int which, int howMuch)
	{
		...
		if (newLevel != -1)
		{
			...
			{
			case 0:
				oldLevel = farmingLevel.Value;
				farmingLevel.Value = newLevel;
				break;
			case 3:
				oldLevel = miningLevel.Value;
				miningLevel.Value = newLevel;
				break;
			case 1:
				oldLevel = fishingLevel.Value;
				fishingLevel.Value = newLevel;
				break;
			case 2:
				oldLevel = foragingLevel.Value;
				foragingLevel.Value = newLevel;
				break;
			case 5:
				oldLevel = luckLevel.Value;
				luckLevel.Value = newLevel;
				break;
			case 4:
				oldLevel = combatLevel.Value;
	...
	public int getEffectiveSkillLevel(int whichSkill)
	{
		...
		int[] effectiveSkillLevels = new int[6] { farmingLevel.Value, fishingLevel.Value, foragingLevel.Value, miningLevel.Value, combatLevel.Value, luckLevel.Value };
	...
	public void onGiftGiven(NPC npc, Object item)
	{
		...
		if (!giftedItems.TryGetValue(npc.name.Value, out var giftData))
		{
			giftData = (giftedItems[npc.name.Value] = new SerializableDictionary<string, int>());
		}
	...
	public void showNotCarrying()
	{
		if (!FarmerSprite.PauseForSingleAnimation && !isMoving())
		{
			bool canOnlyWalk = this.canOnlyWalk || bathingClothes.Value || onBridge.Value;
			switch (FacingDirection)
			{
			case 0:
	...
	public bool hasDailyQuest()
	{
		...
		{
			if (questLog[i].dailyQuest.Value)
			{
				return true;
			}
		}
	...
	public void showToolUpgradeAvailability()
	{
		...
		if (!(toolBeingUpgraded != null) || daysLeftForToolUpgrade.Value > 0 || toolBeingUpgraded.Value == null || Utility.isFestivalDay() || (!(Game1.shortDayNameFromDayOfSeason(day) != "Fri") && hasCompletedCommunityCenter() && !Game1.isRaining) || hasReceivedToolUpgradeMessageYet)
		{
			return;
		}
		if (Game1.newDay)
	...
	public void dayupdate(int timeWentToSleep)
	{
		...
		Stamina = MaxStamina;
		bool wasExhausted = exhausted.Value;
		if (wasExhausted)
		{
			exhausted.Value = false;
			Stamina = MaxStamina / 2 + 1;
		}
		int bedTime = ((timeWentToBed.Value == 0) ? timeWentToSleep : timeWentToBed.Value);
		if (bedTime > 2400)
		{
		...
		if (timeWentToSleep < 2700 && oldStamina > Stamina && !wasExhausted)
		{
			Stamina = oldStamina;
		}
		health = maxHealth;
		...
		foreach (string previousEvent in previousActiveDialogueEvents.Keys)
		{
			...
			{
				activeDialogueEvents[previousEvent + "_memory_oneday"] = 4;
			}
			if (previousActiveDialogueEvents[previousEvent] == 7)
			{
				activeDialogueEvents[previousEvent + "_memory_oneweek"] = 4;
			}
			if (previousActiveDialogueEvents[previousEvent] == 14)
			{
				activeDialogueEvents[previousEvent + "_memory_twoweeks"] = 4;
			}
			if (previousActiveDialogueEvents[previousEvent] == 28)
			{
				activeDialogueEvents[previousEvent + "_memory_fourweeks"] = 4;
			}
			if (previousActiveDialogueEvents[previousEvent] == 56)
			{
				activeDialogueEvents[previousEvent + "_memory_eightweeks"] = 4;
			}
			if (previousActiveDialogueEvents[previousEvent] == 104)
			{
				activeDialogueEvents[previousEvent + "_memory_oneyear"] = 4;
			}
		}
		hasMoved = false;
	...
	public void doDivorce()
	{
		...
		if (spouse != null)
		{
			...
			if (currentSpouse != null)
			{
				...
				if (friendshipData.TryGetValue(currentSpouse.name.Value, out var friendship))
				{
					friendship.Points = 0;
		...
		if (((!(currentSpouse?.isRoommate())) ?? true) && !autoGenerateActiveDialogueEvent("divorced_once"))
		{
	...
	public static void showReceiveNewItemMessage(Farmer who, Item item, int countAdded)
	{
		...
		if (message == null)
		{
			message = ((item.TryGetTempData<bool>("FromStarterGiftBox", out var fromGiftbox) && fromGiftbox && item.QualifiedItemId == "(O)472" && countAdded == 15) ? Game1.content.LoadString("Strings\\StringsFromCSFiles:Farmer.cs.1918") : ((!item.HasContextTag("book_item")) ? ((countAdded > 1) ? Game1.content.LoadString("Strings\\StringsFromCSFiles:Farmer.cs.1922", countAdded, item.DisplayName) : Game1.content.LoadString("Strings\\StringsFromCSFiles:Farmer.cs.1919", item.DisplayName, Lexicon.getProperArticleForWord(item.DisplayName))) : Game1.content.LoadString("Strings\\1_6_Strings:FoundABook", item.DisplayName)));
		}
		Game1.drawObjectDialogue(new List<string> { message });
	...
	public static void showHoldingItem(Farmer who, Item item)
	{
		if (!(item is SpecialItem specialItem))
		{
				...
				if (!(item is Hat))
				{
					if (!(item is Furniture))
					{
						if (!(item is Object) && !(item is Tool))
						{
							...
							{
								TemporaryAnimatedSprite sprite2 = new TemporaryAnimatedSprite(null, default(Microsoft.Xna.Framework.Rectangle), 2500f, 1, 0, who.Position + new Vector2(-4f, -124f), flicker: false, flipped: false)
								{
									motion = new Vector2(0f, -0.1f),
									layerDepth = 1f
								};
						...
						{
							TemporaryAnimatedSprite sprite3 = new TemporaryAnimatedSprite(null, default(Microsoft.Xna.Framework.Rectangle), 2500f, 1, 0, who.Position + new Vector2(0f, -188f), flicker: false, flipped: false)
							{
								motion = new Vector2(0f, -0.1f),
								layerDepth = 1f
							};
					...
					{
						TemporaryAnimatedSprite sprite5 = new TemporaryAnimatedSprite(null, default(Microsoft.Xna.Framework.Rectangle), 2500f, 1, 0, Vector2.Zero, flicker: false, flipped: false)
						{
							motion = new Vector2(0f, -0.1f),
							layerDepth = 1f
						};
						sprite5.CopyAppearanceFromItemId(item.QualifiedItemId);
						sprite5.initialPosition = (sprite5.position = who.Position + new Vector2(32 - sprite5.sourceRect.Width / 2 * 4, -188f));
						Game1.currentLocation.temporarySprites.Add(sprite5);
					}
				}
				else
				{
					TemporaryAnimatedSprite sprite6 = new TemporaryAnimatedSprite(null, default(Microsoft.Xna.Framework.Rectangle), 2500f, 1, 0, who.Position + new Vector2(-8f, -124f), flicker: false, flipped: false, 1f, 0f, Color.White, 1f, 0f, 0f, 0f)
					{
						motion = new Vector2(0f, -0.1f)
					};
			...
			{
				TemporaryAnimatedSprite sprite7 = new TemporaryAnimatedSprite(null, default(Microsoft.Xna.Framework.Rectangle), 2500f, 1, 0, who.Position + new Vector2(0f, -124f), flicker: false, flipped: false, 1f, 0f, Color.White, 1f, 0f, 0f, 0f)
				{
					motion = new Vector2(0f, -0.1f)
				};
	...
	public void resetFriendshipsForNewDay()
	{
		foreach (string name in friendshipData.Keys)
		{
			...
			if (n != null)
			{
				if (n != null && n.datable.Value && !friendshipData[name].IsDating() && !n.isMarried())
				{
					single = true;
				}
	...
	{
		if (!friendshipData.TryGetValue(name, out var friendship) && NPC.CanSocializePerData(name, base.currentLocation))
		{
	...
	{
		FarmerSprite.AnimationFrame[] animationFrames = new FarmerSprite.AnimationFrame[1]
		{
			new FarmerSprite.AnimationFrame(Convert.ToInt32(frame), 100, secondaryArm: false, flip)
		};
		FarmerSprite.setCurrentAnimation(animationFrames);
	...
	public bool addItemToInventoryBool(Item item, bool makeActiveObject = false)
	{
		...
		if (IsLocalPlayer)
		{
			...
			if (makeActiveObject && success && !(item is SpecialItem) && remainder != null && item.Stack <= 1)
			{
				...
				{
					Item i = Items[currentToolIndex.Value];
					Items[currentToolIndex.Value] = Items[newItemPosition];
	...
	public void addItemByMenuIfNecessaryElseHoldUp(Item item, ItemGrabMenu.behaviorOnItemSelect itemSelectedCallback = null, bool forceQueue = false)
	{
		...
		{
			holdUpItemThenMessage(item, countAdded);
		}
	}
	...
	public void SortSeatExitPositions(List<Vector2> list, Vector2 a, Vector2 b, Vector2 c)
	{
		...
		mouse_pos /= 64f;
		List<Vector2> exit_positions = new List<Vector2> { a, b, c };
	...
	public bool couldInventoryAcceptThisItem(string id, int stack, int quality = 0)
	{
		...
		default:
		{
			for (int i = 0; i < maxItems.Value; i++)
			{
				if (Items.Count > i && (Items[i] == null || (Items[i].Stack + stack <= Items[i].maximumStackSize() && Items[i].QualifiedItemId == itemData.QualifiedItemId && Items[i].quality.Value == quality)))
				{
					return true;
				}
			}
	...
	public int freeSpotsInInventory()
	{
		...
		if (slotsUsed >= maxItems.Value)
		{
			return 0;
		}
		return maxItems.Value - slotsUsed;
	}
	...
	public void removeFirstOfThisItemFromInventory(string itemId, int count = 1)
	{
		...
		{
			int toRemove = Math.Min(remaining, ActiveObject.Stack);
			remaining -= toRemove;
			if (ActiveObject.ConsumeStack(toRemove) == null)
			{
				ActiveObject = null;
				showNotCarrying();
			}
	...
	public void ConvertClothingOverrideToClothesItems()
	{
		...
		if (IsOverridingShirt(out var shirtId))
		{
			...
			{
				Clothing clothes2 = new Clothing(shirtId);
				Equip(clothes2, shirtItem);
			}
			shirt.Value = "-1";
		}
	}
	...
	public void changeFriendship(int amount, NPC n)
	{
		...
		if (friendshipData.TryGetValue(n.Name, out var friendship))
		{
			...
			if (n.datable.Value && friendship.Points >= 2000 && !hasOrWillReceiveMail("Bouquet"))
			{
				Game1.addMailForTomorrow("Bouquet");
			}
			if (n.datable.Value && friendship.Points >= 2500 && !hasOrWillReceiveMail("SeaAmulet"))
			{
	...
	public string getPetName()
	{
		...
		{
			foreach (NPC n2 in Utility.getHomeOfFarmer(allFarmer).characters)
			{
				if (n2 is Pet)
				{
					return n2.Name;
	...
	public static void drinkGlug(Farmer who)
	{
		...
		if (Game1.currentLocation == who.currentLocation)
		{
			Game1.playSound((who.itemToEat is Object o && o.preserve.Value == Object.PreserveType.Pickle) ? "eat" : "gulp");
		}
	...
	public void toolPowerIncrease()
	{
		...
		int pitch = Utility.CreateRandom(Game1.dayOfMonth, (double)base.Position.X * 1000.0, base.Position.Y).Next(12, 16) * 100 + toolPower.Value * 100;
	...
	public void takeDamage(int damage, bool overrideParry, Monster damager)
	{
		...
		bool playerParryable = CurrentTool is MeleeWeapon && ((MeleeWeapon)CurrentTool).isOnSpecial && ((MeleeWeapon)CurrentTool).type.Value == 3;
		...
		else
		{
			...
			if (damager != null && isWearingRing("839"))
			{
				...
				_ = Utility.getAwayFromPlayerTrajectory(monsterBox, this) / 2f;
				...
				int multiplier = getNumberOfWornRingsWithID("839");
				damageToMonster *= multiplier;
				base.currentLocation?.damageMonster(monsterBox, damageToMonster, damageToMonster + 1, isBomb: false, this);
			}
	...
	public float getMovementSpeed()
	{
		...
		float movementSpeed2 = Math.Max(1f, (float)base.speed + (Game1.eventUp ? ((float)Math.Max(0, Game1.CurrentEvent.farmerAddedSpeed - 2)) : (addedSpeed + (isRidingHorse() ? 5f : temporarySpeedBuff))));
		if (movementDirections.Count > 1)
		{
	...
	{
		Farmer spouse = Game1.GetPlayer(otherPlayerID);
		if (spouse != null)
		{
	...
	public void updateMovementAnimation(GameTime time)
	{
		...
		else
		{
			...
			running = Math.Abs(speed - 5f) < Math.Abs(speed - 2f) && !bathingClothes.Value && !onBridge.Value;
			if (!num)
			{
				FarmerSprite.StopAnimation();
			}
	...
	public void doneEating()
	{
		...
		if (consumed.QualifiedItemId == "(O)434")
		{
			...
			{
				mid = (favoriteThing.Value.Contains("Stardew") ? Game1.content.LoadString("Strings\\StringsFromCSFiles:Game1.cs.3097") : ((!favoriteThing.Equals("ConcernedApe")) ? (mid + favoriteThing.Value) : Game1.content.LoadString("Strings\\StringsFromCSFiles:Game1.cs.3099")));
			}
		...
		else
		{
			...
			switch (consumed.QualifiedItemId)
			{
			case "(O)773":
				health = maxHealth;
				break;
	...
	public Farmer CreateFakeEventFarmer()
	{
		...
		fake_farmer.changeHairStyle(hair.Value);
	...
	public void eatHeldObject()
	{
		...
		{
			Items[currentToolIndex.Value] = oldActiveItem;
			currentToolIndex.Value = oldToolIndex;
		}
		if (wasStowed)
		{
