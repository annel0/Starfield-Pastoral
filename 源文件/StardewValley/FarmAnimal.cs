...
public class FarmAnimal : Character
{
	...
	public void ReloadTextureIfNeeded(bool forceReload = false)
	{
		...
		{
			string texturePath2 = GetTexturePath();
			if (Sprite.textureName.Value != texturePath2)
			{
				Sprite.LoadTexture(texturePath2);
			}
		}
	}
	...
	{
		string texturePath = "Animals\\" + type.Value;
		if (data != null)
		{
	...
	public void pet(Farmer who, bool is_auto_pet = false)
	{
		...
		if (!wasPet.Value)
		{
			...
			if (wasAutoPet.Value)
			{
				friendshipTowardFarmer.Value = Math.Min(1000, friendshipTowardFarmer.Value + auto_pet_reduction);
			}
			else if (is_auto_pet)
			{
				friendshipTowardFarmer.Value = Math.Min(1000, friendshipTowardFarmer.Value + (15 - auto_pet_reduction));
			}
			else
			{
			...
			if (!is_auto_pet)
			{
				...
				{
					friendshipTowardFarmer.Value = Math.Min(1000, friendshipTowardFarmer.Value + 15);
					happiness.Value = (byte)Math.Min(255, happiness.Value + Math.Max(5, 30 + happinessDrain));
				}
				int emote_index = 20;
				if (wasAutoPet.Value)
				{
				...
				doEmote((moodMessage.Value == 4) ? 12 : emote_index);
			}
			happiness.Value = (byte)Math.Min(255, happiness.Value + Math.Max(5, 30 + happinessDrain));
			if (!is_auto_pet)
			{
	...
	{
		StopAllActions();
		if (!location.TryGetMapPropertyAs("ProduceArea", out Microsoft.Xna.Framework.Rectangle produceArea, true))
		{
			return;
		}
	...
	public string GetProduceID(Random r, bool deluxe = false)
	{
		...
		produceList.RemoveAll((FarmAnimalProduce produce) => (produce.MinimumFriendship > 0 && friendshipTowardFarmer.Value < produce.MinimumFriendship) || (produce.Condition != null && !GameStateQuery.CheckConditions(produce.Condition, base.currentLocation, null, null, null, r)));
	...
	public void dayUpdate(GameLocation environment)
	{
		...
		else if (insideHome != null && IsHome && !home.animalDoorOpen.Value)
		{
			happiness.Value = (byte)Math.Min(255, happiness.Value + happinessDrain * 2);
		}
		...
		if (fullness.Value > 200 || r.NextDouble() < (double)(fullness.Value - 30) / 170.0)
		{
			if (age.Value == ((data != null) ? new int?(data.DaysToMature - 1) : ((int?)null)))
			{
				growFully(r);
			}
			else
		...
		Farmer owner = Game1.GetPlayer(ownerID.Value) ?? Game1.MasterPlayer;
		if (data != null && data.ProfessionForFasterProduce >= 0 && owner.professions.Contains(data.ProfessionForFasterProduce))
		{
			produceSpeedBonus++;
		}
		bool produceToday = daysSinceLastLay.Value >= ((data != null) ? new int?(data.DaysToProduce - produceSpeedBonus) : ((int?)null)) && r.NextDouble() < (double)fullness.Value / 200.0 && r.NextDouble() < (double)happiness.Value / 70.0;
		string whichProduce;
		...
		else
		{
			...
			if (r.NextDouble() < (double)happiness.Value / 150.0)
			{
				float happinessModifier = ((happiness.Value > 200) ? ((float)happiness.Value * 1.5f) : ((float)((happiness.Value <= 100) ? (happiness.Value - 100) : 0)));
				string deluxeProduce = GetProduceID(r, deluxe: true);
				if (data != null && data.DeluxeProduceCareDivisor >= 0f && deluxeProduce != null && friendshipTowardFarmer.Value >= data.DeluxeProduceMinimumFriendship && r.NextDouble() < (double)(((float)friendshipTowardFarmer.Value + happinessModifier) / data.DeluxeProduceCareDivisor) + Game1.player.team.AverageDailyLuck() * (double)data.DeluxeProduceLuckMultiplier)
				{
					whichProduce = deluxeProduce;
				}
				daysSinceLastLay.Value = 0;
				double chanceForQuality = (float)friendshipTowardFarmer.Value / 1000f - (1f - (float)happiness.Value / 225f);
		...
		if (whichProduce != null && home != null)
		{
			...
			producedObject.Quality = produceQuality.Value;
	...
	public int getSellPrice()
	{
		...
		double adjustedFriendship = (double)friendshipTowardFarmer.Value / 1000.0 + 0.3;
	...
	{
		FarmAnimalData data = GetAnimalData();
		if (age.Value <= data?.DaysToMature)
		{
	...
	public override void draw(SpriteBatch b)
	{
		...
		if (shadow == null || shadow.Visible)
		{
			...
			else
			{
				...
				Sprite.drawShadow(b, Game1.GlobalToLocal(Game1.viewport, shadowPos2), shadowScale2);
			}
		}
	...
	public virtual void updateWhenNotCurrentLocation(Building currentBuilding, GameTime time, GameLocation environment)
	{
		...
		doDiveEvent.Poll();
		AnimatedSprite sprite = GetOrLoadTexture();
		...
		if (currentBuilding != null && Game1.random.NextBool(0.002) && currentBuilding.animalDoorOpen.Value && Game1.timeOfDay < 1630 && !environment.IsRainingHere() && !environment.IsWinterHere() && !environment.farmers.Any())
		{
			...
			buildingLocation.animals.TryAdd(myID.Value, this);
			faceDirection(2);
	...
	public virtual bool behaviors(GameTime time, GameLocation location)
	{
		...
		if (Game1.timeOfDay >= 1700 && location.IsOutdoors && controller == null && Game1.random.NextDouble() < 0.002 && home.animalDoorOpen.Value)
		{
			...
			{
				NumPathfindingThisTick++;
				controller = new PathFindController(this, location, PathFindController.isAtEndPoint, 0, null, 200, new Point(home.tileX.Value + home.animalDoor.X, home.tileY.Value + home.animalDoor.Y));
				_followTarget = null;
				_followTargetPosition = null;
			}
		}
		...
		if (Game1.random.NextDouble() < 0.007 && uniqueFrameAccumulator == -1)
		{
			...
			if (newDirection != (FacingDirection + 2) % 4 || IsActuallySwimming())
			{
				if (newDirection < 4)
				{
					...
					if (!base.currentLocation.isOutdoors.Value && base.currentLocation.isCollidingPosition(nextPosition(newDirection), Game1.viewport, this))
					{
	...
	public virtual void HandleHop()
	{
		...
		if (hopOffset != Vector2.Zero)
		{
			...
			if (hopOffset.Y != 0f)
			{
				int move_amount2 = (int)Math.Min(hop_speed, Math.Abs(hopOffset.Y));
				base.Position += new Vector2(0f, move_amount2 * Math.Sign(hopOffset.Y));
				hopOffset.Y = Utility.MoveTowards(hopOffset.Y, 0f, move_amount2);
			}
