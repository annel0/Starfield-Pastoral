...
public class HouseRenovation : ISalable, IHaveItemTypeId
{
		...
		foreach (string key in data.Keys)
		{
			...
			foreach (RenovationValue renovateAction in renovation_data.RenovateActions)
			{
				...
				if (action_data.Type == "Value")
				{
					try
					{
						NetInt field2 = (NetInt)farmhouse.GetType().GetField(action_data.Key).GetValue(farmhouse);
						if (field2 == null)
						{
							valid = false;
							break;
						}
						renovation.onRenovation = (Action<HouseRenovation, int>)Delegate.Combine(renovation.onRenovation, new Action<HouseRenovation, int>(ActionOnRenovation));
				...
				{
					renovation.onRenovation = (Action<HouseRenovation, int>)Delegate.Combine(renovation.onRenovation, new Action<HouseRenovation, int>(MailOnRenovation));
				}
	...
	{
		List<Rectangle> bounds = new List<Rectangle> { bound };
