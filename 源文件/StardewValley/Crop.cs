...
public class Crop : INetObject<NetFields>, IHaveModData
{
	...
	public virtual void drawInMenu(SpriteBatch b, Vector2 screenPosition, Color toTint, float rotation, float scale, float layerDepth)
	{
		...
		{
			ParsedItemData itemData = ItemRegistry.GetDataOrErrorItem("(O)" + indexOfHarvest.Value);
			b.Draw(itemData.GetTexture(), screenPosition, itemData.GetSourceRect(), toTint, rotation, new Vector2(32f, 32f), scale, flip.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, layerDepth);
		}
		else
		{
	...
	public virtual void drawWithOffset(SpriteBatch b, Vector2 tileLocation, Color toTint, float rotation, Vector2 offset)
	{
		...
		{
			ParsedItemData itemData = ItemRegistry.GetDataOrErrorItem("(O)" + indexOfHarvest.Value);
			b.Draw(itemData.GetTexture(), Game1.GlobalToLocal(Game1.viewport, offset + new Vector2(tileLocation.X * 64f, tileLocation.Y * 64f)), itemData.GetSourceRect(), toTint, rotation, new Vector2(8f, 8f), 4f, flip.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (tileLocation.Y + 0.66f) * 64f / 10000f + tileLocation.X * 1E-05f);
			return;
		}
		if (forageCrop.Value)
		{
