...
public class Fence : Object
{
	...
	public override bool checkForAction(Farmer who, bool justCheckingForActivity = false)
	{
		...
		if (!justCheckingForActivity && who != null)
		{
			...
			if (location.objects.TryGetValue(new Vector2(playerTile.X, playerTile.Y - 1), out var upObj) && location.objects.TryGetValue(new Vector2(playerTile.X, playerTile.Y + 1), out var downObj) && location.objects.TryGetValue(new Vector2(playerTile.X + 1, playerTile.Y), out var rightObj) && location.objects.TryGetValue(new Vector2(playerTile.X - 1, playerTile.Y), out var leftObj) && !upObj.isPassable() && !downObj.isPassable() && !leftObj.isPassable() && !rightObj.isPassable())
			{
		...
		{
			if (location.objects.TryGetValue(v, out var obj) && obj is Fence fence && fence.isGate.Value)
			{
	...
	public virtual void toggleGate(bool open, bool is_toggling_counterpart = false, Farmer who = null)
	{
		...
		case 100:
		{
			Vector2 neighborTile2 = tileLocation.Value + new Vector2(-1f, 0f);
			if (location.objects.TryGetValue(neighborTile2, out var neighbor2) && neighbor2 is Fence fence2 && fence2.isGate.Value && fence2.getDrawSum() == 10)
			{
		...
		case 10:
		{
			...
			if (location.objects.TryGetValue(neighborTile4, out var neighbor4) && neighbor4 is Fence fence4 && fence4.isGate.Value && fence4.getDrawSum() == 100)
			{
		...
		case 1000:
		{
			Vector2 neighborTile3 = tileLocation.Value + new Vector2(0f, 1f);
			if (location.objects.TryGetValue(neighborTile3, out var neighbor3) && neighbor3 is Fence fence3 && fence3.isGate.Value && fence3.getDrawSum() == 500)
			{
		...
		case 500:
		{
			...
			if (location.objects.TryGetValue(neighborTile, out var neighbor) && neighbor is Fence fence && fence.isGate.Value && fence.getDrawSum() == 1000)
			{
			...
			if (!isGate.Value)
			{
				...
				if (drawSum == 1500 || drawSum == 110 || drawSum == 1000 || drawSum == 10 || drawSum == 100 || drawSum == 500)
				{
					...
					case 10:
						...
						if (location.objects.GetValueOrDefault(neighbor) is Fence rightFence && rightFence.isGate.Value)
						{
					...
					case 100:
						neighbor = tileLocation.Value + new Vector2(-1f, 0f);
						if (location.objects.GetValueOrDefault(neighbor) is Fence leftFence && leftFence.isGate.Value)
						{
							int neighborSum4 = leftFence.getDrawSum();
							if (neighborSum4 != 10 && neighborSum4 != 110)
							{
								return false;
							}
						}
					...
					case 1000:
						...
						if (location.objects.GetValueOrDefault(neighbor) is Fence downFence && downFence.isGate.Value)
						{
					...
					case 500:
						neighbor = tileLocation.Value + new Vector2(0f, -1f);
						if (location.objects.GetValueOrDefault(neighbor) is Fence upFence && upFence.isGate.Value)
						{
							int neighborSum = upFence.getDrawSum();
							if (neighborSum != 1000 && neighborSum != 1500)
							{
								return false;
							}
						}
					...
					foreach (Vector2 adjacent_tile in array)
					{
						if (!(adjacent_tile == neighbor) && location.objects.TryGetValue(adjacent_tile, out var adjacent) && adjacent is Fence fence && fence.isGate.Value && fence.Type == base.Type)
						{
							return false;
						}
					}
	...
	public override void draw(SpriteBatch b, int x, int y, float alpha = 1f)
	{
		...
		if (health.Value > 1f || repairQueued.Value)
		{
			...
			if (isGate.Value)
			{
				...
				{
				case 10:
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset + new Vector2(x * 64 - 16, y * 64 - 128)), new Rectangle((gatePosition.Value == 88) ? 24 : 0, 192, 24, 48), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 32 + 1) / 10000f);
					return;
				case 100:
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset + new Vector2(x * 64 - 16, y * 64 - 128)), new Rectangle((gatePosition.Value == 88) ? 24 : 0, 240, 24, 48), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 32 + 1) / 10000f);
					return;
				case 1000:
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset + new Vector2(x * 64 + 20, y * 64 - 64 - 20)), new Rectangle((gatePosition.Value == 88) ? 24 : 0, 288, 24, 32), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 - 32 + 2) / 10000f);
					return;
				case 500:
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset + new Vector2(x * 64 + 20, y * 64 - 64 - 20)), new Rectangle((gatePosition.Value == 88) ? 24 : 0, 320, 24, 32), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 96 - 1) / 10000f);
					return;
				case 110:
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset + new Vector2(x * 64 - 16, y * 64 - 64)), new Rectangle((gatePosition.Value == 88) ? 24 : 0, 128, 24, 32), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 32 + 1) / 10000f);
					return;
				case 1500:
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset + new Vector2(x * 64 + 20, y * 64 - 64 - 20)), new Rectangle((gatePosition.Value == 88) ? 16 : 0, 160, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 - 32 + 2) / 10000f);
					b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset + new Vector2(x * 64 + 20, y * 64 - 64 + 44)), new Rectangle((gatePosition.Value == 88) ? 16 : 0, 176, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 96 - 1) / 10000f);
					return;
				}
			...
			else if (heldObject.Value != null)
			{
				...
				offset2 *= 4f;
				heldObject.Value.draw(b, x * 64 + (int)offset2.X, y * 64 + (int)offset2.Y, (float)(y * 64 + 64) / 10000f, 1f);
			}
		}
