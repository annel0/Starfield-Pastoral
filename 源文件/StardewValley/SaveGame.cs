...
public class SaveGame
{
	...
	public static IEnumerator<int> Save()
	{
		...
		if (saveTask.IsFaulted)
		{
			...
			LogError("saveTask failed with an exception", e);
	...
	public static IEnumerator<int> getSaveEnumerator()
	{
		...
		string finalDataName = FilterFileName(Game1.GetSaveGameName()) + "_" + Game1.uniqueIDForThisGame;
	...
	{
		SaveGame data = TryReadSaveFile(file, null, out error);
		if (data != null)
		{
			error = null;
			autoRecovered = false;
		...
		data = TryReadSaveFile(file, "_old", out var error2);
		if (data != null)
		{
			error = null;
			autoRecovered = true;
		...
		data = TryReadSaveFile(file, "_STARDEWVALLEYSAVETMP", out error2);
		if (data != null)
		{
			error = null;
			autoRecovered = true;
	...
	public static IEnumerator<int> getLoadEnumerator(string file)
	{
		...
		else
		{
			...
			if (readSaveTask.IsFaulted)
			{
				...
				LogError("loadNewGameTask failed with an exception.", e);
				throw e;
			}
			if (CancelToTitle)
			{
		...
		{
			Dictionary<string, string> bundleData = loaded.bundleData.ToDictionary();
		...
		else
		{
			...
			if (readSaveTask.IsFaulted)
			{
				...
				LogError("loadFarmerTask failed with an exception", e2);
				throw e2;
			}
		}
		...
		foreach (SpecialOrder order2 in loaded.availableSpecialOrders)
		{
			if (order2 != null)
			{
		...
		foreach (Item donatedItem in loaded.returnedDonations)
		{
		...
		else
		{
			...
			if (readSaveTask.IsFaulted)
			{
				...
				LogError("loadLocationsTask failed with an exception", e3);
	...
	public static void loadDataToLocations(List<GameLocation> fromLocations)
	{
		...
		if (formerLocationNames.Count > 0)
		{
			foreach (GameLocation fromLocation2 in fromLocations)
			{
				foreach (NPC npc in fromLocation2.characters)
				{
					...
					{
						LogDebug($"Updated {npc.Name}'s home from '{curHome}' to '{newHome}'.");
		...
		foreach (GameLocation fromLocation in fromLocations)
		{
			...
			if (realLocation == null)
			{
				...
				{
					LogDebug($"Mapped legacy location '{fromLocation.Name}' to '{realLocationName}'.");
		...
		MigrateLostVillagers(lostVillagers);
	...
	{
		Dictionary<string, string> npcNamesByFormerName = GetFormerNpcNames((string name, CharacterData _) => Game1.getCharacterFromName(name) == null);
	...
	public static Dictionary<string, string> GetFormerLocationNames()
	{
		...
		foreach (KeyValuePair<string, LocationData> pair in Game1.locationData)
		{
			...
			foreach (string formerName in data.FormerLocationNames)
			{
				...
				{
					LogError($"Location '{pair.Key}' in Data/Locations has former name '{formerName}', which can't be added because there's a location with that ID in Data/Locations.");
				}
				else if (formerNames.TryGetValue(formerName, out conflictingId))
				{
					if (conflictingId != pair.Key)
					{
						LogError($"Location '{pair.Key}' in Data/Locations has former name '{formerName}', which can't be added because that name is already mapped to '{conflictingId}'.");
					}
				}
				else
				{
	...
	public static Dictionary<string, string> GetFormerNpcNames(Func<string, CharacterData, bool> filter)
	{
		...
		foreach (KeyValuePair<string, CharacterData> pair in Game1.characterData)
		{
			...
			foreach (string formerName in data.FormerCharacterNames)
			{
				...
				{
					LogError($"NPC '{pair.Key}' in Data/Characters has former name '{formerName}', which can't be added because there's an NPC with that ID in Data/Characters.");
				}
				else if (formerNames.TryGetValue(formerName, out conflictingId))
				{
					if (conflictingId != pair.Key)
					{
						LogError($"NPC '{pair.Key}' in Data/Characters has former name '{formerName}', which can't be added because that name is already mapped to '{conflictingId}'.");
					}
				}
				else
				{
