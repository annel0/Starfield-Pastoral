...
public class Utility
{
	...
	{
		GameLocation location = Game1.getLocationFromName(locationName);
		if (location != null && location.TryGetMapPropertyAs("DefaultWarpLocation", out Point position, false))
		{
			x = position.X;
			y = position.Y;
			return;
	...
	public static Color GetPrismaticColor(int offset = 0, float speedMultiplier = 1f)
	{
		...
		return new Color
		{
			R = (byte)(Lerp((float)(int)PRISMATIC_COLORS[current_index].R / 255f, (float)(int)PRISMATIC_COLORS[next_index].R / 255f, position) * 255f),
			G = (byte)(Lerp((float)(int)PRISMATIC_COLORS[current_index].G / 255f, (float)(int)PRISMATIC_COLORS[next_index].G / 255f, position) * 255f),
			B = (byte)(Lerp((float)(int)PRISMATIC_COLORS[current_index].B / 255f, (float)(int)PRISMATIC_COLORS[next_index].B / 255f, position) * 255f),
			A = (byte)(Lerp((float)(int)PRISMATIC_COLORS[current_index].A / 255f, (float)(int)PRISMATIC_COLORS[next_index].A / 255f, position) * 255f)
		};
	}
	...
	public static string ParseGiftReveals(string str)
	{
		...
		{
			Game1.log.Error("Error parsing gift taste reveals in string '" + original + "'.", exception);
		}
		return str;
	}
	...
	public static List<Vector2> getBorderOfThisRectangle(Microsoft.Xna.Framework.Rectangle r)
	{
		...
		{
			border.Add(new Vector2(r.Right - 1, j));
		}
		for (int i2 = r.Right - 2; i2 >= r.X; i2--)
		{
			border.Add(new Vector2(i2, r.Bottom - 1));
		}
		for (int i3 = r.Bottom - 2; i3 >= r.Y + 1; i3--)
		{
			border.Add(new Vector2(r.X, i3));
		}
		return border;
	}
	...
	public static Monster findClosestMonsterWithinRange(GameLocation location, Vector2 originPoint, int range, bool ignoreUntargetables = false, Func<Monster, bool> match = null)
	{
		...
		{
			if (npc is Monster monster && (!ignoreUntargetables || !(npc is Spiker)) && (match == null || match(monster)))
			{
	...
	public static void fixAllAnimals()
	{
		...
		ForEachLocation(delegate(GameLocation f)
		{
			...
			foreach (FarmAnimal current4 in allFarmAnimals)
			{
				foreach (Building current5 in f.buildings)
				{
					if (current4.CanLiveIn(current5) && current5.GetIndoors() is AnimalHouse animalHouse4 && !animalHouse4.isFull())
					{
						animalHouse4.adoptAnimal(current4);
						break;
					}
				}
			}
			foreach (FarmAnimal current6 in allFarmAnimals)
			{
				if (current6.home == null)
				{
					current6.Position = recursiveFindOpenTileForCharacter(current6, f, new Vector2(40f, 40f), 200) * 64f;
					f.animals.TryAdd(current6.myID.Value, current6);
				}
			}
			return true;
		}, includeInteriors: false);
	...
	public static Event getWeddingEvent(Farmer farmer)
	{
		...
		{
			spouseFarmer = Game1.GetPlayer(spouseFarmerId.Value);
		}
		...
		bool ParseWeddingToken(string[] query, out string replacement, Random random, Farmer player)
		{
			...
			case "setupcontextualweddingattendees":
			{
				...
				foreach (WeddingAttendeeData attendee3 in contextualAttendees)
				{
					sb2.Append(" ");
					sb2.Append(attendee3.Setup);
				}
			...
			case "contextualweddingcelebrations":
			{
				...
				foreach (WeddingAttendeeData attendee2 in contextualAttendees)
				{
					if (attendee2.Celebration != null)
					{
						sb.Append(attendee2.Celebration);
						sb.Append("/");
					}
				}
	...
	public static Item CreateFlavoredItem(string baseID, string preservesID, int quality = 0, int stack = 1)
	{
		...
		if (ItemQueryResolver.TryResolve("FLAVORED_ITEM " + baseID + " " + preservesID, context).FirstOrDefault()?.Item is Item resultItem)
		{
	...
	public static string getRandomItemFromSeason(Season season, bool forQuest, Random random)
	{
		...
		if (forQuest)
		{
			foreach (string s in all_unlocked_cooking_recipes)
			{
				...
				if (!CraftingRecipe.cookingRecipes.TryGetValue(s, out var rawCraftingData))
				{
					continue;
				}
	...
	public static Color? StringToColor(string rawColor)
	{
		...
		else if (rawColor.Contains(' '))
		{
			...
			if ((parts.Length == 3 || parts.Length == 4) && ArgUtility.TryGetInt(parts, 0, out var red2, out var error, "int red") && ArgUtility.TryGetInt(parts, 1, out var green2, out error, "int green") && ArgUtility.TryGetInt(parts, 2, out var blue2, out error, "int blue") && ArgUtility.TryGetOptionalInt(parts, 3, out var alpha2, out error, 255, "int alpha"))
			{
		...
		if (random.NextDouble() < 0.125 + Game1.player.team.AverageDailyLuck() + Game1.player.team.AverageLuckLevel() / 100.0)
		{
			...
			if (lightningRods.Count > 0)
			{
				for (int i = 0; i < 2; i++)
				{
					...
					{
						farm.objects[v2].heldObject.Value = ItemRegistry.Create<Object>("(O)787");
						farm.objects[v2].minutesUntilReady.Value = CalculateMinutesUntilMorning(Game1.timeOfDay);
	...
	public static List<string> GetJukeboxTracks(Farmer player, GameLocation location)
	{
		...
		{
			if (entry2.Value.Available ?? false)
			{
	...
	{
		if (identifier == null || !Game1.currentLightSources.TryGetValue(identifier, out var light))
		{
			return null;
		}
		return light;
	...
	public static FarmEvent pickPersonalFarmEvent()
	{
		...
		else
		{
			if (isMarriedOrRoommates)
			{
				...
				if (flag.HasValue && flag == true && Game1.player.currentLocation == Game1.getLocationFromName(Game1.player.homeLocation.Value) && r.NextDouble() < 0.05 && GameStateQuery.CheckConditions(npcSpouse.GetData()?.SpouseWantsChildren))
				{
			...
			if (isMarriedOrRoommates && Game1.player.team.GetSpouse(Game1.player.UniqueMultiplayerID).HasValue && Game1.player.GetSpouseFriendship().NextBirthingDate == null && r.NextDouble() < 0.05)
			{
				...
				if (Game1.otherFarmers.TryGetValue(spouseID2, out var farmerSpouse))
				{
					Farmer spouse = farmerSpouse;
					if (spouse.currentLocation == Game1.player.currentLocation && (spouse.currentLocation == Game1.getLocationFromName(spouse.homeLocation.Value) || spouse.currentLocation == Game1.getLocationFromName(Game1.player.homeLocation.Value)) && playersCanGetPregnantHere(spouse.currentLocation as FarmHouse))
					{
	...
	public static bool checkForCharacterInteractionAtTile(Vector2 tileLocation, Farmer who)
	{
		...
		if (character != null && !character.IsMonster && !character.IsInvisible)
		{
			...
			else
			{
				...
				if (flag.HasValue && flag == true && character.CanReceiveGifts() && !who.isRidingHorse() && who.friendshipData.ContainsKey(character.Name) && !Game1.eventUp)
				{
	...
	public static bool canGrabSomethingFromHere(int x, int y, Farmer who)
	{
		...
		if (who.IsLocalPlayer)
		{
			...
			{
				if (obj.readyForHarvest.Value || obj.isSpawnedObject.Value || (obj is IndoorPot pot && pot.hoeDirt.Value.readyForHarvest()))
				{
	...
	public static void transferPlacedObjectsFromOneLocationToAnother(GameLocation source, GameLocation destination, Vector2? overflow_chest_position = null, GameLocation overflow_chest_location = null)
	{
		...
		foreach (Vector2 position in new List<Vector2>(source.objects.Keys))
		{
			...
			bool num = destination != null && !destination.objects.ContainsKey(position) && destination.CanItemBePlacedHere(position);
		...
		ForEachLocation(delegate(GameLocation l)
		{
			...
			foreach (Object current in l.objects.Values)
			{
				if (current != fridge)
				{
					if (current is Chest)
					{
						current.ForEachItem(Handle, null);
					}
		...
		try
		{
			...
			if (geodeId.Contains("MysteryBox"))
			{
				if (Game1.stats.Get("MysteryBoxesOpened") > 10 || geodeId == "(O)GoldenMysteryBox")
				{
					...
					{
						if (Game1.player.stats.Get(StatKeys.Mastery(0)) != 0 && r.NextBool(0.005))
						{
			...
			if (Game1.objectData.TryGetValue(geode.ItemId, out var data))
			{
				...
				if (geodeDrops != null && geodeDrops.Count > 0 && (!data.GeodeDropsDefaultItems || r.NextBool()))
				{
					foreach (ObjectGeodeDropData drop in data.GeodeDrops.OrderBy((ObjectGeodeDropData p) => p.Precedence))
					{
						...
						Item item = ItemQueryResolver.TryResolveRandomItem(drop, new ItemQueryContext(null, null, r, $"object '{geode.ItemId}' > geode drop '{drop.Id}'"), avoidRepeat: false, null, null, null, delegate(string query, string error)
						{
	...
	public static bool tryToPlaceItem(GameLocation location, Object item, int x, int y)
	{
		...
		else if (item is Furniture furniture2 && Game1.didPlayerJustLeftClick(ignoreNonMouseHeldInput: true))
		{
	...
	public static void spreadAnimalsAround(Building b, GameLocation environment, IEnumerable<FarmAnimal> animalsList)
	{
		...
		int iterations = 0;
		Queue<Vector2> positionsToCheck = new Queue<Vector2>();
		positionsToCheck.Enqueue(new Vector2(b.tileX.Value + b.animalDoor.X, b.tileY.Value + b.animalDoor.Y + 1));
	...
	public static void HSLtoRGB(double h, double s, double l, out int r, out int g, out int b)
	{
		...
		{
			double_r = QQHtoRGB(p3, p2, h + 120.0);
			double_g = QQHtoRGB(p3, p2, h);
			double_b = QQHtoRGB(p3, p2, h - 120.0);
		}
