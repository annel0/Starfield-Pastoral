...
public class Character : INetObject<NetFields>, IHaveModData
{
	...
	public virtual void MovePosition(GameTime time, xTile.Dimensions.Rectangle viewport, GameLocation currentLocation)
	{
		...
		bool shouldDestroyUnderfootObjects = (byte)num != 0;
		...
		else if (moveUp)
		{
			...
			else if (!currentLocation.isTilePassable(nextPosition(0), viewport) || !shouldDestroyUnderfootObjects)
			{
				Halt();
			}
		...
		else if (moveRight)
		{
			...
			else if (!currentLocation.isTilePassable(nextPosition(1), viewport) || !shouldDestroyUnderfootObjects)
			{
				Halt();
			}
			else if (shouldDestroyUnderfootObjects)
		...
		else if (moveDown)
		{
			...
			else if (!currentLocation.isTilePassable(nextPosition(2), viewport) || !shouldDestroyUnderfootObjects)
			{
				Halt();
			}
			else if (shouldDestroyUnderfootObjects)
		...
		else if (moveLeft)
		{
			...
			else if (!currentLocation.isTilePassable(nextPosition(3), viewport) || !shouldDestroyUnderfootObjects)
			{
				Halt();
			}
			else if (shouldDestroyUnderfootObjects)
		...
		if (shouldDestroyUnderfootObjects && currentLocation != null && isMoving())
		{
