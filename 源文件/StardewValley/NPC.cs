...
public class NPC : Character, IComparable
{
	...
	public virtual void ChooseAppearance(LocalizedContentManager content = null)
	{
		...
		if (location.TryGetMapProperty("UniquePortrait", out var uniquePortraitsProperty) && Enumerable.Contains(ArgUtility.SplitBySpace(uniquePortraitsProperty), base.Name))
		{
		...
		if (location.TryGetMapProperty("UniqueSprite", out var uniqueSpritesProperty) && Enumerable.Contains(ArgUtility.SplitBySpace(uniqueSpritesProperty), base.Name))
		{
			string assetName2 = "Characters\\" + getTextureName() + "_" + location.Name;
			appliedLegacyUniqueSprites = TryLoadSprites(assetName2, out var errorPhrase2, content);
			if (!appliedLegacyUniqueSprites)
			{
				Game1.log.Warn($"NPC {base.Name} can't load sprites from '{assetName2}' (per the {"UniqueSprite"} map property in '{location.NameOrUniqueName}'): {errorPhrase2}. Falling back to default sprites.");
			}
		}
		...
		if (!appliedLegacyUniquePortraits)
		{
			...
			{
				loaded = TryLoadPortraits(appearance.Portrait, out var errorPhrase3, content);
				if (!loaded)
				{
					Game1.log.Warn($"NPC {base.Name} can't load portraits from '{appearance.Portrait}' (per appearance entry '{appearance.Id}' in Data/Characters): {errorPhrase3}. Falling back to default portraits.");
				}
			}
			if (!loaded && isWearingIslandAttire)
			{
				...
				{
					loaded = TryLoadPortraits(beachAsset, out var errorPhrase4, content);
					if (!loaded)
					{
						Game1.log.Warn($"NPC {base.Name} can't load portraits from '{beachAsset}' for island attire: {errorPhrase4}. Falling back to default portraits.");
					}
				}
			}
			if (!loaded && !TryLoadPortraits(defaultAsset, out var errorPhrase5, content))
			{
				Game1.log.Warn($"NPC {base.Name} can't load portraits from '{defaultAsset}': {errorPhrase5}.");
			}
			if (loaded)
			{
		...
		if (!appliedLegacyUniqueSprites)
		{
			string defaultAsset2 = "Characters/" + getTextureName();
			bool loaded2 = false;
			if (appearance != null && appearance.Sprite != null && appearance.Sprite != defaultAsset2)
			{
				loaded2 = TryLoadSprites(appearance.Sprite, out var errorPhrase6, content);
				if (!loaded2)
				{
					Game1.log.Warn($"NPC {base.Name} can't load sprites from '{appearance.Sprite}' (per appearance entry '{appearance.Id}' in Data/Characters): {errorPhrase6}. Falling back to default sprites.");
				}
			}
			if (!loaded2 && isWearingIslandAttire)
			{
				...
				{
					loaded2 = TryLoadSprites(beachAsset2, out var errorPhrase7, content);
					if (!loaded2)
					{
						Game1.log.Warn($"NPC {base.Name} can't load sprites from '{beachAsset2}' for island attire: {errorPhrase7}. Falling back to default sprites.");
					}
				}
			}
			if (!loaded2 && !TryLoadSprites(defaultAsset2, out var errorPhrase8, content))
			{
				Game1.log.Warn($"NPC {base.Name} can't load sprites from '{defaultAsset2}': {errorPhrase8}.");
			}
			if (loaded2)
			{
				LastAppearanceId = appearance?.Id;
	...
	public void performSpecialScheduleChanges()
	{
		...
		{
			bool pamGone = false;
			switch (v.Value.targetLocationName)
			{
			case "BusStop":
				foundBus = true;
				break;
			...
			case "IslandSouth":
			{
				...
				Object sign = ItemRegistry.Create<Object>("(BC)TextSign");
				...
				busStop.tryPlaceObject(new Vector2(25f, 10f), sign);
				foundBus = true;
				pamGone = true;
				break;
			}
	...
	private void updateConstructionAnimation()
	{
		...
		if (Game1.IsMasterGame && base.Name == "Robin" && !isFestivalDay && (!Game1.isGreenRain || Game1.year > 1))
		{
			...
			if (Game1.IsThereABuildingUnderConstruction())
			{
				...
				{
					Game1.warpCharacter(this, b.parentLocationName.Value, new Vector2(b.tileX.Value + b.tilesWide.Value / 2, b.tileY.Value + b.tilesHigh.Value / 2));
					position.X += 16f;
					position.Y -= 32f;
				}
				isPlayingRobinHammerAnimation = false;
	...
	private void doPlayRobinHammerAnimation()
	{
		...
		bool oneDayLeft = Game1.player.daysUntilHouseUpgrade.Value == 1 || Game1.RequireLocation<Town>("Town").daysUntilCommunityUpgrade.Value == 1;
	...
	public int getGiftTasteForThisItem(Item item)
	{
		...
		if (item is Object { Category: var categoryNumber } obj)
		{
			...
			if (Enumerable.Contains(universalLoves, categoryNumberString))
			{
				tasteForItem = 0;
			}
			else if (Enumerable.Contains(universalHates, categoryNumberString))
			{
				tasteForItem = 6;
			}
			else if (Enumerable.Contains(universalLikes, categoryNumberString))
			{
				tasteForItem = 2;
			}
			else if (Enumerable.Contains(universalDislikes, categoryNumberString))
			{
				tasteForItem = 4;
			}
			...
			if (Game1.NPCGiftTastes.TryGetValue(base.Name, out var dispositionData))
			{
				...
				{
					if (categoryNumber != 0 && Enumerable.Contains(items[0], categoryNumberString))
					{
						return 0;
					}
					if (categoryNumber != 0 && Enumerable.Contains(items[3], categoryNumberString))
					{
						return 6;
					}
					if (categoryNumber != 0 && Enumerable.Contains(items[1], categoryNumberString))
					{
						return 2;
					}
					if (categoryNumber != 0 && Enumerable.Contains(items[2], categoryNumberString))
					{
						return 4;
					}
					if (categoryNumber != 0 && Enumerable.Contains(items[4], categoryNumberString))
					{
						return 8;
					}
				}
	...
	public virtual bool tryToReceiveActiveObject(Farmer who, bool probe = false)
	{
		...
		case "(O)809":
		{
			...
			if (!(text == "Krobus"))
			{
				if (text == "Leo" && !Game1.MasterPlayer.mailReceived.Contains("leoMoved"))
				{
					if (!probe)
					{
						Game1.drawObjectDialogue(Game1.parseText(Game1.content.LoadString("Strings\\Characters:MovieInvite_NoTheater", displayName)));
			...
			foreach (MovieInvitation invitation2 in who.team.movieInvitations)
			{
				...
				if (!probe)
				{
					...
					CurrentDialogue.Push(TryGetDialogue("RejectMovieTicket_AlreadyInvitedBySomeoneElse", invitation2.farmer.displayName) ?? TryGetDialogue("RejectMovieTicket") ?? new Dialogue(this, "Strings\\Characters:MovieInvite_InvitedBySomeoneElse", GetDispositionModifiedString("Strings\\Characters:MovieInvite_InvitedBySomeoneElse", invitation2.farmer.displayName)));
					Game1.drawDialogue(this);
				}
				return true;
			}
		...
		case "(O)458":
			if (canReceiveGifts)
			{
				...
				if (friendship.IsDating())
				{
					if (!probe)
					{
						Dialogue dialogue5 = TryGetDialogue($"RejectBouquet_AlreadyAccepted_{friendship.Status}") ?? TryGetDialogue("RejectBouquet_AlreadyAccepted");
						if (dialogue5 != null)
						{
		...
		case "(O)277":
			if (canReceiveGifts)
			{
				if (!probe)
				{
					if (!datable.Value || friendship == null || !friendship.IsDating() || (friendship != null && friendship.IsMarried()))
					{
		...
		case "(O)460":
			if (canReceiveGifts)
			{
				...
				if (who.spouse == base.Name)
				{
					Dialogue dialogue4 = TryGetDialogue($"RejectMermaidPendant_AlreadyAccepted_{friendship?.Status}") ?? TryGetDialogue("RejectMermaidPendant_AlreadyAccepted");
				...
				if (!datable.Value || isMarriedOrEngaged() || isDivorced || (friendship != null && friendship.Points < 1500))
				{
					if (!probe)
					{
						if (Game1.random.NextBool())
				...
				if (datable.Value && friendship != null && friendship.Points < 2500)
				{
					if (!probe)
					{
		...
		default:
		{
			if (canReceiveGifts && activeObj.HasContextTag(ItemContextTagManager.SanitizeContextTag("propose_roommate_" + base.Name)))
			{
				...
				else if (who.isMarriedOrRoommates() || who.isEngaged())
				{
					...
					substitutions = new object[1] { who.getSpouse()?.displayName ?? who.spouse };
				}
				...
				if (failedKey != null)
				{
					Dialogue dialogue3 = ((substitutions != null) ? TryGetDialogue(failedKey, substitutions) : TryGetDialogue(failedKey)) ?? TryGetDialogue("RejectRoommateProposal");
					if (!probe)
					{
						if (dialogue3 != null)
						{
			...
			if (canReceiveGifts && activeObj.canBeGivenAsGift() && !obsoleteNotGiftable)
			{
				...
				if ((friendship != null && friendship.GiftsThisWeek < 2) || who.spouse == base.Name || this is Child || isBirthday() || who.ActiveObject.QualifiedItemId == "(O)StardropTea")
				{
					if (!probe)
					{
						...
						who.completelyStopAnimatingOrDoingAction();
						faceTowardFarmerForPeriod(4000, 3, faceAway: false, who);
						if (datable.Value && who.spouse != null && who.spouse != base.Name && !who.hasCurrentOrPendingRoommate() && Utility.isMale(who.spouse) == Utility.isMale(base.Name) && Game1.random.NextDouble() < 0.3 - (double)((float)who.LuckLevel / 100f) - who.DailyLuck && !isBirthday() && friendship.IsDating())
						{
	...
	public virtual bool checkAction(Farmer who, GameLocation l)
	{
		...
		bool reactingToShorts = who.pantsItem.Value?.QualifiedItemId == "(P)15" && (base.Name == "Lewis" || base.Name == "Marnie");
		...
		else
		{
			...
			if (!(text == "Krobus"))
			{
				if (text == "Dwarf" && who.canUnderstandDwarves && l is Mine)
				{
			...
			int oldDirection = Game1.player.FacingDirection;
			{
				foreach (string key in location_events.Keys)
				{
					if (Event.SplitPreconditions(key)[0] == event_id)
					{
						...
						{
							Event obj2 = new Event(location_events[key], eventAssetName, "event_id");
							obj2.isMemory = true;
	...
	{
		if (!Game1.eventUp && Game1.random.NextBool() && Dialogue != null && Dialogue.TryGetValue(l.name.Value + "_Entry", out var rawText))
		{
	...
	public Dialogue tryToGetMarriageSpecificDialogue(string dialogueKey)
	{
		...
		if (marriageDialogues != null && marriageDialogues.TryGetValue(dialogueKey, out var rawText2))
		{
		...
		if (isRoommate())
		{
			...
			if (marriageDialogues != null && marriageDialogues.TryGetValue(key, out var rawText3))
			{
	...
	public bool checkForNewCurrentDialogue(int heartLevel, bool noPreface = false)
	{
		...
		foreach (string eventMessageKey in Game1.player.activeDialogueEvents.Keys)
		{
			...
			Dialogue dialogue = TryGetDialogue(eventMessageKey);
			if (dialogue == null)
			{
				continue;
			}
		...
		Dialogue dialogue2 = TryGetDialogue(preface + Game1.currentLocation.name.Value + "_" + base.TilePoint.X + "_" + base.TilePoint.Y) ?? TryGetDialogue(preface + Game1.currentLocation.name.Value + "_" + Game1.shortDayNameFromDayOfSeason(Game1.dayOfMonth));
		int hearts = 10;
		while (dialogue2 == null && hearts >= 2)
		{
			if (heartLevel >= hearts)
			{
				dialogue2 = TryGetDialogue(preface + Game1.currentLocation.name.Value + hearts);
			}
			hearts -= 2;
		}
		dialogue2 = dialogue2 ?? TryGetDialogue(preface + Game1.currentLocation.Name);
		if (dialogue2 != null)
		{
	...
	public Dialogue tryToRetrieveDialogue(string preface, int heartLevel, string appendToEnd = "")
	{
		...
		if (!string.IsNullOrEmpty(Game1.player.spouse) && appendToEnd.Equals(""))
		{
			...
			{
				Dialogue s2 = tryToRetrieveDialogue(preface, heartLevel, "_inlaw_" + Game1.player.spouse);
				if (s2 != null)
				{
					return s2;
				}
		...
		if (year == 1)
		{
			Dialogue dialogue = TryGetDialogue(preface + Game1.dayOfMonth + appendToEnd);
			if (dialogue != null)
			{
				return dialogue;
			}
		}
		Dialogue dialogue2 = TryGetDialogue(preface + Game1.dayOfMonth + "_" + year + appendToEnd);
		if (dialogue2 != null)
		{
			return dialogue2;
		}
		...
		for (int hearts = 10; hearts >= 2; hearts -= 2)
		{
			if (heartLevel >= hearts)
			{
				Dialogue dialogue4 = TryGetDialogue(preface + day_name + hearts + "_" + year + appendToEnd) ?? TryGetDialogue(preface + day_name + hearts + appendToEnd);
				if (dialogue4 != null)
				{
		...
		Dialogue dialogue5 = TryGetDialogue(preface + day_name + appendToEnd);
		if (dialogue5 != null)
		{
		...
		{
			dialogue5 = TryGetDialogue("summer_Wed");
		}
		if (dialogue5 != null)
		{
			return dialogue5;
	...
	protected void _PushTemporaryDialogue(string translationKey)
	{
		...
		try
		{
			...
			{
				string relationshipKey = $"{translationKey}_{friendship.Status}";
				if (Game1.content.LoadStringReturnNullIfNotFound(relationshipKey) != null)
				{
			...
			{
				Dialogue temporaryDialogue = new Dialogue(this, translationKey)
				{
					removeOnNextMove = true,
					temporaryDialogueKey = translationKey
				};
				CurrentDialogue.Push(temporaryDialogue);
			}
		}
		catch (Exception exception)
		{
			Game1.log.Error($"NPC '{base.Name}' failed setting temporary dialogue key '{translationKey}'{((translationKey != oldTranslationKey) ? (" (from dialogue key '" + oldTranslationKey + "')") : "")}", exception);
		}
	}
	...
	private void loadEndOfRouteBehavior(string name)
	{
		...
		{
			Game1.log.Error($"NPC {base.Name} failed to apply end-of-route behavior '{name}'{((rawData != null) ? (" with raw data '" + rawData + "'") : "")}.", exception);
		}
	}
	...
	private void setNewDialogue(string dialogueSheetName, string dialogueSheetKey, bool clearOnMovement = false)
	{
		...
		if (dialogueSheetName.Contains("Marriage"))
		{
			...
			{
				Dialogue dialogue = tryToGetMarriageSpecificDialogue(translationKey);
				if (dialogue == null)
				{
					Game1.log.Warn($"NPC '{base.Name}' couldn't set marriage dialogue key '{translationKey}': not found.");
		...
		Dialogue dialogue2 = StardewValley.Dialogue.TryGetDialogue(this, translationPath);
		if (dialogue2 == null)
		{
			Game1.log.Warn($"NPC '{base.Name}' couldn't set dialogue key '{translationPath}': not found.");
	...
	public virtual Dialogue GetGiftReaction(Farmer giver, Object gift, int taste)
	{
		...
		else
		{
			...
			case 7:
				portrait = "$h";
				dialogue = dialogue ?? new Dialogue(this, null, ArgUtility.Get(rawFields, 0));
				break;
			case 0:
			case 2:
				if (dialogue == null)
	...
	public virtual void DrawEmote(SpriteBatch b)
	{
		if (base.IsEmoting && !(this is Child) && !(this is Pet))
		{
			...
			emotePosition = new Vector2(emotePosition.X + (float)dataOffset.X + ((float)Sprite.SourceRect.Width / 2f - 8f) * 4f, emotePosition.Y + (float)dataOffset.Y + (float)emoteYOffset - (float)((Sprite.SpriteHeight + 3) * 4));
		...
		if (locationsRoute != null)
		{
			for (int i = 0; i < locationsRoute.Length; i++)
			{
				...
				if (i < locationsRoute.Length - 1)
				{
					...
					path = addToStackForSchedule(path, PathFindController.findPathForNPCSchedules(locationStartPoint, target, currentLocation, 30000, this));
					locationStartPoint = currentLocation.getWarpPointTarget(target, this);
				}
				else
				{
					path = addToStackForSchedule(path, PathFindController.findPathForNPCSchedules(locationStartPoint, new Point(endingX, endingY), currentLocation, 30000, this));
				}
			}
		}
	...
	protected virtual Dictionary<int, SchedulePathDescription> parseMasterScheduleImpl(string scheduleKey, string rawData, List<string> visited)
	{
		...
		try
		{
			...
			if (split[0].Contains("GOTO"))
			{
				...
				{
					Game1.log.Error($"Failed to load schedule '{scheduleKey}' for NPC '{base.Name}': GOTO references schedule '{newKey}' which couldn't be parsed. Falling back to 'spring'.", exception);
				}
			...
			string previousGameLocation = (isMarried() ? "BusStop" : defaultMap.Value);
			...
			for (int i = routesToSkip; i < split.Length; i++)
			{
				...
				{
					string newKey3 = (getMasterScheduleRawData().ContainsKey("default") ? "default" : "spring");
					return parseMasterScheduleImpl(newKey3, getMasterScheduleEntry(newKey3), visited);
				}
	...
	public bool TryLoadSchedule()
	{
		...
		{
			if (TryLoadSchedule(season + "_" + dayName + "_" + tryHearts2))
			{
				return true;
			}
			tryHearts2--;
		...
		int tryHearts3;
		for (tryHearts3 = heartLevel; tryHearts3 > 0; tryHearts3--)
		{
			if (TryLoadSchedule(dayName + "_" + tryHearts3))
			{
				return true;
			}
			tryHearts3--;
	...
	public Dictionary<string, string> getMasterScheduleRawData()
	{
		if (!_hasLoadedMasterScheduleData)
		{
			...
			try
			{
				...
				{
					_masterScheduleData = Game1.content.Load<Dictionary<string, string>>(assetName);
	...
	public void marriageDuties()
	{
		...
		else if (!Game1.isRaining && r.NextDouble() < 0.4 && !checkTileOccupancyForSpouse(farm, Utility.PointToVector2(farmHouse.getPorchStandingSpot())) && !base.Name.Equals("Krobus"))
		{
			...
			if (r.NextDouble() < 0.6 && Game1.season != Season.Winter && !hasSomeoneWateredCrops)
			{
				...
				if (!origin.Equals(Vector2.Zero))
				{
					...
					{
						if (farm.isTileOnMap(currentPosition) && farm.terrainFeatures.TryGetValue(currentPosition, out var terrainFeature) && terrainFeature is HoeDirt dirt2 && Game1.IsMasterGame && dirt2.needsWatering())
						{
							dirt2.state.Value = 1;
						}
					}
			...
			else if (r.NextDouble() < 0.6 && !hasSomeoneFedTheAnimals)
			{
				...
				if (Game1.IsMasterGame)
				{
					...
					{
						if (building2 is PetBowl bowl2 && !bowl2.watered.Value)
						{
							filledBowl = true;
			...
			else if (!hasSomeoneRepairedTheFences)
			{
				...
				for (; i < Math.Min(50, farm.objects.Length); i++)
				{
					if (!origin2.Equals(Vector2.Zero))
					{
						break;
					}
					if (Utility.TryGetRandom(farm.objects, out var tile2, out var obj) && obj is Fence)
					{
						origin2 = tile2;
					}
				}
				if (!origin2.Equals(Vector2.Zero))
				{
					...
					{
						if (farm.isTileOnMap(currentPosition2) && farm.objects.TryGetValue(currentPosition2, out var obj2) && obj2 is Fence fence && Game1.IsMasterGame)
						{
							fence.repair();
						}
					}
		...
		else if (Game1.isRaining && r.NextDouble() < 0.08 && heartsWithSpouse < 11 && spouse.GetDaysMarried() > 7 && base.Name != "Krobus")
		{
			...
			{
				if (f2.furniture_type.Value == 13 && farmHouse.CanItemBePlacedHere(new Vector2((int)f2.tileLocation.X, (int)f2.tileLocation.Y + 1)))
				{
					setTilePosition((int)f2.tileLocation.X, (int)f2.tileLocation.Y + 1);
					faceDirection(0);
					currentMarriageDialogue.Clear();
		...
		else if (r.NextDouble() < 0.45)
		{
			...
			setTilePosition((int)spot2.X, (int)spot2.Y);
			faceDirection(0);
	...
	public virtual void arriveAtFarmHouse(FarmHouse farmHouse)
	{
		...
		{
			Point bed_spot = farmHouse.getSpouseBedSpot(name.Value);
