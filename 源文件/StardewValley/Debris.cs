			...
			else
			{
				...
				if (canMoveTowardPlayer)
				{
					...
					case DebrisType.OBJECT:
						...
						canMoveTowardPlayer = farmer.couldInventoryAcceptThisItem(itemId.Value, 1, itemQuality);
						...
						break;
					case DebrisType.RESOURCE:
						canMoveTowardPlayer = farmer.couldInventoryAcceptThisItem(itemId.Value, 1);
						break;
					default:
						...
						if (Math.Abs(chunk.position.X + 32f - (float)playerPixel.X) <= 64f && Math.Abs(chunk.position.Y + 32f - (float)playerPixel.Y) <= 64f)
						{
							...
							if (collect(player.Value, chunk))
							{
								if (Game1.debrisSoundInterval <= 0f)
								{
									...
									if (old?.QualifiedItemId != "(O)73" && itemId.Value != "(O)73")
									{
				...
				else
				{
					...
					if (chunk.position.Y >= (float)chunkFinalYLevel && chunk.hasPassedRestingLineOnce.Value)
					{
						...
						bool stillBouncing = chunk.bounces <= (floppingFish.Value ? 65 : 2);
						if (stillBouncing)
						{
							Point tile_point = new Point((int)chunk.position.X / 64, chunkFinalYLevel / 64);
							if (Game1.currentLocation is IslandNorth && (debrisType.Value == DebrisType.ARCHAEOLOGY || debrisType.Value == DebrisType.OBJECT || debrisType.Value == DebrisType.RESOURCE || debrisType.Value == DebrisType.CHUNKS) && Game1.currentLocation.isTileOnMap(tile_point.X, tile_point.Y) && !Game1.currentLocation.hasTileAt(tile_point, "Back"))
							{
								chunkFinalYLevel += 48;
							}
							chunk.bounces++;
