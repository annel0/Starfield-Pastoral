...
public class Game1 : InstanceGame
{
	...
	public static void ResetToolSpriteSheet()
	{
		...
		Texture2D obj = new Texture2D(game1.GraphicsDevice, w, h, mipmap: false, SurfaceFormat.Color)
		{
			Name = texture.Name
		};
	...
	private static void allocateLightmap(int width, int height)
	{
		...
		if (renderTarget2D == null || renderTarget2D.Width != w || lightmap.Height != h)
		{
			_lightmap?.Dispose();
	...
	public virtual void SetWindowSize(int w, int h)
	{
		...
		if (activeClickableMenu?.GetType() == typeof(GameMenu))
		{
			...
			IClickableMenu oldPage = obj.GetCurrentPage();
			IClickableMenu currentPage = ((GameMenu)(activeClickableMenu = new GameMenu(obj.currentTab))).GetCurrentPage();
			if (!(currentPage is CollectionsPage collectionsPage))
			{
				if (!(currentPage is OptionsPage optionsPage))
				{
					if (currentPage is SocialPage socialPage)
					{
	...
	public static void updateViewportForScreenSizeChange(bool fullscreenChange, int width, int height)
	{
		...
		bool sizeDirty = viewport.Width != width || viewport.Height != height;
	...
	private IEnumerator<int> GetLoadContentEnumerator()
	{
		...
		for (int i = 0; i < dynamicPixelRects.Length; i++)
		{
			...
			dynamicPixelRects[i].SetData(white);
		}
	...
	private void _update(GameTime gameTime)
	{
					...
					else
					{
						...
						if (base.IsActive)
						{
							...
							else
							{
								...
								{
									if (!currentKBState.IsKeyDown(k2) && currentMinigame != null)
									{
										currentMinigame.receiveKeyRelease(k2);
									}
								}
								if (options.gamepadControls)
								{
									...
									while (enumerator2.MoveNext())
									{
										...
										currentMinigame?.receiveKeyRelease(Utility.mapGamePadButtonToKey(b2));
									}
		...
		else if (ShouldLoadIncrementally)
		{
			...
			while (loadTimer.Elapsed.TotalMilliseconds < 25.0);
	...
	public static void updateCellarAssignments()
	{
		...
		for (int i = 2; i <= netWorldState.Value.HighestPlayerLimit; i++)
		{
			...
			{
				if (GetPlayer(assignedFarmerId) != null)
				{
					continue;
				}
	...
	public static void updateActiveMenu(GameTime gameTime)
	{
		...
		foreach (Keys k in pressedKeys)
		{
			if (active_menu != null && active_menu.IsActive() && !Enumerable.Contains(oldKBState.GetPressedKeys(), k))
			{
	...
	public static void updateTextEntry(GameTime gameTime)
	{
		...
		foreach (Keys k in pressedKeys)
		{
			if (textEntry != null && !Enumerable.Contains(oldKBState.GetPressedKeys(), k))
			{
		...
		if (options.gamepadControls && textEntry != null)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				textEntry.receiveKeyPress(Utility.mapGamePadButtonToKey(b3));
			}
		...
		else if (killScreen)
		{
			...
			else
			{
				foreach (ReviveLocation revive_location2 in LocationContexts.Default.ReviveLocations)
				{
					...
					{
						warpFarmer(revive_location2.Location, revive_location2.Position.X, revive_location2.Position.Y, flip: false);
						handledRevive = true;
						break;
					}
				}
	...
	public static bool isOneOfTheseKeysDown(KeyboardState state, InputButton[] keys)
	{
		for (int i = 0; i < keys.Length; i++)
		{
			...
			if (k.key != Keys.None && state.IsKeyDown(k.key))
			{
				return true;
			}
		}
	...
	public static bool areAllOfTheseKeysUp(KeyboardState state, InputButton[] keys)
	{
		...
		{
			InputButton k = keys[i];
			if (k.key != Keys.None && !state.IsKeyUp(k.key))
			{
				return false;
			}
		}
	...
	public static void UpdateGameClock(GameTime time)
	{
		...
		if (num > num2 + ((gameLocation != null) ? new int?(gameLocation.ExtraMillisecondsPerInGameMinute * 10) : ((int?)null)))
		{
			if (panMode)
			{
				gameTimeInterval = 0;
	...
	public static Event getAvailableWeddingEvent()
	{
		if (weddingsToday.Count > 0)
		{
			...
			Farmer spouse = GetPlayer(spouseID.Value);
			if (spouse == null)
			{
				return null;
			}
	...
	private bool onFadeToBlackComplete()
	{
		...
		if (!nonWarpFade && locationRequest != null)
		{
			...
			if (locationRequest != null)
			{
				...
				if (MineShaft.IsGeneratedLevel(previousLocation) && currentLocation != null && !MineShaft.IsGeneratedLevel(currentLocation))
				{
					...
					if (showingHealthBar && fadeToBlackAlpha <= 0f)
					{
						for (int i2 = 0; i2 < 3; i2++)
						{
							uiOverlayTempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(366, 412, 5, 6), new Vector2(random.Next(32) + uiViewport.Width - 112, uiViewport.Height - 224 - (player.maxHealth - 100) - 16 + 4), flipped: false, 0.017f, Color.Red)
							{
								...
								local = true,
								scale = 4f,
								delayBeforeAnimationStart = i2 * 150
							});
						}
					}
				}
			...
			for (int i = 0; i < rainDrops.Length; i++)
			{
				if (rainDrops[i].frame == 0)
				{
					...
					if (currentLocation is IslandNorth || currentLocation is Caldera)
					{
						...
						p.Y--;
						if (currentLocation.isTileOnMap(p.X, p.Y) && !currentLocation.hasTileAt(p, "Back") && !currentLocation.hasTileAt(p, "Buildings"))
						{
							rainDrops[i].frame = 0;
						}
					}
	...
	public static void eventFinished()
	{
		...
		{
			exitLocation = new LocationRequest(currentLocation.NameOrUniqueName, currentLocation.isStructure.Value, currentLocation);
		}
	...
	public static void AddLocations()
	{
		...
		foreach (KeyValuePair<string, LocationData> pair in locationData)
		{
			...
			{
				log.Error("Couldn't create the '" + pair.Key + "' location. Is its data in Data/Locations invalid?", exception);
				continue;
			}
			if (location == null)
			{
			...
			if (!currentLocationSet)
			{
				...
				{
					log.Error("Couldn't load tilesheets for the '" + pair.Key + "' location.", exception2);
				}
			}
	...
	public static void fixProblems()
	{
		...
		Utility.ForEachVillager(delegate(NPC nPC)
		{
			...
			if (nPC.DefaultMap == null || !nPC.DefaultMap.ContainsIgnoreCase("cabin") || nPC.DefaultMap != "FarmHouse")
			{
				return true;
			}
			...
			NPC.ReadNpcHomeData(data, nPC.currentLocation, out var locationName, out var _, out var _);
			if (nPC.DefaultMap != locationName)
			{
				if (divorced == null)
				{
		...
		foreach (Farmer player in getAllFarmers())
		{
			if (player.hasQuest("130"))
			{
				...
				if (!found)
				{
					Object item = ItemRegistry.Create<Object>("(O)864");
		...
		{
			[typeof(Axe)] = playerCount,
			[typeof(Pickaxe)] = playerCount,
			[typeof(Hoe)] = playerCount,
			[typeof(WateringCan)] = playerCount,
			[typeof(Wand)] = 0
		};
		foreach (Farmer allFarmer2 in getAllFarmers())
		{
			if (allFarmer2.hasOrWillReceiveMail("ReturnScepter"))
			{
		...
		Utility.ForEachLocation(delegate(GameLocation l)
		{
			...
			foreach (Debris current2 in l.debris)
			{
				Item item2 = current2.item;
				if (item2 != null)
				{
					foreach (Type current3 in missingTools.Keys)
					{
						if (item2.GetType() == current3)
						{
							list.Add(current2);
						}
					}
			...
			foreach (Debris current4 in list)
			{
				l.debris.Remove(current4);
			}
			return true;
		});
		...
		foreach (KeyValuePair<Type, int> pair in missingTools)
		{
			if (pair.Value > 0)
			{
				for (int i2 = 0; i2 < pair.Value; i2++)
				{
					toAdd.Add(pair.Key.ToString());
				}
			}
		}
		for (int i3 = 0; i3 < missingScythes; i3++)
		{
			toAdd.Add("Scythe");
		}
		if (toAdd.Count > 0)
		...
		for (int i4 = 0; i4 < toAdd.Count; i4++)
		{
			Item tool = null;
			switch (toAdd[i4])
			{
	...
	private static IEnumerator<int> _newDayAfterFade()
	{
		...
		player.chestConsumedMineLevels.RemoveWhere((KeyValuePair<int, bool> keyValuePair) => keyValuePair.Key > 120);
		player.currentEyes = 0;
		int seed;
		if (IsMasterGame)
		{
			player.team.announcedSleepingFarmers.Clear();
			seed = Utility.CreateRandomSeed(uniqueIDForThisGame / 100, stats.DaysPlayed * 10 + 1, stats.StepsTaken);
		...
		{
			Farmer spouse_farmer = GetPlayer(item4);
		...
		foreach (Item i3 in shippingBin)
		{
			player.displayedShippedItems.Add(i3);
		}
		if (player.useSeparateWallets || player.IsMainPlayer)
		{
			...
			foreach (Item item2 in shippingBin)
			{
				...
				{
					specialOrder.onItemShipped?.Invoke(player, item2, item_value);
				}
			}
			player.Money += total;
		}
		if (IsMasterGame)
		{
			...
			if (Utility.HasAnyPlayerSeenEvent("191393") && !player.hasOrWillReceiveMail("transferredObjectsJojaMart"))
			{
				...
				GameLocation source3 = RequireLocation("JojaMart");
				if (source3.objects.Length >= 0)
				{
					Utility.transferPlacedObjectsFromOneLocationToAnother(source3, null, new Vector2(89f, 51f), getLocationFromName("Town"));
				}
			}
		}
		if (player.useSeparateWallets && player.IsMainPlayer)
		{
			foreach (Farmer who in getOfflineFarmhands())
			{
				...
				foreach (Item item3 in farmhandShippingBin)
				{
					...
					{
						specialOrder2.onItemShipped?.Invoke(player, item3, item_value2);
					}
				}
				player.team.AddIndividualMoney(who, total2);
		...
		if (IsMasterGame)
		{
			player.team.specialOrders.RemoveWhere(delegate(SpecialOrder specialOrder)
			{
				if (specialOrder.questState.Value != SpecialOrderStatus.Complete && specialOrder.GetDaysLeft() <= 0)
				{
					specialOrder.OnFail();
					return true;
				}
		...
		Utility.ForEachVillager(delegate(NPC nPC)
		{
			player.mailReceived.Remove(nPC.Name);
			player.mailReceived.Remove(nPC.Name + "Cooking");
		...
		if (IsMasterGame)
		{
			Utility.ForEachLocation(delegate(GameLocation location)
			{
				if (location.IsOutdoors && location.IsRainingHere())
				{
					...
					{
						if (pair2.Value is HoeDirt hoeDirt && hoeDirt.state.Value != 2)
						{
		...
		foreach (string mail_key in mailToRemoveOvernight)
		{
			if (IsMasterGame)
			{
				...
				{
					farmer2.RemoveMail(mail_key, farmer2 == MasterPlayer);
				}
			}
			else
			{
	...
	public static void UpdateWeatherForNewDay()
	{
		...
		foreach (KeyValuePair<string, LocationContextData> pair2 in locationContextData)
		{
			...
			if (contextToCopy != null)
			{
				try
				{
					LocationWeather weatherForLocation = netWorldState.Value.GetWeatherForLocation(pair2.Key);
	...
	{
		if (!(getLocationFromName(location_name) is ShopLocation { itemsFromPlayerToSell: var items } shopLocation))
		{
			return;
		}
		for (int i = items.Count - 1; i >= 0; i--)
		{
			if (!(items[i] is Object item))
			{
				items.RemoveAt(i);
			}
			else
			{
				...
				{
					bool soldItem = false;
					if (item.edibility.Value != -300 && random.NextDouble() < 0.04)
					{
						...
						if (n.Age != 2 && n.getSpouse() == null)
						{
							...
							{
								Dialogue dialogue = shopLocation.getPurchasedItemDialogueForNPC(item, n);
								if (dialogue != null)
								{
									n.addExtraDialogue(dialogue);
	...
	public static void queueWeddingsForToday()
	{
		...
		foreach (Farmer farmer in from farmer2 in getOnlineFarmers()
			...
			select farmer2)
		{
			...
			{
				Farmer spouse_farmer = GetPlayer(spouse.Value);
	...
	public static void setGraphicsForSeason(bool onLoad = false)
	{
		foreach (GameLocation l in locations)
		{
			...
			case Season.Spring:
				...
				break;
			case Season.Summer:
				foreach (Object o3 in l.Objects.Values)
				{
					if (!o3.IsWeeds())
					{
						continue;
				...
				break;
			case Season.Fall:
				foreach (Object o2 in l.Objects.Values)
				{
					if (o2.IsWeeds())
					{
	...
	public static void UpdateViewPort(bool overrideFreeze, Point centerPoint)
	{
		...
		if (!viewportFreeze || overrideFreeze)
		{
			...
			bool snapBack = forceSnapOnNextViewportUpdate || Math.Abs(currentViewportTarget.X + (float)(viewport.Width / 2) + (float)viewportBounds.X - (float)playerPixel.X) > 64f || Math.Abs(currentViewportTarget.Y + (float)(viewport.Height / 2) + (float)viewportBounds.Y - (float)playerPixel.Y) > 64f;
		...
		bool forceSnap = forceSnapOnNextViewportUpdate;
		forceSnapOnNextViewportUpdate = false;
	...
	public static void warpCharacter(NPC character, GameLocation targetLocation, Vector2 position)
	{
		...
		{
			if (Utility.TryGetPassiveFestivalData(activePassiveFestival, out var festival) && dayOfMonth >= festival.StartDay && dayOfMonth <= festival.EndDay && festival.Season == season && festival.MapReplacements != null && festival.MapReplacements.TryGetValue(targetLocation.name.Value, out var newName))
			{
	...
	public static void warpFarmer(LocationRequest locationRequest, int tileX, int tileY, int facingDirectionAfterWarp)
	{
		...
		case "Farm":
			...
			case "FarmCave":
			{
				...
				if (getFarm().TryGetMapPropertyAs("FarmCaveEntry", out Point tile3, false))
				{
					tileX = tile3.X;
					tileY = tile3.Y;
					break;
			...
			case "Forest":
			{
				...
				if (getFarm().TryGetMapPropertyAs("ForestEntry", out Point tile4, false))
				{
					tileX = tile4.X;
					tileY = tile4.Y;
					break;
			...
			case "BusStop":
			{
				if (tileX == 79 && tileY == 17 && getFarm().TryGetMapPropertyAs("BusStopEntry", out Point tile2, false))
				{
					tileX = tile2.X;
					tileY = tile2.Y;
				}
			...
			case "Backwoods":
			{
				if (tileX == 40 && tileY == 0 && getFarm().TryGetMapPropertyAs("BackwoodsEntry", out Point tile, false))
				{
					tileX = tile.X;
					tileY = tile.Y;
				}
	...
	private static void notifyServerOfWarp(bool needsLocationInfo)
	{
		if (locationRequest != null)
		{
			byte flags = (byte)((locationRequest.IsStructure ? 1u : 0u) | (uint)(warpingForForcedRemoteEvent ? 2 : 0) | (uint)(needsLocationInfo ? 4 : 0));
			flags = facingDirectionAfterWarp switch
			{
				1 => (byte)(flags | 0x10), 
				2 => (byte)(flags | 0x20), 
				3 => (byte)(flags | 0x40), 
			...
			client.sendMessage(5, (short)xLocationAfterWarp, (short)yLocationAfterWarp, locationRequest.Name, flags);
		}
	}
	...
	{
		if (player.achievements.Contains(which) || gameMode != 3 || !achievements.TryGetValue(which, out var rawData))
		{
			return;
		}
	...
	{
		Farmer forPlayer = GetPlayer(whichPlayer) ?? player;
		currentLocation.debris.Add(new Debris(id, new Vector2(xTile * 64 + 32, yTile * 64 + 32), forPlayer.getStandingPosition()));
	}
	...
	public static Farmer? GetPlayer(long id, bool onlyOnline = false)
	{
		...
		if (otherFarmers.TryGetValue(id, out var onlineFarmhand))
		{
			return onlineFarmhand;
		}
		if (!onlyOnline && netWorldState.Value.farmhandData.TryGetValue(id, out var offlineFarmhand))
		{
	...
	public static bool pressUseToolButton()
	{
		...
		if (player.CurrentTool == null && player.ActiveObject == null)
		{
			...
			c.X = (int)c.X;
			c.Y = (int)c.Y;
			if (currentLocation.Objects.TryGetValue(c, out var o) && !o.readyForHarvest.Value && o.heldObject.Value == null && !(o is Fence) && !(o is CrabPot) && (o.Type == "Crafting" || o.Type == "interactive") && !o.IsTwig())
			{
	...
	public void ResetIslandLocations()
	{
		...
		{
			GameLocation location = locations[i2];
			if (location.InIslandContext())
			{
				...
				try
				{
					locations[i2] = Activator.CreateInstance(location.GetType(), args) as GameLocation;
				}
				catch
				{
					locations[i2] = Activator.CreateInstance(location.GetType()) as GameLocation;
				}
				_locationLookup.Clear();
			}
		}
	...
	private void updatePanModeControls(MouseState currentMouseState, KeyboardState currentKBState)
	{
		...
		{
			int x2 = getMouseX() + viewport.X;
			int y2 = getMouseY() + viewport.Y;
			Warp w = currentLocation.isCollidingWithWarpOrDoor(new Microsoft.Xna.Framework.Rectangle(x2, y2, 1, 1));
			if (w != null)
			{
		...
		{
			Warp w2 = currentLocation.warps[0];
	...
	private void UpdateControlInput(GameTime time)
	{
		...
		hooks.OnGame1_UpdateControlInput(ref currentKBState, ref currentMouseState, ref currentPadState, delegate
		{
			...
			{
				if ((IsHudDrawn || current == chatBox) && wasMouseVisibleThisFrame && current.isWithinBounds(getMouseX(), getMouseY()))
				{
			...
			else
			{
				...
				else
				{
					if (((useToolButtonPressed && !isAnyGamePadButtonBeingPressed()) || (actionButtonPressed && isAnyGamePadButtonBeingPressed())) && pauseTime <= 0f && wasMouseVisibleThisFrame)
					{
						...
						{
							if (IsHudDrawn || current2 == chatBox)
							{
					...
					else
					{
						...
						if (actionButtonPressed || (dialogueUp && useToolButtonPressed))
						{
							...
							{
								if (wasMouseVisibleThisFrame && (IsHudDrawn || current4 == chatBox) && current4.isWithinBounds(getMouseX(), getMouseY()) && !(current4 is LevelUpMenu { informationUp: false }))
								{
	...
	public virtual void DrawWorld(GameTime time, RenderTarget2D target_screen)
	{
		if (hooks.OnRendering(RenderSteps.World, spriteBatch, time, target_screen))
		{
			...
			if (currentLocation.currentEvent != null && !currentLocation.currentEvent.isFestival && currentLocation.currentEvent.farmerActors.Count > 0)
			{
				...
				{
					if ((f.IsLocalPlayer && displayFarmer) || !f.hidden.Value)
					{
			...
			else
			{
				...
				{
					if ((f2.IsLocalPlayer && displayFarmer) || !f2.hidden.Value)
					{
			...
			if (!currentLocation.shouldHideCharacters())
			{
				if (CurrentEvent == null)
				{
					...
					{
						if (!n.swimming.Value && !n.HideShadow && !n.IsInvisible && !checkCharacterTilesForShadowDrawFlag(n))
						{
				...
				else
				{
					...
					{
						if ((CurrentEvent == null || !CurrentEvent.ShouldHideCharacter(n2)) && !n2.swimming.Value && !n2.HideShadow && !checkCharacterTilesForShadowDrawFlag(n2))
						{
				...
				{
					if (!multiplayer.isDisconnecting(f3.UniqueMultiplayerID) && !f3.swimming.Value && !f3.isRidingHorse() && !f3.IsSitting() && (currentLocation == null || !checkCharacterTilesForShadowDrawFlag(f3)))
					{
			...
			for (int j = 0; j < currentLocation.buildingLayers.Count; j++)
			{
				...
				currentLocation.buildingLayers[j].Key.Draw(mapDisplayDevice, viewport, Location.Origin, wrapAround: false, 4, layer_sub_sort * layer);
			}
			...
			if (hooks.OnRendering(RenderSteps.World_Sorted, spriteBatch, time, target_screen))
			{
				if (!currentLocation.shouldHideCharacters())
				{
					if (CurrentEvent == null)
					{
						foreach (NPC n3 in currentLocation.characters)
						{
							if (!n3.swimming.Value && !n3.HideShadow && !n3.isInvisible.Value && checkCharacterTilesForShadowDrawFlag(n3))
							{
					...
					{
						foreach (NPC n4 in CurrentEvent.actors)
						{
							if ((CurrentEvent == null || !CurrentEvent.ShouldHideCharacter(n4)) && !n4.swimming.Value && !n4.HideShadow && checkCharacterTilesForShadowDrawFlag(n4))
							{
								n4.DrawShadow(spriteBatch);
							}
						}
					}
					foreach (Farmer f4 in _farmerShadows)
					{
						if (!f4.swimming.Value && !f4.isRidingHorse() && !f4.IsSitting() && currentLocation != null && checkCharacterTilesForShadowDrawFlag(f4))
						{
				...
				for (int k = 0; k < currentLocation.frontLayers.Count; k++)
				{
					...
					{
						layer2 = (float)k / (float)(currentLocation.frontLayers.Count - 1);
					}
					currentLocation.frontLayers[k].Key.Draw(mapDisplayDevice, viewport, Location.Origin, wrapAround: false, 4, 64f + layer_sub_sort * layer2);
				}
	...
	public virtual void DrawLightmapOnScreen(GameTime time, RenderTarget2D target_screen)
	{
		if (hooks.OnRendering(RenderSteps.World_DrawLightmapOnScreen, spriteBatch, time, target_screen))
		{
			...
			{
				spriteBatch.Draw(lightingRect, vp.Bounds, Color.OrangeRed * 0.45f);
			}
		}
	...
	public virtual void DrawLighting(GameTime time, RenderTarget2D target_screen)
	{
		...
		{
			Color lighting = ((!(currentLocation is MineShaft mine)) ? ((ambientLight.Equals(Color.White) || (currentLocation.isOutdoors.Value && currentLocation.IsRainingHere())) ? outdoorLight : ambientLight) : mine.getLightingColor(time));
			...
			{
				currentLightSource.Value.Draw(spriteBatch, currentLocation, light_multiplier);
			}
		}
	...
	public virtual void DrawSplitScreenWindow()
	{
		...
		if (screen == null || !screen.IsContentLost)
		{
			...
			Viewport viewport = (base.GraphicsDevice.Viewport = defaultDeviceViewport);
	...
	protected virtual void drawHUD()
	{
		...
		Color c = Utility.getRedToGreenLerpColor(player.stamina / (float)player.maxStamina.Value);
		...
		if (!currentLocation.orePanPoint.Equals(Point.Zero) && !Utility.isOnScreen(Utility.PointToVector2(currentLocation.orePanPoint.Value) * 64f + new Vector2(32f, 32f), 64))
		{
			...
			{
				onScreenPosition2.X = graphics.GraphicsDevice.Viewport.Bounds.Right - 8;
			...
			{
				onScreenPosition2.X = currentLocation.orePanPoint.X * 64 - viewport.X;
			}
			if (currentLocation.orePanPoint.Y * 64 > viewport.MaxCorner.Y - 64)
			{
				onScreenPosition2.Y = graphics.GraphicsDevice.Viewport.Bounds.Bottom - 8;
			...
			{
				onScreenPosition2.Y = currentLocation.orePanPoint.Y * 64 - viewport.Y;
			}
			if (onScreenPosition2.X == 8f && onScreenPosition2.Y == 8f)
			{
				rotation2 += (float)Math.PI / 4f;
			}
			if (onScreenPosition2.X == 8f && onScreenPosition2.Y == (float)(graphics.GraphicsDevice.Viewport.Bounds.Bottom - 8))
			{
				rotation2 += (float)Math.PI / 4f;
			}
			if (onScreenPosition2.X == (float)(graphics.GraphicsDevice.Viewport.Bounds.Right - 8) && onScreenPosition2.Y == 8f)
			{
				rotation2 -= (float)Math.PI / 4f;
			}
			if (onScreenPosition2.X == (float)(graphics.GraphicsDevice.Viewport.Bounds.Right - 8) && onScreenPosition2.Y == (float)(graphics.GraphicsDevice.Viewport.Bounds.Bottom - 8))
			{
				rotation2 -= (float)Math.PI / 4f;
			}
			spriteBatch.Draw(mouseCursors, onScreenPosition2, new Microsoft.Xna.Framework.Rectangle(412, 495, 5, 4), Color.Cyan, rotation2, new Vector2(2f, 2f), 4f, SpriteEffects.None, 1f);
		}
	}
	...
	public static void drawDialogueBox(int x, int y, int width, int height, bool speaker, bool drawOnlyBox, string message = null, bool objectDialogueWithPortrait = false, bool ignoreTitleSafe = true, int r = -1, int g = -1, int b = -1)
	{
		...
		{
			theSpeaker = (objectDialogueWithPortrait ? objectDialoguePortraitPerson : currentSpeaker);
			text = ((!objectDialogueWithPortrait) ? theSpeaker.CurrentDialogue.Peek().CurrentEmotion : ((objectDialoguePortraitPerson.Name == player.spouse) ? "$l" : "$neutral"));
			if (text != null)
			{
		...
		if (isQuestion && !dialogueTyping)
		{
			for (int i = 0; i < questionChoices.Count; i++)
			{
				if (currentQuestionChoice == i)
				{
					...
					textPosition.Y = (float)(screenHeight - (5 + addedTileHeightForQuestions + 1) * 64) + ((text2.Trim().Length > 0) ? dialogueFont.MeasureString(text2).Y : 0f) + 128f + (float)(48 * i) - (float)(16 + (questionChoices.Count - 2) * 64) + (float)dialogueY + (float)everythingYOffset;
					spriteBatch.End();
					...
					textPosition.Y = (float)(screenHeight - (5 + addedTileHeightForQuestions + 1) * 64) + ((text2.Trim().Length > 1) ? dialogueFont.MeasureString(text2).Y : 0f) + 128f - (float)((questionChoices.Count - 2) * 64) + (float)(48 * i) + (float)dialogueY + (float)everythingYOffset;
		...
		if (!(currentTool is Slingshot) && !(currentTool is Shears) && !(currentTool is MilkPail) && !(currentTool is Pan))
		{
			...
			case 1:
			{
				if (farmerSprite.currentAnimationIndex > 2)
				{
					...
					tileLocation3.X++;
					tileLocation3.Y--;
					if (!(f.CurrentTool is WateringCan) && f.currentLocation.hasTileAt(tileLocation3, "Front"))
					{
						return;
					}
					tileLocation3.Y++;
				...
				if (!(currentTool is FishingRod rod3))
				{
					if (currentTool is WateringCan)
					{
						if (farmerSprite.currentAnimationIndex == 1)
						{
							...
							tileLocation4.X--;
							tileLocation4.Y--;
							if (f.currentLocation.hasTileAt(tileLocation4, "Front") && f.Position.Y % 64f < 32f)
							{
								return;
							}
						}
				...
				case 0:
					...
					{
						spriteBatch.Draw(spritesheet, new Vector2(fPosition.X - 64f + (float)toolXOffset, fPosition.Y - 160f + (float)toolYOffset), sourceRectangleForTool, color2, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
					}
					break;
				case 1:
					spriteBatch.Draw(spritesheet, new Vector2(fPosition.X - 64f + (float)toolXOffset, fPosition.Y - 160f + 8f + (float)toolYOffset), sourceRectangleForTool, color2, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
					break;
				case 2:
					spriteBatch.Draw(spritesheet, new Vector2(fPosition.X - 96f + 32f + (float)toolXOffset, fPosition.Y - 128f - 24f + (float)toolYOffset), sourceRectangleForTool, color2, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
					break;
				case 3:
					spriteBatch.Draw(spritesheet, new Vector2(fPosition.X - 96f + 24f + (float)toolXOffset, fPosition.Y - 128f - 32f + (float)toolYOffset), sourceRectangleForTool, color2, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
					break;
				case 4:
					...
					{
						spriteBatch.Draw(spritesheet, new Vector2(fPosition.X - 64f + (float)toolXOffset, fPosition.Y - 160f + 4f + (float)toolYOffset), sourceRectangleForTool, color2, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
					}
					break;
				case 5:
					spriteBatch.Draw(spritesheet, new Vector2(fPosition.X - 64f + (float)toolXOffset, fPosition.Y - 160f + (float)toolYOffset), sourceRectangleForTool, color2, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
			...
			case 3:
			{
				if (farmerSprite.currentAnimationIndex > 2)
				{
					...
					tileLocation.X--;
					tileLocation.Y--;
					if (!(f.CurrentTool is WateringCan) && f.currentLocation.hasTileAt(tileLocation, "Front") && f.Position.Y % 64f < 32f)
					{
						return;
					}
					tileLocation.Y++;
				...
				if (!(currentTool is FishingRod rod2))
				{
					if (currentTool is WateringCan)
					{
						if (farmerSprite.currentAnimationIndex == 1)
						{
							...
							tileLocation2.X--;
							tileLocation2.Y--;
							if (f.currentLocation.hasTileAt(tileLocation2, "Front") && f.Position.Y % 64f < 32f)
							{
								return;
							}
						}
					...
					else
					{
						...
						{
						case 0:
							spriteBatch.Draw(spritesheet, Utility.snapToInt(new Vector2(fPosition.X + 32f + 8f + (float)toolXOffset + (float)Math.Min(8, f.toolPower.Value * 4), fPosition.Y - 128f + 8f + (float)toolYOffset + (float)Math.Min(8, f.toolPower.Value * 4))), sourceRectangleForTool, Color.White, (float)Math.PI / 12f + (float)Math.Min(f.toolPower.Value, 2) * ((float)Math.PI / 64f), new Vector2(0f, 16f), 4f, SpriteEffects.FlipHorizontally, layerDepth);
							break;
						case 1:
			...
			if (farmerSprite.currentAnimationIndex > 2 && !(f.CurrentTool is FishingRod { isCasting: false, castedButBobberStillInAir: false, isTimingCast: false }))
			{
				...
				if (f.currentLocation.hasTileAt(tileLocation5, "Front") && f.Position.Y % 64f < 32f && f.Position.Y % 64f > 16f)
				{
					return;
				}
			}
			...
			if (!(currentTool is FishingRod fishingRod))
			{
				...
				case 0:
					if (f.FacingDirection == 0)
					{
						spriteBatch.Draw(spritesheet, Utility.snapToInt(new Vector2(fPosition.X + (float)toolXOffset, fPosition.Y - 128f - 8f + (float)toolYOffset + (float)Math.Min(8, f.toolPower.Value * 4))), sourceRectangleForTool, Color.White, 0f, new Vector2(0f, 16f), 4f, SpriteEffects.None, layerDepth);
					}
					else
					{
						spriteBatch.Draw(spritesheet, Utility.snapToInt(new Vector2(fPosition.X + (float)toolXOffset - 20f, fPosition.Y - 128f + 12f + (float)toolYOffset + (float)Math.Min(8, f.toolPower.Value * 4))), sourceRectangleForTool, Color.White, 0f, new Vector2(0f, 16f), 4f, SpriteEffects.None, layerDepth);
					}
					break;
				case 1:
					if (f.FacingDirection == 0)
			...
			if (farmerSprite.currentAnimationIndex <= 2)
			{
				...
				tileLocation6.Y--;
				if (f.currentLocation.hasTileAt(tileLocation6, "Front"))
				{
					return;
				}
			}
	...
	public static void UpdateHorseOwnership()
	{
		...
		Utility.ForEachBuilding(delegate(Stable item)
		{
			stables.Add(item);
			return true;
		});
		...
		{
			if (stable.owner.Value == -6666666 && GetPlayer(-6666666L) == null)
			{
		...
		foreach (Stable stable2 in stables)
		{
			...
			if (horse2 != null && !claimed_horses.Contains(horse2) && horse2.getOwner() != null && horse2.Name != null && horse2.Name.Length > 0 && horse_name_lookup.TryGetValue(horse2.Name, out var owner) && !horse_lookup.ContainsKey(owner.UniqueMultiplayerID))
			{
				...
				if (verbose)
				{
					log.Verbose("Assigned horse " + horse2.Name + " to " + horse2.getOwner().Name + " (Name match from different owner.)");
				}
			}
		}
		foreach (Stable stable3 in stables)
		{
			...
			if (horse3 != null && !claimed_horses.Contains(horse3) && horse3.getOwner() != null && !horse_lookup.ContainsKey(horse3.getOwner().UniqueMultiplayerID))
			{
				...
				if (verbose)
				{
					log.Verbose("Assigned horse " + horse3.Name + " to " + horse3.getOwner().Name + " (Owner's only stable)");
				}
			}
		}
	...
	public void _PerformRemoveNormalItemFromWorldOvernight(string itemId)
	{
		...
		player.team.returnedDonations.RemoveWhere((Item this_item) => _RecursiveRemoveThisNormalItemItem(this_item, itemId));
		foreach (Inventory value in player.team.globalInventories.Values)
		{
			value.RemoveWhere((Item this_item) => _RecursiveRemoveThisNormalItemItem(this_item, itemId));
		}
	...
	public string takeMapScreenshot(float? in_scale, string screenshot_name, Action onDone)
	{
		...
		if (string.IsNullOrWhiteSpace(screenshot_name))
		{
			...
			screenshotName = SaveGame.FilterFileName(player.name.Value) + $"_{now.Month}-{now.Day}-{now.Year}_{(int)now.TimeOfDay.TotalMilliseconds}";
		}
		return takeMapScreenshot(currentLocation, scale, screenshotName, onDone);
	}
	...
	private unsafe string takeMapScreenshot(GameLocation screenshotLocation, float scale, string screenshot_name, Action onDone)
	{
		...
		GetScreenshotRegion(screenshotLocation, out var start_x, out var start_y, out var width, out var height);
	...
	private static void GetScreenshotRegion(GameLocation screenshotLocation, out int startX, out int startY, out int width, out int height)
	{
		...
		try
		{
			...
			if (fields.Length != 0)
			{
				if (!ArgUtility.TryGetInt(fields, 0, out var topLeftX, out var error, "int topLeftX") || !ArgUtility.TryGetInt(fields, 1, out var topLeftY, out error, "int topLeftY") || !ArgUtility.TryGetInt(fields, 2, out var bottomRightX, out error, "int bottomRightX") || !ArgUtility.TryGetInt(fields, 3, out var bottomRightY, out error, "int bottomRightY"))
				{
