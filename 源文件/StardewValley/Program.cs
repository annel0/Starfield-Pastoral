...
public static class Program
{
	...
	public static string WriteLog(LogType logType, string message, bool append = false)
	{
		...
		else
		{
			...
			filename += $"_{Game1.uniqueIDForThisGame}_{(ulong)((Game1.player == null) ? ((long)Game1.random.Next(999999)) : ((long)Game1.player.millisecondsPlayed))}.txt";
		}
		string logFolderPath = GetLocalAppDataFolder(logFolderName);
		if (logFolderPath == null)
		{
			Game1.log.Error("WriteLog failed on GetLocalAppDataFolder(\"" + logFolderName + "\")");
			return null;
		}
