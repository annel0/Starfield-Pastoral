...
public class Multiplayer
{
	...
	public virtual void updateRoots()
	{
		...
		if (Game1.IsClient)
		{
			...
			{
				if (location.Root != null && _updatedRoots.Add(location.Root.Value))
				{
		...
		else
		{
			Utility.ForEachLocation(delegate(GameLocation gameLocation)
			{
				...
				{
					gameLocation.Root.Clock.InterpolationTicks = interpolationTicks();
					updateRoot(gameLocation.Root);
				}
				return true;
	...
	public virtual void broadcastLocationDeltas()
	{
		...
		Utility.ForEachLocation(delegate(GameLocation gameLocation)
		{
			...
			{
				broadcastLocationDelta(gameLocation);
			}
			return true;
		}, includeInteriors: false, includeGenerated: true);
	}
	...
	public virtual void broadcastEvent(Event evt, GameLocation location, Vector2 positionBeforeEvent, bool use_local_farmer = true, bool notify_when_done = false)
	{
		...
		object[] message = generateForceEventMessage(evt.id, location, (int)positionBeforeEvent.X, (int)positionBeforeEvent.Y, use_local_farmer, notify_when_done);
		if (Game1.IsServer)
		{
			foreach (KeyValuePair<long, Farmer> v in Game1.otherFarmers)
			{
	...
	{
		if (Game1.otherFarmers.Roots.TryGetValue(id, out var otherFarmer) && !disconnectingFarmers.Contains(id))
		{
