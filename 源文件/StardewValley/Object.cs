...
public class Object : Item
{
	...
	public virtual bool performToolAction(Tool t)
	{
		...
		if (base.QualifiedItemId == "(O)590" || base.QualifiedItemId == "(O)SeedSpot")
		{
			if (t is Hoe)
			{
				Random r2 = Utility.CreateDaySaveRandom((0f - tileLocation.X) * 7f, tileLocation.Y * 777f, Game1.netWorldState.Value.TreasureTotemsUsed * 777);
	...
	public virtual void DayUpdate()
	{
		...
		case "(BC)StatueOfBlessings":
			...
			break;
		case "(BC)165":
			if (!(location is AnimalHouse animalHouse) || !(heldObject.Value is Chest chest))
			{
				break;
			}
	...
	public override void actionWhenBeingHeld(Farmer who)
	{
		...
		if (location != null)
		{
			...
			if (lightSource != null && (!bigCraftable.Value || isLamp.Value))
			{
				...
				{
					location.sharedLights.AddLight(new LightSource(lightSource.Id, lightSource.textureIndex.Value, lightSource.position.Value, lightSource.radius.Value, lightSource.color.Value, LightSource.LightContext.None, who.UniqueMultiplayerID, location.NameOrUniqueName));
				}
				location.repositionLightSource(lightSource.Id, who.Position + new Vector2(32f, -64f));
			}
		}
		base.actionWhenBeingHeld(who);
	}
	...
	public virtual bool OutputMachine(MachineData machine, MachineOutputRule outputRule, Item inputItem, Farmer who, GameLocation location, bool probe, bool heldObjectOnly = false)
	{
		...
		if (!heldObjectOnly)
		{
			...
			minutesUntilReady = (int)Utility.ApplyQuantityModifiers(minutesUntilReady, machine.ReadyTimeModifiers, machine.ReadyTimeModifierMode, location, who, heldObject.Value, inputItem);
	...
	public virtual void updateWhenCurrentLocation(GameTime time)
	{
		...
		if (_hasLightSource)
		{
			...
			if (light != null && isOn.Value && !environment.hasLightSource(light.Id))
			{
		...
		if (_hasHeldObject)
		{
			...
			if (heldObject._hasLightSource)
			{
				...
				if (lightSource != null && !environment.hasLightSource(lightSource.Id))
				{
					environment.sharedLights.AddLight(lightSource.Clone());
				}
			}
	...
	public virtual void initializeLightSource(Vector2 tileLocation, bool mineShaft = false)
	{
		...
		if (bigCraftable.Value)
		{
			if (this is Torch && isOn.Value)
			{
				...
				lightSource = new LightSource(GenerateLightSourceId(tileLocation), 4, new Vector2(tileLocation.X * 64f + 32f, tileLocation.Y * 64f + y_offset), 2.5f, new Color(0, 80, 160), LightSource.LightContext.None, 0L, Location?.NameOrUniqueName);
				return;
			}
			if (isLamp.Value)
			{
		...
		if (MinutesUntilReady > 0)
		{
			...
			if (light != null)
			{
				lightSource = new LightSource(GenerateLightSourceId(tileLocation), 4, new Vector2(tileLocation.X * 64f + 32f, tileLocation.Y * 64f), light.Radius, Utility.StringToColor(light.Color) ?? Color.White, LightSource.LightContext.None, 0L, Location?.NameOrUniqueName);
			}
		}
	}
	...
	public virtual void performRemoveAction()
	{
		...
		if (environment != null)
		{
			environment.removeLightSource(lightSource?.Id);
		...
		{
			string id = ((quality.Value != 0) ? (quality.Value - 1).ToString() : preservedParentSheetIndex.Value);
			if (id != null)
			{
	...
	private void readBook(GameLocation location)
	{
		...
		if (Game1.player.stats.Get(itemId.Value) != 0 && base.ItemId != "Book_PriceCatalogue" && base.ItemId != "Book_AnimalCatalogue")
		{
			...
			bool foundAny = false;
			foreach (string tag in GetContextTags())
			{
	...
	protected bool CheckForActionOnMachine(Farmer who, bool justCheckingForActivity = false)
	{
		...
		if (readyForHarvest.Value)
		{
			...
			MachineData machineData = GetMachineData();
			Object outputObj = heldObject.Value;
			if (lastOutputRuleId.Value != null)
			{
				...
				{
					heldObject.Value = null;
					OutputMachine(machineData, outputRule, lastInputItem.Value, who, location, probe: false, heldObjectOnly: true);
					if (heldObject.Value != null)
					{
						outputObj = heldObject.Value;
					}
					else
					{
	...
	public virtual void farmerAdjacentAction(Farmer who, bool diagonal = false)
	{
		...
		case "(O)464":
			...
			{
				int.TryParse(preservedParentSheetIndex.Value, out var preservedParentSheetInt2);
				string sound = "flute";
				if (who.ActiveObject != null)
				{
					sound = getFluteBlockSoundFromHeldObject(who.ActiveObject);
				}
				Game1.playSound(sound, preservedParentSheetInt2, out internalSound);
				scale.Y = 1.3f;
				shakeTimer = 200;
		...
		{
			Vector2 v2 = new Vector2(TileLocation.X, TileLocation.Y - 1f);
			if (Location.objects.TryGetValue(v2, out var tileObj) && tileObj.IsTextSign())
			{
	...
	public virtual bool minutesElapsed(int minutes)
	{
		...
		else
		{
			...
			case "(BC)83":
				showNextIndex.Value = false;
				environment.removeLightSource(GenerateLightSourceId(tileLocation.Value));
				break;
			}
		}
		return false;
	...
	public virtual bool ShouldTimePassForMachine()
	{
		...
		{
			foreach (MachineTimeBlockers item in data.PreventTimePass)
			{
				switch (item)
				{
	...
	public override IEnumerable<Buff> GetFoodOrDrinkBuffs()
	{
		...
		if (edibility.Value <= -300 || !Game1.objectData.TryGetValue(base.ItemId, out var data))
		{
			yield break;
		}
	...
	public virtual void drawAsProp(SpriteBatch b)
	{
		...
		else
		{
			...
			b.Draw(itemData2.GetTexture(), Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, y * 64 + 32)), itemData2.GetSourceRect(), Color.White, 0f, new Vector2(8f, 8f), (scale.Y > 1f) ? getScale().Y : 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)bounds.Bottom / 10000f);
		}
	}
	...
	public virtual void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		if (hovering)
		{
			if (IsTextSign() && !string.IsNullOrEmpty(SignText))
			{
				...
				SpriteText.drawSmallTextBubble(spriteBatch, SignText, position, 256, 0.98f + TileLocation.X * 0.0001f + TileLocation.Y * 1E-06f);
			}
			hovering = false;
		}
		if (bigCraftable.Value)
	...
	public virtual void draw(SpriteBatch spriteBatch, int xNonTile, int yNonTile, float layerDepth, float alpha = 1f)
	{
		...
		else if (!Game1.eventUp || !Game1.CurrentEvent.isTileWalkedOn(xNonTile / 64, yNonTile / 64))
		{
			...
			ParsedItemData itemData2 = ItemRegistry.GetDataOrErrorItem(base.QualifiedItemId);
			spriteBatch.Draw(itemData2.GetTexture(), Game1.GlobalToLocal(Game1.viewport, new Vector2(xNonTile + 32 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), yNonTile + 32 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0))), itemData2.GetSourceRect(0, base.ParentSheetIndex), Color.White * alpha, 0f, new Vector2(8f, 8f), (scale.Y > 1f) ? getScale().Y : 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, layerDepth);
		}
	}
	...
	protected override void GetOneCopyFrom(Item source)
	{
		...
		if (source is Object fromObj)
		{
			...
			questItem.Value = fromObj.questItem.Value;
			questId.Value = fromObj.questId.Value;
						...
						case '6':
							{
								...
								if (!location.TryGetMapPropertyAs("AllowMiniFridges", out bool allowPlacement, false))
								{
	...
	public override string getDescription()
	{
		...
		string preservedId = GetPreservedItemId();
		if (preservedId != null)
		{
	...
	public override int sellToStorePrice(long specificPlayerID = -1L)
	{
		...
		float salePrice = (int)((float)price.Value * (1f + (float)base.Quality * 0.25f));
	...
	public override int salePrice(bool ignoreProfitMargins = false)
	{
		...
		default:
		{
			float salePrice = (int)((float)(price.Value * 2) * (1f + (float)quality.Value * 0.25f));
	...
	protected virtual float getPriceAfterMultipliers(float startPrice, long specificPlayerID = -1L)
	{
		...
		foreach (Farmer player in Game1.getAllFarmers())
		{
			...
			if (player.professions.Contains(1) && (base.Category == -75 || base.Category == -80 || (base.Category == -79 && !isSpawnedObject.Value)))
			{
				multiplier *= 1.1f;
			}
