...
public class Event
{
	...
	private static void SetupEventCommandsIfNeeded()
	{
		...
		if (Commands.Count == 0)
		{
			...
			foreach (MethodInfo method2 in array)
			{
				...
				if (attribute != null)
				{
					...
					{
						RegisterCommandAlias(aliases[j], method2.Name);
					}
				}
			}
		}
		...
		foreach (MethodInfo method3 in array)
		{
			...
			Preconditions[method3.Name] = preconditionDelegate;
		}
		array = methods2;
		foreach (MethodInfo method4 in array)
		{
			...
			if (attribute2 != null)
			{
				...
				for (int j = 0; j < aliases.Length; j++)
				{
					RegisterPreconditionAlias(aliases[j], method4.Name);
				}
			}
		}
	}
			...
			if (actorPositionsAfterMove.Count > 0)
			{
				...
				foreach (string s in array)
				{
					...
					foreach (NPC n2 in actors)
					{
						...
						{
							n2.Halt();
							n2.faceDirection((int)actorPositionsAfterMove[s].Z);
							actorPositionsAfterMove.Remove(s);
							break;
						}
						if (n2.Name.Equals(s))
	...
	private void trySpecialSetUp(GameLocation location)
	{
		...
		case 7:
			...
			case '2':
			{
				...
				for (int i2 = 0; i2 < eventCommands.Length; i2++)
				{
					if (!eventCommands[i2].StartsWith("makeInvisible"))
					{
						continue;
					}
					string[] args2 = ArgUtility.SplitBySpace(eventCommands[i2]);
					...
					{
						eventCommands[i2] = "makeInvisible -1000 -1000";
					}
					else
					{
						eventCommands[i2] = string.Join(" ", args2);
					}
				}
				break;
			}
		...
		for (int i = 0; i < args.Length; i += 4)
		{
			...
			else if (actorName != "farmer")
			{
				...
				if (isFarmerId)
				{
					...
					Farmer f2 = GetFarmerActor(farmerNumber);
					if (f2 != null)
					{
	...
	private void addSpecificTemporarySprite(string key, GameLocation location, string[] args)
	{
			...
			case 't':
				if (key == "ccCelebration")
				{
					...
					for (int i17 = 0; i17 < 32; i17++)
					{
						...
						aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(534, 1413, 11, 16), 99999f, 1, 99999, position2, flicker: false, flipped: false, 1f, 0f, Utility.getRandomRainbowColor(), 4f, 0f, 0f, 0f)
						{
							local = true,
		...
		case 19:
			...
			case 'E':
			{
				...
				for (int k = 0; k < 120; k++)
				{
					aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(626 + k / 28 * 10, 1891, 10, 10), 2000f, 1, 1, new Vector2(Game1.random.Next(Game1.graphics.GraphicsDevice.Viewport.Width), Game1.random.Next(Game1.graphics.GraphicsDevice.Viewport.Height)), flicker: false, flipped: false, 0.01f, 0f, Color.White, 0.1f, 0f, 0f, 0f)
					{
						motion = new Vector2(0f, -2f),
						alphaFade = 0.002f,
						scaleChange = 0.5f,
				...
				if (key == "moonlightJellies")
				{
					...
					underwaterSprites = new TemporaryAnimatedSpriteList
					{
						new TemporaryAnimatedSprite("Maps\\Festivals", new Microsoft.Xna.Framework.Rectangle(256, 16, 16, 16), 250f, 3, 9999, new Vector2(26f, 49f) * 64f, flicker: false, flipped: false, 0.1f, 0f, Color.White, 4f, 0f, 0f, 0f)
						{
							...
							xPeriodicLoopTime = 3000f,
							xPeriodicRange = 16f,
							lightId = GenerateLightSourceId($"moonlightJellies_{lightIndex2++}"),
							lightcolor = Color.Black,
							lightRadius = 1f,
		...
		case 10:
			...
			case 'r':
				...
				aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\parrots", new Microsoft.Xna.Framework.Rectangle(48, 0, 24, 24), 100f, 3, 99999, new Vector2(Game1.viewport.X - 64, 2112f), flicker: false, flipped: true, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
					...
					reachedStopCoordinate = delegate
					{
						TemporaryAnimatedSprite temporaryAnimatedSprite4 = aboveMapSprites[0];
			...
			case 'L':
				if (key == "abbyAtLake")
				{
					...
					{
						lightId = GenerateLightSourceId($"abbyAtLake_{lightIndex++}"),
						lightRadius = 2f
					});
			...
			case 'C':
				if (key == "jojaCeremony")
				{
					...
					for (int i23 = 0; i23 < 16; i23++)
					{
						...
						aboveMapSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(534, 1413, 11, 16), 99999f, 1, 99999, position3, flicker: false, flipped: false, 0.99f, 0f, Color.DeepSkyBlue, 4f, 0f, 0f, 0f)
						{
							local = true,
							motion = new Vector2(0.25f, -1.5f),
							acceleration = new Vector2(0f, -0.001f),
	...
	public void festivalUpdate(GameTime time)
	{
		...
		if (festivalTimer > 0)
		{
			...
			if (festivalTimer <= 0)
			{
				...
				{
					EndPlayerControlSequence();
					if (!TryGetFestivalDataForYear("afterEggHunt", out var rawCommands2))
					{
			...
			case 'e':
			{
				...
				for (int x2 = 0; x2 < pathsLayer.LayerWidth; x2++)
				{
					for (int y2 = 0; y2 < pathsLayer.LayerHeight; y2++)
					{
						...
						{
							festivalProps.Add(new Prop(festivalTexture, tile2.TileIndex, 1, 1, 1, x2, y2));
						}
					}
				}
				festivalTimer = 52000;
		...
		if (isFestival)
		{
			...
			if (tileAction != null)
			{
				try
				{
					...
					case "Shop":
					{
						...
						if (!opened && Utility.TryOpenShopMenu(shop_id, temporaryLocation, null, null, forceOpen: false, playOpenSound: true, delegate(string dialogue4)
						{
			...
			if (who.IsLocalPlayer && (!playerControlSequence || !playerControlSequenceID.Equals("iceFishing")))
			{
				foreach (NPC n in actors)
				{
					...
					if (((tile.X != tileLocation.X || (tile.Y != tileLocation.Y && tile.Y != tileLocation.Y + 1)) && !n.GetBoundingBox().Intersects(tileRect)) || (n.CurrentDialogue.Count < 1 && (n.CurrentDialogue.Count <= 0 || n.CurrentDialogue.Peek().isOnFinalDialogue()) && !n.Equals(festivalHost) && (!n.datable.Value || !isSpecificFestival("spring24")) && (secretSantaRecipient == null || !n.Name.Equals(secretSantaRecipient.Name))))
					{
						continue;
					}
					...
					else if ((Game1.HasDedicatedHost || Game1.serverHost == null || Game1.player.Equals(Game1.serverHost.Value)) && n.Equals(festivalHost) && (n.CurrentDialogue.Count == 0 || n.CurrentDialogue.Peek().isOnFinalDialogue()) && hostMessageKey != null)
					{
					...
					if (who.dancePartner.Value == null)
					{
						...
						else if (n.CurrentDialogue.Peek().isOnFinalDialogue())
						{
							...
							if (who.spouse != null && n.Name == who.spouse)
							{
								...
								if (n.isRoommate())
								{
									TryGetFestivalDialogueForYear(n, n.Name + "_roommate", out dialogue2);
								}
								if (dialogue2 == null)
								{
									TryGetFestivalDialogueForYear(n, n.Name + "_spouse", out dialogue2);
								}
								if (dialogue2 != null)
								{
									n.CurrentDialogue.Clear();
					...
					if (who.spouse != null && n.Name == who.spouse && !isSpecificFestival("spring24"))
					{
						...
						if (n.isRoommate())
						{
							TryGetFestivalDialogueForYear(n, n.Name + "_roommate", out dialogue3);
						}
						if (dialogue3 == null)
						{
							TryGetFestivalDialogueForYear(n, n.Name + "_spouse", out dialogue3);
						}
			...
			if (festivalData != null && isSpecificFestival("spring13"))
			{
				...
				for (int i2 = festivalProps.Count - 1; i2 >= 0; i2--)
				{
					if (festivalProps[i2].isColliding(tile2))
					{
						who.festivalScore++;
						festivalProps.RemoveAt(i2);
		...
		if (mostFishScore >= numberOfFishToWin)
		{
			...
			else
			{
				...
				for (int k = 0; k < iceFishWinners.Count; k++)
				{
					...
					winnerDialogue = winnerDialogue + " " + iceFishWinners[k].displayName;
					if (k < iceFishWinners.Count - 1)
					{
						winnerDialogue += ",";
					}
