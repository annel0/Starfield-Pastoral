...
public class Farm : GameLocation
{
	...
	public virtual Vector2 GetStarterShippingBinLocation()
	{
		...
		{
			if (!TryGetMapPropertyAs("ShippingBinLocation", out Vector2 position, false))
			{
	...
	{
		if (!TryGetMapPropertyAs("PetBowlLocation", out Vector2 tile, false))
		{
	...
	{
		if (TryGetMapPropertyAs("GreenhouseLocation", out Vector2 position, false))
		{
			return position;
		}
	...
	public void onNewGame()
	{
		...
		else if (Game1.GetFarmTypeID() == "MeadowlandsFarm")
		{
			...
			for (int k = 7; k < 20; k++)
			{
				objects.Add(new Vector2(62f, k), new Fence(new Vector2(62f, k), "322", k == 13));
			}
	...
	public override void DayUpdate(int dayOfMonth)
	{
		...
		{
			if (characters[i] is Pet pet && (hasTileAt(pet.TilePoint, "Buildings") || hasTileAt(pet.TilePoint.X + 1, pet.TilePoint.Y, "Buildings") || !CanSpawnCharacterHere(pet.Tile) || !CanSpawnCharacterHere(new Vector2(pet.TilePoint.X + 1, pet.TilePoint.Y))))
			{
				pet.WarpToPetBowl();
			}
		}
		...
		{
			int slimesEscaped = characters.RemoveWhere((NPC npc) => npc is GreenSlime && Game1.random.NextDouble() < 0.035);
			if (slimesEscaped > 0)
			{
		...
		if (ShouldSpawnBeachFarmForage())
		{
			while (Game1.random.NextDouble() < 0.9)
			{
				...
				if (!CanItemBePlacedHere(v) || hasTileAt((int)v.X, (int)v.Y, "AlwaysFront"))
				{
					continue;
				}
		...
		if (ShouldSpawnForestFarmForage() && !Game1.IsWinter)
		{
			...
			{
				Vector2 v2 = new Vector2(Game1.random.Next(18), Game1.random.Next(map.Layers[0].LayerHeight));
				if (Game1.random.NextBool() || Game1.whichFarm != 2)
				{
					v2 = getRandomTile();
				}
				if (CanItemBePlacedHere(v2, itemIsPassable: false, CollisionMask.All, CollisionMask.None) && !hasTileAt((int)v2.X, (int)v2.Y, "AlwaysFront") && ((Game1.whichFarm == 2 && v2.X < 18f) || doesTileHavePropertyNoNull((int)v2.X, (int)v2.Y, "Type", "Back").Equals("Grass")))
				{
					...
					dropObject(obj2, v2 * 64f, Game1.viewport, initialPlacement: true);
				}
			}
			if (objects.Length > 0)
			{
				...
				{
					if (Utility.TryGetRandom(objects, out key, out var o2) && o2.IsWeeds())
					{
		...
		if (terrainFeatures.Length > 0 && Game1.season == Season.Fall && Game1.dayOfMonth > 1 && Game1.random.NextDouble() < 0.05)
		{
			...
			{
				if (Utility.TryGetRandom(terrainFeatures, out var _, out var feature) && feature is Tree tree && tree.growthStage.Value >= 5 && !tree.tapped.Value && !tree.isTemporaryGreenRainTree.Value)
				{
	...
	public void addCrows()
	{
		...
		for (int i = 0; i < potentialCrows; i++)
		{
			...
			{
				if (!Utility.TryGetRandom(terrainFeatures, out var tile, out var feature) || !(feature is HoeDirt dirt2))
				{
					continue;
				}
				...
				if (!scarecrow)
				{
					dirt2.destroyCrop(showAnimation: false);
	...
	public void showShipment(Item item, bool playThrowSound = true)
	{
		...
		Vector2 initialPosition = new Vector2(71f, 13f) * 64f + new Vector2(8 + Game1.random.Next(6), 2f) * 4f;
		...
		foreach (bool isColorOverlay in array)
		{
			...
			{
				temporarySprites.Add(new TemporaryAnimatedSprite(itemData.TextureName, itemData.GetSourceRect(isColorOverlay ? 1 : 0), initialPosition, flipped: false, 0f, Color.White)
				{
					interval = 9999f,
					scale = 4f,
					alphaFade = 0.045f,
	...
	public override Item getFish(float millisecondsAfterNibble, string bait, int waterDepth, Farmer who, double baitPotency, Vector2 bobberTile, string location = null)
	{
		if (_fishLocationOverride == null)
		{
			...
			if (fields.Length != 0)
			{
				if (!ArgUtility.TryGet(fields, 0, out var targetLocation, out var error, allowBlank: true, "string targetLocation") || !ArgUtility.TryGetFloat(fields, 1, out var chance, out error, "float chance"))
				{
	...
	protected override void resetLocalState()
	{
		...
		if (Game1.MasterPlayer.mailReceived.Contains("Farm_Eternal") && !Game1.player.mailReceived.Contains("Farm_Eternal_Parrots") && !IsRainingHere())
		{
			...
			for (int k = 0; k < 20; k++)
			{
				DelayedAction.playSoundAfterDelay("batFlap", 5000 + k * 250);
			}
	...
	public virtual Vector2 GetSpouseOutdoorAreaCorner()
	{
		...
		{
			if (!TryGetMapPropertyAs("SpouseAreaLocation", out Vector2 position, false))
			{
	...
	public virtual Point GetMainMailboxPosition()
	{
		...
		{
			if (!TryGetMapPropertyAs("MailboxLocation", out Point position, false))
			{
			...
			if (buildingData?.ActionTiles != null)
			{
				foreach (BuildingActionTile action in buildingData.ActionTiles)
				{
					...
					{
						mapMainMailboxPosition = new Point(farmhouse.tileX.Value + action.Tile.X, farmhouse.tileY.Value + action.Tile.Y);
						break;
					}
				}
			}
	...
	public virtual Point GetGrandpaShrinePosition()
	{
		...
		{
			if (!TryGetMapPropertyAs("GrandpaShrineLocation", out Point position, false))
			{
	...
	public virtual Point GetMainFarmHouseEntry()
	{
		...
		{
			if (!TryGetMapPropertyAs("FarmHouseEntry", out Point position, false))
			{
			...
			if (farmhouse != null)
			{
				mainFarmhouseEntry = new Point(farmhouse.tileX.Value + farmhouse.humanDoor.X, farmhouse.tileY.Value + farmhouse.humanDoor.Y + 1);
			}
		}
	...
	public bool ShouldSpawnMountainOres()
	{
		...
		{
			_mountainForageRectangle = (TryGetMapPropertyAs("SpawnMountainFarmOreRect", out Microsoft.Xna.Framework.Rectangle area, false) ? area : Microsoft.Xna.Framework.Rectangle.Empty);
		}
