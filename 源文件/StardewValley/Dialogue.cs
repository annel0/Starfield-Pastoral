...
public class Dialogue
{
	...
	protected virtual void parseDialogueString(string masterString, string translationKey)
	{
		...
		for (int i = 0; i < masterDialogueSplit.Length; i++)
		{
			...
			for (int j2 = 1; j2 < rawSplit.Length; j2 += 2)
			{
				...
				playerResponses.Add(new NPCDialogueResponse(null, -1, "quickResponse" + j2, Game1.parseText(choice)));
			...
			{
				if (farmer.DialogueQuestionsAnswered.Contains(prerequisiteSplit[j3]))
				{
					choseOne = true;
					break;
				}
	...
	{
		if (dialogues.Count > 0 && speaker != null && speaker.shouldWearIslandAttire.Value && Game1.player.friendshipData.TryGetValue(speaker.Name, out var friendship) && friendship.IsDivorced() && CurrentEmotion == "$u")
		{
	...
	public virtual void prepareCurrentDialogueForDisplay()
	{
		...
		if (flag.HasValue && flag == true && !farmer.canUnderstandDwarves)
		{
	...
	public string ReplacePlayerEnteredStrings(string str)
	{
		...
		if (str.Contains('%'))
		{
			...
			string favoriteThing = Utility.FilterUserName(farmer.favoriteThing.Value);
	...
	public static string randomName()
	{
		...
		default:
		{
			...
			if (Game1.random.NextBool() && !Enumerable.Contains(vowels, lastLetter))
			{
				name += Game1.random.Choose(consonantEndings);
			}
			else if (Enumerable.Contains(vowels, lastLetter))
			{
			...
			{
				if (Enumerable.Contains(vowels, name[i2].ToString()) && Enumerable.Contains(vowels, name[i2 - 2].ToString()))
				{
					switch (name[i2 - 1])
					{
					case 'c':
			...
			return ReplaceBadRandomName(name);
		}
		}
	}
	...
	private void checkEmotions()
	{
		...
		if (digits > 0)
		{
			string emote = (CurrentEmotion = text.Substring(emoteIndex, digits + 1));
