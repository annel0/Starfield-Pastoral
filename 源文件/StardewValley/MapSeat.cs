	...
	public List<Vector2> GetSeatPositions(bool ignore_offsets = false)
	{
		...
		if (!(value == "playground"))
		{
			...
			else if (seatType.Value.StartsWith("custom "))
			{
				...
				Vector2 seat2 = new Vector2(tilePosition.X + customDrawValues.Value.X, tilePosition.Y);
				if (!ignore_offsets)
				{
					seat2.Y += customDrawValues.Value.Y;
				}
		...
		{
			Vector2 seat3 = new Vector2(tilePosition.X + 0.75f, tilePosition.Y);
			if (!ignore_offsets)
			{
				seat3.Y -= 0.1f;
			}
