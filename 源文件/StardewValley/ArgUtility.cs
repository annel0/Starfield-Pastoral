...
public static class ArgUtility
{
	...
	public static string[] SplitQuoteAware(string input, char delimiter, StringSplitOptions splitOptions = StringSplitOptions.None, bool keepQuotesAndEscapes = false)
	{
		...
		{
			for (int i2 = values.Count - 1; i2 >= 0; i2--)
			{
				values[i2] = values[i2].Trim();
				if (splitOptionsRemoveEmpty && values[i2].Length == 0)
				{
					values.RemoveAt(i2);
				}
			}
	...
	public static string UnsplitQuoteAware(string[] input, char delimiter, int startAt = 0, int count = int.MaxValue)
	{
		...
		int i = startAt;
		for (int endAt = startAt + count - 1; i <= endAt; i++)
		{
			string arg = input[i];
			if (arg.Contains('"'))
			{
	...
	public static bool TryGetEnum<TEnum>(string[] array, int index, out TEnum value, out string error, [CallerArgumentExpression("value")] string name = null) where TEnum : struct
	{
		...
		if (!Utility.TryParseEnum<TEnum>(raw, out value))
		{
			...
			value = default(TEnum);
			error = GetValueParseError(array, index, name, required: true, $"an enum of type '{type.FullName ?? type.Name}' (should be one of {string.Join(", ", Enum.GetNames(typeof(TEnum)))})");
			return false;
		}
		error = null;
		return true;
	...
	public static bool TryGetOptionalEnum<TEnum>(string[] array, int index, out TEnum value, out string error, TEnum defaultValue = default(TEnum), [CallerArgumentExpression("value")] string name = null) where TEnum : struct
	{
		...
		{
			Type type = typeof(TEnum);
			error = GetValueParseError(array, index, name, required: false, "an enum of type '" + (type.FullName ?? type.Name) + "'");
			value = defaultValue;
			return false;
		}
		error = null;
	...
	{
		if (!TryGetInt(array, index, out var x, out error, (name != null) ? (name + " > x") : null) || !TryGetInt(array, index + 1, out var y, out error, (name != null) ? (name + " > y") : null))
		{
			value = Point.Zero;
			return false;
		}
	...
	{
		if (!TryGetInt(array, index, out var x, out error, (name != null) ? (name + " > x") : null) || !TryGetInt(array, index + 1, out var y, out error, (name != null) ? (name + " > y") : null) || !TryGetInt(array, index + 2, out var width, out error, (name != null) ? (name + " > width") : null) || !TryGetInt(array, index + 3, out var height, out error, (name != null) ? (name + " > height") : null))
		{
	...
	public static bool TryGetVector2(string[] array, int index, out Vector2 value, out string error, bool integerOnly = false, [CallerArgumentExpression("value")] string name = null)
	{
		...
		if (integerOnly)
		{
			if (TryGetInt(array, index, out var x, out error, xName) && TryGetInt(array, index + 1, out var y, out error, yName))
			{
				value = new Vector2(x, y);
				return true;
			}
		}
		else if (TryGetFloat(array, index, out x2, out error, xName) && TryGetFloat(array, index + 1, out y2, out error, yName))
		{
