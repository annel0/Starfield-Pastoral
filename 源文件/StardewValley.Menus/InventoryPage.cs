...
public class InventoryPage : IClickableMenu
{
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		foreach (ClickableComponent c in equipmentIcons)
		{
			...
			case "Trinket":
				if (Game1.player.stats.Get("trinketSlots") != 0 && checkHeldItem((Item item) => item == null || item is Trinket))
				{
			...
			{
				if (Game1.player.Items[i] == null || checkHeldItem((Item item) => Game1.player.Items[i].canStackWith(item)))
				{
		...
		if (checkHeldItem((Item item) => item?.QualifiedItemId == "(O)434"))
		{
		...
		{
			if (checkHeldItem((Item item) => item is Ring))
			{
				if (Game1.player.leftRing.Value == null)
				{
			...
			else if (checkHeldItem((Item item) => item is Hat))
			{
				if (Game1.player.hat.Value == null)
				{
			...
			else if (checkHeldItem((Item item) => item is Boots))
			{
				if (Game1.player.boots.Value == null)
				{
			...
			else if (checkHeldItem((Item item) => item is Clothing clothing && clothing.clothesType.Value == Clothing.ClothesType.SHIRT))
			{
				if (Game1.player.shirtItem.Value == null)
				{
			...
			else if (checkHeldItem((Item item) => item is Clothing clothing && clothing.clothesType.Value == Clothing.ClothesType.PANTS))
			{
				if (Game1.player.pantsItem.Value == null)
				{
			...
			else if (checkHeldItem((Item item) => item is Trinket) && Game1.player.stats.Get("trinketSlots") != 0)
			{
				bool success = false;
				for (int i2 = 0; i2 < Game1.player.trinketItems.Count; i2++)
				{
					if (Game1.player.trinketItems[i2] == null)
					{
						Game1.player.trinketItems[i2] = takeHeldItem() as Trinket;
						success = true;
						break;
					}
				}
			...
			if (inventory.getInventoryPositionOfClick(x, y) >= 12)
			{
				...
				for (i3 = 0; i3 < 12; i3++)
				{
					if (Game1.player.Items[i3] == null || checkHeldItem((Item item) => Game1.player.Items[i3].canStackWith(item)))
					{
			...
			{
				int i4;
				for (i4 = 12; i4 < Game1.player.Items.Count; i4++)
				{
					if (Game1.player.Items[i4] == null || checkHeldItem((Item item) => Game1.player.Items[i4].canStackWith(item)))
					{
						if (Game1.player.CurrentToolIndex == i4 && checkHeldItem())
						{
							Game1.player.CursorSlotItem.actionWhenBeingHeld(Game1.player);
						}
						setHeldItem(Utility.addItemToInventory(takeHeldItem(), i4, inventory.actualInventory));
						if (checkHeldItem())
						{
							Game1.player.CursorSlotItem.actionWhenStopBeingHeld(Game1.player);
						}
		...
		if (trashCan.containsPoint(x, y) && checkHeldItem((Item item) => item?.canBeTrashed() ?? false))
		{
			Utility.trashItem(takeHeldItem());
		...
		else if (!isWithinBounds(x, y) && checkHeldItem((Item item) => item?.canBeTrashed() ?? false))
		{
