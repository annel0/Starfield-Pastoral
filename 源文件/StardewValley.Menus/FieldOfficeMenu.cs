...
public class FieldOfficeMenu : MenuWithInventory
{
	...
	private bool donate(int index, int x, int y)
	{
		if (pieceHolders[index].containsPoint(x, y) && pieceHolders[index].item == null)
		{
			...
			pieceHolders[index].item = ItemRegistry.Create(item.QualifiedItemId);
			checkForSetFinish();
			gotReward = office.donatePiece(index);
			madeADonation = true;
			Game1.playSound("newArtifact");
			Game1.multiplayer.globalChatInfoMessage("FieldOfficeDonation", Game1.player.Name, TokenStringBuilder.ItemNameFor(item));
			return true;
		}
		return false;
	}
