...
public class CollectionsPage : IClickableMenu
{
		...
		: base(x, y, width, height)
	{
		...
		foreach (KeyValuePair<int, string> kvp in Game1.achievements)
		{
			...
			if (farmerHas2 || (split[2].Equals("true") && (split[3].Equals("-1") || farmerHasAchievements(split[3]))))
			{
				int xPos2 = baseX + widthUsed[5] % collectionWidth * 68;
				int yPos2 = baseY + widthUsed[5] / collectionWidth * 68;
				collections[5][0].Add(new ClickableTextureComponent(kvp.Key + " " + farmerHas2, new Rectangle(xPos2, yPos2, 64, 64), null, "", Game1.mouseCursors, Game1.getSourceRectForStandardTileSheet(Game1.mouseCursors, 25), 1f));
				widthUsed[5]++;
			}
			else
			{
				...
				collections[5][0].Add(new ClickableTextureComponent("??? false", new Rectangle(xPos3, yPos3, 64, 64), null, "???", Game1.mouseCursors, Game1.getSourceRectForStandardTileSheet(Game1.mouseCursors, 25), 1f));
				widthUsed[5]++;
			}
		}
		if (Game1.player.secretNotesSeen.Count > 0)
		{
			...
			foreach (int i2 in secretNotesData.Keys)
			{
				if (i2 >= GameLocation.JOURNAL_INDEX)
				{
					if (!show_journals)
					{
						continue;
				...
				if (i2 >= GameLocation.JOURNAL_INDEX)
				{
					collections[6][0].Add(new ClickableTextureComponent(i2 + " " + Game1.player.secretNotesSeen.Contains(i2), new Rectangle(xPos4, yPos4, 64, 64), null, "", Game1.objectSpriteSheet, Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, 842, 16, 16), 4f, Game1.player.secretNotesSeen.Contains(i2)));
				}
				else
				{
					collections[6][0].Add(new ClickableTextureComponent(i2 + " " + Game1.player.secretNotesSeen.Contains(i2), new Rectangle(xPos4, yPos4, 64, 64), null, "", Game1.objectSpriteSheet, Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, 79, 16, 16), 4f, Game1.player.secretNotesSeen.Contains(i2)));
				}
				widthUsed[6]++;
			}
		}
		...
		foreach (string s2 in Game1.player.mailReceived)
		{
			if (mail.TryGetValue(s2, out var rawText))
			{
				...
				letters.Add(new ClickableTextureComponent(s2 + " true " + ((split2.Length > 1) ? split2[1] : "???"), new Rectangle(xPos5, yPos5, 64, 64), null, "", Game1.mouseCursors, new Rectangle(190, 423, 14, 11), 4f, drawShadow: true)
				{
					myID = letters.Count,
					...
					downNeighborID = ((yPos5 + 68 > yPositionOnScreen + height - 128) ? (-7777) : (letters.Count + collectionWidth)),
	...
	{
		if (oldPage is CollectionsPage oldCollectionsPage)
		{
			sideTabs[currentTab].bounds.X -= widthToMoveActiveTab;
			currentTab = oldCollectionsPage.currentTab;
			currentPage = oldCollectionsPage.currentPage;
			sideTabs[currentTab].bounds.X += widthToMoveActiveTab;
		}
	}
		...
		foreach (ClickableTextureComponent c2 in collections[currentTab][currentPage])
		{
			if (c2.containsPoint(x, y, 2))
			{
				...
				if (currentTab == 5 || (data_split.Length > 1 && Convert.ToBoolean(data_split[1])) || (data_split.Length > 2 && Convert.ToBoolean(data_split[2])))
				{
					if (currentTab == 7)
					{
						hoverText = Game1.parseText(c2.name.Substring(c2.name.IndexOf(' ', c2.name.IndexOf(' ') + 1) + 1), Game1.smallFont, 256);
					}
					else
					{
	...
	public string createDescription(string id)
	{
		...
		default:
		{
			...
			string displayName = Game1.content.LoadStringReturnNullIfNotFound("Strings\\Objects:" + data.ItemId + "_CollectionsTabName") ?? data.DisplayName;
			string dataDescription = Game1.content.LoadStringReturnNullIfNotFound("Strings\\Objects:" + data.ItemId + "_CollectionsTabDescription") ?? data.Description;
			description = description + displayName + Environment.NewLine + Environment.NewLine + Game1.parseText(dataDescription, Game1.smallFont, 256) + Environment.NewLine + Environment.NewLine;
			switch (data.ObjectType)
			{
			case "Arch":
			{
				description += (Game1.player.archaeologyFound.TryGetValue(id, out var fields2) ? Game1.content.LoadString("Strings\\UI:Collections_Description_ArtifactsFound", fields2[0]) : "");
				break;
			}
			case "Cooking":
			{
			...
			case "Fish":
			{
				if (Game1.player.fishCaught.TryGetValue("(O)" + id, out var fields))
				{
			...
			default:
			{
				description = ((!(data.ObjectType == "Minerals") && data.Category != -2) ? (description + Game1.content.LoadString("Strings\\UI:Collections_Description_NumberShipped", Game1.player.basicShipped.TryGetValue(id, out var timesFound) ? timesFound : 0)) : (description + Game1.content.LoadString("Strings\\UI:Collections_Description_MineralsFound", Game1.player.mineralsFound.TryGetValue(id, out var timesFound2) ? timesFound2 : 0)));
				break;
			}
			}
	...
	public override void draw(SpriteBatch b)
	{
		...
		foreach (ClickableTextureComponent c in collections[currentTab][currentPage])
		{
			...
			bool drawColorFaded = (currentTab == 4 && Convert.ToBoolean(nameParts[2])) || (currentTab == 5 && !drawColor && c.hoverText != "???");
