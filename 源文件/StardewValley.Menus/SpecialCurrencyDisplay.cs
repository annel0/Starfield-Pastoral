...
public class SpecialCurrencyDisplay
{
	...
	public virtual void ShowCurrency(string currency, Func<bool> keepOpen = null, float timeToLive = 5f)
	{
		...
		foreach (CurrencyRenderInfo rendered in displayedCurrencies)
		{
		...
		if (registeredCurrencyDisplays.TryGetValue(currency, out var data))
		{
			displayedCurrencies.Add(new CurrencyRenderInfo(data, keepOpen, timeToLive));
		}
		else
		{
	...
	public virtual void OnCurrencyChange(NetIntDelta field, int oldValue, int newValue)
	{
		...
		foreach (CurrencyRenderInfo render in displayedCurrencies)
		{
		...
		foreach (CurrencyDisplayType currency in registeredCurrencyDisplays.Values)
		{
			if ((object)currency.field == field)
			{
				...
				displayedCurrencies.Add(render2);
				return;
			}
		}
	...
	public virtual void Update(GameTime time)
	{
		...
		{
			CurrencyRenderInfo render = displayedCurrencies[i];
	...
	public virtual void Draw(SpriteBatch b)
	{
		...
		foreach (CurrencyRenderInfo render in displayedCurrencies)
		{
			...
			Vector2 drawPosition = GetUpperLeft(render.slidePosition);
			if (leftOffset > 0)
			{
