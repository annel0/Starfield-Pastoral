...
public class Toolbar : IClickableMenu
{
	...
	public void shifted(bool right)
	{
		...
		for (int i2 = buttons.Count - 1; i2 >= 0; i2--)
		{
			buttons[i2].scale = 1f + (float)(11 - i2) * 0.03f;
		}
	}
	...
	public override void draw(SpriteBatch b)
	{
		...
		for (int k = 0; k < 12; k++)
		{
			...
			Vector2 toDraw2 = new Vector2(Game1.uiViewport.Width / 2 - 384 + k * 64, yPositionOnScreen - 96 + 8);
			if (Game1.player.Items.Count > k && Game1.player.Items[k] != null)
			{
				Game1.player.Items[k].drawInMenu(b, toDraw2, (Game1.player.CurrentToolIndex == k) ? 0.9f : (buttons[k].scale * 0.8f), transparency, 0.88f);
			}
		}
		if (hoverItem != null)
		{
