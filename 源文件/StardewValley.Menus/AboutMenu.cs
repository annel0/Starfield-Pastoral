...
public class AboutMenu : IClickableMenu
{
	...
	public void SetUpCredits()
	{
		foreach (string line in Game1.temporaryContent.Load<List<string>>("Strings\\credits"))
		{
			...
			{
				credits.Add(new TextCreditsBlock(line));
			}
		}
		Vector2 topLeft = Utility.getTopLeftPositionForCenteringOnScreen(width, base.height);
		...
		downButton = new ClickableTextureComponent(new Rectangle((int)topLeft.X + width - 80, (int)topLeft.Y + base.height - 32, 64, 64), Game1.mouseCursors, Game1.getSourceRectForStandardTileSheet(Game1.mouseCursors, 11), 0.8f)
		{
			myID = 95555,
			upNeighborID = -99998,
			rightNeighborID = -99998,
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		else
		{
			...
			int oldYpos = yPos;
			int i = 0;
			while (yPos < yPositionOnScreen + base.height - 64 && credits.Count > currentCreditsIndex + i)
			{
	...
	{
		Vector2 topLeft = Utility.getTopLeftPositionForCenteringOnScreen(width, base.height - 100);
		if (!Game1.options.showClearBackgrounds)
		{
			b.Draw(Game1.fadeToBlackRect, Game1.graphics.GraphicsDevice.Viewport.Bounds, Color.Black * 0.75f);
		}
		IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(473, 36, 24, 24), (int)topLeft.X, (int)topLeft.Y, width, base.height, Color.White, 4f, drawShadow: false);
		int yPos = yPositionOnScreen + 96;
		int i = 0;
		while (yPos < yPositionOnScreen + base.height - 64 && credits.Count > currentCreditsIndex + i)
		{
