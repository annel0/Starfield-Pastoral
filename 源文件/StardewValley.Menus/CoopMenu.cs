...
public class CoopMenu : LoadGameMenu
{
	...
	protected virtual void onLobbyUpdate(object lobby)
	{
		try
		{
			...
			if (!Enumerable.Contains(farmUsers, selfID) && !newFarmhands)
			{
				return;
			}
			farm.PreviouslyJoined = Enumerable.Contains(farmUsers, selfID);
			if (menuSlots == null)
			{
				return;
			}
	...
	private void enterIPPressed()
	{
		...
		}, last_entered_ip, "join_menu", filterInput: false));
	}
	...
	private void drawTabs(SpriteBatch b)
	{
		if (!_splitScreen && isSetUp)
		{
			...
			if (joinTab.visible)
			{
				...
				bool colorHover = currentTab != Tab.JOIN_TAB && joinTab.containsPoint(Game1.getMouseX(), Game1.getMouseY());
			...
			if (hostTab.visible)
			{
				bool colorSelect2 = currentTab == Tab.HOST_TAB;
				bool colorHover2 = currentTab != Tab.HOST_TAB && hostTab.containsPoint(Game1.getMouseX(), Game1.getMouseY());
				IClickableMenu.drawTextureBox(b, Game1.menuTexture, new Rectangle(0, 256, 60, 60), hostTab.bounds.X, hostTab.bounds.Y, hostTab.bounds.Width, hostTab.bounds.Height + ((!smallScreenFormat) ? 64 : 0), colorSelect2 ? selectColor : (colorHover2 ? hoverColor : Color.White), 1f, drawShadow: false);
				Utility.drawTextWithColoredShadow(b, hostTab.label, Game1.dialogueFont, new Vector2(hostTab.bounds.Center.X, hostTab.bounds.Y + 40) - Game1.dialogueFont.MeasureString(hostTab.label) / 2f, Game1.textColor, colorHover2 ? hoverShadow : (colorSelect2 ? selectShadow : Game1.textShadowDarkerColor), 1.01f);
			}
		}
	}
