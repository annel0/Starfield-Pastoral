...
public class FarmhandMenu : LoadGameMenu
{
	...
	{
		if (button == Buttons.B && readyToClose())
		{
			exitThisMenu();
		}
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		{
			if (slotButtons[i].containsPoint(x, y) && i < MenuSlots.Count && MenuSlots[currentItemIndex + i] is FarmhandSlot slot && slot.BelongsToAnotherPlayer())
			{
	...
	public override void performHoverAction(int x, int y)
	{
		...
		for (int i = 0; i < slotButtons.Count; i++)
		{
			if (currentItemIndex + i < MenuSlots.Count && slotButtons[i].containsPoint(x, y) && MenuSlots[currentItemIndex + i] is FarmhandSlot farmhandSlot && farmhandSlot.BelongsToAnotherPlayer())
			{
