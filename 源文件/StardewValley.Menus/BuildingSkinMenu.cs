...
public class BuildingSkinMenu : IClickableMenu
{
		...
		: base(Game1.uiViewport.Width / 2 - WindowWidth / 2, Game1.uiViewport.Height / 2 - WindowHeight / 2, WindowWidth, WindowHeight)
	{
		...
		RepositionElements();
		SetSkin(Math.Max(Skins.FindIndex((SkinEntry skinEntry) => skinEntry.Id == Building.skinId.Value), 0));
	...
	public override void draw(SpriteBatch b)
	{
		...
		Building?.drawInMenu(b, (int)buildingDrawCenter.X - (int)((float)Building.tilesWide.Value / 2f * 64f), (int)buildingDrawCenter.Y - sourceRect.Height * 4 / 2);
		b.End();
