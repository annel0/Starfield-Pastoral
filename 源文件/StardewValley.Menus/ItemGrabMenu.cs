...
public class ItemGrabMenu : MenuWithInventory
{
	...
	{
		IList<Item> playerInventory = inventory.actualInventory;
		IList<Item> chestInventory = ItemsToGrabMenu.actualInventory;
		...
		for (int playerIndex = 0; playerIndex < playerInventory.Count; playerIndex++)
		{
			...
			if (playerItem != null)
			{
				if (firstEmptyChestIndex == -1 && chestInventory.Count < ItemsToGrabMenu.capacity)
				{
			...
			if (mergedAny)
			{
				TransferredItemSprite itemSprite = new TransferredItemSprite(originalPlayerItem.getOne(), inventory.inventory[playerIndex].bounds.X, inventory.inventory[playerIndex].bounds.Y);
				_transferredItemSprites.Add(itemSprite);
			}
		}
		foreach (int slotIndex in affectedChestSlots)
		{
			ItemsToGrabMenu.ShakeItem(slotIndex);
		}
	}
