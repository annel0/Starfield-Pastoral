...
public class JunimoNoteMenu : IClickableMenu
{
	...
	public virtual void HandlePartialDonation(Item item, ClickableTextureComponent slot)
	{
		...
		if (slot.item == null)
		{
			...
			amountToDonate = Math.Min(currentPartialIngredientDescription.Value.stack, item.Stack);
			partialDonationItem.Stack = amountToDonate;
		...
		{
			amountToDonate = Math.Min(currentPartialIngredientDescription.Value.stack - partialDonationItem.Stack, item.Stack);
			partialDonationItem.Stack += amountToDonate;
			item = item.ConsumeStack(amountToDonate);
		}
		if (amountToDonate > 0)
		{
			Item donatedItem = heldItem.getOne();
			...
			{
				partialDonationComponents.Add(donatedItem);
			}
		...
		else if (amountToDonate > 0 && playSound)
		{
			Game1.playSound("sell");
		}
	}
		...
		if (specificBundlePage)
		{
			...
			if (partialDonationItem != null)
			{
				for (int i = 0; i < ingredientSlots.Count; i++)
				{
					...
					{
						int count = 0;
						foreach (Item contributedItem in partialDonationComponents)
						{
	...
	public override void performHoverAction(int x, int y)
	{
		...
		if (specificBundlePage)
		{
			...
			if (heldItem != null)
			{
				...
				{
					if (c2.bounds.Contains(x, y) && CanBePartiallyOrFullyDonated(heldItem) && (partialDonationItem == null || c2.item == partialDonationItem))
					{
						c2.sourceRect.X = 530;
						c2.sourceRect.Y = 262;
					}
	...
	public override void draw(SpriteBatch b)
	{
		...
		else
		{
			...
			for (int i = 0; i < ingredientList.Count; i++)
			{
				...
				ClickableTextureComponent c2 = ingredientList[i];
	...
	public override void gameWindowSizeChanged(Rectangle oldBounds, Rectangle newBounds)
	{
		...
		{
			Point p = getBundleLocationFromNumber(j);
			bundles[j].bounds.X = p.X;
			bundles[j].bounds.Y = p.Y;
			bundles[j].sprite.position = new Vector2(p.X, p.Y);
		}
		if (!specificBundlePage)
		{
			return;
		...
		{
			BundleIngredientDescription ingredient = currentPageBundle.ingredients[l];
			...
			if (metadata?.TypeIdentifier == "(O)")
			{
				...
				ingredientList.Add(new ClickableTextureComponent("", ingredientListRectangles[l], "", item.DisplayName, texture, sourceRect, 4f)
				{
					myID = l + 1000,
					item = item,
					upNeighborID = -99998,
