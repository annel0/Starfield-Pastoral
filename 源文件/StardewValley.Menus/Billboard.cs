...
public class Billboard : IClickableMenu
{
	...
	public virtual List<BillboardEvent> GetEventsForDay(int day, Dictionary<int, List<NPC>> birthdays)
	{
		...
		if (Utility.TryGetPassiveFestivalDataForDay(day, Game1.season, null, out var id2, out var festivalData, ignoreConditionsCheck: true) && (festivalData?.ShowOnCalendar ?? false))
		{
			string festivalName2 = TokenParser.ParseText(festivalData.DisplayName);
			...
			{
				curEvents.Add(new BillboardEvent(BillboardEventType.PassiveFestival, new string[1] { id2 }, festivalName2));
			}
		}
		if (Game1.IsSummer && (day == 20 || day == 21))
		{
			string festivalName3 = Game1.content.LoadString("Strings\\1_6_Strings:TroutDerby");
			curEvents.Add(new BillboardEvent(BillboardEventType.FishingDerby, LegacyShims.EmptyArray<string>(), festivalName3));
		}
		else if (Game1.IsWinter && (day == 12 || day == 13))
		{
			string festivalName4 = Game1.content.LoadString("Strings\\1_6_Strings:SquidFest");
			curEvents.Add(new BillboardEvent(BillboardEventType.FishingDerby, LegacyShims.EmptyArray<string>(), festivalName4));
		}
		...
		foreach (Farmer farmer in onlineFarmers)
		{
			...
			else
			{
				...
				{
					Farmer spouse_farmer = Game1.GetPlayer(spouseId.Value);
	...
	public override void draw(SpriteBatch b)
	{
		...
		else
		{
			...
			bool drawAllStars = Game1.stats.Get("BillboardQuestsDone") % 3 == 0 && Game1.questOfTheDay != null && Game1.questOfTheDay.completed.Value;
			for (int j = 0; j < (drawAllStars ? 3 : (Game1.stats.Get("BillboardQuestsDone") % 3)); j++)
			{
