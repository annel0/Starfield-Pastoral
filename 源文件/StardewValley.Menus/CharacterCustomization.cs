...
public class CharacterCustomization : IClickableMenu
{
	...
	public override void receiveLeftClick(int x, int y, bool playSound = true)
	{
		...
		{
			foreach (ClickableTextureComponent c2 in farmTypeButtons)
			{
				if (c2.containsPoint(x, y) && !c2.name.Contains("Gray"))
				{
					optionButtonClick(c2.name);
					c2.scale -= 0.5f;
		...
		{
			foreach (ClickableComponent c4 in leftSelectionButtons)
			{
				if (c4.containsPoint(x, y))
				{
					selectionClick(c4.name, -1);
					if (c4.scale != 0f)
					{
						c4.scale -= 0.25f;
		...
		{
			foreach (ClickableComponent c5 in rightSelectionButtons)
			{
				if (c5.containsPoint(x, y))
				{
					selectionClick(c5.name, 1);
					if (c5.scale != 0f)
					{
						c5.scale -= 0.25f;
	...
	public override void performHoverAction(int x, int y)
	{
		...
		foreach (ClickableTextureComponent c2 in rightSelectionButtons)
		{
		...
		foreach (ClickableTextureComponent c4 in genderButtons)
		{
			if (c4.containsPoint(x, y))
			{
	...
	public override void draw(SpriteBatch b)
	{
		...
		foreach (ClickableComponent c3 in labels)
		{
			...
			else if (c3 == separateWalletLabel)
			{
				offset = 21f - Game1.smallFont.MeasureString(c3.name).X / 2f;
				subYOffset = 4f;
				sub = (Game1.player.team.useSeparateWallets.Value ? separateWalletString : sharedWalletString);
			}
			else
			{
