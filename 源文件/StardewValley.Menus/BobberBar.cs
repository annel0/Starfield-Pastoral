	...
	public override void draw(SpriteBatch b)
	{
		...
		if (scale == 1f)
		{
			...
			if (challengeBaitFishes > -1)
			{
				...
				Utility.drawWithShadow(b, Game1.mouseCursors_1_6, new Vector2((float)(xPosition2 - 24) + everythingShake.X, (float)(yPos - 16) + everythingShake.Y), new Rectangle(240, 31, 15, 38), Color.White, 0f, Vector2.Zero, 4f);
				for (int y = 0; y < 3; y++)
				{
					if (y < challengeBaitFishes)
					{
						Utility.drawWithShadow(b, Game1.mouseCursors_1_6, new Vector2(xPosition2 - 12, (float)yPos + (float)(y * 20) * 2f) + everythingShake, new Rectangle(236, 205, 19, 19), Color.White, 0f, new Vector2(0f, 0f), 2f, flipped: false, 0.88f);
					}
					else
					{
						b.Draw(Game1.mouseCursors_1_6, new Vector2(xPosition2 - 12, (float)yPos + (float)(y * 20) * 2f) + everythingShake, new Rectangle(217, 205, 19, 19), Color.White, 0f, new Vector2(0f, 0f), 2f, SpriteEffects.None, 0.88f);
					}
				}
			}
		}
