...
public class MapPage : IClickableMenu
{
		...
		: base(x, y, width, height)
	{
		...
		scrollText = mapPosition?.Data.GetScrollText(playerTile);
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			{
				if (points.TryGetValue(tooltip2.NamespacedId, out var component2))
				{
	...
	public virtual void drawMiniPortraits(SpriteBatch b, float alpha = 1f)
	{
		...
		foreach (Farmer player in Game1.getOnlineFarmers())
		{
			...
			MapAreaPositionWithContext? positionData = (player.IsLocalPlayer ? mapPosition : WorldMapManager.GetPositionData(player.currentLocation, tile));
			if (positionData.HasValue && !(positionData.Value.Data.Region.Id != mapRegion.Id))
			{
	...
	public virtual void drawMap(SpriteBatch b, bool drawBorders = true, float alpha = 1f)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			{
				Rectangle destRect2 = overlay.GetOffsetMapPixelArea(mapBounds.X, mapBounds.Y);
