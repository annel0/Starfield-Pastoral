...
public class ChatTextBox(Texture2D textBoxTexture, Texture2D caretTexture, SpriteFont font, Color textColor) : TextBox(textBoxTexture, caretTexture, font, textColor)
{
	...
	public override void RecieveTextInput(string text)
	{
		...
		if (!(currentWidth + ChatBox.messageFont(LocalizedContentManager.CurrentLanguageCode).MeasureString(text).X >= (float)(base.Width - 16)))
		{
			...
			finalText.Last().message = Utility.FilterDirtyWordsIfStrictPlatform(finalText.Last().message);
			updateWidth();
		}
	}
