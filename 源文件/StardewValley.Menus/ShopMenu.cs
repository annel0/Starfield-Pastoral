...
public class ShopMenu : IClickableMenu
{
	...
	{
		ClickableComponent lastValidButton = forSaleButtons[0];
		for (int i = 0; i < forSaleButtons.Count; i++)
		{
			...
			if (i >= forSale.Count)
			{
				...
				{
					currentlySnappedComponent = lastValidButton;
			...
			if (heldItem == null && !readOnly)
			{
				...
				if (toSell != null)
				{
					...
					else
					{
						...
						if (toSell is Object sellObj && sellObj.edibility.Value != -300)
						{
	...
	private bool tryToPurchaseItem(ISalable item, ISalable held_item, int stockToBuy, int x, int y)
	{
		...
		if (held_item == null)
		{
			...
			if (getPlayerCurrencyAmount(Game1.player, currency) >= price && (extraTradeItem == null || HasTradeItem(extraTradeItem, extraTradeItemCount)))
			{
				...
				if (onPurchase != null && onPurchase(item, Game1.player, stockToBuy, stock))
				{
					exitThisMenu();
				}
			}
		...
		if (heldItem == null && !readOnly)
		{
			...
			if (toSell != null)
			{
				...
				else
				{
					...
					else if (sold_item is Object obj && obj.edibility.Value != -300 && Game1.random.NextDouble() < 0.03999999910593033 && Game1.currentLocation is ShopLocation shopLocation)
					{
	...
	public override void performHoverAction(int x, int y)
	{
		...
		foreach (ClickableComponent c in inventory.inventory)
		{
			...
			Item i2 = inventory.getItemFromClickableComponent(c);
			if (i2 == null || (inventory.highlightMethod != null && !inventory.highlightMethod(i2)))
			{
				continue;
			}
			if (_isStorageShop)
			{
				hoverText = i2.getDescription();
				boldTitleText = i2.DisplayName;
				hoveredItem = i2;
				continue;
			}
			hoverText = i2.DisplayName + " x" + i2.Stack;
			if (i2 is Object hovered_object && hovered_object.needsToBeDonated())
			{
				hoverText = hoverText + "\n\n" + i2.getDescription() + "\n";
			}
			hoverPrice = (int)((float)i2.sellToStorePrice(-1L) * sellPercentage) * i2.Stack;
		}
	}
	...
	public void updatePosition()
	{
		...
		int portraitDrawPosition = xPositionOnScreen - 320;
		if ((portraitTexture == null && string.IsNullOrEmpty(potraitPersonDialogue)) || portraitDrawPosition <= 0 || !Game1.options.showMerchantPortraits)
		{
	...
	public override void draw(SpriteBatch b)
	{
		...
		for (int i = 0; i < forSaleButtons.Count; i++)
		{
			ClickableComponent forSaleButton = forSaleButtons[i];
			...
			ISalable item = forSale[currentItemIndex + i];
			ItemStockInformation stock = itemPriceAndStock[item];
			StackDrawType stackDrawType = GetStackDrawType(stock, item);
			string displayName = item.DisplayName;
			IClickableMenu.drawTextureBox(b, theme.ItemRowBackgroundTexture, theme.ItemRowBackgroundSourceRect, forSaleButton.bounds.X, forSaleButton.bounds.Y, forSaleButton.bounds.Width, forSaleButton.bounds.Height, (forSaleButton.containsPoint(Game1.getOldMouseX(), Game1.getOldMouseY()) && !scrolling) ? theme.ItemRowBackgroundHoverColor : Color.White, 4f, drawShadow: false);
			if (item.Stack > 1)
			{
			...
			{
				b.Draw(theme.ItemIconBackgroundTexture, new Vector2(forSaleButton.bounds.X + 32 - 12, forSaleButton.bounds.Y + 24 - 4), theme.ItemIconBackgroundSourceRect, Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
				Vector2 drawPos = new Vector2(forSaleButton.bounds.X + 32 - 8, forSaleButton.bounds.Y + 24);
				...
				{
					b.Draw(Game1.mouseCursors2, new Vector2(forSaleButton.bounds.X + 32 - 8, forSaleButton.bounds.Y + 24), new Rectangle(64, 240, 16, 16), Color.White * ((!failedCanPurchaseCheck) ? 1f : 0.25f), 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, 1f);
				}
				string formattedDisplayName = displayName;
				bool hasPrice = stock.Price > 0;
				if (SpriteText.getWidthOfString(formattedDisplayName) > width - (hasPrice ? (150 + SpriteText.getWidthOfString(stock.Price + " ")) : 100) && formattedDisplayName.Length > (hasPrice ? 27 : 37))
				{
					formattedDisplayName = formattedDisplayName.Substring(0, hasPrice ? 27 : 37);
					formattedDisplayName += "...";
				}
				SpriteText.drawString(b, formattedDisplayName, forSaleButton.bounds.X + 96 + 8, forSaleButton.bounds.Y + 28, 999999, -1, 999999, failedCanPurchaseCheck ? 0.5f : 1f, 0.88f, junimoText: false, -1, "", theme.ItemRowTextColor);
			}
			else
			{
				SpriteText.drawString(b, displayName, forSaleButton.bounds.X + 32 + 8, forSaleButton.bounds.Y + 28, 999999, -1, 999999, failedCanPurchaseCheck ? 0.5f : 1f, 0.88f, junimoText: false, -1, "", theme.ItemRowTextColor);
			}
			...
			if (stock.Price > 0)
			{
				SpriteText.drawString(b, stock.Price + " ", right - SpriteText.getWidthOfString(stock.Price + " ") - 60, forSaleButton.bounds.Y + 28, 999999, -1, 999999, (getPlayerCurrencyAmount(Game1.player, currency) >= stock.Price && !failedCanPurchaseCheck) ? 1f : 0.5f, 0.88f, junimoText: false, -1, "", theme.ItemRowTextColor);
				Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(forSaleButton.bounds.Right - 52, forSaleButton.bounds.Y + 40 - 4), new Rectangle(193 + currency * 9, 373, 9, 10), Color.White * ((!failedCanPurchaseCheck) ? 1f : 0.25f), 0f, Vector2.Zero, 4f, flipped: false, -1f, -1, -1, (!failedCanPurchaseCheck) ? 0.35f : 0f);
				right -= SpriteText.getWidthOfString(stock.Price + " ") + 96;
			...
			if (stock.TradeItem != null)
			{
				...
				bool hasEnoughToTrade = HasTradeItem(requiredItem, requiredItemCount);
				if (canPurchaseCheck != null && !canPurchaseCheck(currentItemIndex + i))
				{
				...
				SpriteText.drawString(b, "x" + requiredItemCount, right - (int)textWidth - 16, tradeTextDrawY, 999999, -1, 999999, hasEnoughToTrade ? 1f : 0.5f, 0.88f, junimoText: false, -1, "", theme.ItemRowTextColor);
			}
		}
		if (IsOutOfStock())
		{
			string text = Game1.content.LoadString("Strings\\StringsFromCSFiles:ShopMenu.cs.11583");
			SpriteText.drawString(b, text, xPositionOnScreen + width / 2 - SpriteText.getWidthOfString(text) / 2, yPositionOnScreen + height / 2 - 128, 999999, -1, 999999, 1f, 0.88f, junimoText: false, -1, "", theme?.ItemRowTextColor);
		}
		inventory.draw(b);
		for (int i2 = animations.Count - 1; i2 >= 0; i2--)
		{
			if (animations[i2].update(Game1.currentGameTime))
			{
				animations.RemoveAt(i2);
			}
			else
			{
				animations[i2].draw(b, localPosition: true);
			}
		}
		poof?.draw(b);
		upArrow.draw(b);
		...
		if (hoverText != "")
		{
			Item actualItem = hoveredItem as Item;
			ISalable salable = hoveredItem;
			if (salable != null && salable.IsRecipe)
			{
				IClickableMenu.drawToolTip(b, " ", boldTitleText, actualItem, heldItem != null, -1, currency, getHoveredItemExtraItemIndex(), getHoveredItemExtraItemAmount(), new CraftingRecipe(actualItem?.BaseName ?? hoveredItem.Name), (hoverPrice > 0) ? hoverPrice : (-1));
			}
			else
			{
				IClickableMenu.drawToolTip(b, hoverText, boldTitleText, actualItem, heldItem != null, -1, currency, getHoveredItemExtraItemIndex(), getHoveredItemExtraItemAmount(), null, (hoverPrice > 0) ? hoverPrice : (-1));
			}
		}
