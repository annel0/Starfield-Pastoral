...
public class ClickableTextureComponent : ClickableComponent
{
	...
	public virtual void draw(SpriteBatch b, Color c, float layerDepth, int frameOffset = 0, int xOffset = 0, int yOffset = 0)
	{
		...
		if (texture != null)
		{
			...
			if (drawShadow)
			{
				Utility.drawWithShadow(b, texture, new Vector2((float)(bounds.X + xOffset) + (float)(sourceRect.Width / 2) * baseScale, (float)(bounds.Y + yOffset) + (float)(sourceRect.Height / 2) * baseScale), r, c, 0f, new Vector2(sourceRect.Width / 2, sourceRect.Height / 2), scale, flipped: false, layerDepth);
			}
			else
			{
