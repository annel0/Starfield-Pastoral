...
public class SaveGameMenu : IClickableMenu, IDisposable
{
	...
	public override void draw(SpriteBatch b)
	{
		...
		else if (margin < 0 || Game1.IsClient)
		{
			...
			{
				int totalPlayers = Game1.getOnlineFarmers().Count - (Game1.HasDedicatedHost ? 1 : 0);
				_stringBuilder.Clear();
				_stringBuilder.Append(Game1.content.LoadString("Strings\\UI:ReadyCheck", Game1.newDaySync.numReadyForSave(), totalPlayers));
