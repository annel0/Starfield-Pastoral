...
public class SkillsPage : IClickableMenu
{
		...
		: base(x, y, width, height)
	{
		...
		for (int l = 0; l < 5; l++)
		{
			...
			skillAreas.Add(new ClickableTextureComponent(index.ToString() ?? "", new Rectangle(drawX - 128 - 48, drawY + l * 68, 148, 36), index.ToString() ?? "", text, null, Rectangle.Empty, 1f)
			{
				myID = l,
	...
	public override void performHoverAction(int x, int y)
	{
		...
		foreach (ClickableTextureComponent c3 in skillAreas)
		{
			if (c3.containsPoint(x, y) && c3.hoverText.Length > 0)
			{
				hoverText = c3.hoverText;
				hoverTitle = Farmer.getSkillDisplayNameFromIndex(Convert.ToInt32(c3.name));
				break;
			}
		}
	...
	public override void draw(SpriteBatch b)
	{
		...
		Game1.player.FarmerRenderer.draw(b, new FarmerSprite.AnimationFrame(Game1.player.bathingClothes.Value ? 108 : playerPanelFrames[playerPanelIndex], 0, secondaryArm: false, flip: false), Game1.player.bathingClothes.Value ? 108 : playerPanelFrames[playerPanelIndex], new Rectangle(playerPanelFrames[playerPanelIndex] * 16, Game1.player.bathingClothes.Value ? 576 : 0, 16, 32), new Vector2(x + 32, y + 16 - 4), Vector2.Zero, 0.8f, 2, Color.White, 0f, 1f, Game1.player);
		...
		Utility.drawTextWithShadow(b, Game1.content.LoadString("Strings\\UI:Inventory_PortraitHover_Level", Game1.player.houseUpgradeLevel.Value + 1), Game1.smallFont, new Vector2(x + xHouseOffset + 72, y), Game1.textColor);
