...
public class ShippingMenu : IClickableMenu
{
	...
	public void parseItems(IList<Item> items)
	{
		...
		foreach (Item item in items)
		{
			if (item != null)
			{
				int category = getCategoryIndexForObject(item);
		...
		else if (Game1.dayOfMonth != 28 && !outro)
		{
			if (!Game1.wasRainingYesterday)
			{
				...
				else if (Game1.random.NextDouble() < 5E-05)
				{
					...
					{
						TemporaryAnimatedSprite bird2 = new TemporaryAnimatedSprite("LooseSprites\\Cursors", sourceRect, Game1.random.Next(60, 101), 4, 100, startingPosition + new Vector2((i + 1) * Game1.random.Next(15, 18), (i + 1) * -20), flicker: false, flipped: false, 0.01f, 0f, Color.Black, 4f, 0f, 0f, 0f, local: true);
						bird2.motion = new Vector2(-1f, 0f);
						animations.Add(bird2);
						bird2 = new TemporaryAnimatedSprite("LooseSprites\\Cursors", sourceRect, Game1.random.Next(60, 101), 4, 100, startingPosition + new Vector2((i + 1) * Game1.random.Next(15, 18), (i + 1) * 20), flicker: false, flipped: false, 0.01f, 0f, Color.Black, 4f, 0f, 0f, 0f, local: true);
						bird2.motion = new Vector2(-1f, 0f);
						animations.Add(bird2);
					}
				}
	...
	public override void draw(SpriteBatch b)
	{
		...
		if (Game1.wasRainingYesterday)
		{
			...
			for (int i = 0; i < width; i += 639)
			{
				b.Draw(Game1.mouseCursors, new Vector2(i * 4, Game1.uiViewport.Height - 192), new Rectangle(0, isWinter ? 1034 : 737, 639, 48), (isWinter ? (Color.White * 0.25f) : new Color(30, 62, 50)) * (0.5f - (float)introTimer / 3500f), 0f, Vector2.Zero, 4f, SpriteEffects.FlipHorizontally, 1f);
				b.Draw(Game1.mouseCursors, new Vector2(i * 4, Game1.uiViewport.Height - 128), new Rectangle(0, isWinter ? 1034 : 737, 639, 32), (isWinter ? (Color.White * 0.5f) : new Color(30, 62, 50)) * (1f - (float)introTimer / 3500f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
			}
			...
			for (int j = -244; j < Game1.uiViewport.Width + 244; j += 244)
			{
				b.Draw(Game1.mouseCursors, new Vector2((float)j + weatherX % 244f, -32f), new Rectangle(643, 1142, 61, 53), Color.SlateGray * 0.85f * (1f - (float)introTimer / 3500f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.9f);
			}
			...
			{
				b.Draw(Game1.mouseCursors, new Vector2((float)k + weatherX * 1.5f % 244f, -128f), new Rectangle(643, 1142, 61, 53), Color.LightSlateGray * (1f - (float)introTimer / 3500f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.9f);
			}
		}
		else
		{
			...
			for (int l = 0; l < width; l += 639)
			{
				b.Draw(Game1.mouseCursors, new Vector2(l * 4, 0f), new Rectangle(0, 1453, 639, 195), Color.White * (1f - (float)introTimer / 3500f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
			}
			...
			foreach (ClickableTextureComponent c in categories)
			{
				if (introTimer < 2500 - i2 * 500)
				{
					...
					if (c.visible)
					{
						...
						categoryItems[i2][0].drawInMenu(b, start + new Vector2(-88 + extraWidth, yOffset + 16), 1f, 1f, 0.9f, StackDrawType.Hide);
					}
					...
					for (int m = 0; m < 6; m++)
					{
						b.Draw(Game1.mouseCursors, start + new Vector2(-itemSlotWidth + extraWidth - 192 - 24 + m * 6 * 4, 12f), new Rectangle(355, 476, 7, 11), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.88f);
					}
					categoryDials[i2].draw(b, start + new Vector2(-itemSlotWidth + extraWidth - 192 - 48 + 4, 20f), categoryTotals[i2]);
		...
		else
		{
			...
			for (int n = currentTab * itemsPerCategoryPage; n < currentTab * itemsPerCategoryPage + itemsPerCategoryPage; n++)
			{
				if (categoryItems[currentPage].Count > n)
				{
					Item item = categoryItems[currentPage][n];
