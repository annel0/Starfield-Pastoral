...
public class ChooseFromIconsMenu : IClickableMenu
{
	...
	public void setUpIcons(string which)
	{
		...
		if (hasTooltips)
		{
			foreach (ClickableTextureComponent i2 in icons)
			{
				i2.hoverText = Game1.parseText(i2.hoverText, Game1.smallFont, toolTipWidth - 32);
				maxTooltipHeight = Math.Max(maxTooltipHeight, (int)Game1.smallFont.MeasureString(i2.hoverText).Y);
				maxTooltipWidth = Math.Max(maxTooltipWidth, (int)Game1.smallFont.MeasureString(i2.hoverText).X);
			}
			maxTooltipHeight += 48;
			maxTooltipWidth += 48;
		}
		...
		for (int j = 0; j < icons.Count; j += maxIconsPerRow)
		{
			int rowCount = Math.Min(icons.Count - j, maxIconsPerRow);
			int x = xPositionOnScreen + width / 2 - rowCount * iconWidth / 2;
			for (int k = 0; k < rowCount; k++)
			{
				int index = k + j;
				icons[index].bounds.X = x + k * iconWidth;
	...
	public override void update(GameTime time)
	{
		...
		temporarySprites.RemoveAll((TemporaryAnimatedSprite sprite) => sprite.update(time));
	}
