	...
	public override void update(GameTime time)
	{
		...
		if (gamepadMode)
		{
			...
			if (Math.Abs(Game1.input.GetGamePadState().ThumbSticks.Right.X) > 0.5f || Math.Abs(Game1.input.GetGamePadState().ThumbSticks.Right.Y) > 0.5f)
			{
				...
				for (int i = 0; i < _emoteButtons.Count; i++)
				{
					float dot = Vector2.Dot(value2: new Vector2((float)_emoteButtons[i].bounds.Center.X - ((float)xPositionOnScreen + (float)width / 2f), (float)_emoteButtons[i].bounds.Center.Y - ((float)yPositionOnScreen + (float)height / 2f)), value1: offset);
					if (dot > highest_dot)
					{
						highest_dot = dot;
						_selectedEmote = emotes[i];
