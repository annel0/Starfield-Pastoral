...
public class ProfileMenu : IClickableMenu
{
	...
	protected void _SetCharacter(SocialPage.SocialEntry entry)
	{
		...
		if (Current.Character is NPC npcData)
		{
			CharacterData data = npcData.GetData();
			string assetName = "Characters/" + npcData.getTextureName();
			try
			{
				_animatedSprite = new AnimatedSprite(assetName, 0, data?.Size.X ?? 16, data?.Size.Y ?? 32);
			}
			catch (Exception exception)
			{
				Game1.log.Error($"Profile menu couldn't load sprite '{assetName}' for NPC '{npcData.Name}', defaulting to their current sprite.", exception);
				_animatedSprite = npcData.Sprite.Clone();
			...
			foreach (ParsedItemData itemData in ItemRegistry.GetObjectTypeDefinition().GetAllData())
			{
				...
				Object item = ItemRegistry.Create<Object>(itemData.QualifiedItemId);
				...
				for (int i = 0; i < itemCategories.Length; i++)
				{
					...
					if (!(categoryName == "Profile_Gift_Category_LikedGifts"))
					{
						if (categoryName == "Profile_Gift_Category_Misc")
						{
							...
							foreach (ProfileItemCategory category in array)
							{
								if (category.validCategories != null && Enumerable.Contains(category.validCategories, item.Category))
								{
									isAccountedFor = true;
									break;
								}
						...
						else if (Enumerable.Contains(itemCategories[i].validCategories, item.Category))
						{
							if (!_sortedItems.TryGetValue(i, out var categoryItems2))
							{
					...
					{
						if (!_sortedItems.TryGetValue(i, out var categoryItems3))
						{
	...
	public void ConstrainSelectionToView()
	{
		...
		if (clickableComponent != null && clickableComponent.region == 502 && !_itemDisplayRect.Contains(currentlySnappedComponent.bounds))
		{
			...
			{
				int scroll2 = (int)Math.Floor(((double)currentlySnappedComponent.bounds.Top - (double)_itemDisplayRect.Top) / (double)scrollStep) * scrollStep;
				Scroll(scroll2);
			}
		}
	...
	public override void draw(SpriteBatch b)
	{
		...
		if (draw_tooltip)
		{
			...
			{
				name = Game1.content.LoadStringReturnNullIfNotFound("Strings\\Objects:" + hoveredItem.ItemId + "_CollectionsTabName") ?? name;
				description = Game1.content.LoadStringReturnNullIfNotFound("Strings\\Objects:" + hoveredItem.ItemId + "_CollectionsTabDescription") ?? description;
			}
			IClickableMenu.drawToolTip(b, description, name, hoveredItem);
		}
	}
