...
public class Slots : IMinigame
{
	...
	public Slots(int toBet = -1, bool highStakes = false)
	{
		...
		int buttonTopY = 44;
		spinButton10 = CreateSpinButton(32, buttonTopY, "Strings\\StringsFromCSFiles:Slots.cs.12117");
		spinButton100 = CreateSpinButton(37, buttonTopY + 64, "Strings\\StringsFromCSFiles:Slots.cs.12118");
		doneButton = CreateSpinButton(30, buttonTopY + 128, "Strings\\StringsFromCSFiles:NameSelect.cs.3864");
	...
	{
		int extraButtonWidth = GetButtonSizeOffset();
	...
	public void draw(SpriteBatch b)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			Vector2 topLeft = new Vector2(minSlotX + i * 104, Game1.graphics.GraphicsDevice.Viewport.Height / 2 - 128);
			b.Draw(Game1.mouseCursors, topLeft, new Rectangle(306, 320, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.99f);
			float faceValue = (slots[i] + 1f) % 8f;
			int previous = getIconIndex(((int)faceValue + 8 - 1) % 8);
			int current = getIconIndex((previous + 1) % 8);
			b.Draw(Game1.objectSpriteSheet, topLeft - new Vector2(0f, -64f * (faceValue % 1f)), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, previous, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.99f);
			b.Draw(Game1.objectSpriteSheet, topLeft - new Vector2(0f, 64f - 64f * (faceValue % 1f)), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, current, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.99f);
		...
		spinButton10.bounds.X = slotMiddle - spinButton10.bounds.Width / 2;
		spinButton100.bounds.X = slotMiddle - spinButton100.bounds.Width / 2;
		doneButton.bounds.X = slotMiddle - doneButton.bounds.Width / 2;
		int extraButtonWidth = GetButtonSizeOffset();
		b.Draw(Game1.mouseCursors, new Vector2(spinButton10.bounds.X, spinButton10.bounds.Y), new Rectangle(441, 385, 32 + extraButtonWidth, 13), Color.White * ((!spinning && Game1.player.clubCoins >= 10) ? 1f : 0.5f), 0f, Vector2.Zero, 4f * spinButton10.scale, SpriteEffects.None, 0.99f);
		b.Draw(Game1.mouseCursors, new Vector2(spinButton100.bounds.X, spinButton100.bounds.Y), new Rectangle(441, 398, 37 + extraButtonWidth, 13), Color.White * ((!spinning && Game1.player.clubCoins >= 100) ? 1f : 0.5f), 0f, Vector2.Zero, 4f * spinButton100.scale, SpriteEffects.None, 0.99f);
		b.Draw(Game1.mouseCursors, new Vector2(doneButton.bounds.X, doneButton.bounds.Y), new Rectangle(441, 411, 30 + extraButtonWidth, 13), Color.White * ((!spinning) ? 1f : 0.5f), 0f, Vector2.Zero, 4f * doneButton.scale, SpriteEffects.None, 0.99f);
		...
		for (int j = 0; j < 8; j++)
		{
			...
			SpriteText.drawString(b, "x" + payout, (int)basePos.X + 192 + 16, (int)basePos.Y + (j + 2) * 68 + 24, 9999, -1, 99999, 1f, 0.88f, junimoText: false, -1, "", SpriteText.color_White);
		}
