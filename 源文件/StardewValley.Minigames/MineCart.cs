...
public class MineCart : IMinigame
{
	...
	public class Track : Entity
	{
		...
		public override void _Draw(SpriteBatch b)
		{
			...
			{
				for (float y = base.drawnPosition.Y; y < (float)_game.screenHeight; y += (float)(_game.tileSize * 4))
				{
					b.Draw(_game.texture, _game.TransformDraw(new Vector2(base.drawnPosition.X, y + 16f)), new Rectangle(320, 784, 16, 64), Color.White, 0f, Vector2.Zero, _game.GetPixelScale(), SpriteEffects.None, 0.5f + base.drawnPosition.Y * 1E-05f + 0.01f);
					b.Draw(_game.texture, _game.TransformDraw(new Vector2(base.drawnPosition.X, y + 16f)), new Rectangle(368, 784, 16, 64), _game.trackShadowTint * darkness, 0f, Vector2.Zero, _game.GetPixelScale(), SpriteEffects.None, 0.5f + base.drawnPosition.Y * 1E-05f + 0.005f);
					darkness += 0.1f;
				}
				return;
			}
			bool flipper = _showSecondTile;
			for (float y2 = base.drawnPosition.Y; y2 < (float)_game.screenHeight; y2 += (float)_game.tileSize)
			{
				b.Draw(_game.texture, _game.TransformDraw(new Vector2(base.drawnPosition.X, y2)), (_game.currentTheme == 4) ? new Rectangle(16 + (flipper ? 1 : 0) * 16, 160, 16, 16) : new Rectangle(16 + (flipper ? 1 : 0) * 16, 32, 16, 16), _game.trackTint, 0f, Vector2.Zero, _game.GetPixelScale(), SpriteEffects.None, 0.5f + base.drawnPosition.Y * 1E-05f + 0.01f);
				b.Draw(_game.texture, _game.TransformDraw(new Vector2(base.drawnPosition.X, y2)), (_game.currentTheme == 4) ? new Rectangle(16 + (flipper ? 1 : 0) * 16, 160, 16, 16) : new Rectangle(16 + (flipper ? 1 : 0) * 16, 32, 16, 16), _game.trackShadowTint * darkness, 0f, Vector2.Zero, _game.GetPixelScale(), SpriteEffects.None, 0.5f + base.drawnPosition.Y * 1E-05f + 0.005f);
				darkness += 0.1f;
				flipper = !flipper;
			}
		}
		...
		public int GetYAtPoint(float x)
		{
			...
			{
				TrackType.UpSlope => (int)(position.Y - 2f - (float)local_x), 
				TrackType.DownSlope => (int)(position.Y - 2f - 16f + (float)local_x), 
				TrackType.IceDownSlope => (int)(position.Y - 2f - 16f + (float)local_x), 
				TrackType.SlimeUpSlope => (int)(position.Y - 2f - (float)local_x), 
				_ => (int)(position.Y - 2f), 
			};
		}
	}
	...
	public class PlayerMineCartCharacter : MineCartCharacter, ICollideable
	{
		...
		public override void OnLand()
		{
			...
			else
			{
				...
				{
					Game1.playSound("slimeHit");
					bool purple = GetTrack().GetType() != typeof(Track);
					for (int i = 0; i < 3; i++)
					{
	...
	public class MineCartCharacter : BaseCharacter
	{
		...
		protected override void _Update(float time)
		{
			...
			if (was_grounded && old_track_type != currentTrackType)
			{
	...
	{
		int tilePosition = (int)(x / (float)tileSize);
		return _tracks.GetValueOrDefault(tilePosition);
	}
	...
	public void UpdateCutsceneTick()
	{
		...
		case 5:
			...
			break;
		case 7:
			UpdateCutsceneForEnding(ref fade_out_time);
			break;
		}
	...
	public bool tick(GameTime time)
	{
		...
		_entities.RemoveAll((Entity entity2) => entity2?.ShouldReap() ?? false);
		...
		if (gameState == GameStates.Ingame)
		{
			...
			{
				int i2 = 0;
				while (i2 < checkpointPositions.Count && player.position.X >= (float)(checkpointPositions[i2] * tileSize))
				{
					foreach (Entity entity2 in _entities)
					{
						if (entity2 is CheckpointIndicator indicator && (int)(indicator.position.X / (float)tileSize) == checkpointPositions[i2])
						{
							indicator.Activate();
							break;
						}
					}
					checkpointPosition = ((float)checkpointPositions[i2] + 0.5f) * (float)tileSize;
					ReapEntities();
					checkpointPositions.RemoveAt(i2);
					i2--;
					i2++;
				}
			}
			...
			if (respawnCounter <= 0)
			{
				if (player_tile_position > _lastTilePosition)
				{
					...
					for (int i3 = 0; i3 < number_of_motions; i3++)
					{
						score += 10;
					}
				}
			...
			if (_goalIndicator != null && distanceToTravel != -1 && player.position.X >= _goalIndicator.position.X && distanceToTravel != -1 && player.position.Y <= _goalIndicator.position.Y * (float)tileSize + 4f && !reachedFinish && fadeDelta < 0f)
			{
				...
				foreach (Entity entity in _entities)
				{
					if (entity is GoalIndicator indicator2)
					{
						indicator2.Activate();
					}
		...
		for (int i4 = sparkShower.Count - 1; i4 >= 0; i4--)
		{
			sparkShower[i4].dy += 0.105f * (delta_time / 0.0166666f);
			sparkShower[i4].x += sparkShower[i4].dx * (delta_time / 0.0166666f);
			sparkShower[i4].y += sparkShower[i4].dy * (delta_time / 0.0166666f);
			sparkShower[i4].c.B = (byte)(0.0 + Math.Max(0.0, Math.Sin(totalTimeMS / (Math.PI * 20.0 / (double)sparkShower[i4].dx)) * 255.0));
			if (reachedFinish)
			{
				sparkShower[i4].c.R = (byte)(0.0 + Math.Max(0.0, Math.Sin((totalTimeMS + 50.0) / (Math.PI * 20.0 / (double)sparkShower[i4].dx)) * 255.0));
				sparkShower[i4].c.G = (byte)(0.0 + Math.Max(0.0, Math.Sin((totalTimeMS + 100.0) / (Math.PI * 20.0 / (double)sparkShower[i4].dx)) * 255.0));
				if (sparkShower[i4].c.R == 0)
				{
					sparkShower[i4].c.R = byte.MaxValue;
				}
				if (sparkShower[i4].c.G == 0)
				{
					sparkShower[i4].c.G = byte.MaxValue;
				}
			}
			if (sparkShower[i4].y > (float)screenHeight)
			{
				sparkShower.RemoveAt(i4);
			}
		}
		return false;
	}
	...
	public void Die()
	{
		...
		for (int j = 0; j < 6; j++)
		{
			Track runway_track = GetTrackForXPosition((checkpointPosition / (float)tileSize + (float)j) * (float)tileSize);
			if (runway_track?.obstacle != null)
			{
	...
	public void draw(SpriteBatch b)
	{
		...
		case GameStates.Title:
		{
			...
			for (int num = 0; num < 5 && num < _currentHighScores.Count; num++)
			{
				...
				KeyValuePair<string, int> score = _currentHighScores[num];
				int score_text_width = (int)Game1.dialogueFont.MeasureString(score.Value.ToString() ?? "").X / 4;
				b.DrawString(Game1.dialogueFont, "#" + (num + 1), TransformDraw(draw_position), color, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.199f);
				...
				b.DrawString(Game1.dialogueFont, score.Value.ToString() ?? "", TransformDraw(draw_position + score_offset - new Vector2(score_text_width, 0f)), color, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.199f);
				Vector2 shadow_offset = new Vector2(1f, 1f);
				b.DrawString(Game1.dialogueFont, "#" + (num + 1), TransformDraw(draw_position + shadow_offset), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.1999f);
				...
				b.DrawString(Game1.dialogueFont, score.Value.ToString() ?? "", TransformDraw(draw_position + score_offset - new Vector2(score_text_width, 0f) + shadow_offset), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.1999f);
				draw_position.Y += 10f;
			}
			break;
		}
		...
		if (!gamePaused && (gameState == GameStates.Ingame || gameState == GameStates.Cutscene || gameState == GameStates.FruitsSummary || gameState == GameStates.Map))
		{
			...
			{
				draw_position2.X = 4f;
				for (int num2 = 0; num2 < livesLeft; num2++)
				{
					...
					if (draw_position2.X > 90f && num2 < livesLeft - 1)
					{
						draw_position2.X = 4f;
				...
				for (int num3 = livesLeft; num3 < 3; num3++)
				{
			...
			for (int num4 = 0; num4 < 3; num4++)
			{
				...
				if (currentFruitCheckMagnitude > 0f && num4 == currentFruitCheckIndex - 1)
				{
				...
				if (_collectedFruit.Contains((CollectableFruits)num4))
				{
					b.Draw(texture, TransformDraw(draw_position2 + shake_magnitude), new Rectangle(160 + num4 * 16, 0, 16, 16), Color.White, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.07f);
					b.Draw(texture, TransformDraw(draw_position2 + new Vector2(1f, 1f) + shake_magnitude), new Rectangle(160 + num4 * 16, 0, 16, 16), Color.Black, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.075f);
				}
				else
				{
					b.Draw(texture, TransformDraw(draw_position2 + shake_magnitude), new Rectangle(160 + num4 * 16, 16, 16, 16), Color.White, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.07f);
					b.Draw(texture, TransformDraw(draw_position2 + shake_magnitude + new Vector2(1f, 1f)), new Rectangle(160 + num4 * 16, 16, 16, 16), Color.Black, 0f, new Vector2(0f, 0f), GetPixelScale(), SpriteEffects.None, 0.075f);
				}
				draw_position2.X += 18f;
			}
			if (gameMode == 3)
		...
		if (gamePaused)
		{
			...
			b.DrawString(position: TransformDraw(new Vector2
			{
				X = screenWidth / 2,
				Y = screenHeight / 4
			} - new Vector2(Game1.dialogueFont.MeasureString(current_text).X / 2f / 4f, 0f)), spriteFont: Game1.dialogueFont, text: current_text, color: Color.White, rotation: 0f, origin: Vector2.Zero, scale: GetPixelScale() / 4f, effects: SpriteEffects.None, layerDepth: 0.1f);
		}
