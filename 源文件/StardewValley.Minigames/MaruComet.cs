...
public class MaruComet : IMinigame
{
	...
	public bool tick(GameTime time)
	{
		...
		for (int i2 = flybysClose.Count - 1; i2 >= 0; i2--)
		{
			flybysClose[i2] = new Vector2(flybysClose[i2].X + 0.8f * (float)time.ElapsedGameTime.Milliseconds * 1.5f, flybysClose[i2].Y - 0.8f * (float)time.ElapsedGameTime.Milliseconds * 1.5f);
			if (cometColorOrigin.Y + flybysClose[i2].Y < centerOfScreen.Y - (float)(143 * zoom / 2))
			{
				flybysClose.RemoveAt(i2);
			}
		}
		for (int i3 = flybysFar.Count - 1; i3 >= 0; i3--)
		{
			flybysFar[i3] = new Vector2(flybysFar[i3].X + 0.8f * (float)time.ElapsedGameTime.Milliseconds * 0.5f, flybysFar[i3].Y - 0.8f * (float)time.ElapsedGameTime.Milliseconds * 0.5f);
			if (cometColorOrigin.Y + flybysFar[i3].Y < centerOfScreen.Y - (float)(143 * zoom / 2))
			{
				flybysFar.RemoveAt(i3);
			}
		}
	...
	public void draw(SpriteBatch b)
	{
		...
		SpriteText.drawStringHorizontallyCenteredAt(b, currentString, (int)centerOfScreen.X, (int)text_draw_y, currentStringCharacter, -1, 99999, 1f, 0.99f, junimoText: false, SpriteText.color_Purple, Game1.game1.localMultiplayerWindow.Width);
