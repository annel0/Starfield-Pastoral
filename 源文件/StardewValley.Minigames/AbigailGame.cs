...
public class AbigailGame : IMinigame
{
		...
		public override bool move(Vector2 playerPosition, GameTime time)
		{
			...
			case 0:
			{
				...
				if (shootTimer < 0)
				{
					...
					{
						trajectory3 = Utility.getTranslatedVector2(trajectory3, playerMovementDirections.Last(), 3f);
					}
	...
	public void usePowerup(int which)
	{
		...
		case 4:
		{
			...
			else
			{
				...
				{
					c2.takeDamage(30);
					bullets.Add(new CowboyBullet(c2.position.Center, 2, 1));
				}
			}
			for (int i = 0; i < 30; i++)
			{
	...
	public void updateBullets(GameTime time)
	{
		...
		for (int i2 = enemyBullets.Count - 1; i2 >= 0; i2--)
		{
			enemyBullets[i2].position.X += enemyBullets[i2].motion.X;
			enemyBullets[i2].position.Y += enemyBullets[i2].motion.Y;
			if (enemyBullets[i2].position.X <= 0 || enemyBullets[i2].position.Y <= 0 || enemyBullets[i2].position.X >= 762 || enemyBullets[i2].position.Y >= 762)
			{
				enemyBullets.RemoveAt(i2);
			}
			else if (map[(enemyBullets[i2].position.X + 6) / 16 / 3, (enemyBullets[i2].position.Y + 6) / 16 / 3] == 7)
			{
				enemyBullets.RemoveAt(i2);
			}
			else if (playerInvincibleTimer <= 0 && deathTimer <= 0f && playerBoundingBox.Intersects(new Rectangle(enemyBullets[i2].position.X, enemyBullets[i2].position.Y, 15, 15)))
			{
				playerDie();
				break;
			}
	...
	public bool tick(GameTime time)
	{
		...
		if (gopherRunning)
		{
			...
			for (int i2 = monsters.Count - 1; i2 >= 0; i2--)
			{
				if (gopherBox.Intersects(monsters[i2].position))
				{
					addGuts(monsters[i2].position.Location, monsters[i2].type);
					monsters.RemoveAt(i2);
					Game1.playSound("Cowboy_monsterDie");
				}
			}
		...
		if (motionPause <= 0)
		{
			for (int i3 = powerups.Count - 1; i3 >= 0; i3--)
			{
				if (Utility.distance(playerBoundingBox.Center.X, powerups[i3].position.X + TileSize / 2, playerBoundingBox.Center.Y, powerups[i3].position.Y + TileSize / 2) <= (float)(TileSize + 3) && (powerups[i3].position.X < TileSize || powerups[i3].position.X >= 16 * TileSize - TileSize || powerups[i3].position.Y < TileSize || powerups[i3].position.Y >= 16 * TileSize - TileSize))
				{
					if (powerups[i3].position.X + TileSize / 2 < playerBoundingBox.Center.X)
					{
						powerups[i3].position.X++;
					}
					if (powerups[i3].position.X + TileSize / 2 > playerBoundingBox.Center.X)
					{
						powerups[i3].position.X--;
					}
					if (powerups[i3].position.Y + TileSize / 2 < playerBoundingBox.Center.Y)
					{
						powerups[i3].position.Y++;
					}
					if (powerups[i3].position.Y + TileSize / 2 > playerBoundingBox.Center.Y)
					{
						powerups[i3].position.Y--;
				...
				powerups[i3].duration -= time.ElapsedGameTime.Milliseconds;
				if (powerups[i3].duration <= 0)
				{
					powerups.RemoveAt(i3);
				}
			}
			for (int i4 = activePowerups.Count - 1; i4 >= 0; i4--)
			{
				int key = activePowerups.ElementAt(i4).Key;
			...
			if (deathTimer <= 0f && playerMovementDirections.Count > 0 && !scrollingMap)
			{
				...
				powerups.RemoveAll(delegate(CowboyPowerup item)
				{
					if (playerBoundingBox.Intersects(new Rectangle(item.position.X, item.position.Y, TileSize, TileSize)) && !playerBoundingBox.Intersects(noPickUpBox))
					{
						if (heldItem != null)
						{
							usePowerup(item.which);
							return true;
						}
						if (getPowerUp(item))
						{
			...
			if (deathTimer <= 0f && playerShootingDirections.Count > 0 && shotTimer <= 0)
			{
				...
				for (int i8 = 0; i8 < fireSpeedLevel; i8++)
				{
			...
			else if (deathTimer <= 0f && !waitingForPlayerToMoveDownAMap && !shopping && !shootoutLevel)
			{
				...
				for (int p = 0; p < 4; p++)
				{
					...
					if (spawnQueue[p][0].X == 1 || spawnQueue[p][0].X == 4)
					{
						...
						int tries2 = 0;
						while (isCollidingWithMonster(new Rectangle((int)tile.X * TileSize, (int)tile.Y * TileSize, TileSize, TileSize), null) && tries2 < 10)
						{
					...
					case 0:
					{
						...
						{
							if (Game1.random.NextBool() && !isCollidingWithMonster(new Rectangle(x2 * 16 * 3, 0, 48, 48), null))
							{
								CowboyMonster monster5 = new CowboyMonster(spawnQueue[p][0].X, new Point(x2 * TileSize, 0));
								if (whichRound > 0)
								{
					...
					case 1:
					{
						...
						{
							if (Game1.random.NextBool() && !isCollidingWithMonster(new Rectangle(720, y2 * TileSize, 48, 48), null))
							{
								CowboyMonster monster3 = new CowboyMonster(spawnQueue[p][0].X, new Point(15 * TileSize, y2 * TileSize));
								if (whichRound > 0)
								{
					...
					case 2:
					{
						for (int x = 7; x < 10; x++)
						{
							...
							{
								CowboyMonster monster4 = new CowboyMonster(spawnQueue[p][0].X, new Point(x * TileSize, 15 * TileSize));
								if (whichRound > 0)
								{
					...
					case 3:
					{
						for (int y = 7; y < 10; y++)
						{
							...
							{
								CowboyMonster monster2 = new CowboyMonster(spawnQueue[p][0].X, new Point(0, y * TileSize));
								if (whichRound > 0)
								{
				...
				if (waveTimer <= 0 && monsters.Count == 0 && isSpawnQueueEmpty())
				{
					...
					if (whichRound > 0)
					{
						for (int i9 = 0; i9 < monsterChances.Count; i9++)
						{
							_ = monsterChances[i9];
							monsterChances[i9] *= 1.1f;
						}
					}
			...
			for (int i10 = monsters.Count - 1; i10 >= 0; i10--)
			{
				monsters[i10].move(playerPosition, time);
				if (i10 < monsters.Count && monsters[i10].position.Intersects(playerBoundingBox) && playerInvincibleTimer <= 0)
				{
					if (zombieModeTimer <= 0)
					{
						playerDie();
					...
					if (monsters[i10].type != -2)
					{
						addGuts(monsters[i10].position.Location, monsters[i10].type);
						monsters.RemoveAt(i10);
						Game1.playSound("Cowboy_monsterDie");
					}
				}
				if (playingWithAbigail && i10 < monsters.Count && monsters[i10].position.Intersects(player2BoundingBox) && player2invincibletimer <= 0)
				{
					Game1.playSound("Cowboy_monsterDie");
	...
	public int[,] getMap(int wave)
	{
		...
		case -1:
		{
			for (int num = 0; num < 16; num++)
			{
				...
				{
					if (newMap[num, num2] == 0 || newMap[num, num2] == 1 || newMap[num, num2] == 2 || newMap[num, num2] == 5)
					{
						newMap[num, num2] = 3;
					}
				}
		...
		case 8:
		{
			for (int num5 = 0; num5 < 16; num5++)
			{
				...
				{
					if (newMap[num5, num6] == 5)
					{
						newMap[num5, num6] = Game1.random.Choose(0, 1);
					}
				}
			}
			for (int num7 = 0; num7 < 16; num7++)
			{
				newMap[num7, 8] = Game1.random.Choose(8, 9);
			}
			newMap[8, 4] = 7;
			newMap[8, 12] = 7;
			newMap[9, 12] = 7;
		...
		case 7:
		{
			for (int n = 0; n < 16; n++)
			{
				newMap[n, 5] = ((n % 2 == 0) ? 9 : 8);
				newMap[n, 10] = ((n % 2 == 0) ? 9 : 8);
			}
			newMap[4, 5] = 10;
			newMap[8, 5] = 10;
			newMap[12, 5] = 10;
		...
		case 10:
		{
			...
			{
				newMap[num8, 1] = ((num8 % 2 == 0) ? 9 : 8);
				newMap[num8, 14] = ((num8 % 2 == 0) ? 9 : 8);
			}
			newMap[8, 1] = 10;
			newMap[7, 1] = 10;
			newMap[9, 1] = 10;
		...
		case 12:
		{
			...
			for (int m = 0; m < 16; m++)
			{
				newMap[m, 0] = ((m % 2 == 0) ? 9 : 8);
				newMap[m, 15] = ((m % 2 == 0) ? 9 : 8);
			}
	...
	public void draw(SpriteBatch b)
	{
		...
		else
		{
			...
			else
			{
				...
				for (int num = 0; num < whichWave + whichRound * 12; num++)
				{
					b.Draw(Game1.mouseCursors, topLeftScreenCoordinate + new Vector2(TileSize * 16 + 3, num * 3 * 6), new Rectangle(512, 1760, 5, 5), Color.White, 0f, Vector2.Zero, 3f, SpriteEffects.None, 0.5f);
				}
