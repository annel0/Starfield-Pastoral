...
public class Mummy : Monster
{
	...
	{
		crumbleEvent.Poll();
		if (reviveTimer.Value > 0 && Sprite.CurrentAnimation == null && Sprite.currentFrame != 19)
		{
	...
	{
		if (reviveTimer.Value <= 0 && withinPlayerThreshold())
		{
			base.IsWalkingTowardPlayer = true;
		}
	...
	protected override void updateAnimation(GameTime time)
	{
		...
		else if (reviveTimer.Value > 0)
		{
			reviveTimer.Value -= time.ElapsedGameTime.Milliseconds;
			if (reviveTimer.Value < 2000)
			{
				shake(reviveTimer.Value);
			}
			if (reviveTimer.Value <= 0)
			{
				if (Game1.IsMasterGame)
