...
public class Grub : Monster
{
	...
	public override int takeDamage(int damage, int xTrajectory, int yTrajectory, bool isBomb, double addedPrecision, Farmer who)
	{
		...
		else
		{
			...
			if (base.Health <= 0)
			{
				...
				Utility.makeTemporarySpriteJuicier(new TemporaryAnimatedSprite(44, base.Position, isHardModeMonster.Value ? Color.LimeGreen : Color.Orange, 10)
				{
					holdLastFrame = true,
					alphaFade = 0.01f,
					interval = 50f
	...
	{
		if ((base.Health > 8 || (hard.Value && base.Health >= base.MaxHealth)) && !pupating.Value)
		{
	...
	{
		b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(Sprite.SpriteWidth * 4 / 2, GetBoundingBox().Height / 2) + ((shakeTimer > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), Sprite.SourceRect, hard.Value ? Color.Lime : Color.White, rotation, new Vector2(Sprite.SpriteWidth / 2, (float)Sprite.SpriteHeight * 3f / 4f), Math.Max(0.2f, scale.Value) * 4f, (flip || (Sprite.CurrentAnimation != null && Sprite.CurrentAnimation[Sprite.currentAnimationIndex].flip)) ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)base.StandingPixel.Y / 10000f)));
	}
	...
	protected override void updateMonsterSlaveAnimation(GameTime time)
	{
		...
		else if (base.Health <= 8 || (hard.Value && base.Health < base.MaxHealth))
		{
			metamorphCounter -= time.ElapsedGameTime.Milliseconds;
	...
	public override void behaviorAtGameTick(GameTime time)
	{
		...
		if (pupating.Value)
		{
			...
			if (metamorphCounter <= 0)
			{
				...
				base.Health = -500;
				Game1.createRadialDebris(base.currentLocation, Sprite.textureName.Value, new Rectangle(208, 424, 32, 40), 4, standingPixel.X, standingPixel.Y, 25, base.TilePoint.Y);
				Game1.createRadialDebris(base.currentLocation, Sprite.textureName.Value, new Rectangle(208, 424, 32, 40), 8, standingPixel.X, standingPixel.Y, 15, base.TilePoint.Y);
				if (base.currentLocation is MineShaft mine)
				{
					base.currentLocation.characters.Add(mine.BuffMonsterIfNecessary(new Fly(base.Position, hard.Value)
					{
				...
				{
					base.currentLocation.characters.Add(new Fly(base.Position, hard.Value)
					{
						currentLocation = base.currentLocation
					});
				}
		...
		else if (base.Health <= base.MaxHealth / 2 - 2 || (hard.Value && base.Health < base.MaxHealth))
		{
			metamorphCounter -= time.ElapsedGameTime.Milliseconds;
			if (metamorphCounter <= 0)
			{
