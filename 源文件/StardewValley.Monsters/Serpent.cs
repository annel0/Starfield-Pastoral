...
public class Serpent : Monster
{
	...
	protected override void localDeathAnimation()
	{
		...
		if (IsRoyalSerpent())
		{
			...
			for (int i = -1; i < segments.Count; i++)
			{
				...
				TemporaryAnimatedSprite current_sprite = new TemporaryAnimatedSprite(color: new Color
				{
					R = (byte)Utility.Lerp(255f, 255f, color_fade),
					G = (byte)Utility.Lerp(0f, 166f, color_fade),
					B = (byte)Utility.Lerp(0f, 0f, color_fade),
					A = byte.MaxValue
				}, textureName: Sprite.textureName.Value, sourceRect: source_rect, animationInterval: 800f, animationLength: 1, numberOfLoops: 0, position: segment_position, flicker: false, flipped: false, layerDepth: 0.9f, alphaFade: 0.001f, scale: 4f * scale.Value, scaleChange: 0.01f, rotation: current_rotation + (float)Math.PI, rotationChange: (float)((double)Game1.random.Next(3, 5) * Math.PI / 64.0))
				{
	...
	public override void drawAboveAllLayers(SpriteBatch b)
	{
		...
		for (int i = -1; i < segmentCount.Value; i++)
		{
			...
			float max_offset = (float)segmentCount.Value * -0.25f / 10000f - 5E-05f;
