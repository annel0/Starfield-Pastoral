...
public class RockCrab : Monster
{
	...
	public override void shedChunks(int number)
	{
		...
		Game1.createRadialDebris(base.currentLocation, Sprite.textureName.Value, new Rectangle(0, 120, 16, 16), 8, standingPixel.X, standingPixel.Y, number, base.TilePoint.Y, Color.White, 4f * scale.Value);
	}
	...
	public override int takeDamage(int damage, int xTrajectory, int yTrajectory, bool isBomb, double addedPrecision, Farmer who)
	{
		...
		else if (Sprite.currentFrame % 4 == 0 && !shellGone.Value)
		{
			actualDamage = 0;
	...
	public override void update(GameTime time, GameLocation location)
	{
		...
		if (!shellGone.Value && !base.Player.isRafting)
		{
	...
	public override void behaviorAtGameTick(GameTime time)
	{
		...
		if (!withinPlayerThreshold() && !shellGone.Value)
		{
			Halt();
		}
		else if (withinPlayerThreshold() && !shellGone.Value && name.Equals("Truffle Crab"))
		{
			shellGone.Value = true;
		}
		else
