...
public class GreenSlime : Monster
{
	...
	public override void draw(SpriteBatch b)
	{
		...
		for (int i = 0; i <= stackedSlimes.Value; i++)
		{
			...
			b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(32f, boundsHeight / 2 + yOffset) + stack_adjustment, Sprite.SourceRect, prismatic.Value ? Utility.GetPrismaticColor(348 + specialNumber.Value, 5f) : color.Value, 0f, new Vector2(8f, 16f), 4f * Math.Max(0.2f, scale.Value - 0.4f * ((float)ageUntilFullGrown.Value / 120000f)), SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)(standingY + i * 2) / 10000f)));
			b.Draw(Game1.shadowTexture, getLocalPosition(Game1.viewport) + new Vector2(32f, (float)(boundsHeight / 2 * 7) / 4f + (float)yOffset + 8f * scale.Value - (float)((ageUntilFullGrown.Value > 0) ? 8 : 0)) + stack_adjustment, Game1.shadowTexture.Bounds, Color.White, 0f, new Vector2(Game1.shadowTexture.Bounds.Center.X, Game1.shadowTexture.Bounds.Center.Y), 3f + scale.Value - (float)ageUntilFullGrown.Value / 120000f - ((Sprite.currentFrame % 4 % 3 != 0 || i != 0) ? 1f : 0f) + (float)yOffset / 30f, SpriteEffects.None, (float)(standingY - 1 + i * 2) / 10000f);
			if (ageUntilFullGrown.Value <= 0)
			{
				if (top_slime && (cute.Value || hasSpecialItem.Value))
				{
					...
					b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + stack_adjustment + new Vector2(32f, boundsHeight - 16 + ((readyToJump <= 0) ? (4 * (-2 + Math.Abs(Sprite.currentFrame % 4 - 2))) : (4 + 4 * (Sprite.currentFrame % 4 % 3))) + yOffset) * scale.Value, new Rectangle(xDongleSource, 168 + yDongleSource, 16, 24), hasSpecialItem.Value ? Color.White : color.Value, 0f, new Vector2(8f, 16f), 4f * Math.Max(0.2f, scale.Value - 0.4f * ((float)ageUntilFullGrown.Value / 120000f)), flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)standingY / 10000f + 0.0001f)));
				}
	...
	public void mateWith(GreenSlime mateToPursue, GameLocation location)
	{
		if (location.canSlimeMateHere())
		{
			...
			baby.resilience.Value = r.Choose(resilience.Value, mateToPursue.resilience.Value);
			baby.resilience.Value = Math.Max(0, resilience.Value + r.Next(-1, 2));
