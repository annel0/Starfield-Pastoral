	...
	protected override void updateAnimation(GameTime time)
	{
		...
		if (nextParticle <= 0f)
		{
			...
			if (variant.Value == GhostVariant.Putrid)
			{
				...
				{
					int standingY = base.StandingPixel.Y;
					TemporaryAnimatedSprite drip = new TemporaryAnimatedSprite(Sprite.textureName.Value, new Microsoft.Xna.Framework.Rectangle(Game1.random.Next(4) * 16, 168, 16, 24), 100f, 1, 10, base.Position + new Vector2(Utility.RandomFloat(-16f, 16f), Utility.RandomFloat(-16f, 0f) - (float)yOffset), flicker: false, flipped: false, (float)standingY / 10000f, 0.01f, Color.White, 4f, -0.01f, 0f, 0f);
		...
		if (base.currentLocation == Game1.currentLocation)
		{
			...
			{
				light.position.Value = new Vector2(base.Position.X + 32f, base.Position.Y + 64f + (float)yOffset);
			}
