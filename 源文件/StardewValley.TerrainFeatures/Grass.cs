...
public class Grass : TerrainFeature
{
	...
	public override void doCollisionAction(Rectangle positionOfCollider, int speedOfCollision, Vector2 tileLocation, Character who)
	{
		...
		if (who is Farmer && Game1.player.CurrentTool is MeleeWeapon { isOnSpecial: not false } weapon && weapon.type.Value == 0 && Math.Abs(shakeRotation) < 0.001f && performToolAction(Game1.player.CurrentTool, -1, tileLocation))
		{
	...
	public bool reduceBy(int number, bool showDebris)
	{
		...
		{
			Vector2 tileLocation = Tile;
			Game1.createRadialDebris(Game1.currentLocation, textureName(), new Rectangle(2, 8 + grassSourceOffset.Value, 8, 8), 1, (int)((tileLocation.X + 1f) * 64f), ((int)tileLocation.Y + 1) * 64, Game1.random.Next(2, 5), (int)tileLocation.Y + 1, Color.White, 4f);
			Game1.createRadialDebris(Game1.currentLocation, textureName(), new Rectangle(2, 8 + grassSourceOffset.Value, 8, 8), 1, (int)((tileLocation.X + 1.1f) * 64f), (int)((tileLocation.Y + 1.1f) * 64f), Game1.random.Next(2, 5), (int)tileLocation.Y + 1, Color.White, 4f);
			Game1.createRadialDebris(Game1.currentLocation, textureName(), new Rectangle(2, 8 + grassSourceOffset.Value, 8, 8), 1, (int)((tileLocation.X + 0.9f) * 64f), (int)((tileLocation.Y + 1.1f) * 64f), Game1.random.Next(2, 5), (int)tileLocation.Y + 1, Color.White, 4f);
	...
	public override void dayUpdate()
	{
		...
		if ((grassType.Value == 1 || grassType.Value == 7) && (environment.GetSeason() != Season.Winter || environment.HasMapPropertyWithValue("AllowGrassGrowInWinter")) && numberOfWeeds.Value < 4)
		{
	...
	public void setUpRandom()
	{
		...
		bool front = Location?.hasTileAt((int)tileLocation.X, (int)tileLocation.Y, "Front") ?? false;
		for (int i = 0; i < 4; i++)
		{
	...
	public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation)
	{
		...
		if ((weapon != null && weapon.type.Value != 2) || explosion > 0)
		{
			...
			numberOfWeeds.Value -= numberOfWeedsToDestroy;
	...
	public override void drawInMenu(SpriteBatch spriteBatch, Vector2 positionOnScreen, Vector2 tileLocation, float scale, float layerDepth)
	{
		...
		for (int i = 0; i < numberOfWeeds.Value; i++)
		{
			...
			spriteBatch.Draw(position: (i != 4) ? (tileLocation * 64f + new Vector2((float)(i % 2 * 64 / 2 + r.Next(-2, 2) * 4 - 4) + 30f, i / 2 * 64 / 2 + r.Next(-2, 2) * 4 + 40)) : (tileLocation * 64f + new Vector2((float)(16 + r.Next(-2, 2) * 4 - 4) + 30f, 16 + r.Next(-2, 2) * 4 + 40)), texture: texture.Value, sourceRectangle: new Rectangle(whichWeed * 15, grassSourceOffset.Value, 15, 20), color: Color.White, rotation: shakeRotation / (float)(r.NextDouble() + 1.0), origin: Vector2.Zero, scale: scale, effects: SpriteEffects.None, layerDepth: layerDepth + (32f * scale + 300f) / 20000f);
		}
	}
	...
	public override void draw(SpriteBatch spriteBatch)
	{
		...
		for (int i = 0; i < numberOfWeeds.Value; i++)
		{
			...
			spriteBatch.Draw(texture.Value, Game1.GlobalToLocal(Game1.viewport, pos), new Rectangle(whichWeed[i] * 15, grassSourceOffset.Value, 15, 20), Color.White, shakeRotation / (float)(shakeRandom[i] + 1.0), new Vector2(7.5f, 17.5f), 4f, flip[i] ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (pos.Y + 16f - 20f) / 10000f + pos.X / 10000000f);
		}
	}
}
