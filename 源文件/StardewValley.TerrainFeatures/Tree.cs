...
public class Tree : TerrainFeature
{
	...
	public override bool tickUpdate(GameTime time)
	{
		...
		Vector2 tileLocation = Tile;
		if (growthStage.Value >= 5 && !falling.Value && !stump.Value && Game1.player.GetBoundingBox().Intersects(new Microsoft.Xna.Framework.Rectangle(64 * ((int)tileLocation.X - 1), 64 * ((int)tileLocation.Y - 5), 192, 288)))
		{
		...
		else
		{
			...
			if ((double)Math.Abs(shakeRotation) > Math.PI / 2.0)
			{
				...
				if (data != null)
				{
					...
					{
						int numToDrop = (int)((lastHitBy.professions.Contains(12) ? 1.25 : 1.0) * (double)(12 + extraWoodCalculator(tileLocation)));
						if (lastHitBy.stats.Get("Book_Woodcutting") != 0 && r.NextDouble() < 0.05)
						{
							numToDrop *= 2;
						}
						Game1.createRadialDebris(location, 12, (int)tileLocation.X + (shakeLeft.Value ? (-4) : 4), (int)tileLocation.Y, numToDrop, resource: true);
						Game1.createRadialDebris(location, 12, (int)tileLocation.X + (shakeLeft.Value ? (-4) : 4), (int)tileLocation.Y, (int)((lastHitBy.professions.Contains(12) ? 1.25 : 1.0) * (double)(12 + extraWoodCalculator(tileLocation))), resource: false);
					}
					if (data.DropWoodOnChop)
					{
						Game1.createMultipleObjectDebris("(O)92", (int)tileLocation.X + (shakeLeft.Value ? (-4) : 4), (int)tileLocation.Y, 5, lastPlayerToHit.Value, location);
					}
					int numHardwood = 0;
					...
					if (chopItems != null && chopItems.Count > 0)
					{
						...
						{
							Item item = TryGetDrop(drop, r, lastHitBy, "ChopItems", null, false);
							if (item != null)
							{
								if (drop.ItemId == "709")
								{
								...
								{
									Game1.createMultipleItemDebris(item, new Vector2(tileLocation.X + (float)(shakeLeft.Value ? (-4) : 4), tileLocation.Y) * 64f, -2, location);
								}
							}
						}
					...
					if (numHardwood > 0)
					{
						Game1.createMultipleObjectDebris("(O)709", (int)tileLocation.X + (shakeLeft.Value ? (-4) : 4), (int)tileLocation.Y, numHardwood, lastPlayerToHit.Value, location);
					}
					...
					{
						Game1.createMultipleObjectDebris(data.SeedItemId, (int)tileLocation.X + (shakeLeft.Value ? (-4) : 4), (int)tileLocation.Y, r.Next(1, 3), lastPlayerToHit.Value, location);
					}
				}
	...
	public override void dayUpdate()
	{
		...
		{
			GameLocation location = Location;
			if ((location == null || !location.IsGreenRainingHere()) && isTemporaryGreenRainTree.Value)
			{
				isTemporaryGreenRainTree.Value = false;
				if (treeType.Value == "10")
				{
		...
		if (growthStage.Value >= 5 && !stump.Value && environment is Farm && Game1.random.NextBool(data?.SeedSpreadChance ?? 0.15f))
		{
			...
			{
				environment.terrainFeatures.Add(location2, new Tree(treeType.Value, 0));
			}
		}
		if (isTemporaryGreenRainTree.Value && environment.IsGreenhouse && (localSeason == Season.Winter || localSeason == Season.Fall))
		{
			hasSeed.Value = false;
		}
		else
		...
		bool accelerateMoss = growthStage.Value >= 5 && !Game1.IsWinter && (treeType.Value == "10" || treeType.Value == "11") && !isTemporaryGreenRainTree.Value;
		if (growthStage.Value >= 5 && !Game1.IsWinter && !accelerateMoss)
		{
			for (int x = (int)tile.X - 2; (float)x <= tile.X + 2f; x++)
			{
				for (int y = (int)tile.Y - 2; (float)y <= tile.Y + 2f; y++)
				{
					...
					if (Location.terrainFeatures.GetValueOrDefault(v) is Tree tree && tree.growthStage.Value >= 5 && (tree.treeType.Value == "10" || tree.treeType.Value == "11") && !tree.isTemporaryGreenRainTree.Value && tree.hasMoss.Value)
					{
						accelerateMoss = true;
						break;
					}
		...
		if (flag.HasValue && flag == true && growthStage.Value >= 14 && !stump.Value && Game1.random.NextBool(mossChance))
		{
			hasMoss.Value = true;
		}
	}
	...
	public override bool seasonUpdate(bool onLoad)
	{
		...
		{
			Object tileObject = Location.getObjectAtTile((int)Tile.X, (int)Tile.Y);
			if (tileObject != null && tileObject.IsTapper())
			{
				UpdateTapperProduct(tileObject, null, onlyPerformRemovals: true);
			}
		}
		loadSprite();
		return false;
	...
	public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation)
	{
		...
		if (growthStage.Value >= 5)
		{
			...
			if (t is Axe)
			{
				...
				if (location is Town && tileLocation.X < 100f && !isTemporaryGreenRainTree.Value)
				{
				...
				if (!stump.Value && t.getLastFarmerToUse() != null && location.HasUnlockedAreaSecretNotes(t.getLastFarmerToUse()) && Game1.random.NextDouble() < 0.005)
				{
			...
			{
				Debris d = treeType.Value switch
				{
	...
	protected void performSeedDestroy(Tool t, Vector2 tileLocation)
	{
		...
		{
			Farmer lastHitBy = Game1.GetPlayer(lastPlayerToHit.Value) ?? Game1.MasterPlayer;
			if (lastPlayerToHit.Value != 0L && lastHitBy.getEffectiveSkillLevel(2) >= 1)
			{
	...
	protected bool TryGetTapperOutput(List<WildTreeTapItemData> tapItems, string previousItemId, Random r, float timeMultiplier, out Object output, out int minutesUntilReady)
	{
		if (tapItems != null)
		{
			...
			foreach (WildTreeTapItemData tapData in tapItems)
			{
				...
				Farmer lastHitBy = Game1.GetPlayer(lastPlayerToHit.Value) ?? Game1.MasterPlayer;
				Item item = TryGetDrop(tapData, r, lastHitBy, "TapItems", (string id) => id.Replace("PREVIOUS_OUTPUT_ID", previousItemId));
				if (item != null)
				{
	...
	protected void performBushDestroy(Vector2 tileLocation)
	{
		...
		foreach (WildTreeChopItemData drop in data.ChopItems)
		{
			Item item = TryGetDrop(drop, r, lastHitBy, "ChopItems");
			if (item != null)
			{
			...
			if (data != null)
			{
				...
				else
				{
					...
					else
					{
						if (data.DropWoodOnChop)
						{
							Game1.createRadialDebris(location, 12, (int)tileLocation.X, (int)tileLocation.Y, (int)((lastHitBy.professions.Contains(12) ? 1.25 : 1.0) * (double)(5 + extraWoodCalculator(tileLocation))), resource: true);
						}
						...
						if (chopItems2 != null && chopItems2.Count > 0)
						{
							...
							{
								Item item2 = TryGetDrop(drop2, r, lastHitBy, "ChopItems");
								if (item2 != null)
								{
	...
	public override void drawInMenu(SpriteBatch spriteBatch, Vector2 positionOnScreen, Vector2 tileLocation, float scale, float layerDepth)
	{
		...
		if (growthStage.Value < 5)
		{
			Microsoft.Xna.Framework.Rectangle sourceRect = growthStage.Value switch
			{
			...
			spriteBatch.Draw(texture.Value, positionOnScreen - new Vector2(0f, (float)sourceRect.Height * scale), sourceRect, Color.White, 0f, Vector2.Zero, scale, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, layerDepth + (positionOnScreen.Y + (float)sourceRect.Height * scale) / 20000f);
			return;
		}
		if (!falling.Value)
		{
	...
	public override void draw(SpriteBatch spriteBatch)
	{
		...
		if (growthStage.Value < 5)
		{
			...
			spriteBatch.Draw(texture.Value, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileLocation.X * 64f + 32f, tileLocation.Y * 64f - (float)(sourceRect.Height * 4 - 64) + (float)((growthStage.Value >= 3) ? 128 : 64))), sourceRect, fertilized.Value ? Color.HotPink : Color.White, shakeRotation, new Vector2(8f, (growthStage.Value >= 3) ? 32 : 16), 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (growthStage.Value == 0) ? 0.0001f : (baseSortPosition / 10000f));
		}
		else
		{
			if (!stump.Value || falling.Value)
			{
				if (IsLeafy())
				{
					spriteBatch.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileLocation.X * 64f - 51f, tileLocation.Y * 64f - 16f)), shadowSourceRect, Color.White * ((float)Math.PI / 2f - Math.Abs(shakeRotation)), 0f, Vector2.Zero, 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 1E-06f);
				}
				else
				{
