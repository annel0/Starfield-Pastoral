	...
	public override bool tickUpdate(GameTime time)
	{
		...
		else
		{
			...
			if ((double)Math.Abs(shakeRotation) > Math.PI / 2.0)
			{
				...
				Game1.createRadialDebris(location, 12, (int)tileLocation.X + (shakeLeft.Value ? (-4) : 4), (int)tileLocation.Y, (int)((lastHitBy.professions.Contains(12) ? 1.25 : 1.0) * 12.0), resource: true);
				Game1.createRadialDebris(location, 12, (int)tileLocation.X + (shakeLeft.Value ? (-4) : 4), (int)tileLocation.Y, (int)((lastHitBy.professions.Contains(12) ? 1.25 : 1.0) * 12.0), resource: false);
