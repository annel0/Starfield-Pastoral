...
public class Bush : LargeTerrainFeature
{
	...
	{
		Season season = GetCosmeticSeason();
		int seasonNumber = (int)season;
		switch (size.Value)
		{
		case 0:
			sourceRect.Value = new Rectangle(seasonNumber * 16 * 2 + tileSheetOffset.Value * 16, 224, 16, 32);
			break;
		case 1:
		{
			if (townBush.Value)
			...
			int xOffset = seasonNumber * 16 * 4 + tileSheetOffset.Value * 16 * 2;
		...
		case 3:
		{
			...
			{
			case Season.Spring:
				sourceRect.Value = new Rectangle(Math.Min(2, age / 10) * 16 + tileSheetOffset.Value * 16, 256, 16, 32);
				break;
			case Season.Summer:
				sourceRect.Value = new Rectangle(64 + Math.Min(2, age / 10) * 16 + tileSheetOffset.Value * 16, 256, 16, 32);
				break;
			case Season.Fall:
				sourceRect.Value = new Rectangle(Math.Min(2, age / 10) * 16 + tileSheetOffset.Value * 16, 288, 16, 32);
				break;
			case Season.Winter:
				sourceRect.Value = new Rectangle(64 + Math.Min(2, age / 10) * 16 + tileSheetOffset.Value * 16, 288, 16, 32);
				break;
			}
			break;
		}
	...
	{
		if (Location == null || !Location.SeedsIgnoreSeasonsHere())
		{
			if (inPot.Value)
			{
	...
	public bool inBloom()
	{
		...
		Season season = Location?.GetSeason() ?? Game1.season;
	...
	public override void loadSprite()
	{
		...
		if (size.Value != 4)
		{
			if (size.Value == 1 && !readyForHarvest() && r.NextDouble() < 0.2 + extra && inBloom())
			{
				tileSheetOffset.Value = 1;
			}
	...
	public override bool performUseAction(Vector2 tileLocation)
	{
		...
		if (shakeTimer <= 0f)
		{
			...
			if (maxShake == 0f && (size.Value != 3 || season != Season.Winter || IsSheltered()))
			{
	...
	public void shake(Vector2 tileLocation, bool doEvenIfStillShaking)
	{
		...
		base.NeedsUpdate = true;
		if (!townBush.Value && readyForHarvest() && inBloom())
		{
	...
	public override bool seasonUpdate(bool onLoad)
	{
		...
		{
			Season season = Location.GetSeason();
			tileSheetOffset.Value = ((size.Value == 1 && season == Season.Summer && Game1.random.NextBool()) ? 1 : 0);
			loadSprite();
		}
		return false;
	}
	...
	public override bool performToolAction(Tool t, int explosion, Vector2 tileLocation)
	{
		...
		if (size.Value == 3 && t is MeleeWeapon { ItemId: "66" })
		{
			shake(tileLocation, doEvenIfStillShaking: true);
		}
		else if (t is Axe axe && isDestroyable())
		{
			...
			shake(tileLocation, doEvenIfStillShaking: true);
			if (axe.upgradeLevel.Value >= 1 || size.Value == 3)
			{
				health -= ((size.Value == 3) ? 0.5f : ((float)axe.upgradeLevel.Value / 5f));
				if (health <= -1f)
				{
	...
	public override void drawInMenu(SpriteBatch spriteBatch, Vector2 positionOnScreen, Vector2 tileLocation, float scale, float layerDepth)
	{
		...
		spriteBatch.Draw(texture.Value, positionOnScreen + new Vector2(0f, -64f * scale), new Rectangle(32, 96, 16, 32), Color.White, 0f, Vector2.Zero, scale, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, layerDepth + (positionOnScreen.Y + 448f * scale - 1f) / 20000f);
	}
	...
	public override void draw(SpriteBatch spriteBatch)
	{
		...
		if (drawShadow.Value)
		{
			...
			{
				spriteBatch.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2((tileLocation.X + ((getEffectiveSize() == 1) ? 0.5f : 1f)) * 64f - 51f, tileLocation.Y * 64f - 16f + yDrawOffset)), shadowSourceRect, Color.White, 0f, Vector2.Zero, 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 1E-06f);
			}
			else
			{
		...
		spriteBatch.Draw(texture.Value, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileLocation.X * 64f + (float)((getEffectiveSize() + 1) * 64 / 2), (tileLocation.Y + 1f) * 64f - (float)((getEffectiveSize() > 0 && (!townBush.Value || getEffectiveSize() != 1) && size.Value != 4) ? 64 : 0) + yDrawOffset)), sourceRect.Value, Color.White, shakeRotation, new Vector2((getEffectiveSize() + 1) * 16 / 2, 32f), 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)(getBoundingBox().Center.Y + 48) / 10000f - tileLocation.X / 1000000f);
	}
}
