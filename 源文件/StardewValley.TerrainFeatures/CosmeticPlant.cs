...
public class CosmeticPlant : Grass
{
	...
	{
		Vector2 tileLocation = Tile;
		spriteBatch.Draw(texture.Value, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileLocation.X * 64f, tileLocation.Y * 64f) + new Vector2(32 + xOffset.Value, 60 + yOffset.Value)), new Rectangle(grassType.Value * 16, 0, 16, 24), Color.White, shakeRotation, new Vector2(8f, 23f), 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, ((float)(getBoundingBox().Y - 4) + tileLocation.X / 900f + 0.01f) / 10000f);
	}
}
