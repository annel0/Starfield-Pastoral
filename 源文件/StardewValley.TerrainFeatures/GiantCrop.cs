...
public class GiantCrop : ResourceClump
{
	...
	public override bool performToolAction(Tool t, int damage, Vector2 tileLocation)
	{
		...
		Game1.createRadialDebris(Game1.currentLocation, 12, (int)tileLocation.X + width.Value / 2, (int)tileLocation.Y + height.Value / 2, r.Next(4, 9), resource: false);
		if (shakeTimer <= 0f)
		{
			shakeTimer = 100f;
		...
		if (t.hasEnchantmentOfType<ShavingEnchantment>() && r.NextBool((float)power / 5f) && data?.HarvestItems != null)
		{
			foreach (GiantCropHarvestItemData drop in data.HarvestItems)
			{
				...
				if (item != null)
				{
					...
					Debris d = new Debris(item, new Vector2((tileLocation.X + (float)(width.Value / 2)) * 64f, (tileLocation.Y + (float)(height.Value / 2)) * 64f), Game1.player.getStandingPosition());
		...
		if (health.Value <= 0f)
		{
			...
			if (data?.HarvestItems != null)
			{
				...
				{
					Item item2 = TryGetDrop(drop2, r, player, isShaving: false, healthDeducted);
					if (item2 == null)
					{
						continue;
					}
			...
			Game1.createRadialDebris(Game1.currentLocation, 12, (int)tileLocation.X + width.Value / 2, (int)tileLocation.Y + width.Value / 2, r.Next(4, 9), resource: false);
			...
			for (int x = 0; x < width.Value; x++)
			{
				for (int y = 0; y < height.Value; y++)
				{
					...
					if (width.Value >= 2 && height.Value >= 2 && (x == 0 || x == width.Value - 2) && (y == 0 || y == height.Value - 2))
					{
