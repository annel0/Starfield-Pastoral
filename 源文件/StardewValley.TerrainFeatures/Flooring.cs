...
public class Flooring : TerrainFeature
{
	...
	private List<Neighbor> gatherNeighbors()
	{
		...
		for (int i = 0; i < offsets.Length; i++)
		{
			...
			else if (terrainFeatures.TryGetValue(tile, out feature) && feature is Flooring flooring && flooring.whichFloor.Value == whichFloor.Value)
			{
				Neighbor n2 = new Neighbor(flooring, item.Direction, item.InvDirection);
				results.Add(n2);
			}
		}
		return results;
