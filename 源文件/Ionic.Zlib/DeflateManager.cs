...
internal sealed class DeflateManager
{
	...
	internal void set_data_type()
	{
		...
		for (; n < InternalConstants.LITERALS; n++)
		{
			bin_freq += dyn_ltree[n * 2];
		}
	...
	internal int longest_match(int cur_match)
	{
		...
		byte scan_end2 = window[scan + best_len];
		...
		{
			int match = cur_match;
			if (window[match + best_len] != scan_end2 || window[match + best_len - 1] != scan_end1 || window[match] != window[scan] || window[++match] != window[scan + 1])
			{
				continue;
			}
			scan += 2;
			...
			if (len > best_len)
			{
				...
				scan_end2 = window[scan + best_len];
			}
		}
