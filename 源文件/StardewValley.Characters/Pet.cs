...
public class Pet : NPC
{
	...
	public Item TryGetGiftItem(List<PetGift> gifts)
	{
		...
		gifts.RemoveAll(delegate(PetGift petGift)
		{
			if (friendshipTowardFarmer.Value >= petGift.MinimumFriendshipThreshold && GameStateQuery.CheckConditions(petGift.Condition))
			{
				totalWeight += petGift.Weight;
				return false;
			}
			return true;
		});
	...
	public virtual void drawHat(SpriteBatch b, Vector2 shake)
	{
		...
		if (!(value == "Cat"))
		{
			if (value == "Dog")
			{
				...
				case 20:
					direction = ((!flipped) ? 1 : 3);
					hatOffset.X += 26f;
					hatOffset.Y += ((whichBreed.Value == "2") ? 16 : ((whichBreed.Value == "1") ? 24 : 20));
					break;
				case 21:
					direction = ((!flipped) ? 1 : 3);
					hatOffset.X += 22f;
					hatOffset.Y += ((whichBreed.Value == "2") ? 12 : ((whichBreed.Value == "1") ? 20 : 16));
					break;
				case 22:
					direction = ((!flipped) ? 1 : 3);
					hatOffset.X += 18f;
					hatOffset.Y += ((whichBreed.Value == "2") ? 8 : ((whichBreed.Value == "1") ? 8 : 12));
					break;
				case 17:
					hatOffset.Y += 12f;
					break;
				...
				case 27:
					direction = 2;
					hatOffset.Y += 12 + ((whichBreed.Value == "2") ? (-4) : 0);
					break;
				case 28:
				case 29:
					scale = 1.3333334f;
				...
				case 32:
					direction = ((!flipped) ? 1 : 3);
					hatOffset.X += 26f;
					hatOffset.Y += ((whichBreed.Value == "2") ? 12 : 16);
					break;
				case 33:
					direction = ((!flipped) ? 1 : 3);
					hatOffset.X += 26f;
					hatOffset.Y += ((whichBreed.Value == "2") ? 16 : 20);
					break;
				case 34:
					direction = ((!flipped) ? 1 : 3);
					hatOffset.X += 26f;
					hatOffset.Y += ((whichBreed.Value == "2") ? 20 : 24);
					break;
				}
				string value2 = whichBreed.Value;
				if (!(value2 == "2"))
				{
					if (value2 == "3" && direction == 3 && Sprite.CurrentFrame > 16)
					{
						hatOffset.X += 4f;
					}
				}
		...
		else
		{
			...
			if ((whichBreed.Value == "3" || whichBreed.Value == "4") && direction == 3)
			{
				hatOffset.X -= 4f;
			}
		}
	...
	public override void draw(SpriteBatch b)
	{
		...
		Vector2 shake = ((shakeTimer > 0 && !isSleepingOnFarmerBed.Value) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero);
