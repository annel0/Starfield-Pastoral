...
public class CrabPot : Object
{
	...
	public override bool performObjectDropInAction(Item dropInItem, bool probe, Farmer who, bool returnFalseIfItemConsumed = false)
	{
		...
		if (dropInItem is Object { Category: -21 } dropIn && NeedsBait(who))
		{
			if (!probe)
			{
				if (who != null)
	...
	{
		Object prevBait = bait.Value;
		if (base.AttemptAutoLoad(inventory, who) && prevBait != bait.Value)
		{
	...
	public override bool checkForAction(Farmer who, bool justCheckingForActivity = false)
	{
		...
		if (tileIndexToShow == 714)
		{
			...
			if (item != null)
			{
				...
				if (Utility.CreateDaySaveRandom(Game1.uniqueIDForThisGame, Game1.stats.DaysPlayed * 77, tileLocation.X * 777f + tileLocation.Y).NextDouble() < 0.25 && Game1.player.stats.Get("Book_Crabbing") != 0 && who.couldInventoryAcceptThisItem(item.QualifiedItemId, numToCatch * 2, item.Quality))
				{
					numToCatch *= 2;
				}
	...
	{
		GameLocation location = Location;
		Farmer ownedByPlayer = Game1.GetPlayer(owner.Value) ?? Game1.MasterPlayer;
		bool isMariner = ownedByPlayer.professions.Contains(10);
		if (NeedsBait(ownedByPlayer) || heldObject.Value != null)
		{
			return;
		}
		tileIndexToShow = 714;
		...
		if (!r.NextBool(chanceForJunk))
		{
			...
			foreach (KeyValuePair<string, string> v in DataLoader.Fish(Game1.content))
			{
				...
				heldObject.Value = ItemRegistry.Create<Object>("(O)" + v.Key, quantity, quality);
				break;
			}
		}
		if (heldObject.Value == null)
		{
			...
			{
				heldObject.Value = ItemRegistry.Create<Object>("(O)" + r.ChooseFrom(marinerList), quantity, quality);
			}
			else
			{
