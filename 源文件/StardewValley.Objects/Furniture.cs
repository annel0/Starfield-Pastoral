...
public class Furniture : Object, ISittable
{
	...
	public virtual List<Vector2> GetSeatPositions(bool ignore_offsets = false)
	{
		...
		case 2:
		{
			...
			case 2:
			{
				...
				{
					seat_positions.Add(TileLocation + new Vector2((float)j + 0.5f, 0f));
				}
	...
	public override bool performObjectDropInAction(Item dropInItem, bool probe, Farmer who, bool returnFalseIfItemConsumed = false)
	{
		...
		if (IsTable() && heldObject.Value == null && !dropIn.bigCraftable.Value && !(dropIn is Wallpaper) && (!(dropIn is Furniture furniture) || (furniture.getTilesWide() == 1 && furniture.getTilesHigh() == 1)))
		{
			if (!probe)
			{
	...
	public virtual void addLights()
	{
		...
		if (isLampStyleLightSource())
		{
			...
			if (base.lightSource == null)
			{
				base.lightSource = new LightSource(GenerateLightSourceId(), 4, new Vector2(boundingBox.X + 32, boundingBox.Y + ((furniture_type.Value == 7) ? (-64) : 64)), (furniture_type.Value == 7) ? 2f : 1f, (base.QualifiedItemId == "(F)1369") ? (Color.RoyalBlue * 0.7f) : Color.Black, LightSource.LightContext.None, 0L, environment.NameOrUniqueName);
				environment.sharedLights.AddLight(base.lightSource.Clone());
			}
		}
		else if (base.QualifiedItemId == "(F)1440")
		{
			base.lightSource = new LightSource(GenerateLightSourceId(), 4, new Vector2(boundingBox.X + 96, (float)boundingBox.Y - 32f), 1.5f, Color.Black, LightSource.LightContext.None, 0L, environment.NameOrUniqueName);
			environment.sharedLights.AddLight(base.lightSource.Clone());
		}
		...
		{
			string lightSourceId = GenerateLightSourceId();
			Vector2 lightPosition = new Vector2(tileLocation.X * 64f + 32f + 2f, tileLocation.Y * 64f + 12f);
			for (int i = 0; i < getTilesWide(); i++)
			{
				base.lightSource = new LightSource($"{lightSourceId}_tile{i}", 8, lightPosition, 2f, Color.Black, LightSource.LightContext.None, 0L, environment.NameOrUniqueName);
				environment.sharedLights.AddLight(base.lightSource.Clone());
				lightPosition.X += 64f;
			}
		}
	...
	public virtual void removeLights()
	{
		...
		if (isLampStyleLightSource() || base.QualifiedItemId == "(F)1440")
		{
			sourceRect.Value = defaultSourceRect.Value;
			sourceIndexOffset.Value = 0;
			environment?.removeLightSource(GenerateLightSourceId());
		...
		else if (this is FishTankFurniture)
		{
			...
			for (int i = 0; i < getTilesWide(); i++)
			{
				environment?.removeLightSource($"{lightSourceId}_tile{i}");
			}
			base.lightSource = null;
		}
	}
	...
	public virtual void rotate()
	{
		...
		{
			int rotationAmount = ((rotations.Value == 4) ? 1 : 2);
	...
	public static Furniture GetFurnitureInstance(string itemId, Vector2? position = null)
	{
		...
		default:
		{
			string furnitureType = ArgUtility.Get(getData(itemId), 1);
			...
			default:
				if (furnitureType?.StartsWith("bed") ?? false)
				{
		...
		for (int x = 0; x < tilesWide; x++)
		{
			for (int y = 0; y < tilesHigh; y++)
			{
				...
				foreach (Furniture f in l.furniture)
				{
					...
					if ((f.furniture_type.Value != 12 || furniture_type.Value == 12) && f.GetBoundingBox().Contains((int)curPixel.X, (int)curPixel.Y) && !f.AllowPlacementOnThisTile((int)tile.X + x, (int)tile.Y + y))
					{
						return false;
					}
				}
				...
				if (furniture_type.Value == 15 && y == 0)
				{
					if (l.IsTileOccupiedBy(curTile, collisionMask, ignorePassables))
					{
						return false;
				...
				if (l.terrainFeatures.GetValueOrDefault(curTile) is HoeDirt { crop: not null })
				{
					return false;
				}
			}
	...
	public override bool placementAction(GameLocation location, int x, int y, Farmer who = null)
	{
		...
		{
			if (f.furniture_type.Value == 11 && f.heldObject.Value == null && f.GetBoundingBox().Intersects(boundingBox.Value))
			{
	...
	public virtual int GetAdditionalFurniturePlacementStatus(GameLocation location, int x, int y, Farmer who = null)
	{
		if (location.CanPlaceThisFurnitureHere(this))
		{
			...
			if (furniture_type.Value == 6 || furniture_type.Value == 17 || furniture_type.Value == 13 || base.QualifiedItemId == "(F)1293")
			{
				...
				{
					if ((furniture_type.Value == 6 || furniture_type.Value == 17 || furniture_type.Value == 13 || offset != 0) && decoratable_location.isTileOnWall(anchor.X, anchor.Y - offset) && decoratable_location.GetWallTopY(anchor.X, anchor.Y - offset) + offset == anchor.Y)
					{
						foundWall = true;
					}
	...
	public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		if (isDrawingLocationFurniture)
		{
			...
			SpriteEffects spriteEffects = (flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None);
			...
			{
				spriteBatch.Draw(texture, actualDrawPosition, drawn_source_rect, color, 0f, Vector2.Zero, 4f, spriteEffects, (furniture_type.Value == 12) ? (2E-09f + tileLocation.Y / 100000f) : ((float)(boundingBox.Value.Bottom - ((furniture_type.Value == 6 || furniture_type.Value == 17 || furniture_type.Value == 13) ? 48 : 8)) / 10000f));
		...
		if (heldObject.Value != null)
		{
			...
			else
			{
				...
				{
					spriteBatch.Draw(heldItemData.GetTexture(), Game1.GlobalToLocal(Game1.viewport, new Vector2(boundingBox.Center.X - 32, boundingBox.Center.Y - (drawHeldObjectLow.Value ? 32 : 85))), heldItemData.GetSourceRect(), Color.White * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(boundingBox.Bottom + 1) / 10000f);
				}
			}
		}
		...
		{
			Rectangle bounds2 = GetBoundingBoxAt(x, y);
			spriteBatch.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(boundingBox.Center.X - 20, (float)boundingBox.Center.Y - 105.6f)), new Rectangle(276 + (int)((Game1.currentGameTime.TotalGameTime.TotalMilliseconds + (double)(x * 3047) + (double)(y * 88)) % 400.0 / 100.0) * 12, 1985, 12, 11), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(bounds2.Bottom - 2) / 10000f);
		}
		if (Game1.debugMode)
		{
	...
	public virtual void drawAtNonTileSpot(SpriteBatch spriteBatch, Vector2 location, float layerDepth, float alpha = 1f)
	{
		...
		spriteBatch.Draw(itemData.GetTexture(), location, drawn_source_rect, Color.White * alpha, 0f, Vector2.Zero, 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, layerDepth);
	}
	...
	protected override void GetOneCopyFrom(Item source)
	{
		...
		if (source is Furniture fromFurniture)
		{
			...
			isOn.Value = false;
			rotations.Value = fromFurniture.rotations.Value;
			currentRotation.Value = fromFurniture.currentRotation.Value - ((rotations.Value == 4) ? 1 : 2);
			rotate();
		}
	}
}
