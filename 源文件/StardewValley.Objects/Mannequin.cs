...
public class Mannequin : Object
{
				...
				else
				{
					...
					if (Game1.random.NextDouble() < 0.33)
					{
						for (int i = 0; i < 30; i++)
						{
							...
							for (int y2 = 1; y2 < Location.Map.Layers[0].LayerHeight; y2++)
							{
								Vector2 spot = new Vector2(xPos, y2);
								if (Location.isTileLocationOpen(spot) && Location.isTilePlaceable(spot) && !dec_location2.isTileOnWall(xPos, y2) && !Location.IsTileOccupiedBy(spot))
								{
									facing.Value = 2;
									renderCache = null;
					...
					for (int i2 = 0; i2 < 30; i2++)
					{
						...
						{
							Vector2 spot2 = new Vector2(x, yPos);
							if (Location.isTileLocationOpen(spot2) && Location.isTilePlaceable(spot2) && !dec_location2.isTileOnWall(x, yPos) && !Location.IsTileOccupiedBy(spot2))
							{
	...
	protected override void GetOneCopyFrom(Item source)
	{
		...
		if (source is Mannequin from)
		{
			hat.Value = from.hat.Value?.getOne() as Hat;
			shirt.Value = from.shirt.Value?.getOne() as Clothing;
			pants.Value = from.pants.Value?.getOne() as Clothing;
			boots.Value = from.boots.Value?.getOne() as Boots;
		}
	}
