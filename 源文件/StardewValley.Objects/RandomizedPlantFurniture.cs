...
public class RandomizedPlantFurniture : Furniture
{
	...
	public virtual void DrawFurniture(SpriteBatch sb, Vector2 location, float alpha, Vector2 origin, float scale, float base_sort_y)
	{
		...
		sb.Draw(texture, location, drawn_source_rect, Color.White * alpha, 0f, origin, scale, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, base_sort_y);
		...
		sb.Draw(texture, location + new Vector2(offset_x, -8f * scale), drawn_source_rect, Color.White * alpha, 0f, origin, scale, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, base_sort_y + 1E-05f);
		drawn_source_rect = new Rectangle(0, 0, 16, 16);
		drawn_source_rect.X += topIndex.Value % 8 * 16;
		drawn_source_rect.Y += topIndex.Value / 8 * 16;
		sb.Draw(texture, location + new Vector2(offset_x, -24f * scale), drawn_source_rect, Color.White * alpha, 0f, origin, scale, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, base_sort_y + 1E-05f);
	}
}
