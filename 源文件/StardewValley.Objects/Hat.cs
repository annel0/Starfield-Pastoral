...
public class Hat : Item
{
	...
	public override void drawInMenu(SpriteBatch spriteBatch, Vector2 location, float scaleSize, float transparency, float layerDepth, StackDrawType drawStackNumber, Color color, bool drawShadow)
	{
		...
		spriteBatch.Draw(texture, location + new Vector2(32f, 32f), drawnSourceRect, isPrismatic.Value ? (Utility.GetPrismaticColor() * transparency) : (color * transparency), 0f, new Vector2(10f, 10f), 4f * scaleSize, SpriteEffects.None, layerDepth);
	...
	public void draw(SpriteBatch spriteBatch, Vector2 location, float scaleSize, float transparency, float layerDepth, int direction, bool useAnimalTexture = false)
	{
		...
		spriteBatch.Draw(texture, location + new Vector2(10f, 10f), drawnSourceRect, isPrismatic.Value ? (Utility.GetPrismaticColor() * transparency) : (Color.White * transparency), 0f, new Vector2(3f, 3f), 3f * scaleSize, SpriteEffects.None, layerDepth);
	}
	...
	{
		ParsedItemData itemData = ItemRegistry.GetDataOrErrorItem(base.QualifiedItemId);
		if (Name != null && Name != "Error Item" && itemData.IsErrorItem)
		{
