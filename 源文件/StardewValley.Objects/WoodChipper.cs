...
public class WoodChipper : Object
{
	...
	public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		if (depositedItem.Value != null && shakeTimer > 0 && _isAnimatingChip)
		{
			...
			Vector2 draw_position = new Vector2
			{
				X = Utility.Lerp(start_position.X, end_position.X, completion),
				Y = Utility.Lerp(start_position.Y, end_position.Y, completion)
			};
			...
			spriteBatch.Draw(itemTexture, draw_position, itemDraw.GetSourceRect(), Color.White * alpha, 0f, new Vector2(8f, 8f), 4f * draw_scale, depositedItem.Value.flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, draw_layer + 0.00175f);
		}
		...
		{
			ParsedItemData itemDraw2 = ItemRegistry.GetDataOrErrorItem(heldObject.Value.QualifiedItemId);
			Texture2D itemTexture2 = itemDraw2.GetTexture();
			spriteBatch.Draw(itemTexture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, (float)(y * 64 - 64 - 8) + yOffset)), itemDraw2.GetSourceRect(), Color.White * 0.75f, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, (float)((y + 1) * 64) / 10000f + 1E-05f + tileLocation.X / 10000f);
			if (heldObject.Value is ColoredObject coloredObject)
			{
				Rectangle coloredSourceRect = itemDraw2.GetSourceRect(1, base.ParentSheetIndex);
				spriteBatch.Draw(itemTexture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, (float)(y * 64 - 64 - 8) + yOffset)), coloredSourceRect, coloredObject.color.Value * 0.75f, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, (float)((y + 1) * 64) / 10000f + 1E-05f + tileLocation.X / 10000f);
			}
		}
	}
}
