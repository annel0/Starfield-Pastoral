...
public class ColoredObject : Object
{
		...
		: base(itemId, stack)
	{
		...
		{
			ColorSameIndexAsParentSheetIndex = !data.ColorOverlayFromNextIndex;
		}
	}
	...
	public override void drawInMenu(SpriteBatch spriteBatch, Vector2 location, float scaleSize, float transparency, float layerDepth, StackDrawType drawStackNumber, Color colorOverride, bool drawShadow)
	{
		...
		Vector2 origin = (bigCraftable.Value ? new Vector2(32f, 64f) : new Vector2(8f, 8f));
		float scale = ((!bigCraftable.Value) ? (4f * scaleSize) : ((scaleSize < 0.2f) ? scaleSize : (scaleSize / 2f)));
	...
	private void drawSmokedFish(SpriteBatch spriteBatch, Vector2 location, float scaleSize, float layerDepth, float transparency = 1f)
	{
		...
		int interval = 700 + (price.Value + 17) * 7777 % 200;
	...
	public override void drawWhenHeld(SpriteBatch spriteBatch, Vector2 objectPosition, Farmer f)
	{
		...
		else
		{
			...
			spriteBatch.Draw(itemData2.GetTexture(), objectPosition, itemData2.GetSourceRect(0, base.ParentSheetIndex), color.Value, 0f, Vector2.Zero, 4f, SpriteEffects.None, Math.Max(0f, (float)(f.StandingPixel.Y + 4) / 10000f));
		}
	}
	...
	public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		else if (!Game1.eventUp || Location.IsFarm)
		{
			...
			if (!ColorSameIndexAsParentSheetIndex)
			{
				...
				spriteBatch.Draw(texture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32, y * 64 + 32)), itemData2.GetSourceRect(0, base.ParentSheetIndex), Color.White, 0f, new Vector2(8f, 8f), (scale.Y > 1f) ? getScale().Y : 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)bounds.Bottom / 10000f);
				spriteBatch.Draw(texture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), y * 64 + 32 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0))), coloredSourceRect2, color.Value, 0f, new Vector2(8f, 8f), (scale.Y > 1f) ? getScale().Y : 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)bounds.Bottom / 10000f);
			}
			else
			{
				spriteBatch.Draw(texture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + 32 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), y * 64 + 32 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0))), itemData2.GetSourceRect(0, base.ParentSheetIndex), color.Value, 0f, new Vector2(8f, 8f), (scale.Y > 1f) ? getScale().Y : 4f, flipped.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)bounds.Bottom / 10000f);
			}
		}
	}
