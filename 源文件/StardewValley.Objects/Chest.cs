...
public class Chest : Object
{
	...
	public bool TryMoveToSafePosition(int? preferDirection = null)
	{
		...
		bool TryMoveRecursively(Vector2 tile_position, int depth, Vector2? prioritize_direction)
		{
			...
			foreach (Vector2 offset in offsets)
			{
				...
				{
					if (!location.objects.ContainsKey(new_position) && location.objects.Remove(TileLocation))
					{
						kickStartTile.Value = TileLocation;
			...
			if (depth < 3)
			{
				foreach (Vector2 offset2 in offsets)
				{
					...
					if (location.isPointPassable(new Location((int)(new_position2.X + 0.5f) * 64, (int)(new_position2.Y + 0.5f) * 64), Game1.viewport) && TryMoveRecursively(new_position2, depth + 1, prioritize_direction))
					{
						return true;
					}
				}
	...
	public override bool performObjectDropInAction(Item dropInItem, bool probe, Farmer who, bool returnFalseIfItemConsumed = false)
	{
		if (dropInItem != null && dropInItem.QualifiedItemId != base.QualifiedItemId && dropInItem.HasContextTag("swappable_chest") && HasContextTag("swappable_chest") && Location != null)
		{
			if (!probe)
			{
				...
				Chest newChest = new Chest(playerChest: true, TileLocation, dropInItem.ItemId);
				int newCapacity = newChest.GetActualCapacity();
				if (newCapacity < GetActualCapacity() && newCapacity < Items.CountItemStacks())
				{
					return false;
				}
				...
				newChest.netItems.Value = netItems.Value;
				newChest.playerChoiceColor.Value = playerChoiceColor.Value;
				newChest.Tint = Tint;
				newChest.modData.CopyFrom(base.modData);
				GameLocation location = Location;
				location.Objects.Remove(TileLocation);
				location.Objects.Add(TileLocation, newChest);
	...
	public void dumpContents()
	{
		...
		if (giftbox.Value)
		{
			TemporaryAnimatedSprite sprite = new TemporaryAnimatedSprite("LooseSprites\\Giftbox", new Microsoft.Xna.Framework.Rectangle(0, giftboxIndex.Value * 32, 16, 32), 80f, 11, 1, tileLocation.Value * 64f - new Vector2(0f, 52f), flicker: false, flipped: false, tileLocation.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				destroyable = false,
				holdLastFrame = true
			};
			if (location.netObjects.TryGetValue(tileLocation.Value, out var tileObj) && tileObj == this)
			{
	...
	public override bool checkForAction(Farmer who, bool justCheckingForActivity = false)
	{
		...
		if (items.Count == 0 && (!playerChest.Value || giftbox.Value))
		{
	...
	public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		if (playerChest.Value && (base.QualifiedItemId == "(BC)130" || base.QualifiedItemId == "(BC)232" || base.QualifiedItemId.Equals("(BC)BigChest") || base.QualifiedItemId.Equals("(BC)BigStoneChest")))
		{
			...
			Microsoft.Xna.Framework.Rectangle drawRect = itemData2.GetSourceRect(0, spriteIndex);
			Microsoft.Xna.Framework.Rectangle lidRect = itemData2.GetSourceRect(0, lidIndex);
			Microsoft.Xna.Framework.Rectangle coloredLidRect = itemData2.GetSourceRect(0, coloredLidIndex);
			spriteBatch.Draw(texture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f, (draw_y - 1f) * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0))), drawRect, playerChoiceColor.Value * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order);
			...
			spriteBatch.Draw(texture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f, (draw_y - 1f) * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0))), lidRect, Color.White * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order + 2E-05f);
			spriteBatch.Draw(texture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f, (draw_y - 1f) * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0))), coloredLidRect, playerChoiceColor.Value * alpha * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order + 1E-05f);
			return;
		}
		if (playerChest.Value)
		{
			...
			spriteBatch.Draw(texture3, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (draw_y - 1f) * 64f)), itemData3.GetSourceRect(), tint.Value * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order);
			spriteBatch.Draw(texture3, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (draw_y - 1f) * 64f)), itemData3.GetSourceRect(0, currentLidFrame), tint.Value * alpha * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order + 1E-05f);
			return;
		}
		if (giftbox.Value)
		{
			...
			{
				int textureY = giftboxIndex.Value * 32;
	...
	public virtual void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f, bool local = false)
	{
		...
		if (itemData2 != null)
		{
			...
			Microsoft.Xna.Framework.Rectangle drawRect = itemData2.GetSourceRect(0, drawIndex);
			Microsoft.Xna.Framework.Rectangle lidRect = itemData2.GetSourceRect(0, overlayIndex);
			Microsoft.Xna.Framework.Rectangle coloredLidRect = itemData2.GetSourceRect(0, coloredLidIndex);
