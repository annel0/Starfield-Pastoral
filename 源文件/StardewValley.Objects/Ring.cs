...
public class Ring : Item
{
	...
	public override void onEquip(Farmer who)
	{
		...
		case "516":
			...
			location.sharedLights.AddLight(new LightSource(lightSourceId, 1, new Vector2(who.Position.X + 21f, who.Position.Y + 64f), 5f, new Color(0, 50, 170), LightSource.LightContext.None, who.UniqueMultiplayerID));
			break;
		case "517":
			lightSourceId = GenerateLightSourceId(who);
			location.sharedLights.AddLight(new LightSource(lightSourceId, 1, new Vector2(who.Position.X + 21f, who.Position.Y + 64f), 10f, new Color(0, 30, 150), LightSource.LightContext.None, who.UniqueMultiplayerID));
			break;
		case "888":
		case "527":
			lightSourceId = GenerateLightSourceId(who);
			location.sharedLights.AddLight(new LightSource(lightSourceId, 1, new Vector2(who.Position.X + 21f, who.Position.Y + 64f), 10f, new Color(0, 80, 0), LightSource.LightContext.None, who.UniqueMultiplayerID));
			break;
		}
	}
	...
	public virtual void update(GameTime time, GameLocation environment, Farmer who)
	{
		...
		environment.repositionLightSource(lightSourceId, new Vector2(who.Position.X + 21f, who.Position.Y) + offset);
		if (!environment.isOutdoors.Value && !(environment is MineShaft) && !(environment is VolcanoDungeon))
		{
			LightSource l = environment.getLightSource(lightSourceId);
			if (l != null)
			{
				l.radius.Value = 3f;
			}
