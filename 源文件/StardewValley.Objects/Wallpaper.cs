		...
		if (location is DecoratableLocation decoratableLocation)
		{
			...
			if (isFloor.Value)
			{
				...
				if (floor_id != null)
				{
					if (GetSetData() != null)
					{
						decoratableLocation.SetFloor(GetSetData().Id + ":" + parentSheetIndex.Value, floor_id);
					}
					else
					{
						decoratableLocation.SetFloor(parentSheetIndex.Value.ToString(), floor_id);
					}
			...
			else
			{
				...
				if (wall_id != null)
				{
					if (GetSetData() != null)
					{
						decoratableLocation.SetWallpaper(GetSetData().Id + ":" + parentSheetIndex.Value, wall_id);
					}
					else
					{
						decoratableLocation.SetWallpaper(parentSheetIndex.Value.ToString(), wall_id);
					}
					location.playSound("coin");
					return true;
				}
