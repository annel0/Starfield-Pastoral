...
public class IndoorPot : Object
{
	...
	public override bool checkForAction(Farmer who, bool justCheckingForActivity = false)
	{
		if (who != null)
		{
			...
			if (heldObject.Value != null)
			{
				Object obj = heldObject.Value;
				int originalQuality = obj.Quality;
				obj.Quality = Location.GetHarvestSpawnedObjectQuality(who, obj.isForage(), TileLocation);
				bool num = who.addItemToInventoryBool(obj);
				if (num)
				{
					heldObject.Value = null;
					readyForHarvest.Value = false;
					Game1.playSound("coin");
					Location.OnHarvestedForage(who, obj);
					return num;
				}
				heldObject.Value.Quality = originalQuality;
				return num;
			}
	...
	public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
	{
		...
		spriteBatch.Draw(itemData.GetTexture(), destination, itemData.GetSourceRect(showNextIndex.Value ? 1 : 0), Color.White * alpha, 0f, Vector2.Zero, SpriteEffects.None, Math.Max(0f, (float)((y + 1) * 64 - 24) / 10000f) + (float)x * 1E-05f);
