...
public class SpriteText
{
	...
	public static int getHeightOfString(string s, int widthConstraint = 999999)
	{
		...
		for (int j = 0; j < s.Length; j++)
		{
			...
			position.X += 8f * FontPixelZoom + (float)accumulatedHorizontalSpaceBetweenCharacters + (float)getWidthOffsetForChar(s[j]) * FontPixelZoom;
			if (j > 0)
			{
				position.X += (float)getWidthOffsetForChar(s[j - 1]) * FontPixelZoom;
			}
	...
	public static string getSubstringBeyondHeight(string s, int width, int height)
	{
		...
		for (int j = 0; j < s.Length; j++)
		{
			...
			if (j > 0)
			{
				position.X += 8f * FontPixelZoom + (float)accumulatedHorizontalSpaceBetweenCharacters + (float)(getWidthOffsetForChar(s[j]) + getWidthOffsetForChar(s[j - 1])) * FontPixelZoom;
			}
			accumulatedHorizontalSpaceBetweenCharacters = (int)(0f * FontPixelZoom);
	...
	public static int positionOfNextSpace(string s, int index, int currentXPosition, int accumulatedHorizontalSpaceBetweenCharacters)
	{
		...
		for (int j = index; j < s.Length; j++)
		{
			if (isUsingNonSpriteSheetFont())
			{
				...
				currentXPosition = ((!characterMap.TryGetValue(s[j], out var fc2)) ? (currentXPosition + (int)((float)FontFile.Common.LineHeight * FontPixelZoom)) : (currentXPosition + (int)((float)fc2.XAdvance * FontPixelZoom)));
				continue;
			}
			if (s[j] == ' ' || s[j] == '^')
			{
