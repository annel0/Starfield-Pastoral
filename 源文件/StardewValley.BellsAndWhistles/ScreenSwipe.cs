...
public class ScreenSwipe
{
	...
	private int ViewportWidth => Game1.uiViewport.Width;

	private int ViewportHeight => Game1.uiViewport.Height;
	...
	public ScreenSwipe(int which, float swipeVelocity = -1f, int durationAfterSwipe = -1, int w = -1, int h = -1)
	{
		...
		Vector2 screenCenter = new Vector2(ViewportWidth / 2, ViewportHeight / 2);
		if (which == 0)
		{
	...
	{
		if (durationAfterSwipe > 0 && bgDest.Width <= ViewportWidth)
		{
			bgDest.Width += (int)((double)swipeVelocity * time.ElapsedGameTime.TotalMilliseconds);
			if (bgDest.Width > ViewportWidth)
			{
		...
		if (bgDest.Width > ViewportWidth && durationAfterSwipe > 0)
		{
