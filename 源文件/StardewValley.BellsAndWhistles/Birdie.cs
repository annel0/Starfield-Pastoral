	...
	public override bool update(GameTime time, GameLocation environment)
	{
		...
		case 0:
			if (sprite.CurrentAnimation == null)
			{
				...
				{
					new FarmerSprite.AnimationFrame((short)(baseFrame + 2), 480),
					new FarmerSprite.AnimationFrame((short)(baseFrame + 3), 170, secondaryArm: false, flip),
					new FarmerSprite.AnimationFrame((short)(baseFrame + 4), 170, secondaryArm: false, flip)
				};
		...
		case 4:
			...
			{
				int delta = (flip ? 1 : (-1));
				if (!environment.isCollidingPosition(getBoundingBox(delta, 0), Game1.viewport, isFarmer: false, 0, glider: false, null, pathfinding: false, projectile: false, ignoreCharacterRequirement: true))
				{
					position.X += delta;
				}
			}
			...
			{
				float delta2 = (flip ? 0.5f : (-0.5f));
				if (Math.Abs(position.X + delta2 - startingPosition.X) < 8f)
				{
					position.X += delta2;
				}
				else
