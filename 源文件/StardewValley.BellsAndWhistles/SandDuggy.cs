...
public class SandDuggy : INetObject<NetFields>
{
	...
	public virtual int FindRandomFreePoint()
	{
		...
		{
			Point holeTile = holeLocations[i];
			if (!locationRef.Value.isObjectAtTile(holeTile.X, holeTile.Y) && !locationRef.Value.isTerrainFeatureAt(holeTile.X, holeTile.Y) && !locationRef.Value.terrainFeatures.ContainsKey(Utility.PointToVector2(holeTile)))
			{
		...
		validHoleLocations.RemoveAll(delegate(int index)
		{
			...
			foreach (Farmer current in locationRef.Value.farmers)
			{
				if (NearFarmer(location, current))
				{
					return true;
				}
			}
	...
	public virtual void Update(GameTime time)
	{
		...
		if (currentHoleIndex.Value >= 0)
		{
			...
			if (NearFarmer(synched_position, Game1.player) && FindRandomFreePoint() != currentHoleIndex.Value)
			{
