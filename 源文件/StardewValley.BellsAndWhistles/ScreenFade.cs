...
public class ScreenFade
{
	...
	public void UpdateFadeAlpha(GameTime time)
	{
		if (fadeIn)
		{
			...
			fadeToBlackAlpha = (Game1.IsDedicatedHost ? 1.2f : fadeToBlackAlpha);
		}
		else if (!Game1.messagePause && !Game1.dialogueUp)
		{
			fadeToBlackAlpha -= ((Game1.eventUp || Game1.farmEvent != null) ? 0.0008f : 0.0019f) * (float)time.ElapsedGameTime.Milliseconds;
			fadeToBlackAlpha = (Game1.IsDedicatedHost ? (-0.2f) : fadeToBlackAlpha);
		}
	}
	...
	public void UpdateGlobalFade()
	{
		if (fadeIn)
		{
			...
			fadeToBlackAlpha = (Game1.IsDedicatedHost ? 0f : Math.Max(0f, fadeToBlackAlpha - globalFadeSpeed));
			return;
		}
		...
		fadeToBlackAlpha = (Game1.IsDedicatedHost ? 1f : Math.Min(1f, fadeToBlackAlpha + globalFadeSpeed));
	}
}
