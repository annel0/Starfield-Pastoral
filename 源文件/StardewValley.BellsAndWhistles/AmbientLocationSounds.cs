...
public class AmbientLocationSounds
{
	...
	public static void update(GameTime time)
	{
		...
		if (volumeOverrideForLocChange >= 0f)
		{
			...
			{
				if (shortestDistanceForCue[j] <= (float)farthestSoundDistance * 1.5f)
				{
					float volume = Math.Min(volumeOverrideForLocChange, Math.Min(1f, 1f - shortestDistanceForCue[j] / ((float)farthestSoundDistance * 1.5f)));
