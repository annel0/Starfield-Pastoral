...
public class ParrotUpgradePerch : INetObject<NetFields>
{
		...
		else
		{
			...
			case "House":
			{
				for (int j = 0; j < 16; j++)
				{
					...
					TemporaryAnimatedSprite t2 = new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(49 + 16 * Game1.random.Next(3), 229, 16, 6), Utility.getRandomPositionInThisRectangle(upgradeRect.Value, Game1.random) * 64f, Game1.random.NextBool(), 0f, Color.White)
					{
						motion = new Vector2(Game1.random.Next(-2, 3), -16f),
						acceleration = new Vector2(0f, 0.5f),
						rotationChange = (float)Game1.random.Next(-4, 5) * 0.05f,
					...
					t2 = new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(49 + 16 * Game1.random.Next(3), 229, 16, 6), Utility.getRandomPositionInThisRectangle(upgradeRect.Value, Game1.random) * 64f, Game1.random.NextBool(), 0f, Color.White)
					{
						motion = new Vector2(Game1.random.Next(-2, 3), -16f),
						acceleration = new Vector2(0f, 0.5f),
						rotationChange = (float)Game1.random.Next(-4, 5) * 0.05f,
	...
	public static void ActivateGoldenParrot()
	{
		...
		foreach (GameLocation l in Game1.locations)
		{
			...
			if (!(l is IslandNorth island_north))
			{
				...
				{
					if (!(l is IslandEast east))
					{
						if (!(l is IslandHut hut))
						{
							if (!(l is IslandSouthEast se))
							{
								if (!(l is IslandShrine shrine))
								{
									if (!(l is IslandWest west))
									{
										if (l is IslandFarmCave fcave)
										{
	...
	public virtual void Update(GameTime time)
	{
		...
		if (IsAvailable(use_cached_value: true))
		{
			bool playerNearby = currentState.Value == UpgradeState.Idle && !upgradeMutex.IsLocked() && Math.Abs(Game1.player.TilePoint.X - tilePosition.X) <= 1 && Math.Abs(Game1.player.TilePoint.Y - tilePosition.Y) <= 1;
			if (playerNearby != isPlayerNearby)
			{
	...
	public virtual void Draw(SpriteBatch b)
	{
		if (IsAvailable(use_cached_value: true) && (parrotPresent || upgradeName.Value == "Hut"))
		{
			...
			b.Draw(texture, Game1.GlobalToLocal(Game1.viewport, (Utility.PointToVector2(tilePosition.Value) + new Vector2(0.5f, -1f)) * 64f) + shake, new Microsoft.Xna.Framework.Rectangle(frame * 24, (upgradeName.Value == "GoldenParrot") ? 96 : 0, 24, 24), Color.White, 0f, new Vector2(12f, 16f), 4f, SpriteEffects.None, (((float)tilePosition.Y + 1f) * 64f - 1f) / 10000f);
		}
	}
