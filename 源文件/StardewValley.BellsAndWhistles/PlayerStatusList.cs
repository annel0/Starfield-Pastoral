...
public class PlayerStatusList : INetObject<NetFields>
{
	...
	protected void _ResortList()
	{
		...
		{
			if (Game1.IsMasterGame && !_sortedFarmers.Contains(farmer2))
			{
				_statusList.Remove(farmer2.UniqueMultiplayerID);
			}
			if (!_statusList.ContainsKey(farmer2.UniqueMultiplayerID))
			{
				_sortedFarmers.Remove(farmer2);
			}
		}
		switch (sortMode)
		{
	...
	public void Draw(SpriteBatch b, Vector2 draw_position, float draw_scale = 4f, float draw_layer = 0.45f, HorizontalAlignment horizontal_origin = HorizontalAlignment.Left, VerticalAlignment vertical_origin = VerticalAlignment.Top)
	{
		...
		if (horizontal_origin == HorizontalAlignment.Right)
		{
			...
			else
			{
				...
				{
					if (!farmer.IsDedicatedPlayer && _formattedStatusList.TryGetValue(farmer.UniqueMultiplayerID, out var state))
					{
		...
		foreach (Farmer farmer2 in _sortedFarmers)
		{
			...
			if (_formattedStatusList.TryGetValue(farmer2.UniqueMultiplayerID, out var state2))
			{
				Vector2 draw_offset = Vector2.Zero;
				farmer2.FarmerRenderer.drawMiniPortrat(b, draw_position, draw_layer, draw_scale * 0.75f, 2, farmer2);
				if (displayMode == DisplayMode.Icons && _iconDefinitions.TryGetValue(state2, out var spriteDefinition))
				{
