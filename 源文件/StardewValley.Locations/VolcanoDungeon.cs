...
public class VolcanoDungeon : IslandLocation
{
	...
	public virtual void GenerateLevel(bool use_level_level_as_layout = false)
	{
		...
		ApplyToColor(new Color(0, 255, 0), delegate(int num, int num2)
		{
			if (!startPosition.HasValue)
			{
				startPosition = new Point(num, num2);
			}
			if (level.Value == 0)
			{
				warps.Add(new Warp(num, num2 + 2, "IslandNorth", 40, 24, flipFarmer: false));
			}
			else
			{
				warps.Add(new Warp(num, num2 + 2, GetLevelName(level.Value - 1), num - startPosition.Value.X, 0, flipFarmer: false));
			}
		});
		ApplyToColor(new Color(255, 0, 0), delegate(int num, int num2)
		{
			if (!endPosition.HasValue)
			{
				endPosition = new Point(num, num2);
			}
			if (level.Value == 9)
			{
				warps.Add(new Warp(num, num2 - 2, "Caldera", 21, 39, flipFarmer: false));
			}
			else
			{
				warps.Add(new Warp(num, num2 - 2, GetLevelName(level.Value + 1), num - endPosition.Value.X, 1, flipFarmer: false));
			}
		});
		setPieceAreas.Clear();
		Color set_piece_color = new Color(255, 255, 0);
		ApplyToColor(set_piece_color, delegate(int num, int num2)
		{
			int j;
			for (j = 0; j < 32 && !(GetPixel(num + j, num2, Color.Black) != set_piece_color) && !(GetPixel(num, num2 + j, Color.Black) != set_piece_color); j++)
			{
			}
			setPieceAreas.Add(new Microsoft.Xna.Framework.Rectangle(num, num2, j, j));
			for (int k = 0; k < j; k++)
			{
				for (int l = 0; l < j; l++)
				{
					SetPixelMap(num + k, num2 + l, Color.White);
				}
			}
		});
		possibleSwitchPositions = new Dictionary<int, List<Point>>();
		possibleGatePositions = new Dictionary<int, List<Point>>();
		ApplyToColor(new Color(128, 128, 128), delegate(int x2, int y2)
		{
			AddPossibleSwitchLocation(0, x2, y2);
		});
		ApplySetPieces();
		GenerateWalls(Color.Black, 0, 4, 4, 4, start_in_wall: true, delegate(int x2, int y2)
		{
			SetPixelMap(x2, y2, Color.Chartreuse);
		}, use_corner_hack: true);
		GenerateWalls(Color.Chartreuse, 0, 13, 1);
		ApplyToColor(Color.Blue, delegate(int num, int num2)
		{
			waterTiles[num, num2] = true;
			SetTile(backLayer, num, num2, 4);
			setTileProperty(num, num2, "Back", "Water", "T");
			if (generationRandom.NextDouble() < 0.1)
			{
				sharedLights.AddLight(new LightSource($"VolcanoDungeon_{level.Value}_Lava_{num}_{num2}", 4, new Vector2(num, num2) * 64f, 2f, new Color(0, 50, 50), LightSource.LightContext.None, 0L, base.NameOrUniqueName));
			}
		});
	...
	public virtual void GenerateEntities()
	{
		...
		ApplyToColor(new Color(0, 255, 255), delegate(int x2, int y2)
		{
			spawn_points.Add(new Point(x2, y2));
		});
		List<Point> spiker_spawn_points = new List<Point>();
		ApplyToColor(new Color(0, 128, 255), delegate(int x2, int y2)
		{
			...
			for (int x = 0; x < size; x++)
			{
				for (int y = 0; y <= size; y++)
				{
					...
					if (path_index >= GetTileIndex(10, 14) && path_index <= GetTileIndex(15, 14))
					{
						...
						double chance = 1.0;
						if (FrameworkExtensions.TryGetValue(tile.Properties, "Chance", out var property) && !double.TryParse(property, out chance))
						{
							chance = 1.0;
						}
						if (generationRandom.NextDouble() < chance)
