...
public class Summit : GameLocation
{
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		if (currentEvent == null && Game1.background != null && !Game1.background.cursed && temporarySprites.Count == 0 && Game1.random.NextDouble() < ((Game1.timeOfDay < 1800) ? 0.0006 : ((Game1.season == Season.Summer && Game1.dayOfMonth == 20) ? 1.0 : 0.001)))
		{
			...
			if (Game1.timeOfDay < 1800)
			{
				...
				{
					TemporaryAnimatedSprite bird2 = localSeason switch
					{
				...
				else if (Game1.random.NextDouble() < 0.15 && Game1.stats.Get("childrenTurnedToDoves") > 1)
				{
					for (int j = 0; j < Game1.stats.Get("childrenTurnedToDoves"); j++)
					{
						...
						TemporaryAnimatedSprite bird3 = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(388, 1894, 24, 21), Game1.random.Next(80, 121), 6, 200, startingPosition + new Vector2((j + 1) * (Game1.random.Next(25, 27) * 4), Game1.random.Next(-32, 33) * 4), flicker: false, flipped: false, 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f, local: true)
						{
							layerDepth = 0f
						};
				...
				{
					sourceRect = Rectangle.Empty;
					TemporaryAnimatedSprite bird5 = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(206, 1827, 15, 25), 30f, 4, 99900, startingPosition, flicker: false, flipped: false, 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f, local: true)
					{
			...
			{
				TemporaryAnimatedSprite t2 = new TemporaryAnimatedSprite("LooseSprites\\Cursors", sourceRect, animationSpeed, (localSeason == Season.Winter) ? 2 : 4, loops, startingPosition, flicker: false, flipped: false, 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f, local: true)
				{
					layerDepth = 0f
				};
	...
	public string getEndSlideshow()
	{
		...
		foreach (KeyValuePair<string, CharacterData> entry2 in Game1.characterData)
		{
			...
			if (data2.EndSlideShow == EndSlideShowBehavior.TrailingGroup && TryDrawNpc(name2, data2, 120, i))
			{
				i += 500;
			}
		}
		...
		foreach (KeyValuePair<string, string> v in dictionary)
		{
			...
			{
				Game1.log.Error("Couldn't add fish '" + v.Key + "' to summit event credits.", exception);
			}
		}
		...
		foreach (KeyValuePair<string, string> v2 in DataLoader.Monsters(Game1.content))
		{
			...
			case "Shadow Guy":
			{
				spriteHeight = 32;
				spriteStartIndex = 4;
				Texture2D t3 = Game1.content.Load<Texture2D>("Characters\\Monsters\\Shadow Brute");
				base.TemporarySprites.Add(new TemporaryAnimatedSprite(null, new Rectangle(spriteWidth * spriteStartIndex % t3.Width, spriteWidth * spriteStartIndex / t3.Width * spriteHeight, spriteWidth, spriteHeight), 100f, animationLength, 999999, new Vector2(Game1.viewport.Width + 192, (float)Game1.viewport.Height * 0.5f - (float)(spriteHeight * 4) - 16f), flicker: false, flipped: true, 0.9f, 0f, Color.White, 4f, 0f, 0f, 0f, local: true)
				{
					motion = new Vector2(-3f, 0f),
					delayBeforeAnimationStart = i,
				...
				t3 = Game1.content.Load<Texture2D>("Characters\\Monsters\\Shadow Shaman");
				spriteHeight = 24;
				...
				t3 = Game1.content.Load<Texture2D>("Characters\\Monsters\\Shadow Sniper");
				spriteHeight = 32;
				spriteWidth = 32;
			...
			case "Bat":
			{
				Texture2D t2 = Game1.content.Load<Texture2D>("Characters\\Monsters\\Frost Bat");
				base.TemporarySprites.Add(new TemporaryAnimatedSprite(null, new Rectangle(spriteWidth * spriteStartIndex % t2.Width, spriteWidth * spriteStartIndex / t2.Width * spriteHeight, spriteWidth, spriteHeight), 100f, animationLength, 999999, new Vector2(Game1.viewport.Width + 192, (float)Game1.viewport.Height * 0.5f - (float)(spriteHeight * 4) - 16f), flicker: false, flipped: true, 0.9f, 0f, Color.White, 4f, 0f, 0f, 0f, local: true)
				{
					motion = new Vector2(-3f, 0f),
					delayBeforeAnimationStart = i,
					yPeriodic = (animationLength == 1),
				...
				t2 = Game1.content.Load<Texture2D>("Characters\\Monsters\\Lava Bat");
				spriteHeight = 24;
				...
				t2 = Game1.content.Load<Texture2D>("Characters\\Monsters\\Iridium Bat");
			...
			try
			{
				...
				base.TemporarySprites.Add(new TemporaryAnimatedSprite(null, new Rectangle(spriteWidth * spriteStartIndex % t4.Width, spriteWidth * spriteStartIndex / t4.Width * spriteHeight + 1, spriteWidth, spriteHeight - 1), 100f, animationLength, 999999, new Vector2(Game1.viewport.Width + 192, (float)Game1.viewport.Height * 0.5f - (float)(spriteHeight * 4) - 16f + (float)yOffset), flicker: false, flipped: true, 0.9f, 0f, Color.White, 4f, 0f, 0f, 0f, local: true)
				{
					motion = new Vector2(-3f, 0f),
					delayBeforeAnimationStart = i,
					yPeriodic = (animationLength == 1),
