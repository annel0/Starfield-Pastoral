		...
		if (doesTileHaveProperty(tileLocation.X, tileLocation.Y, "Action", "Buildings") == "BoatTicket")
		{
			...
			else if (Game1.MasterPlayer.hasOrWillReceiveMail("willyBoatFixed"))
			{
				...
				else
				{
					...
					{
						createQuestionDialogueWithCustomWidth(Game1.content.LoadString("Strings\\Locations:BuyTicket", displayPrice), createYesNoResponses(), "Boat");
					}
					else
					{
						createQuestionDialogue(Game1.content.LoadString("Strings\\Locations:BuyTicket", displayPrice), createYesNoResponses(), "Boat");
					}
				}
			}
			return true;
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		if ((float)_boatDirection != 0f)
		{
			...
			else
			{
				...
				sprite.acceleration = new Vector2(-0.25f * (float)Math.Sign(_boatDirection), 0f);
		...
		if (_boatAnimating)
		{
			...
			Vector2 position2 = new Vector2(80f, -32f) * 4f + GetBoatPosition();
