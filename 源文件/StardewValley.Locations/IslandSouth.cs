...
public class IslandSouth : IslandLocation
{
	public class IslandActivityAssigments
	{
		...
		public virtual void FindActivityForCharacters(Dictionary<Character, string> last_activity_assignments)
		{
			...
			{
				if (!currentAssignments.ContainsKey(character2) && !TryAssignment(character2, towelLoungePoints, "Resort_Towel", character2.name.Value.ToLower() + "_beach_towel", animation_required: true, 0.5, last_activity_assignments) && !TryAssignment(character2, wanderPoints, "Resort_Wander", "square_3_3", animation_required: false, 0.4, last_activity_assignments) && !TryAssignment(character2, umbrellaPoints, "Resort_Umbrella", character2.name.Value.ToLower() + "_beach_umbrella", animation_required: true, (character2.Name == "Abigail") ? 0.5 : 0.1) && (character2.Age != 0 || !TryAssignment(character2, chairPoints, "Resort_Chair", "_beach_chair", animation_required: false, 0.4, last_activity_assignments)))
				{
					TryAssignment(character2, shoreLoungePoints, "Resort_Shore", null, animation_required: false, 1.0, last_activity_assignments);
				}
			}
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		if ((float)_boatDirection != 0f)
		{
			...
			else
			{
				...
				TemporaryAnimatedSprite sprite = new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(0, 0, 64, 64), 50f, 9, 1, position, flicker: false, flipped: false, 0f, 0.025f, Color.White, 1f, 0f, 0f, 0f);
		...
		Vector2 position2 = new Vector2(2f, 2.5f) * 64f + GetBoatPosition();
		TemporaryAnimatedSprite sprite2 = new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(0, 1600, 64, 128), 200f, 9, 1, position2, flicker: false, flipped: false, 1f, 0.025f, Color.White, 1f, 0.025f, 0f, 0f);
	...
	public override bool isTilePlaceable(Vector2 tileLocation, bool itemIsPassable = false)
	{
		...
		if (!westernTurtleMoved.Value && turtle2Spot.Contains(non_tile_position))
		{
			return false;
		}
	...
	public static void SetupIslandSchedules()
	{
		...
		foreach (NPC visitor4 in visitors)
		{
			...
			bool should_dress = HasIslandAttire(visitor4);
			bool had_first_activity = false;
			if (should_dress)
			{
				Point dressing_room = GetDressingRoomPoint(visitor4);
			...
			if (should_dress)
			{
				Point dressing_room2 = GetDressingRoomPoint(visitor4);
