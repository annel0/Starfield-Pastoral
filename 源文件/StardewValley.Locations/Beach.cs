...
public class Beach : GameLocation
{
	...
	{
		base.initNetFields();
		base.NetFields.AddField(bridgeFixed, "bridgeFixed").AddField(derbyMutex.NetFields, "derbyMutex.NetFields");
	...
	public override void DayUpdate(int dayOfMonth)
	{
		...
		while (Game1.random.NextDouble() < (double)chance)
		{
			if (Game1.random.NextDouble() < 0.1)
			{
				...
				{
					dropObject(ItemRegistry.Create<Object>("(O)152"), position2 * 64f, Game1.viewport, initialPlacement: true);
				}
			}
			chance /= 2f;
		}
		if (IsSummerHere() && Game1.dayOfMonth >= 12 && Game1.dayOfMonth <= 14)
		{
			...
			while (Game1.random.NextDouble() < (double)chance)
			{
				...
				{
					dropObject(ItemRegistry.Create<Object>(id2), position3 * 64f, Game1.viewport, initialPlacement: true);
				}
				chance /= 1.1f;
			}
		}
		if (Game1.IsWinter)
		{
			characters.RemoveWhere((NPC npc) => npc.Name.Contains("derby_contestent"));
		}
	}
	...
	{
		switch (getTileIndexAt(tileLocation, "Buildings", "untitled tile sheet"))
		{
		case 284:
	...
	public override void MakeMapModifications(bool force = false)
	{
		...
		if (Game1.IsWinter && Game1.dayOfMonth >= 12 && Game1.dayOfMonth <= 13)
		{
			if (getCharacterFromName("winter_derby_contestent0") == null && (Game1.IsMasterGame || !Game1.player.sleptInTemporaryBed.Value))
			{
			...
			if (Game1.dayOfMonth == 13)
			{
				...
				setMapTile(13, 6, 51, "Front", tileSheetId);
				setMapTile(13, 5, 43, "AlwaysFront", tileSheetId);
			}
		...
		else if (_appliedMapOverrides.Contains("Beach_SquidFest") || getTileIndexAt(11, 7, "Buildings", "16") == 45)
		{
