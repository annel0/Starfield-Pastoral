...
public class IslandNorth : IslandLocation
{
	...
	public override void TransferDataFromSavedLocation(GameLocation l)
	{
		...
		{
			bridgeFixed.Value = location.bridgeFixed.Value;
	...
	public override bool isTilePlaceable(Vector2 tile_location, bool itemIsPassable = false)
	{
		...
		if (!caveOpened.Value && boulderPosition.Contains(non_tile_position))
		{
			return false;
		}
	...
	public override void DayUpdate(int dayOfMonth)
	{
		...
		digSiteUpdate();
		terrainFeatures.RemoveWhere((KeyValuePair<Vector2, TerrainFeature> pair) => pair.Value is HoeDirt { crop: not null } hoeDirt && hoeDirt.crop.forageCrop.Value);
	...
	public void digSiteUpdate()
	{
		...
		if (Utility.getNumObjectsOfIndexWithinRectangle(digSiteBounds, new string[9] { "(O)816", "(O)817", "(O)818", "(O)819", "(O)32", "(O)38", "(O)40", "(O)42", "(O)590" }, this) < 60)
		{
			for (int i = 0; i < numberOfAdditionsToTry; i++)
			{
				...
				else if (Game1.random.NextDouble() < 0.1)
				{
					...
					if (CanItemBePlacedHere(position, itemIsPassable: false, CollisionMask.All, CollisionMask.None) && !IsTileOccupiedBy(position) && !hasTileAt(xCoord, yCoord, "AlwaysFront") && !hasTileAt(xCoord, yCoord, "Front") && !isBehindBush(position) && doesTileHaveProperty(xCoord, yCoord, "Diggable", "Back") != null && doesTileHaveProperty(xCoord, yCoord, "Diggable", "Back") == "T")
					{
		...
		else
		{
			...
			for (int i2 = 0; i2 < times; i2++)
			{
				...
				{
					objects.Add(position2, ItemRegistry.Create<Object>(r.Choose("(O)785", "(O)676", "(O)677")));
				}
			}
		}
	}
	...
	public override bool performOrePanTenMinuteUpdate(Random r)
	{
		if (Game1.MasterPlayer.mailReceived.Contains("ccFishTank") && orePanPoint.Value.Equals(Point.Zero) && r.NextBool())
		{
			for (int tries = 0; tries < 3; tries++)
			{
				...
				if (isOpenWater(p.X, p.Y) && FishingRod.distanceToLand(p.X, p.Y, this) <= 1 && !hasTileAt(p, "Buildings"))
				{
