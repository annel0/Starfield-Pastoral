...
public class DesertFestival : Desert
{
	...
	public virtual void CollectRacePrizes()
	{
		...
		if (specialRewardsCollected.TryGetValue(Game1.player.UniqueMultiplayerID, out var collectedSpecialReward) && !collectedSpecialReward)
		{
	...
	public override bool performAction(string action, Farmer who, Location tile_location)
	{
		...
		case "DesertRacerMan":
		{
			...
			else if (rewardsToCollect.TryGetValue(Game1.player.UniqueMultiplayerID, out toCollect) && toCollect > 0)
			{
			...
			{
				if (raceGuesses.TryGetValue(Game1.player.UniqueMultiplayerID, out var guessedRacer) && currentRaceState.Value == RaceState.Go)
				{
			...
			else if (nextRaceGuesses.TryGetValue(Game1.player.UniqueMultiplayerID, out guessedRacer2))
			{
	...
	{
		if (sabotages.TryGetValue(Game1.player.UniqueMultiplayerID, out var sabotagedRacer))
		{
			if (_localSabotageText == -1)
			{
				_localSabotageText = Game1.random.Next(1, 4);
			}
			Game1.drawObjectDialogue(Game1.content.LoadString("Strings\\1_6_Strings:Shady_Guy_Selected_" + _localSabotageText, Game1.content.LoadString("Strings\\1_6_Strings:Racer_" + sabotagedRacer)));
		}
	}
	...
	public virtual void AnnounceRace(string text)
	{
		...
		{
			_raceTextTimer = 4f;
			if (raceGuesses.TryGetValue(Game1.player.UniqueMultiplayerID, out var guessedRacer))
			{
				if (lastRaceWinner.Value == guessedRacer)
				{
	...
	public static void SetupFestivalDay()
	{
		...
		if (characters.Count > 0)
		{
			...
			SetupMerchantSchedule(character2, 0);
		}
		if (characters.Count > 0)
		{
