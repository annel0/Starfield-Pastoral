...
public class FarmHouse : DecoratableLocation
{
	...
	private void AddStarterGiftBox(Farm farm)
	{
		...
		{
			if (!ArgUtility.TryGet(fields, i, out var giftId, out var error, allowBlank: false, "string giftId") || !ArgUtility.TryGetOptionalInt(fields, i + 1, out var count, out error, 0, "int count"))
			{
		...
		if (!farm.TryGetMapPropertyAs("FarmHouseStarterSeedsPosition", out Vector2 tile, false))
		{
	...
	private void AddStarterFurniture(Farm farm)
	{
		...
		if (fields.Any())
		{
			...
			{
				if (!ArgUtility.TryGetInt(fields, i, out var index, out var error, "int index") || !ArgUtility.TryGetVector2(fields, i + 1, out var tile, out error, integerOnly: false, "Vector2 tile") || !ArgUtility.TryGetInt(fields, i + 3, out var rotations, out error, "int rotations"))
				{
	...
	{
		string id = GetStarterFlooring(farm);
		if (id != null)
		{
			SetFloor(id, null);
		}
	...
	{
		string id = GetStarterWallpaper(farm);
		if (id != null)
		{
	...
	{
		if (TryGetMapPropertyAs("KitchenStandingLocation", out Point position, false))
		{
			return position;
		}
		switch (upgradeLevel)
	...
	{
		if (TryGetMapPropertyAs("EntryLocation", out Point position, false))
		{
			return position;
		}
		switch (upgradeLevel)
	...
	public Point getRandomOpenPointInHouse(Random r, int buffer = 0, int tries = 30)
	{
		for (int numTries = 0; numTries < tries; numTries++)
		{
			...
			foreach (Point point2 in rect.GetPoints())
			{
				int x = point2.X;
				int y = point2.Y;
				obstacleFound = !hasTileAt(x, y, "Back") || !CanItemBePlacedHere(new Vector2(x, y)) || isTileOnWall(x, y);
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		foreach (Vector2 offset in adjacentTilesOffsets)
		{
			...
			if (isCharacterAtTile(v) is Monster monster)
			{
				...
				if (monster.Health <= 0)
				{
					debris.Add(new Debris(monster.Sprite.textureName.Value, Game1.random.Next(6, 16), Utility.PointToVector2(centerPixel)));
	...
	protected override void resetSharedState()
	{
		...
		for (int i = characters.Count - 1; i >= 0; i--)
		{
			if (characters[i] is Pet { TilePoint: var tile } pet)
			{
				...
				if (!isTileOnMap(tile.X, tile.Y) || hasTileAt(bounds.Left / 64, tile.Y, "Buildings") || hasTileAt(bounds.Right / 64, tile.Y, "Buildings"))
				{
					pet.WarpToPetBowl();
					break;
				}
		...
		for (int i2 = characters.Count - 1; i2 >= 0; i2--)
		{
			for (int j = i2 - 1; j >= 0; j--)
			{
				if (i2 < characters.Count && j < characters.Count && (characters[j].Equals(characters[i2]) || (characters[j].Name.Equals(characters[i2].Name) && characters[j].IsVillager && characters[i2].IsVillager)) && j != i2)
				{
					characters.RemoveAt(j);
				}
			}
			for (int j2 = farm.characters.Count - 1; j2 >= 0; j2--)
			{
				if (i2 < characters.Count && j2 < characters.Count && farm.characters[j2].Equals(characters[i2]))
				{
					farm.characters.RemoveAt(j2);
				}
			}
		}
	}
	...
	public virtual void setMapForUpgradeLevel(int level)
	{
		...
		bool need_bed_upgrade = previousUpgradeLevel == 0 && upgradeLevel >= 0;
		if (previousUpgradeLevel >= 0)
		{
			...
			if (upgradeLevel != 3 || need_bed_upgrade)
			{
				for (int i = 0; i < map.Layers[0].LayerWidth; i++)
				{
					...
					{
						if (doesTileHaveProperty(i, j, "DefaultBedPosition", "Back") == null)
						{
							continue;
						}
						...
						if (previousUpgradeLevel != 1 || bed_furniture == null || (bed_furniture.tileLocation.X == 39f && bed_furniture.tileLocation.Y == 22f))
						{
							...
							if (previousUpgradeLevel == 0 && bed_furniture != null)
							{
								...
								furniture.Add(new BedFurniture(bedId2, new Vector2(i, j)));
							}
							else if (bed_furniture != null)
							{
								...
								furniture.Add(new BedFurniture(bed_furniture.ItemId, new Vector2(i, j)));
							}
						}
						break;
					}
	...
	{
		if (TryGetMapPropertyAs("SpouseRoomPosition", out Point position, false))
		{
			return position;
		}
		if (upgradeLevel != 1)
	...
	public virtual void loadSpouseRoom()
	{
		...
		for (int i = 0; i < areaToRefurbish.Width; i++)
		{
			...
			{
				int tileIndex = refurbishedBuildingsLayer.GetTileIndexAt(fromOrigin.X + i, fromOrigin.Y + y);
				if (tileIndex != -1)
				{
					adjustMapLightPropertiesForLamp(tileIndex, areaToRefurbish.X + i, areaToRefurbish.Y + y, "Buildings");
				}
				if (y < areaToRefurbish.Height - 1)
				{
					tileIndex = refurbishedFrontLayer.GetTileIndexAt(fromOrigin.X + i, fromOrigin.Y + y);
					if (tileIndex != -1)
					{
						adjustMapLightPropertiesForLamp(tileIndex, areaToRefurbish.X + i, areaToRefurbish.Y + y, "Front");
					}
				}
			}
		}
