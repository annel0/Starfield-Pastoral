...
public class Racer : INetObject<NetFields>
{
		...
		if (Game1.IsMasterGame)
		{
				...
				if (location.finishedRacers.Count <= 0)
				{
					if (burstDuration > 0f)
					{
						...
						if (burstDuration <= 0f)
						{
							...
							if (progress > last_place && Game1.random.NextDouble() < (double)Math.Min(0.05f + (float)sabotages.Value * 0.2f, 0.5f))
							{
								tripping.Value = true;
			...
			while (frame_travel > 0f)
			{
				...
				if (location.currentRaceState.Value == DesertFestival.RaceState.Go && currentTrackIndex >= 0)
				{
					...
					{
						track_position2 = location.GetTrackPosition(currentTrackIndex, horizontalPosition);
						segmentStart = segmentEnd;
	...
	{
		float sort_y = (position.Y + (float)racerIndex.Value * 0.1f) / 10000f;
		...
		source_rect.Y = racerIndex.Value * 16;
