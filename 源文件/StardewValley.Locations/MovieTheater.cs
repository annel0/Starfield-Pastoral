...
public class MovieTheater : GameLocation
{
	...
	protected void _InitializeMap()
	{
		...
		if (paths_layer != null)
		{
			for (int x = 0; x < paths_layer.LayerWidth; x++)
			{
				...
				{
					if (paths_layer.Tiles[x, y] != null && paths_layer.GetTileIndexAt(x, y) == 7 && FrameworkExtensions.TryGetValue(paths_layer.Tiles[x, y].Properties, "group", out var property) && int.TryParse(property, out var hangout_group))
					{
	...
	public void OnRequestStartMovieEvent(long uid)
	{
		...
		if (CurrentState == 0)
		{
			...
			{
				foreach (Character character2 in playerGroup2)
				{
	...
	public void OnMovieViewerLockEvent(MovieViewerLockEvent e)
	{
		...
		{
			Farmer farmer = Game1.GetPlayer(uid, onlyOnline: true);
			if (farmer != null)
			{
	...
	private void addSpecificRandomNPC(int whichRandomNPC)
	{
		...
		{
		case 0:
			setMapTile(2, 9, 215, "Buildings", "movieTheater_tileSheet", "MessageSpeech MovieTheater_CraneMan" + r.Choose("2", ""));
			setMapTile(2, 8, 199, "Front", "movieTheater_tileSheet");
			break;
		case 1:
			setMapTile(19, 7, 216, "Buildings", "movieTheater_tileSheet", "MessageSpeech MovieTheater_Welwick" + r.Choose("2", ""));
			setMapTile(19, 6, 200, "Front", "movieTheater_tileSheet");
			break;
		case 2:
			setAnimatedMapTile(21, 7, new int[4] { 217, 217, 217, 218 }, 700L, "Buildings", "movieTheater_tileSheet", "MessageSpeech MovieTheater_ShortsMan" + r.Choose("2", ""));
			setAnimatedMapTile(21, 6, new int[4] { 201, 201, 201, 202 }, 700L, "Front", "movieTheater_tileSheet");
			break;
		case 3:
			setMapTile(5, 9, 219, "Buildings", "movieTheater_tileSheet", "MessageSpeech MovieTheater_Mother" + r.Choose("2", ""));
			setMapTile(6, 9, 220, "Buildings", "movieTheater_tileSheet", "MessageSpeech MovieTheater_Child" + r.Choose("2", ""));
		...
		case 4:
			...
			break;
		case 5:
			setAnimatedMapTile(10, 7, new int[4] { 251, 251, 251, 252 }, 900L, "Buildings", "movieTheater_tileSheet", "MessageSpeech MovieTheater_Lupini" + r.Choose("2", ""));
			setAnimatedMapTile(10, 6, new int[4] { 235, 235, 235, 236 }, 900L, "Front", "movieTheater_tileSheet");
			break;
		case 6:
			setAnimatedMapTile(5, 7, new int[4] { 249, 249, 249, 250 }, 600L, "Buildings", "movieTheater_tileSheet", "MessageSpeech MovieTheater_ConcessionMan" + r.Choose("2", ""));
		...
		case 7:
			...
			break;
		case 8:
			setMapTile(3, 8, 247, "Buildings", "movieTheater_tileSheet", "MessageSpeech MovieTheater_RedCapGuy" + r.Choose("2", ""));
			setMapTile(3, 7, 231, "Front", "movieTheater_tileSheet");
			break;
		case 9:
			setMapTile(2, 11, 253, "Buildings", "movieTheater_tileSheet", "MessageSpeech MovieTheater_Governor" + r.Choose("2", ""));
			setMapTile(2, 10, 237, "Front", "movieTheater_tileSheet");
			break;
		case 10:
			setMapTile(9, 7, 221, "Buildings", "movieTheater_tileSheet", "NPCSpeechMessageNoRadius Gunther MovieTheater_Gunther" + r.Choose("2", ""));
			setMapTile(9, 6, 205, "Front", "movieTheater_tileSheet");
			break;
		case 11:
			setMapTile(19, 10, 208, "Buildings", "movieTheater_tileSheet", "NPCSpeechMessageNoRadius Marlon MovieTheater_Marlon" + r.Choose("2", ""));
			setMapTile(19, 9, 192, "Front", "movieTheater_tileSheet");
			break;
		case 12:
			setMapTile(12, 4, 209, "Buildings", "movieTheater_tileSheet", "MessageSpeech MovieTheater_Marcello" + r.Choose("2", ""));
			setMapTile(12, 3, 193, "Front", "movieTheater_tileSheet");
			break;
		case 13:
			setMapTile(17, 12, 241, "Buildings", "movieTheater_tileSheet", "NPCSpeechMessageNoRadius Morris MovieTheater_Morris" + r.Choose("2", ""));
	...
	public static List<MovieData> GetMoviesForSeason(WorldDate date)
	{
		...
		if (movies.Count == 0)
		{
			...
			{
				if (MovieSeasonMatches(movie2, date.Season))
				{
					movies.Add(movie2);
				}
			}
		...
		foreach (Farmer farmer in farmers)
		{
			for (int j = 0; j < Game1.player.team.movieInvitations.Count; j++)
			{
				...
				if (invite.farmer == farmer && GetFirstInvitedPlayer(invite.invitedNPC) == farmer)
				{
					...
					NPC character = AddMoviePatronNPC(invite.invitedNPC.name.Value, 14, 15, 0);
					_playerInvitedPatrons.Add(character.name.Value, 1);
					_availableHangoutPoints[group].Remove(point);
					int direction = 2;
					IPropertyCollection tileProperties = map.GetLayer("Paths").Tiles[point.X, point.Y].Properties;
					if (tileProperties != null && FrameworkExtensions.TryGetValue(tileProperties, "direction", out var rawDirection))
					{
	...
	protected void _PopulateNPCOnlyGroups(List<List<Character>> player_groups, List<List<Character>> groups)
	{
		...
		{
			foreach (Character character2 in group)
			{
		...
		for (int j = 0; j < group_count; j++)
		{
			...
			foreach (string character3 in array)
			{
				bool found_friendship = false;
				foreach (Farmer allFarmer in Game1.getAllFarmers())
				{
					if (allFarmer.friendshipData.ContainsKey(character3))
					{
				...
				if (GetResponseForMovie(Game1.getCharacterFromName(character3)) == "dislike" || GetResponseForMovie(Game1.getCharacterFromName(character3)) == "reject")
				{
					valid = false;
					break;
				}
			...
			if (valid)
			{
				...
				array = characters;
				foreach (string character4 in array)
				{
					NPC patron = AddMoviePatronNPC(character4, 1000, 1000, 2);
					new_group.Add(patron);
					used_characters.Add(character4);
					_characterGroupLookup[character4] = characters.Length > 1;
				}
	...
	public void RequestEndMovie(long uid)
	{
		...
		if (CurrentState == 1)
		{
			...
			{
				for (int j = 0; j < _viewingGroups[group].Count; j++)
				{
					if (!(_viewingGroups[group][j] is NPC))
					{
						continue;
					}
					NPC patron_character = GetMoviePatron(_viewingGroups[group][j].Name);
	...
	public void PathCharacterToLocation(NPC character, Point point, int direction)
	{
		if (character.currentLocation == this)
		{
			...
			controller.pathToEndPoint = PathFindController.findPathForNPCSchedules(character.TilePoint, point, this, 30000, character);
	...
	public bool OnPurchaseConcession(ISalable salable, Farmer who, int countTaken, ItemStockInformation stock)
	{
		foreach (MovieInvitation invitation in who.team.movieInvitations)
		{
			if (invitation.farmer == who && GetFirstInvitedPlayer(invitation.invitedNPC) == Game1.player && _spawnedMoviePatrons.ContainsKey(invitation.invitedNPC.Name))
			{
				...
				_purchasedConcessions[invitation.invitedNPC.Name] = concession.Id;
	...
	public override void performTouchAction(string[] action, Vector2 playerStandingPosition)
	{
		...
		{
			if (!ArgUtility.TryGetPoint(action, 1, out var exitTile, out var error, "Point exitTile"))
			{
	...
	public static List<MovieConcession> GetConcessionsForGuest(string npc_name)
	{
		...
		for (int num = concessions.Count; num < min_concessions; num++)
		{
			int i2 = 0;
