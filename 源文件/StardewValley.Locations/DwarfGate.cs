	...
	public virtual void ApplyTiles()
	{
		...
		foreach (Point point in localSwitches.Keys)
		{
			...
			if (localSwitches[point])
			{
				...
				locationRef.Value.setMapTile(point.X, point.Y, VolcanoDungeon.GetTileIndex(1, 31), "Back", "dungeon").Properties.Remove("TouchAction");
			}
			else
			{
				locationRef.Value.setMapTile(point.X, point.Y, VolcanoDungeon.GetTileIndex(0, 31), "Back", "dungeon").Properties["TouchAction"] = "DwarfSwitch";
			}
		}
		switch (total_switches)
		{
		case 1:
			locationRef.Value.setMapTile(tilePosition.X - 1, tilePosition.Y, VolcanoDungeon.GetTileIndex(10 + local_pressed_switches, 23), "Buildings", "dungeon");
			break;
		case 2:
			locationRef.Value.setMapTile(tilePosition.X - 1, tilePosition.Y, VolcanoDungeon.GetTileIndex(12 + local_pressed_switches, 23), "Buildings", "dungeon");
			break;
		case 3:
			locationRef.Value.setMapTile(tilePosition.X - 1, tilePosition.Y, VolcanoDungeon.GetTileIndex(10 + local_pressed_switches, 22), "Buildings", "dungeon");
			break;
		}
