...
public class MineShaft : GameLocation
{
	...
	public static void yearUpdate()
	{
		...
		if (permanentMineChanges.TryGetValue(5, out var change))
		{
			change.platformContainersLeft = 6;
		}
		if (permanentMineChanges.TryGetValue(45, out change))
		{
			change.platformContainersLeft = 6;
		}
		if (permanentMineChanges.TryGetValue(85, out change))
		{
			change.platformContainersLeft = 6;
		}
	}
		...
		for (int y = 0; y < buildingsLayer.LayerHeight; y++)
		{
			for (int x = 0; x < buildingsLayer.LayerWidth; x++)
			{
				...
				if (tileIndex != -1)
				{
					...
					case 115:
					{
						string lightSourceId = $"Mines_{mineLevel}_{x}_{y}";
						tileBeneathLadder = new Vector2(x, y + 1);
						sharedLights.AddLight(new LightSource(lightSourceId + "_1", 4, new Vector2(x, y - 2) * 64f + new Vector2(32f, 0f), 0.25f, new Color(0, 20, 50), LightSource.LightContext.None, 0L, base.NameOrUniqueName));
						sharedLights.AddLight(new LightSource(lightSourceId + "_2", 4, new Vector2(x, y - 1) * 64f + new Vector2(32f, 0f), 0.5f, new Color(0, 20, 50), LightSource.LightContext.None, 0L, base.NameOrUniqueName));
						sharedLights.AddLight(new LightSource(lightSourceId + "_3", 4, new Vector2(x, y) * 64f + new Vector2(32f, 0f), 0.75f, new Color(0, 20, 50), LightSource.LightContext.None, 0L, base.NameOrUniqueName));
						sharedLights.AddLight(new LightSource(lightSourceId + "_4", 4, new Vector2(x, y + 1) * 64f + new Vector2(32f, 0f), 1f, new Color(0, 20, 50), LightSource.LightContext.None, 0L, base.NameOrUniqueName));
						found++;
						break;
					}
					case 112:
					...
					if (!lighting.Equals(Color.White))
					{
						...
						case 97:
						case 113:
							sharedLights.AddLight(new LightSource($"Mines_{mineLevel}_{x}_{y}_5", 4, new Vector2(x, y) * 64f, 2.5f, new Color(0, 50, 100), LightSource.LightContext.None, 0L, base.NameOrUniqueName));
							switch (tileIndex)
							{
							case 66:
				...
				{
					sharedLights.AddLight(new LightSource($"Mines_{mineLevel}_{x}_{y}_Lava", 4, new Vector2(x, y) * 64f, 2f, new Color(0, 220, 220), LightSource.LightContext.None, 0L, base.NameOrUniqueName));
				}
			}
		}
	...
	private void setMonsterTextureToDangerousVersion(Monster monster)
	{
		...
		{
			Game1.log.Error($"Failed loading '{newAssetName}' texture for dangerous {monster.Name}.", exception);
		}
	}
		...
		if (mineLevel > 1 && (mineLevel % 5 != 0 || mineLevel >= 121) && (mineRandom.NextBool() || isDinoArea))
		{
			...
			for (int i = 0; i < numBarrels; i++)
			{
				...
				while (isTileOnMap(p.X, p.Y))
				{
					...
					{
						if (Utility.GetDayOfPassiveFestival("DesertFestival") > 0 && getMineArea() == 121 && !hasAddedDesertFestivalStatue && hasTileAt((int)objectPos.X, (int)objectPos.Y - 1, "Buildings"))
						{
		...
		if (mineLevel % 10 != 0 || (getMineArea() == 121 && !isForcedChestLevel(mineLevel) && !netIsTreasureRoom.Value))
		{
			...
			for (int j = 0; j < backLayer2.LayerWidth; j++)
			{
				for (int k = 0; k < backLayer2.LayerHeight; k++)
				{
					...
					if (isTileClearForMineObjects(j, k))
					{
						if (mineRandom.NextDouble() <= stoneChance)
						{
							...
							if (getMineArea() == 40 && mineRandom.NextDouble() < 0.15)
							{
								...
								base.Objects.Add(objectPos2, new Object(which.ToString(), 1)
								{
									Fragility = 2,
									CanBeGrabbed = true
								});
							...
							if (rainbowLights.Value && mineRandom.NextDouble() < 0.55)
							{
								...
								{
									string which2 = ((mineRandom.Next(5) != 0) ? "(O)420" : "(O)422");
									Object obj = ItemRegistry.Create<Object>(which2);
									obj.IsSpawnedObject = true;
									base.Objects.Add(objectPos2, obj);
								}
								continue;
							}
							Object litter = createLitterObject(0.001, 5E-05, gemStoneChance, objectPos2);
							if (litter != null)
							{
						...
						else if (mineRandom.NextDouble() <= monsterChance && getDistanceFromStart(j, k) > 5f)
						{
							...
							{
								monsterToAdd = BuffMonsterIfNecessary(getMonsterForThisLevel(mineLevel, j, k));
							}
							...
							if (monsterToAdd.GetBoundingBox().Width <= 64 || isTileClearForMineObjects(j + 1, k))
							{
						...
						{
							Vector2 objectPos3 = new Vector2(j, k);
						...
						else if (mineRandom.NextDouble() <= 0.005 && !isDarkArea() && !mustKillAllMonstersToAdvance() && (GetAdditionalDifficulty() <= 0 || (getMineArea() == 40 && mineLevel % 40 < 30)))
						{
							...
							Vector2 objectPos4 = new Vector2(j, k);
							...
							resourceClumps.Add(new ResourceClump(whichClump, 2, 2, objectPos4));
						}
					...
					else if (isContainerPlatform(j, k) && CanItemBePlacedHere(new Vector2(j, k)) && mineRandom.NextDouble() < 0.4 && (firstTime || canAdd(0, barrelsAdded)))
					{
						Vector2 objectPos5 = new Vector2(j, k);
					...
					else
					{
						...
						Monster monsterToAdd2 = BuffMonsterIfNecessary(getMonsterForThisLevel(mineLevel, j, k));
						if (monsterToAdd2.GetBoundingBox().Width <= 64 || isTileClearForMineObjects(j + 1, k))
						{
			...
			if (stonesLeftOnThisLevel > 35)
			{
				...
				{
					if (!Utility.TryGetRandom(objects, out var stone, out var obj2) || !obj2.IsBreakableStone())
					{
						continue;
					}
					...
					for (int x = (int)stone.X - radius / 2; (float)x < stone.X + (float)(radius / 2); x++)
					{
						for (int y = (int)stone.Y - radius / 2; (float)y < stone.Y + (float)(radius / 2); y++)
						{
							...
							if (!objects.TryGetValue(tile, out var tileObj) || !tileObj.IsBreakableStone())
							{
								continue;
							}
							objects.Remove(tile);
			...
			{
				Vector2 possibleSpot = new Vector2(mineRandom.Next(backLayer2.LayerWidth), mineRandom.Next(backLayer2.LayerHeight));
	...
	{
		Random r = Utility.CreateRandom(level, Game1.stats.DaysPlayed, x, (double)y * 9999.0);
		if (Game1.mine == null)
		{
	...
	public void loadLevel(int level)
	{
		...
		{
			mapNumberToLoad = forceLayout.Value;
			string assetName = "Maps\\Mines\\" + mapNumberToLoad;
			if (!mapContent.DoesAssetExist<Map>(assetName))
			{
				Game1.log.Warn($"Can't force mine layout to {mapNumberToLoad} because there's no '{assetName}' asset, falling back to default logic.");
				mapNumberToLoad = -1;
			}
		}
		if (mapNumberToLoad < 0)
		{
			...
			if (getMineArea(level) == 121)
			{
				...
				for (mapNumberToLoad = mineRandom.Next(40); mapNumberToLoad == lastLevel?.loadedMapNumber; mapNumberToLoad = mineRandom.Next(40))
				{
				}
			...
			if (lowestLevelReached >= 120 && mapNumberToLoad != 10 && mapNumberToLoad % 5 != 0 && mineLevel > 1 && mineLevel != 77377)
			{
				...
				if (mineLevel > 120)
				{
					chance2 += Math.Min(0.06, (double)mineLevel / 10000.0);
				}
				if (alt_random.NextDouble() < chance2)
				{
					int[] source = new int[4] { 40, 47, 50, 51 };
					mapNumberToLoad = alt_random.Next(40, 61);
					if (Enumerable.Contains(source, mapNumberToLoad) && alt_random.NextDouble() < 0.75)
					{
						mapNumberToLoad = alt_random.Next(40, 61);
					}
					...
					if (Enumerable.Contains(source, mapNumberToLoad))
					{
	...
	{
		Point elevatorSpot = (ElevatorLightSpot = Utility.findTile(this, 80, "Buildings", "mine"));
		if (elevatorSpot.X >= 0)
		{
			if (canAdd(3, 0))
			{
			...
			{
				setMapTile(elevatorSpot.X, elevatorSpot.Y, 48, "Buildings", "mine");
			}
		}
	}
	...
	public override bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
	{
		if (who.IsLocalPlayer)
		{
			...
			case 174:
			{
				...
				createQuestionDialogue(Game1.content.LoadString("Strings\\Locations:Mines_Shaft"), options2, "Shaft");
				return true;
			}
			case 194:
				playSound("openBox");
	...
	public override void drawAboveAlwaysFrontLayer(SpriteBatch b)
	{
		...
		SpriteText.drawString(b, (Game1.player.team.calicoEggSkullCavernRating.Value + 1).ToString() ?? "", (int)eggPos.X + 28 - SpriteText.getWidthOfString((Game1.player.team.calicoEggSkullCavernRating.Value + 1).ToString() ?? "") / 2, (int)eggPos.Y + 4);
	}
	...
	public Monster getMonsterForThisLevel(int level, int xTile, int yTile)
	{
		...
		else if (getMineArea() == 80)
		{
			...
			if (mineRandom.NextDouble() < 0.2 && distanceFromLadder > 8f && mineLevel >= 90 && hasTileAt(xTile, yTile, "Back") && !hasTileAt(xTile, yTile, "Front"))
			{
		...
		else
		{
			if (getMineArea() == 121)
			{
				...
				if (GetAdditionalDifficulty() > 0 && mineRandom.NextDouble() < 0.2 && distanceFromLadder > 8f && hasTileAt(xTile, yTile, "Back") && !hasTileAt(xTile, yTile, "Front"))
				{
					return new SquidKid(position);
				}
	...
	private static void clearInactiveMines(bool keepUntickedLevels = true)
	{
		...
		{
			if (farmer.locationBeforeForcedEvent.Value == null || !IsGeneratedLevel(farmer.locationBeforeForcedEvent.Value, out var playerMineLevel))
			{
				continue;
			}
		...
		foreach (MineShaft mine in activeMines)
		{
			if (!mine.farmers.Any() && !Enumerable.Contains(disconnectLevels, mine.NameOrUniqueName))
			{
				continue;
			}
		...
		activeMines.RemoveAll(delegate(MineShaft mineShaft)
		{
			if (mineShaft.mineLevel == 77377)
			{
				return false;
			}
			if (Enumerable.Contains(disconnectLevels, mineShaft.NameOrUniqueName))
			{
				return false;
			}
			if (mineShaft.mineLevel > 120)
			{
				if (mineShaft.mineLevel <= maxSkullLevel)
				{
					return false;
				}
			}
			else if (mineShaft.mineLevel <= maxMineLevel)
			{
				return false;
			}
			if (mineShaft.lifespan == 0 && keepUntickedLevels)
			{
				return false;
			}
			if (Game1.IsServer && Game1.locationRequest?.Location is MineShaft mineShaft2 && mineShaft.NameOrUniqueName == mineShaft2.NameOrUniqueName)
			{
				return false;
			}
			mineShaft.OnRemoved();
	...
	public static bool IsGeneratedLevel(string locationName, out int level, out int? forceLayout)
	{
		...
		if (splitIndex > 0)
		{
			if (int.TryParse(rawLevel.Substring(0, splitIndex), out level) && int.TryParse(rawLevel.Substring(splitIndex + 1), out var forceLayoutValue))
			{
				forceLayout = forceLayoutValue;
				return true;
			}
			level = 0;
			forceLayout = null;
	...
	{
		if (!IsGeneratedLevel(name, out var mineLevel, out var forceLayout))
		{
		...
		if (forceLayout.HasValue)
		{
			name = GetLevelName(mineLevel);
		}
		foreach (MineShaft mine in activeMines)
		{
			if (mine.Name == name)
			{
				if (forceLayout.HasValue && mine.loadedMapNumber != forceLayout)
				{
					Game1.log.Warn($"Can't set mine level {mineLevel} to layout {forceLayout} because it's already active with layout {mine.loadedMapNumber}.");
				}
				return mine;
			}
		}
