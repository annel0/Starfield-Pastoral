...
public class Mountain : GameLocation
{
	...
	public void ApplyTreehouseIfNecessary()
	{
		...
		{
			TileSheet tilesheet = map.RequireTileSheet("untitled tile sheet2");
	...
	private void restoreBridge()
	{
		...
		TileSheet tileSheet = map.RequireTileSheet(0, "outdoors");
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		if (landslide.Value && (int)((Game1.currentGameTime.TotalGameTime.TotalMilliseconds - 400.0) / 1600.0) % 2 != 0 && Utility.isOnScreen(new Point(landSlideRect.X / 64, landSlideRect.Y / 64), 128))
		{
	...
	{
		if (landslide.Value && position.Intersects(landSlideRect))
		{
			return true;
		}
		if (railroadAreaBlocked.Value && position.Intersects(railroadBlockRect))
		{
			return true;
		}
	...
	public override bool isTilePlaceable(Vector2 tileLocation, bool itemIsPassable = false)
	{
		...
		if (landslide.Value && landSlideRect.Contains(non_tile_position))
		{
			return false;
		}
		if (railroadAreaBlocked.Value && railroadBlockRect.Contains(non_tile_position))
		{
			return false;
		}
