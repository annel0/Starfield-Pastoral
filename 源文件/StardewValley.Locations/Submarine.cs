...
public class Submarine : GameLocation
{
	...
	{
		if (getTileIndexAt(tileLocation, "Buildings", "submarine tiles") == 217)
		{
			if (doneUntilReset)
			{
	...
	private void changeSubmergeLight(bool red, bool clear = false)
	{
		if (clear)
		{
			setMapTile(3, 4, 98, "Buildings", "submarine tiles");
			setMapTile(4, 4, 99, "Buildings", "submarine tiles");
			setMapTile(3, 5, 122, "Buildings", "submarine tiles");
			setMapTile(4, 5, 123, "Buildings", "submarine tiles");
		}
		else if (red)
		{
			setMapTile(3, 4, 425, "Buildings", "submarine tiles");
			setMapTile(4, 4, 426, "Buildings", "submarine tiles");
			setMapTile(3, 5, 449, "Buildings", "submarine tiles");
			setMapTile(4, 5, 450, "Buildings", "submarine tiles");
		}
		else
		{
			setMapTile(3, 4, 427, "Buildings", "submarine tiles");
			setMapTile(4, 4, 428, "Buildings", "submarine tiles");
			setMapTile(3, 5, 451, "Buildings", "submarine tiles");
			setMapTile(4, 5, 452, "Buildings", "submarine tiles");
		}
	}
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		if (submergeTimer > 0f)
		{
			if (ascending.Value && !localAscending)
			{
				localAscending = true;
			}
			...
			else
			{
				...
				{
					Vector2 pos = new Vector2(Game1.random.Next(12, map.DisplayWidth - 64), ascending.Value ? 1 : 640);
					int which = Game1.random.Next(3);
					Game1.background.tempSprites.Add(new TemporaryAnimatedSprite
					{
						motion = new Vector2(0f, (float)((!ascending.Value) ? 1 : (-1)) * (-3f + (float)which)),
		...
		else if (submerged.Value && !doneUntilReset)
		{
			...
			{
				Vector2 pos2 = new Vector2(Game1.random.Next(384, map.DisplayWidth - 64), 320f);
