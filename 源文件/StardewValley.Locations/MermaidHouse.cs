...
public class MermaidHouse : GameLocation
{
	...
	{
		switch (getTileIndexAt(tileLocation, "Buildings", "mermaid_tiles"))
		{
		case 56:
			playClamTone(0, who);
			return true;
	...
	public override void UpdateWhenCurrentLocation(GameTime time)
	{
		...
		if (Game1.shouldTimePass())
		{
			...
			sparkles.RemoveWhere((TemporaryAnimatedSprite sparkle) => sparkle.update(time));
			alwaysFrontTempSprites.RemoveWhere((TemporaryAnimatedSprite sprite) => sprite.update(time));
			...
			if (stopWatch.ElapsedMilliseconds > 27692 && stopWatch.ElapsedMilliseconds < 55385)
			{
				...
				{
					bubbles[i] = new Vector2(bubbles[i].X, bubbles[i].Y - 0.1f * (float)time.ElapsedGameTime.Milliseconds);
				}
			}
