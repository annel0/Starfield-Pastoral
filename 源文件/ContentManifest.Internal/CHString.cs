		...
		for (; readHead < jsonText.Length; readHead++)
		{
			...
			case '\\':
			{
				...
				case 'u':
				{
					...
					string decodedString = char.ConvertFromUtf32(0 | ((ParseHexChar(jsonText[readHead + 1]) & 0xF) << 12) | ((ParseHexChar(jsonText[readHead + 2]) & 0xF) << 8) | ((ParseHexChar(jsonText[readHead + 3]) & 0xF) << 4) | (ParseHexChar(jsonText[readHead + 4]) & 0xF));
