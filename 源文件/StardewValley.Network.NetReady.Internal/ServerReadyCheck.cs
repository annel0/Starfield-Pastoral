...
internal sealed class ServerReadyCheck : BaseReadyCheck
{
	...
	public override void Update()
	{
		...
		{
			if (IsFarmerRequired(farmer.UniqueMultiplayerID) && !Game1.multiplayer.isDisconnecting(farmer))
			{
		...
		if (ready != base.NumberReady || required != base.NumberRequired)
		{
			...
			{
				SendMessage(ReadyCheckMessageType.UpdateAmounts, ready - ((base.State == ReadyState.Ready) ? 1 : 0), required - 1);
			}
			else
			{
