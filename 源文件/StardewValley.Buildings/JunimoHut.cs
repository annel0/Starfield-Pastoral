...
public class JunimoHut(Vector2 tileLocation) : Building("Junimo Hut", tileLocation)
{
	...
	public void updateLightState()
	{
		...
		string lightSourceId = $"{"JunimoHut"}_{tileX}_{tileY}";
		if (wasLit.Value)
		{
			if (Utility.getLightSource(lightSourceId) == null)
			{
				Game1.currentLightSources.Add(new LightSource(lightSourceId, 4, new Vector2(tileX.Value + 1, tileY.Value + 1) * 64f + new Vector2(32f, 32f), 0.5f, LightSource.LightContext.None, 0L, parentLocationName.Value));
			}
	...
	public override void updateWhenFarmNotCurrentLocation(GameTime time)
	{
		...
		JunimoHarvester j = new JunimoHarvester(location, new Vector2(tileX.Value + 1, tileY.Value + 1) * 64f + new Vector2(0f, 32f), this, junimoNumber, gemColor);
		j.isPrismatic.Value = isPrismatic;
		location.characters.Add(j);
		myJunimos.Add(j);
		junimoSendOutTimer = 1000;
		if (Utility.isOnScreen(Utility.Vector2ToPoint(new Vector2(tileX.Value + 1, tileY.Value + 1)), 64, location))
		{
			try
			{
	...
	{
		GameLocation location = GetParentLocation();
		for (int x = tileX.Value + 1 - cropHarvestRadius; x < tileX.Value + 2 + cropHarvestRadius; x++)
		{
			for (int y = tileY.Value - cropHarvestRadius + 1; y < tileY.Value + 2 + cropHarvestRadius; y++)
			{
	...
	public override void draw(SpriteBatch b)
	{
		...
		b.Draw(texture.Value, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileX.Value * 64, tileY.Value * 64 + tilesHigh.Value * 64)), sourceRect, color * alpha, 0f, new Vector2(0f, texture.Value.Bounds.Height), 4f, SpriteEffects.None, (float)((tileY.Value + tilesHigh.Value - 1) * 64) / 10000f);
		if (raisinDays.Value > 0 && !Game1.IsWinter)
		{
