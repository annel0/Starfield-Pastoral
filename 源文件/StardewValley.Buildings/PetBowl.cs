...
public class PetBowl(Vector2 tileLocation) : Building("Pet Bowl", tileLocation)
{
	...
	public override void draw(SpriteBatch b)
	{
		...
		if (watered.Value)
		{
			...
			float sortY = (tileY.Value + tilesHigh.Value) * 64;
			if (data != null)
			{
		...
		if (nameTimer > 0)
		{
			...
			float sortY2 = (tileY.Value + tilesHigh.Value) * 64;
			if (data2 != null)
			{
			...
			sortY2 += 1.5f;
			sortY2 /= 10000f;
			SpriteText.drawSmallTextBubble(b, nameTimerMessage, Game1.GlobalToLocal(new Vector2(((float)tileX.Value + 1.5f) * 64f, tileY.Value * 64 - 32)), -1, sortY2 + 1E-06f);
		}
	}
}
