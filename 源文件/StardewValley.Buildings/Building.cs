...
public class Building : INetObject<NetFields>, IHaveModData
{
	...
	public virtual string GetPaintDataKey(Dictionary<string, string> paintData)
	{
		...
		string lookupName = ((value == "Farmhouse") ? "House" : ((!(value == "Cabin")) ? buildingType.Value : "Stone Cabin"));
	...
	public virtual void LoadFromBuildingData(BuildingData data, bool forUpgrade = false, bool forConstruction = false)
	{
		...
		{
			if (GetBuildingChest(buildingChest2.Id) == null)
			{
	...
	public virtual void InitializeIndoor(BuildingData data, bool forConstruction, bool forUpgrade)
	{
		...
		foreach (IndoorItemAdd item in data.IndoorItems)
		{
			...
			if (item.ClearTile)
			{
				...
				{
					int i = 0;
					for (int height = newFurniture.getTilesHigh(); i < height; i++)
					{
						int j = 0;
						for (int width = newFurniture.getTilesWide(); j < width; j++)
						{
							interior.cleanUpTileForMapOverride(new Point((int)tileVector.X + j, (int)tileVector.Y + i), item.ItemId);
						}
					}
				}
				else
	...
	public virtual bool doAction(Vector2 tileLocation, Farmer who)
	{
		...
		else
		{
			...
			GameLocation interior = GetIndoors();
			if (who.IsLocalPlayer && tileLocation.X == (float)(humanDoor.X + tileX.Value) && tileLocation.Y == (float)(humanDoor.Y + tileY.Value) && interior != null)
			{
				if (who.mount != null)
				{
			...
			if (data != null)
			{
				...
				door.X /= 64;
				door.Y /= 64;
				if (daysOfConstructionLeft.Value <= 0 && door != Microsoft.Xna.Framework.Rectangle.Empty && door.Contains(Utility.Vector2ToPoint(tileLocation)) && Game1.didPlayerJustRightClick(ignoreNonMouseHeldInput: true))
				{
	...
	public virtual void performActionOnBuildingPlacement()
	{
		...
		for (int y = 0; y < tilesHigh.Value; y++)
		{
			...
			{
				Vector2 currentGlobalTilePosition = new Vector2(tileX.Value + x, tileY.Value + y);
				if (!(location.terrainFeatures.GetValueOrDefault(currentGlobalTilePosition) is Flooring) || !(GetData()?.AllowsFlooringUnderneath ?? false))
				{
		...
		foreach (BuildingPlacementTile additionalPlacementTile in GetAdditionalPlacementTiles())
		{
			...
			{
				Vector2 currentGlobalTilePosition2 = new Vector2(tileX.Value + areaTile.X, tileY.Value + areaTile.Y);
				if ((!onlyNeedsToBePassable || (location.terrainFeatures.TryGetValue(currentGlobalTilePosition2, out var feature) && !feature.isPassable())) && (!(location.terrainFeatures.GetValueOrDefault(currentGlobalTilePosition2) is Flooring) || !(GetData()?.AllowsFlooringUnderneath ?? false)))
				{
	...
	public virtual void performActionOnConstruction(GameLocation location, Farmer who)
	{
		...
		Vector2 buildingCenter = new Vector2((float)tileX.Value + (float)tilesWide.Value * 0.5f, (float)tileY.Value + (float)tilesHigh.Value * 0.5f);
		...
		{
			location.localSound("axchop", buildingCenter);
			for (int x = tileX.Value; x < tileX.Value + tilesWide.Value; x++)
			{
				for (int y = tileY.Value; y < tileY.Value + tilesHigh.Value; y++)
				{
					for (int i = 0; i < 5; i++)
					{
			...
			for (int j = 0; j < 8; j++)
			{
				DelayedAction.playSoundAfterDelay("dirtyHit", 250 + j * 150, location, buildingCenter, -1, local: true);
			}
		}
		else
		{
			...
			int l = 0;
			for (int bottomEdge = mainSourceRect.Height / 16 * 2; l <= bottomEdge; l++)
			{
				int m = 0;
				for (int rightEdge = sourceRectForMenu.Width / 16 * 2; m < rightEdge; m++)
				{
					location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(666, 1851, 8, 8), 40f, 4, 2, new Vector2(tileX.Value, tileY.Value) * 64f + new Vector2(m * 64 / 2, l * 64 / 2 - mainSourceRect.Height * 4 + tilesHigh.Value * 64) + new Vector2(Game1.random.Next(-32, 32), Game1.random.Next(-32, 32)), flicker: false, flipped: false)
					{
						layerDepth = (float)((tileY.Value + tilesHigh.Value) * 64) / 10000f + (float)m / 10000f,
						pingPong = true,
						delayBeforeAnimationStart = (mainSourceRect.Height / 16 * 2 - l) * 100,
						scale = 4f,
						alphaFade = 0.01f,
					...
					location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(666, 1851, 8, 8), 40f, 4, 2, new Vector2(tileX.Value, tileY.Value) * 64f + new Vector2(m * 64 / 2, l * 64 / 2 - mainSourceRect.Height * 4 + tilesHigh.Value * 64) + new Vector2(Game1.random.Next(-32, 32), Game1.random.Next(-32, 32)), flicker: false, flipped: false)
					{
						layerDepth = (float)((tileY.Value + tilesHigh.Value) * 64) / 10000f + (float)m / 10000f + 0.0001f,
						pingPong = true,
						delayBeforeAnimationStart = (mainSourceRect.Height / 16 * 2 - l) * 100,
						scale = 4f,
						alphaFade = 0.01f,
	...
	public virtual void Update(GameTime time)
	{
		...
		if (chimneyTimer <= 0)
		{
			if (interior.hasActiveFireplace())
			{
				...
				Microsoft.Xna.Framework.Rectangle mainSourceRect = getSourceRect();
				Vector2 cornerPosition = new Vector2(tileX.Value * 64, tileY.Value * 64 + tilesHigh.Value * 64 - mainSourceRect.Height * 4);
				BuildingData data = GetData();
	...
	public virtual void UpdateTransparency()
	{
		if (fadeWhenPlayerIsBehind.Value)
		{
			...
			Microsoft.Xna.Framework.Rectangle boundingBox = new Microsoft.Xna.Framework.Rectangle(tileX.Value * 64, (tileY.Value + (-(sourceRect.Height / 16) + tilesHigh.Value)) * 64, tilesWide.Value * 64, (sourceRect.Height / 16 - tilesHigh.Value) * 64 + 32);
	...
	public virtual Vector2 getUpgradeSignLocation()
	{
		...
		return new Vector2(((float)tileX.Value + signOffset.X) * 64f, ((float)tileY.Value + signOffset.Y) * 64f - signHeight * 4f);
	}
	...
	public virtual void CheckItemConversionRule(BuildingItemConversion conversion, ItemQueryContext itemQueryContext)
	{
		...
		for (int j = 0; j < convertAmount; j++)
		{
			...
			for (int i = 0; i < conversion.ProducedItems.Count; i++)
			{
				...
				if (GameStateQuery.CheckConditions(producedItem.Condition, GetParentLocation()))
				{
					...
					Item item3 = destinationChest.addItem(item2);
					if (item3 == null || item3.Stack != producedCount)
					{
		...
		{
			Item item4 = sourceChest.Items[k];
			if (item4 == null)
			{
				continue;
			}
			...
			{
				if (!item4.HasContextTag(requiredTag2))
				{
					fail2 = true;
					break;
				}
			...
			if (!fail2)
			{
				int consumedAmount = Math.Min(requiredAmount, item4.Stack);
				sourceChest.Items[k] = item4.ConsumeStack(consumedAmount);
	...
	public virtual Microsoft.Xna.Framework.Rectangle ApplySourceRectOffsets(Microsoft.Xna.Framework.Rectangle source)
	{
		...
		{
			int seasonOffset = Game1.GetSeasonIndexForLocation(GetParentLocation());
	...
	protected virtual GameLocation createIndoors(BuildingData data, string nameOfIndoorsWithoutUnique)
	{
		...
		if (data != null && !string.IsNullOrEmpty(data.IndoorMap))
		{
			...
			try
			{
				localIndoors = (GameLocation)Activator.CreateInstance(locationType, mapAssetName, buildingType.Value);
			}
			catch (Exception)
			{
				try
				{
					localIndoors = (GameLocation)Activator.CreateInstance(locationType, mapAssetName);
				}
				catch (Exception exception2)
				{
					Game1.log.Error($"Error trying to instantiate indoors for '{buildingType}'", exception2);
					localIndoors = new GameLocation("Maps\\" + nameOfIndoorsWithoutUnique, buildingType.Value);
				}
			}
		}
		if (localIndoors != null)
		{
			localIndoors.uniqueName.Value = nameOfIndoorsWithoutUnique + GuidHelper.NewGuid();
			localIndoors.IsFarm = true;
			localIndoors.isStructure.Value = true;
			localIndoors.ParentBuilding = this;
			updateInteriorWarps(localIndoors);
		}
		return localIndoors;
	}
	...
	public virtual Microsoft.Xna.Framework.Rectangle getRectForAnimalDoor(BuildingData data)
	{
		if (data != null)
		{
			...
			return new Microsoft.Xna.Framework.Rectangle((rect.X + tileX.Value) * 64, (rect.Y + tileY.Value) * 64, rect.Width * 64, rect.Height * 64);
		}
	...
	public virtual void load()
	{
		...
		else
		{
			...
			if (indoorInstance != null && indoors.Value != null)
			{
				...
				{
					pair.Value.reload(indoorInstance);
				}
	...
	public virtual void drawInMenu(SpriteBatch b, int x, int y)
	{
		...
		float baseSortY = tilesHigh.Value * 64;
	...
	public virtual void drawBackground(SpriteBatch b)
	{
		...
		Vector2 drawPosition = new Vector2(tileX.Value * 64, tileY.Value * 64 + tilesHigh.Value * 64);
		foreach (BuildingDrawLayer drawLayer in data.DrawLayers)
		{
			if (!drawLayer.DrawInBackground)
			{
	...
	public virtual void draw(SpriteBatch b)
	{
		...
		float baseSortY = (tileY.Value + tilesHigh.Value) * 64;
		float sortY = baseSortY;
		if (data != null)
		{
			sortY -= data.SortTileOffset * 64f;
		...
		if (data != null)
		{
			foreach (Chest chest in buildingChests)
			{
				...
				{
					sortY = ((float)tileY.Value + chestData.DisplayTile.Y + 1f) * 64f;
					sortY += 1f;
					...
					float drawX = ((float)tileX.Value + chestData.DisplayTile.X) * 64f;
					float drawY = ((float)tileY.Value + chestData.DisplayTile.Y - 1f) * 64f;
			...
			if (data.DrawLayers != null)
			{
				foreach (BuildingDrawLayer drawLayer in data.DrawLayers)
				{
					...
					if (drawLayer.OnlyDrawIfChestHasContents != null)
					{
						Chest chest2 = GetBuildingChest(drawLayer.OnlyDrawIfChestHasContents);
		...
		if (data != null)
		{
			...
			{
				sortY = ((float)tileY.Value + data.UpgradeSignTile.Y + 1f) * 64f;
				sortY += 2f;
				sortY /= 10000f;
	...
	public virtual void drawShadow(SpriteBatch b, int localX = -1, int localY = -1)
	{
		...
		Vector2 basePosition = ((localX == -1) ? Game1.GlobalToLocal(new Vector2(tileX.Value * 64, (tileY.Value + tilesHigh.Value) * 64)) : new Vector2(localX, localY + sourceRectForMenu.Height * 4));
		...
		for (int x = 1; x < tilesWide.Value - 1; x++)
		{
		...
		b.Draw(Game1.mouseCursors, basePosition + new Vector2((tilesWide.Value - 1) * 64, 0f), rightShadow, Color.White * ((localX == -1) ? alpha : 1f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
	}
	...
	{
		int drawPercentage = Math.Min(16, Math.Max(0, (int)(16f - (float)newConstructionTimer.Value / 1000f * 16f)));
		float drawPercentageReal = (float)(2000 - newConstructionTimer.Value) / 2000f;
		if (magical.Value || daysOfConstructionLeft.Value <= 0)
		{
			...
			if (magical.Value)
			{
				...
				{
					b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileX.Value * 64 + i * 16, (float)(tileY.Value * 64 - mainSourceRect.Height * 4 + tilesHigh.Value * 64) + (float)(mainSourceRect.Height * 4) * (1f - drawPercentageReal))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) - ((i % 2 == 0) ? 32 : 8)), new Microsoft.Xna.Framework.Rectangle(536 + (newConstructionTimer.Value + i * 4) % 56 / 8 * 8, 1945, 8, 8), (i % 2 == 1) ? (Color.Pink * alpha) : (Color.LightPink * alpha), 0f, new Vector2(0f, 0f), 4f + (float)Game1.random.Next(100) / 100f, SpriteEffects.None, (float)((tileY.Value + tilesHigh.Value) * 64) / 10000f + 0.0001f);
					if (i % 2 == 0)
					{
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileX.Value * 64 + i * 16, (float)(tileY.Value * 64 - mainSourceRect.Height * 4 + tilesHigh.Value * 64) + (float)(mainSourceRect.Height * 4) * (1f - drawPercentageReal))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) + ((i % 2 == 0) ? 32 : 8)), new Microsoft.Xna.Framework.Rectangle(536 + (newConstructionTimer.Value + i * 4) % 56 / 8 * 8, 1945, 8, 8), Color.White * alpha, 0f, new Vector2(0f, 0f), 4f + (float)Game1.random.Next(100) / 100f, SpriteEffects.None, (float)((tileY.Value + tilesHigh.Value) * 64) / 10000f + 0.0001f);
					}
				}
				return;
			}
			...
			{
				b.Draw(Game1.animations, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileX.Value * 64 - 16 + j * 16, (float)(tileY.Value * 64 - mainSourceRect.Height * 4 + tilesHigh.Value * 64) + (float)(mainSourceRect.Height * 4) * (1f - drawPercentageReal))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) - ((j % 2 == 0) ? 32 : 8)), new Microsoft.Xna.Framework.Rectangle((newConstructionTimer.Value + j * 20) % 304 / 38 * 64, 768, 64, 64), Color.White * alpha * ((float)newConstructionTimer.Value / 500f), 0f, new Vector2(0f, 0f), 1f, SpriteEffects.None, (float)((tileY.Value + tilesHigh.Value) * 64) / 10000f + 0.0001f);
				if (j % 2 == 0)
				{
					b.Draw(Game1.animations, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileX.Value * 64 - 16 + j * 16, (float)(tileY.Value * 64 - mainSourceRect.Height * 4 + tilesHigh.Value * 64) + (float)(mainSourceRect.Height * 4) * (1f - drawPercentageReal))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) - ((j % 2 == 0) ? 32 : 8)), new Microsoft.Xna.Framework.Rectangle((newConstructionTimer.Value + j * 20) % 400 / 50 * 64, 2944, 64, 64), Color.White * alpha * ((float)newConstructionTimer.Value / 500f), 0f, new Vector2(0f, 0f), 1f, SpriteEffects.None, (float)((tileY.Value + tilesHigh.Value) * 64) / 10000f + 0.0001f);
				}
			}
			return;
		}
		bool drawFloor = daysOfConstructionLeft.Value == 1;
		for (int x = tileX.Value; x < tileX.Value + tilesWide.Value; x++)
		{
			for (int y = tileY.Value; y < tileY.Value + tilesHigh.Value; y++)
			{
				if (x == tileX.Value + tilesWide.Value / 2 && y == tileY.Value + tilesHigh.Value - 1)
				{
					if (drawFloor)
					{
					...
					b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(x, y) * 64f) + new Vector2(0f, 64 - drawPercentage * 4) + ((newConstructionTimer.Value > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Microsoft.Xna.Framework.Rectangle(367, 309, 16, drawPercentage), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 64 - 1) / 10000f);
				}
				else if (x == tileX.Value && y == tileY.Value)
				{
					if (drawFloor)
					{
					...
					b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(x, y) * 64f) + new Vector2(0f, 64 - drawPercentage * 4) + ((newConstructionTimer.Value > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Microsoft.Xna.Framework.Rectangle(351, 293, 16, drawPercentage), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 64 - 1) / 10000f);
				}
				else if (x == tileX.Value + tilesWide.Value - 1 && y == tileY.Value)
				{
					if (drawFloor)
					{
					...
					b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(x, y) * 64f) + new Vector2(0f, 64 - drawPercentage * 4) + ((newConstructionTimer.Value > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Microsoft.Xna.Framework.Rectangle(383, 293, 16, drawPercentage), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 64 - 1) / 10000f);
				}
				else if (x == tileX.Value + tilesWide.Value - 1 && y == tileY.Value + tilesHigh.Value - 1)
				{
					if (drawFloor)
					{
				...
				else if (x == tileX.Value && y == tileY.Value + tilesHigh.Value - 1)
				{
					if (drawFloor)
					{
					...
					b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(x, y) * 64f) + new Vector2(0f, 64 - drawPercentage * 4) + ((newConstructionTimer.Value > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Microsoft.Xna.Framework.Rectangle(351, 325, 16, drawPercentage), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64) / 10000f);
				}
				else if (x == tileX.Value + tilesWide.Value - 1)
				{
					if (drawFloor)
					{
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(x, y) * 64f) + new Vector2(0f, 64 - drawPercentage * 4 + 16), new Microsoft.Xna.Framework.Rectangle(383, 261, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
				...
				else if (y == tileY.Value + tilesHigh.Value - 1)
				{
					if (drawFloor)
					{
					...
					b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(x, y) * 64f) + new Vector2(0f, 64 - drawPercentage * 4) + ((newConstructionTimer.Value > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Microsoft.Xna.Framework.Rectangle(367, 325, 16, drawPercentage), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64) / 10000f);
				}
				else if (x == tileX.Value)
				{
					if (drawFloor)
					{
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(x, y) * 64f) + new Vector2(0f, 64 - drawPercentage * 4 + 16), new Microsoft.Xna.Framework.Rectangle(351, 261, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
					}
					b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(x, y) * 64f) + new Vector2(0f, 64 - drawPercentage * 4) + ((newConstructionTimer.Value > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Microsoft.Xna.Framework.Rectangle(351, 309, 16, drawPercentage), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64) / 10000f);
				}
				else if (y == tileY.Value)
				{
					if (drawFloor)
					{
					...
					b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(x, y) * 64f) + new Vector2(0f, 64 - drawPercentage * 4) + ((newConstructionTimer.Value > 0) ? new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2)) : Vector2.Zero), new Microsoft.Xna.Framework.Rectangle(367, 293, 16, drawPercentage), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 64 - 1) / 10000f);
				}
				else if (drawFloor)
				{
					b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(x, y) * 64f) + new Vector2(0f, 64 - drawPercentage * 4 + 16), new Microsoft.Xna.Framework.Rectangle(367, 261, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1E-05f);
