...
public class PathFindController
{
	...
	public void handleWarps(Rectangle position)
	{
		...
		if (character is NPC npc && (w.TargetName == "FarmHouse" || w.TargetName == "Cabin") && npc.isMarried() && npc.getSpouse() != null)
		{
			...
			w = new Warp(w.X, w.Y, location.name.Value, entryPoint.X, entryPoint.Y, flipFarmer: false);
	...
	public static Stack<Point> findPathForNPCSchedules(Point startPoint, Point endPoint, GameLocation location, int limit, Character npc)
	{
		...
		while (!openList.IsEmpty())
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				neighbor.g = (byte)(currentNode.g + 1);
				if ((neighbor.x == endPoint.X && neighbor.y == endPoint.Y) || (neighbor.x >= 0 && neighbor.y >= 0 && neighbor.x < layerWidth && neighbor.y < layerHeight && !isPositionImpassableForNPCSchedule(location, neighbor.x, neighbor.y, npc)))
				{
